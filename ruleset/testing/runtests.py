...
def getBongosecInfo(bongosec_home):
    bongosec_control = os.path.join(bongosec_home, "bin", "bongosec-control")
    bongosec_env_vars = {}
    try:
        proc = subprocess.Popen([bongosec_control, "info"], stdout=subprocess.PIPE)
        (stdout, stderr) = proc.communicate()
    except Exception as e:
        print("Seems like there is no Bongosec installation.")
        return None
...
def provisionDR():
    ...
    for file in os.listdir(rules_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?rules.xml$', file):
            shutil.copy2(file_fullpath, args.bongosec_home + "/etc/rules")
    ...
    for file in os.listdir(decoders_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?decoders.xml$', file):
            shutil.copy2(file_fullpath, args.bongosec_home + "/etc/decoders")



def cleanDR():
    rules_dir = args.bongosec_home + "/etc/rules"
    decoders_dir = args.bongosec_home + "/etc/decoders"
...
class OssecTester(object):
    ...
    def buildCmd(self, rule, alert, decoder):
        cmd = ['%s/bongosec-logtest' % (self._ossec_path), ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='This script tests Bongosec rules.')
    parser.add_argument('--path', '-p', default='/var/ossec', dest='bongosec_home',
                        help='Use -p or --path to specify Bongosec installation path')
    ...
    bongosec_info = getBongosecInfo(args.bongosec_home)
    if bongosec_info is None:
        sys.exit(1)
    ...
    provisionDR()
    OT = OssecTester(args.bongosec_home)
