...
param (
    [string]$MSI_NAME = "bongosec-agent.msi",
    [string]$SIGN = "no",
...
if(($help.isPresent)) {
    "
    This tool can be used to generate the Windows Bongosec agent msi package.

    PARAMETERS TO BUILD BONGOSEC-AGENT MSI (OPTIONALS):
    ...
    USAGE:
        ...
        * BONGOSEC:
          $ ./generate_bongosec_msi.ps1  -MSI_NAME {{ NAME }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}
            Build a devel msi:    $ ./generate_bongosec_msi.ps1 -MSI_NAME bongosec-agent_4.9.0-0_windows_0ceb378.msi -SIGN no
            Build a prod msi:     $ ./generate_bongosec_msi.ps1 -MSI_NAME bongosec-agent-4.9.0-1.msi -SIGN yes
    "
    Exit
}
...
function BuildBongosecMsi(){
    ...
    if($SIGN -eq "yes"){
        ...
        $filesToSign = @(
            ...
            "..\bongosec_modules\syscollector\build\bin\syscollector.dll",
    ...
    & $CANDLE_EXE -nologo .\bongosec-installer.wxs -out "bongosec-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\bongosec-installer.wixobj" -out $MSI_NAME -ext WixUtilExtension -ext WixUiExtension
...
function ExtractDebugSymbols(){
	...
	$exeFiles +=  Get-ChildItem -Filter "bongosec_modules\syscollector\build\bin\syscollector.dll"
  ...
  #compress every pdb file in current folder
    ...
    $ZIP_NAME = $MSI_NAME -replace 'bongosec-agent', 'bongosec-agent-debug-symbols' -replace '\.msi$', '.zip'
