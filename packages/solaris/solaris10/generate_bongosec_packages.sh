...
# Bongosec Solaris 10 Package builder.


# CONFIGURATION VARIABLES
bongosec_branch="$(echo "$2" | cut -d "/" -f2)"
...
if [ -z "${bongosec_branch}" ]; then
    bongosec_branch="main"
fi
...
set_control_binary() {
  if [ -e ${SOURCE}/VERSION.json ]; then
    bongosec_version="v$(sed -n 's/.*"version"[ \t]*:[ \t]*"\([^"]*\)".*/\1/p' ${SOURCE}/VERSION.json)"
...
{
    bongosec_version="$(sed -n 's/.*"version"[ \t]*:[ \t]*"\([^"]*\)".*/\1/p' ${SOURCE}/VERSION.json)"
...
clone(){
    cd ${CURRENT_PATH}
    git clone --depth=1 $REPOSITORY -b $bongosec_branch ${SOURCE} || return 1
    cd $SOURCE
    compute_version_revision
...
package(){
    cd ${CURRENT_PATH}
    find ${install_path} | awk 'length > 0' > "bongosec-agent_$VERSION.list"
    ...
    echo "i pkginfo=pkginfo" > "bongosec-agent_$VERSION.proto"
    echo "i checkinstall=checkinstall.sh" >> "bongosec-agent_$VERSION.proto"
    echo "i preinstall=preinstall.sh" >> "bongosec-agent_$VERSION.proto"
    echo "i postinstall=postinstall.sh" >> "bongosec-agent_$VERSION.proto"
    echo "i preremove=preremove.sh" >> "bongosec-agent_$VERSION.proto"
    echo "i postremove=postremove.sh" >> "bongosec-agent_$VERSION.proto"
    echo "f none /etc/init.d/bongosec-agent  0755 root root" >> "bongosec-agent_$VERSION.proto"
    echo "s none /etc/rc2.d/S97bongosec-agent=/etc/init.d/bongosec-agent" >> "bongosec-agent_$VERSION.proto"
    echo "s none /etc/rc3.d/S97bongosec-agent=/etc/init.d/bongosec-agent" >> "bongosec-agent_$VERSION.proto"
    cat "bongosec-agent_$VERSION.list" | pkgproto >> "bongosec-agent_$VERSION.proto"

    echo $VERSION
    pkgmk -o -r / -d . -f "bongosec-agent_$VERSION.proto"
    pkg_name="bongosec-agent_$VERSION-sol10-$ARCH.pkg"
    pkgtrans -s ${CURRENT_PATH} "${pkg_name}" bongosec-agent
...
clean(){
    ...
    cd ${CURRENT_PATH}
    rm -rf ${SOURCE}
    rm -rf bongosec-agent bongosec *.list *proto
    rm -f *.new
