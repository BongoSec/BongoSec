...
# Modified by Santiago Bassett (http://www.bongosec.github.io) - Feb 2016
...
UPGRADE_FILE_FLAG="${DIR}/BONGOSEC_PKG_UPGRADE"
...
if [ -z "${upgrade}" ]; then
    echo "Generating Bongosec configuration for a fresh installation."
    ...
    if [ -f "${INSTALLATION_SCRIPTS_DIR}/gen_ossec.sh" ]; then
        ...
        chown root:bongosec ${DIR}/etc/ossec.conf
...
if [ -n "${upgrade}" ] && [ -n "${restart}" ]; then
    echo "Restarting Bongosec..."
    ${DIR}/bin/bongosec-control restart
fi
