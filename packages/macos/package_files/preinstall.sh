...
if [ -d "${DIR}" ]; then
    echo "A Bongosec agent installation was found in ${DIR}. Will perform an upgrade."
    upgrade="true"
    ...
    if [ -d ${DIR}/logs/ossec ]; then
        echo "Renaming ${DIR}/logs/ossec to ${DIR}/logs/bongosec"
    ...
    if pkgutil --pkgs | grep -i bongosec-agent-etc > /dev/null 2>&1 ; then
        echo "Removing previous package receipt for bongosec-agent-etc"
...
echo "Hiding the fixed bongosec user"
