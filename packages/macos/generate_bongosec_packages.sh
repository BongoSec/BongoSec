#!/bin/bash
set -e
# Program to build and package OSX bongosec-agent
...
ARCH="intel64"
BONGOSEC_SOURCE_REPOSITORY="https://github.com/bongosec/bongosec"
SERVICE_PATH="/Library/LaunchDaemons/com.bongosec.agent.plist"
STARTUP_PATH="/Library/StartupItems/BONGOSEC/StartupParameters.plist"
LAUNCHER_SCRIPT_PATH="/Library/StartupItems/BONGOSEC/Bongosec-launcher"
STARTUP_SCRIPT_PATH="/Library/StartupItems/BONGOSEC/BONGOSEC"
...
function clean_and_exit() {
    ...
    if [ -z "$BRANCH_TAG" ]; then
        make -C $BONGOSEC_PATH/src clean clean-deps
    fi
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in Bongosec's installation. This also includes library files.
...
function prepare_building_folder() {
    ...
    version="$1"
    pkg_final_name="$2"
    build_info_file="${BONGOSEC_PACKAGES_PATH}/specs/build-info.json"
    preinstall_script="${BONGOSEC_PACKAGES_PATH}/package_files/preinstall.sh"
    postinstall_script="${BONGOSEC_PACKAGES_PATH}/package_files/postinstall.sh"
    packaged_directory=$CURRENT_PATH/bongosec-agent/payload

    if [ -d "$CURRENT_PATH/bongosec-agent" ]; then

        echo "\nThe bongosec agent building directory is present on this machine."
        echo "Removing it from the system."

        rm -rf $CURRENT_PATH/bongosec-agent
    fi

    munkipkg --create --json $CURRENT_PATH/bongosec-agent

    cp -f $build_info_file $CURRENT_PATH/bongosec-agent/

    sed -i '' "s|VERSION|$version|g" $CURRENT_PATH/bongosec-agent/$(basename $build_info_file)
    sed -i '' "s|PACKAGE_NAME|$pkg_final_name|g" $CURRENT_PATH/bongosec-agent/$(basename $build_info_file)

    cp $preinstall_script $CURRENT_PATH/bongosec-agent/scripts/preinstall
    cp $postinstall_script $CURRENT_PATH/bongosec-agent/scripts/postinstall

    sed -i '' "s|PACKAGE_ARCH|$ARCH|g" $CURRENT_PATH/bongosec-agent/scripts/preinstall
...
function build_package() {
    ...
    if [ -n "$BRANCH_TAG" ]; then
        ...
        BONGOSEC_PATH="${SOURCES_DIRECTORY}/bongosec"
        git clone --depth=1 -b ${BRANCH_TAG} ${BONGOSEC_SOURCE_REPOSITORY} "${BONGOSEC_PATH}"
    else
        BONGOSEC_PATH="${CURRENT_PATH}/../.."
    fi
    short_commit_hash="$(cd "${BONGOSEC_PATH}" && git rev-parse --short=7 HEAD)"

    export CONFIG="${BONGOSEC_PATH}/etc/preloaded-vars.conf"
    BONGOSEC_PACKAGES_PATH="${BONGOSEC_PATH}/packages/macos"
    ENTITLEMENTS_PATH="${BONGOSEC_PACKAGES_PATH}/entitlements.plist"

    VERSION="$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $BONGOSEC_PATH/VERSION.json)"

    # Define output package name
    if [ $IS_STAGE == "no" ]; then
        pkg_name="bongosec-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="bongosec-agent-${VERSION}-${REVISION}.${ARCH}"
    fi

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe bongosec agent is already installed on this machine."
        echo "Removing it from the system."

        ${CURRENT_PATH}/uninstall.sh
    fi

    ${BONGOSEC_PACKAGES_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${BONGOSEC_PATH}" ${JOBS} ${DEBUG} ${MAKE_COMPILATION}
    ...
    # create package
    if munkipkg $CURRENT_PATH/bongosec-agent ; then
        echo "The bongosec agent package for macOS has been successfully built."
        mv $CURRENT_PATH/bongosec-agent/build/* $DESTINATION/
        symbols_pkg_name="bongosec-agent-debug-symbols-${VERSION}-${REVISION}.${ARCH}-macos"
        cp -R "${BONGOSEC_PATH}/src/symbols"  "${DESTINATION}"
