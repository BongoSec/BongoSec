...
source_directory=/bongosec-sources
...
build_environment() {
    ...
    curl -k -SO http://packages.bongosec.github.io/utils/gcc/gcc_9.4_HPUX_build.tar.gz
    ...
    cd ${build_tools_path}
    curl -k -SO http://packages.bongosec.github.io/utils/cmake/cmake_3.22.2_HPUX_build.tar.gz
...
compute_version_revision() {
    bongosec_version="$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $source_directory/VERSION.json)"

    if [ -z "$bongosec_revision" ]; then
        stage_value=$(awk -F'"' '/"stage"[ \t]*:/ {print $4}' $source_directory/VERSION.json)
        bongosec_revision=$(echo "$stage_value" | sed 's/[^0-9]*\([0-9][0-9]*\).*/\1/')
    fi

    # Add commit hash to the VERSION.json file
    short_commit_hash=$(/usr/local/bin/curl -ks "https://api.github.com/repos/bongosec/bongosec/commits/${bongosec_branch}" | awk -F '"' '/"sha":/ {print substr($4, 1, 7); exit}')
...
download_source() {
    ...
    /usr/local/bin/curl -k -L -o "/bongosec.zip" "https://github.com/bongosec/bongosec/archive/${bongosec_branch}.zip"
    /usr/local/bin/unzip /bongosec.zip
    mv bongosec-* ${source_directory}
    compute_version_revision
}

check_version() {
    bongosec_version="v$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $source_directory/VERSION.json)"
...
compile() {
    ...
    config
    check_version
    gmake deps RESOURCES_URL=http://packages.bongosec.github.io/deps/${deps_version} TARGET=agent
...
show_help() {
    ...
    echo "    -r, --revision Revision to naming Bongosec agent package."
