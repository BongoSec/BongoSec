...
# Program to build the Bongosec App for OpenSearch Dashboards
...
BRANCH_TAG=""
CHECKSUMDIR=""
CONTAINER_NAME="bongosec-plugin-for-opensearch-dashboards-builder"
...
help() {
    echo -e ""
    echo -e "NAME"
    echo -e "        $(basename "$0") - Build Bongosec plugin files."
    echo -e ""
    echo -e "SYNOPSIS"
    ...
    echo -e ""
    echo -e "        -s, --store <path>"
    echo -e "                [Optional] Set the destination path of package, by default /tmp/bongosec-app."
    echo -e ""
...
build_package(){
    ...
    # Build the Bongosec plugin for OpenSearch Dashboards package using the build docker image
    docker run --rm -t -v "${OUTDIR}":/bongosec_app:Z \
