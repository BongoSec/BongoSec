...
setup_build(){
    ...
    cp -pr ${specs_path}/bongosec-${BUILD_TARGET}/debian ${sources_dir}/debian
...
get_package_and_checksum(){
    bongosec_version="$1"
    short_commit_hash="$2"
    base_name="bongosec-${BUILD_TARGET}_${bongosec_version}-${REVISION}"
    symbols_base_name="bongosec-${BUILD_TARGET}-dbg_${bongosec_version}-${REVISION}"
    ...
    if [[ "${checksum}" == "yes" ]]; then
        ...
        cd ${pkg_path} && sha512sum bongosec-${BUILD_TARGET}_*deb > /var/local/bongosec/${deb_file}.sha512
        sha512sum bongosec-${BUILD_TARGET}-*deb > /var/local/bongosec/${symbols_deb_file}.sha512
    fi

    find ${pkg_path} -type f -name "bongosec-${BUILD_TARGET}_*deb" -exec mv {} /var/local/bongosec/${deb_file} \;
    find ${pkg_path} -type f -name "bongosec-${BUILD_TARGET}-dbg_*deb" -exec mv {} /var/local/bongosec/${symbols_deb_file} \;
}
