...
build_package() {
  ...
  source_code="http://api.github.com/repos/bongosec/bongosec/tarball/${reference}"

  rm -f bongosec.tar.gz && curl -L ${source_code} -k -o bongosec.tar.gz -s
  rm -rf bongosec-bongosec-* bongosec-agent-*
  extracted_directory=$(gunzip -c bongosec.tar.gz | tar -xvf - | tail -n 1 | cut -d' ' -f2 | cut -d'/' -f1)
  bongosec_version=$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $extracted_directory/VERSION.json)

  # Add commit hash to the VERSION.json file
  pushd $extracted_directory
  short_commit_hash=$(curl -ks "https://api.github.com/repos/bongosec/bongosec/commits/${reference}" | awk -F '"' '/"sha":/ {print substr($4, 1, 7); exit}')
  ...
  cp -pr ${extracted_directory} bongosec-agent-${bongosec_version}
  ...
  package_name=bongosec-agent-${bongosec_version}
  tar cf ${package_name}.tar ${package_name} && gzip ${package_name}.tar
  mv ${package_name}.tar.gz ${rpm_build_dir}/SOURCES/

  cp ${current_path}/SPECS/bongosec-agent-aix.spec ${rpm_build_dir}/SPECS/${package_name}-aix.spec
  ...
  --define "_init_scripts ${init_scripts}" --define "_sysconfdir ${sysconfdir}" --define "_version ${bongosec_version}" \
  ...
  find /etc/ -name "*bongosec*" -exec rm {} \;
