/*
 * Bongosec Inventory Harvester - Network Interface element
...
public:
    ...
    static DataHarvester<InventoryNetIfaceHarvester> build(TContext* data)
    {
        ...
        {
            element.data.bongosec.cluster.name = instancePolicyManager.getClusterName();
            element.data.bongosec.cluster.node = instancePolicyManager.getClusterNodeName();
        }
