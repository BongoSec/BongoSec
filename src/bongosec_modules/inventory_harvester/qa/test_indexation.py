...
TEST_DATA_ROOT = Path("bongosec_modules/inventory_harvester/qa/test_data")
QUEUE_DIR = Path("queue")
TEMPLATE_ROOT = Path("bongosec_modules/inventory_harvester/indexer/template")
...
def discover_templates():
    ...
    return [path for path in TEMPLATE_ROOT.rglob("bongosec-states-*.json") if valid(path)]
...
def create_index_from_template(template_path: Path):
    ...
    if pattern and "*" in pattern:
        if "fim-files" in template_name:
            index_name = "bongosec-states-files-cluster01"
        elif "fim-registries" in template_name:
            index_name = "bongosec-states-registries-cluster01"
        else:
...
def opensearch():
    ...
    client.containers.run(
        ...
        environment={
            ...
            "OPENSEARCH_INITIAL_ADMIN_PASSWORD": "BongosecTest99$",
        },
        name="opensearch",
        stdout=True,
        stderr=True,
...
def test_data_indexation(opensearch, test_folder):
    ...
    is_bongosec_db = "bongosec_db" in str(test_path)

    cmd = (
        Path(
            "build/bongosec_modules/inventory_harvester/testtool/inventory_harvester_testtool"
        )
        if Path(
            "build/bongosec_modules/inventory_harvester/testtool/inventory_harvester_testtool"
        ).exists()
        else Path(
            "bongosec_modules/inventory_harvester/build/testtool/inventory_harvester_testtool"
        )
    )
    ...
    try:
        ...
        if is_bongosec_db:
            LOGGER.info("Loading templates manually for 'bongosec_db' test case...")
            load_all_templates()
