...
## How to use the tool
In order to run the `syscollector_test_tool` (located in `src/bongosec_modules/syscollector/build/bin` folder) utility the only step to be followed is just to execute the tool (without parameters):
```
