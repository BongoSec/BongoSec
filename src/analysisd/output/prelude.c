...
/** Bongosec to prelude severity mapping. **/
static const char *(bongosec2prelude_sev[]) = {"info", "info", "info", "info",
                               "low", "low", "low", "low",
...
add_idmef_object(idmef_message_t *msg, const char *object, const char *value)
{
    ...
    if (ret < 0) {
        merror("Bongosec2Prelude: IDMEF: Cannot add object '%s': %s.",
...
err:
    merror("Bongosec2Prelude: %s: IDMEF error: %s.",
...
void OS_PreludeLog(const Eventinfo *lf)
{
    ...
    if ( ret < 0 ) {
        merror("Bongosec2Prelude: Cannot create IDMEF message");
        return;
    }
    ...
    add_idmef_object(idmef, "alert.assessment.impact.severity",
                     ...
                     bongosec2prelude_sev[lf->generated_rule->level]);
    ...
    /* Begin Classification Infomations */
    {
        ...
        /* Check Bongosec rules for reference */
        if (lf->generated_rule->sigid) {
            ...
            add_idmef_object(idmef, "alert.classification.reference(-1).meaning", "Bongosec Ruleset");

            snprintf(_prelude_data, 256, "https://github.com/bongosec/bongosec/tree/master/ruleset");
         ...
         * that points back to Bongosec ruleset for more infomation.
         */
