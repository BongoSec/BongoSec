                              ...
                              void *d2)
{
    ...
    const char *oswmodule = "wodle";                            /* Wodle - Bongosec Module  */
    ...
    const char *osgcp_pub = "gcp-pubsub";                       /* Google Cloud PubSub - Bongosec Module */
    const char *osgcp_bucket = "gcp-bucket";                    /* Google Cloud Bucket - Bongosec Module */
    const char *wlogtest = "rule_test";                         /* Bongosec Logtest */
    const char *agent_upgrade = "agent-upgrade";                /* Agent Upgrade Module */
    const char *task_manager = "task-manager";                  /* Task Manager Module */
    const char *bongosec_db = "wdb";                               /* Bongosec-DB Daemon */
#ifndef WIN32
        ...
        if (chld_node && (strcmp(node[i]->element, osglobal) == 0)) {
            ...
            if ((modules & CWMODULE)) {
                mwarn(
                    ...
                    "See https://documentation.bongosec.github.io",
                    osvulndetector,
                    osvulndetection);
            ...
            if ((modules & CWMODULE) && !(modules & CAGENT_CONFIG) && (Read_AgentUpgrade(xml, node[i], d1) < 0)) {
                ...
                mwarn("%s configuration is only set in the manager.", node[i]->element);
            #endif
        }  else if (chld_node && (strcmp(node[i]->element, bongosec_db) == 0)) {
            #if !defined(CLIENT)
                if ((modules & BONGOSECDB) && (Read_BongosecDB(xml, chld_node) < 0)) {
                    goto fail;
                }
            #else
                mwarn("%s configuration is only set in the manager.", node[i]->element);
