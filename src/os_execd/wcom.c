...
size_t wcom_dispatch(char *command, char ** output) {
    ...
    } else if (strcmp(rcv_comm, "restart") == 0 || strcmp(rcv_comm, "restart-bongosec") == 0) {
...
size_t wcom_restart(char ** output) {
    ...
    if (lock <= 0) {
        ...
        if (access("active-response/bin/restart.sh", F_OK) == 0) {
            ...
            exec_cmd[0] = "bin/bongosec-control";
            exec_cmd[1] = "restart";
        }
        ...
        snprintf(command, sizeof(command), "%s/%s", AR_BINDIR, "restart-bongosec.exe");
        char *cmd[2] = { command, NULL };
        char *cmd_parameters = "{\"version\":1,\"origin\":{\"name\":\"\",\"module\":\"bongosec-execd\"},\"command\":\"add\",\"parameters\":{\"extra_args\":[],\"alert\":{},\"program\":\"restart-bongosec.exe\"}}";
...
size_t wcom_check_manager_config(char **output) {
    static const char *daemons[] = {"bin/bongosec-authd", "bin/bongosec-remoted",
                                    "bin/bongosec-execd", "bin/bongosec-analysisd", "bin/bongosec-logcollector",
                                    "bin/bongosec-integratord", "bin/bongosec-syscheckd", "bin/bongosec-maild",
                                    "bin/bongosec-modulesd", "bin/bongosec-clusterd", "bin/bongosec-agentlessd",
                                    "bin/bongosec-integratord", "bin/bongosec-dbd", "bin/bongosec-csyslogd", NULL
                                    };
