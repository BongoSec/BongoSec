...
void AgentdStart(int uid, int gid, const char *user, const char *group)
{
    ...
    if (agt->force_reconnect_interval) {
        minfo("Using force reconnect interval, Bongosec Agent will reconnect every %ld %s", w_seconds_to_time_value(agt->force_reconnect_interval), w_seconds_to_time_unit(agt->force_reconnect_interval, TRUE));
    }
