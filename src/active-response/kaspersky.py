#!/usr/bin/env python
 ...
 ###
 # Integration of Bongosec agent with Kaspersky endpoint security for Linux
...
# Sets the file paths
##################################################################################################################
bongosec_path = os.path.abspath(os.path.join(__file__, "../../.."))
bongosec_queue = '{0}/queue/sockets/queue'.format(bongosec_path)
now = time.strftime("%a %b %d %H:%M:%S %Z %Y")
ar_log_file = '{0}/logs/active-responses.log'.format(bongosec_path)
...
def send_msg(bongosec_queue, msg):
	...
	try:
		s.connect(bongosec_queue)
	except:
		print('Error: Bongosec must be running.')
		sys.exit(1)
...
def main():
    ar_log()
    set_logger('bongosec-kaspersky', foreground=args.verbose)
    run_kaspersky()
