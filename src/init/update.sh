...
# Shell script update functions for Bongosec
...
##########
# Checks if Bongosec is installed by taking the installdir from the services
...
getPreinstalledDirByType()
{
    ...
    if hash ps 2>&1 > /dev/null && hash grep 2>&1 > /dev/null && [ -n "$(ps -e | egrep ^\ *1\ .*systemd$)" ]; then
        ...
        SED_EXTRACT_PREINSTALLEDDIR="s/^ExecStart=\/usr\/bin\/env \(.*\)\/bin\/bongosec-control start$/\1/p"

        if [ "X$pidir_service_name" = "Xbongosec-manager" ] || [ "X$pidir_service_name" = "Xbongosec-local" ]; then #manager, hibrid or local
            type="manager"
        else
            type="agent"
        fi

        # Get the unit file and extract the Bongosec home path
        PREINSTALLEDDIR=$(systemctl cat bongosec-${type}.service 2>/dev/null | sed -n "${SED_EXTRACT_PREINSTALLEDDIR}")
        ...
        if [ -f /usr/lib/systemd/system/bongosec-${type}.service ]; then
            SERVICE_UNIT_PATH=/usr/lib/systemd/system/bongosec-${type}.service
        fi
        # Others
        if [ -f /etc/systemd/system/bongosec-${type}.service ]; then
            SERVICE_UNIT_PATH=/etc/systemd/system/bongosec-${type}.service
        fi
    ...
    if [ -r "/etc/redhat-release" ]; then
        if [ -d /etc/rc.d/init.d ]; then
            if [ -f /etc/rc.d/init.d/${pidir_service_name} ]; then
                PREINSTALLEDDIR=`sed -n 's/^BONGOSEC_HOME=\(.*\)$/\1/p' /etc/rc.d/init.d/${pidir_service_name}`
                if [ -d "$PREINSTALLEDDIR" ]; then
                    return 0;
                else
                    return 1;
    ...
    if [ -r "/etc/gentoo-release" ]; then
        if [ -f /etc/init.d/${pidir_service_name} ]; then
            PREINSTALLEDDIR=`sed -n 's/^BONGOSEC_HOME=\(.*\)$/\1/p' /etc/init.d/${pidir_service_name}`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
    ...
    if [ -r "/etc/slackware-version" ]; then
        if [ -f /etc/rc.d/rc.${pidir_service_name} ]; then
            PREINSTALLEDDIR=`sed -n 's/^BONGOSEC_HOME=\(.*\)$/\1/p' /etc/rc.d/rc.${pidir_service_name}`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
    ...
    if [ "X${NUNAME}" = "XDarwin" ]; then
        if [ -f /Library/StartupItems/BONGOSEC/BONGOSEC ]; then
            PREINSTALLEDDIR=`sed -n 's/^ *//; s/^\s*\(.*\)\/bin\/bongosec-control start$/\1/p' /Library/StartupItems/BONGOSEC/BONGOSEC`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
    ...
    if [ "X${UN}" = "XHP-UX" ]; then
        if [ -f /sbin/init.d/${pidir_service_name} ]; then
            PREINSTALLEDDIR=`sed -n 's/^BONGOSEC_HOME=\(.*\)$/\1/p' /sbin/init.d/${pidir_service_name}`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
    ...
    if [ "X${UN}" = "XOpenBSD" -o "X${UN}" = "XNetBSD" -o "X${UN}" = "XFreeBSD" -o "X${UN}" = "XDragonFly" ]; then
        # Checking for the presence of bongosec-control on rc.local
        grep bongosec-control /etc/rc.local > /dev/null 2>&1
        if [ $? = 0 ]; then
            PREINSTALLEDDIR=`sed -n 's/^\(.*\)\/bin\/bongosec-control start$/\1/p' /etc/rc.local`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
        ...
        if [ -e "/etc/rc.d/rc.local" ]; then
            grep bongosec-control /etc/rc.d/rc.local > /dev/null 2>&1
            if [ $? = 0 ]; then
                PREINSTALLEDDIR=`sed -n 's/^\(.*\)\/bin\/bongosec-control start$/\1/p' /etc/rc.d/rc.local`
                if [ -d "$PREINSTALLEDDIR" ]; then
                    return 0;
                else
                    return 1;
...
##########
# Checks if Bongosec is installed in the specified path by searching for the control binary.
#
# isBongosecInstalled()
##########
isBongosecInstalled()
{
    if [ -f "${1}/bin/bongosec-control" ]; then
        return 0;
...
##########
# Checks if Bongosec is installed by trying with each installation type.
# If it finds an installation, it sets the PREINSTALLEDDIR variable.
# After that it checks if Bongosec is truly installed there, if it is installed it returns TRUE.
# If it isn't installed continue searching in other installation types and replacing PREINSTALLEDDIR variable.
# It returns FALSE if Bongosec isn't installed in any of this.
#
# getPreinstalledDir()
##########
getPreinstalledDir()
{
    # Checking ossec-init.conf for old bongosec versions
    if [ -f "${OSSEC_INIT}" ]; then
        . ${OSSEC_INIT}
        if [ -d "$DIRECTORY" ]; then
            PREINSTALLEDDIR="$DIRECTORY"
            if isBongosecInstalled $PREINSTALLEDDIR; then
                return 0;
            fi
        fi
    fi

    # Getting preinstalled dir for Bongosec manager and hibrid installations
    pidir_service_name="bongosec-manager"
    if getPreinstalledDirByType && isBongosecInstalled $PREINSTALLEDDIR; then
        return 0;
    fi

    # Getting preinstalled dir for Bongosec agent installations
    pidir_service_name="bongosec-agent"
    if getPreinstalledDirByType && isBongosecInstalled $PREINSTALLEDDIR; then
        return 0;
    fi

    # Getting preinstalled dir for Bongosec local installations
    pidir_service_name="bongosec-local"
    if getPreinstalledDirByType && isBongosecInstalled $PREINSTALLEDDIR; then
        return 0;
    fi
...
getPreinstalledType()
{
    ...
    if [ -f "${OSSEC_INIT}" ]; then
        ...
        TYPE=`$PREINSTALLEDDIR/bin/bongosec-control info -t`
    fi
...
getPreinstalledVersion()
{
    ...
    if [ -f "${OSSEC_INIT}" ]; then
        ...
        VERSION=`$PREINSTALLEDDIR/bin/bongosec-control info -v`
    fi
...
{
    NAME=""
    # Checking ossec-init.conf for old bongosec versions. New versions
...
UpdateStartOSSEC()
{
    ...
    if [ `stat /proc/1/exe 2> /dev/null | grep "systemd" | wc -l` -ne 0 ]; then
        ...
        $INSTALLDIR/bin/bongosec-control start
    fi
}
...
UpdateStopOSSEC()
{
    ...
    if [ `stat /proc/1/exe 2> /dev/null | grep "systemd" | wc -l` -ne 0 ]; then
        systemctl stop bongosec-$TYPE
    elif [ `stat /proc/1/exe 2> /dev/null | grep "init.d" | wc -l` -ne 0 ]; then
        service bongosec-$TYPE stop
    fi

    # Make sure Bongosec is stopped
    if [ "X$PREINSTALLEDDIR" = "X" ]; then
        getPreinstalledDir
    fi
    ...
    if [ -f "$PREINSTALLEDDIR/bin/ossec-control" ]; then
        ...
        $PREINSTALLEDDIR/bin/bongosec-control stop > /dev/null 2>&1
    fi
   ...
   # We also need to remove all syscheck queue file (format changed)
    ...
    # Deleting plain-text agent information if exists (it was migrated to Bongosec DB in v4.1)
    if [ -d "$PREINSTALLEDDIR/queue/agent-info" ]; then
        rm -rf $PREINSTALLEDDIR/queue/agent-info > /dev/null 2>&1
    fi

    # Deleting plain-text rootcheck information if exists (it was migrated to Bongosec DB in v4.1)
...
UpdateOldVersions()
{
    if [ "$INSTYPE" = "server" ]; then
        ...
        DEPRECATED=`cat ./src/init/bongosec/deprecated_ruleset.txt`
    ...
    # If it is Bongosec 2.0 or newer, exit
    if [ "X$USER_OLD_NAME" = "XBongosec" ]; then
        return
    fi
    ...
    if [ ! "$INSTYPE" = "agent" ]; then
        ...
        chown root:bongosec $BACKUP_RULESET > /dev/null 2>&1

        # Backup decoders: Bongosec v1.0.1 to v1.1.1
        old_decoders="ossec_decoders bongosec_decoders"
        ...
        # Backup decoders: Bongosec v1.0 and OSSEC
