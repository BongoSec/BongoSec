...
if [[ "$OS" == "Darwin" ]]; then
    if [ "${BONGOSEC_HOME}" != "/Library/Ossec" ]; then
    ...
    if [ "${BONGOSEC_HOME}" != "/var/ossec" ]; then
...
if [[ "$OS" == "Darwin" ]]; then
    installer -pkg ./var/upgrade/bongosec-agent* -target / >> ./logs/upgrade.log 2>&1
elif [[ "$OS" == "Linux" ]]; then
    if find ./var/upgrade/ -mindepth 1 -maxdepth 1 -type f -name "*.rpm" | read; then
        if command -v rpm >/dev/null 2>&1; then
            rpm -UFvh ./var/upgrade/bongosec-agent* >> ./logs/upgrade.log 2>&1
        else
        ...
        if command -v dpkg >/dev/null 2>&1; then
            dpkg -i --force-confdef ./var/upgrade/bongosec-agent* >> ./logs/upgrade.log 2>&1
        else
        ...
        if command -v apk >/dev/null 2>&1; then
            apk add --allow-untrusted --force ./var/upgrade/bongosec-agent* >> ./logs/upgrade.log 2>&1
        else
...
# Restart Agent
echo "$(date +"%Y/%m/%d %H:%M:%S") - Checking for Bongosec Agent control script." >> ./logs/upgrade.log

if [ -f "./bin/bongosec-control" ]; then
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Restarting Bongosec Agent." >> ./logs/upgrade.log
    ./bin/bongosec-control restart >> ./logs/upgrade.log 2>&1
elif [ -f "./bin/ossec-control" ]; then
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Upgrade failed: bongosec-control not found. Attempting to restart using ossec-control." >> ./logs/upgrade.log
    ./bin/ossec-control restart >> ./logs/upgrade.log 2>&1
    echo -ne "2" > ./var/upgrade/upgrade_result
    rm -f $LOCK
    exit 1
else
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Upgrade failed: Neither bongosec-control nor ossec-control were found." >> ./logs/upgrade.log
    echo -ne "2" > ./var/upgrade/upgrade_result
    rm -f $LOCK
    exit 1
fi
...
while [ "$status" != "connected" -a $COUNTER -gt 0 ]; do
    . ./var/run/bongosec-agentd.state >> ./logs/upgrade.log 2>&1
