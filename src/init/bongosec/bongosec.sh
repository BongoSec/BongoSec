...
# Install functions for Bongosec
# bongosec.github.io (https://github.com/bongosec)
...
InstallSELinuxPolicyPackage(){
    ...
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/bongosec.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/bongosec.pp /tmp && semodule -i /tmp/bongosec.pp
                rm -f /tmp/bongosec.pp
                semodule -e bongosec
            fi
        fi
    fi
...
BongosecUpgrade()
{
    ...
    if [ -f "$PREINSTALLEDDIR/var/db/global.db" ]; then
        ...
        if [ -f "$PREINSTALLEDDIR/queue/db/global.db" ]; then
            ...
            chown bongosec:bongosec $PREINSTALLEDDIR/queue/db/global.db
    ...
    # Remove existing SQLite databases for Bongosec DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/bongosec
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated Bongosec tools
    ...
    if (grep "^ossec:" /etc/group > /dev/null 2>&1) || (dscl . -read /Groups/ossec > /dev/null 2>&1)  ; then
        if [ "X$1" = "Xserver" ]; then
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -print0 | xargs -0 chown root:bongosec
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -print0 | xargs -0 chown bongosec:bongosec
        else
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -exec chown root:bongosec {} \;
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -exec chown bongosec:bongosec {} \;
        fi
    fi
