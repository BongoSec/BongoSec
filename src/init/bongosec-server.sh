...
# bongosec-control        This shell script takes care of starting
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting Bongosec $VERSION..."
    fi

    TEST=$(${DIR}/bin/bongosec-logtest-legacy -t  2>&1 | grep "ERROR")
    if [ ! -z "$TEST" ]; then
        if [ $USE_JSON = true ]; then
        ...
        touch ${DIR}/var/run/bongosec-analysisd.failed
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ## If bongosec-maild is disabled, don't try to start it.
        if [ X"$i" = "Xbongosec-maild" ]; then
             grep "<email_notification>no<" ${DIR}/etc/ossec.conf >/dev/null 2>&1
             if [ $? = 0 ]; then
                 continue
             fi
        fi
        ## If bongosec-clusterd is disabled, don't try to start it.
        if [ X"$i" = "Xbongosec-clusterd" ]; then
        ...
        ## If bongosec-authd is disabled, don't try to start it.
        if [ X"$i" = "Xbongosec-authd" ]; then
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ]; then
                    echo "${pfile}: Process ${pid} not used by Bongosec, removing..."
                fi
...
info()
{
    if [ "X${1}" = "X" ]; then
        if [ $USE_JSON = true ]; then
            echo -n '{"error":0,"data":['
            echo -n '{"BONGOSEC_VERSION":"'${VERSION}'"},'
            echo -n '{"BONGOSEC_REVISION":"'${REVISION}'"},'
            echo -n '{"BONGOSEC_TYPE":"'${TYPE}'"}'
            echo -n ']}'
        else
            echo "BONGOSEC_VERSION=\"${VERSION}\""
            echo "BONGOSEC_REVISION=\"${REVISION}\""
            echo "BONGOSEC_TYPE=\"${TYPE}\""
        fi
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/bongosec-execd//')
    restart_service
    ;;
status)
    lock
