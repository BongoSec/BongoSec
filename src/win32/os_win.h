...
/* Install the BONGOSEC-HIDS agent service */
int InstallService(char *path);

/* Uninstall the BONGOSEC-HIDS agent service */
int UninstallService();

/* Check if the BONGOSEC-HIDS agent service is running
 * Returns 1 on success (running) or 0 if not running
 */
int CheckServiceRunning();

/* Start BONGOSEC-HIDS service */
int os_start_service();

/* Stop BONGOSEC-HIDS service */
int os_stop_service();
