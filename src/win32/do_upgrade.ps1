...
function remove_upgrade_files {
    ...
    Remove-Item -Path ".\bongosec-agent*.msi" -ErrorAction SilentlyContinue
    Remove-Item -Path ".\do_upgrade.ps1" -ErrorAction SilentlyContinue
}


function get_bongosec_installation_directory {
    Start-NativePowerShell {
        $path1 = "HKLM:\SOFTWARE\WOW6432Node\Bongosec, Inc.\Bongosec Agent"
        $key1 = "BongosecInstallDir"

        $path2 = "HKLM:\SOFTWARE\WOW6432Node\ossec"
        $key2 = "Install_Dir"

        $BongosecInstallDir = $null

        try {
            $BongosecInstallDir = (Get-ItemProperty -Path $path1 -ErrorAction SilentlyContinue).$key1
        }
        catch {
            $BongosecInstallDir = $null
        }

        if ($null -eq $BongosecInstallDir) {
            try {
                $BongosecInstallDir = (Get-ItemProperty -Path $path2 -ErrorAction SilentlyContinue).$key2
            }
            catch {
                $BongosecInstallDir = $null
            }
        ...
        if ($null -eq $BongosecInstallDir) {
            Write-output "$(Get-Date -format u) - Couldn't find Bongosec in the registry. Upgrade will assume current path is correct" >> .\upgrade\upgrade.log
            $BongosecInstallDir = (Get-Location).Path.TrimEnd('\')
        }

        return $BongosecInstallDir
    }
}

# Check process status
function check-process {
    $process_id = (Get-Process bongosec-agent).id
    $counter = 10
    while($process_id -eq $null -And $counter -gt 0) {
        $counter--
        Start-Service -Name "Bongosec"
        Start-Sleep 2
        $process_id = (Get-Process bongosec-agent).id
    }
...
# Check new version and restart the Bongosec service
function check-installation {
    $actual_version = get-version
    $counter = 5
    while($actual_version -eq $current_version -And $counter -gt 0) {
        write-output "$(Get-Date -format u) - Waiting for the Bongosec-Agent installation to end." >> .\upgrade\upgrade.log
        $counter--
        Start-Sleep 2
        $actual_version = get-version
    }
    write-output "$(Get-Date -format u) - Starting Bongosec-Agent service." >> .\upgrade\upgrade.log
    Start-Service -Name "Bongosec"
}

# Function to extract the version from the MSI using msiexec
function get_msi_version {
    $msiPath = (Get-Item ".\bongosec-agent*.msi").FullName
...
function install {
    ...
    try {
        $msiPath = (Get-Item ".\bongosec-agent*.msi").Name
...
# Check that the Bongosec installation runs on the expected path
$bongosecDir = get_bongosec_installation_directory
$normalizedBongosecDir = $bongosecDir.TrimEnd('\')
$currentDir = (Get-Location).Path.TrimEnd('\')

if ($normalizedBongosecDir -ne $currentDir) {
    Write-Output "$(Get-Date -format u) - Current working directory is not the Bongosec installation directory. Aborting." >> .\upgrade\upgrade.log
...
function Get-AgentStatus {
    Select-String -Path '.\bongosec-agent.state' -Pattern "^status='(.+)'" | %{$_.Matches[0].Groups[1].value}
}
