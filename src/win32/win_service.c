...
static LPTSTR g_lpszServiceName        = "BongosecSvc";
static LPTSTR g_lpszServiceDisplayName = "Bongosec";
static LPTSTR g_lpszServiceDescription = "Bongosec Windows Agent";
...
int os_stop_service()
{
    ...
    * newer instances of bongosec-agent.
    */
    ...
    if (ossecServiceStatusHandle) {
        switch (dwOpcode) {
            case SERVICE_CONTROL_STOP:
                ...
                // Kill children processes spawned by modules, only in bongosec-agent
                wm_kill_children();
                stop_wmodules();
