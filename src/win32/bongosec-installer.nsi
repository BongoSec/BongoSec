...
; output file
!ifndef OutFile
    !define OutFile "bongosec-agent-${VERSION}.exe"
!endif
...
VIAddVersionKey CompanyName "Bongosec Inc."
VIAddVersionKey LegalCopyright "2023 - Bongosec Inc."
VIAddVersionKey FileDescription "Bongosec Agent installer"
VIAddVersionKey FileVersion "${VERSION}"
VIAddVersionKey ProductVersion "${VERSION}"
VIAddVersionKey InternalName "Bongosec Agent"
...
Section "Bongosec Agent (required)" MainSec
    ...
    ; install files
    File bongosec-agent.exe
    File bongosec-agent-eventchannel.exe
    ...
    File /oname=active-response\bin\restart-bongosec.exe restart-bongosec.exe
    ...
    File /oname=libbongosecext.dll ..\libbongosecext.dll
    File /oname=libbongosecshared.dll ..\libbongosecshared.dll
    ...
    File /oname=syscollector.dll ..\bongosec_modules\syscollector\build\bin\syscollector.dll
    File /oname=libfimdb.dll ..\syscheckd/build/bin/libfimdb.dll
    File /oname=queue\syscollector\norm_config.json ..\bongosec_modules\syscollector\norm_config.json
    File VERSION.json
    ...
    ; use appropriate version of "bongosec-agent.exe"
    ${If} ${AtLeastWinVista}
        Delete "$INSTDIR\bongosec-agent.exe"
        Rename "$INSTDIR\bongosec-agent-eventchannel.exe" "$INSTDIR\bongosec-agent.exe"
    ${Else}
        Delete "$INSTDIR\bongosec-agent-eventchannel.exe"
    ${Endif}
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "Publisher" "Bongosec, Inc."
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "DisplayIcon" '"$INSTDIR\favicon.ico"'
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "HelpLink" "https://bongosec.github.io"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "URLInfoAbout" "https://bongosec.github.io"
    ...
    ServiceInstall:
        nsExec::ExecToLog '"$INSTDIR\bongosec-agent.exe" install-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
...
Section "Uninstall"
    ...
    ServiceUninstall:
        nsExec::ExecToLog '"$INSTDIR\bongosec-agent.exe" uninstall-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
