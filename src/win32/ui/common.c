...
int gen_server_info(HWND hwnd)
{
    ...
    /* Set status data */
    SendMessage(hStatus, SB_SETTEXT, 0, (LPARAM)"https://bongosec.github.io");
...
int config_read(__attribute__((unused)) HWND hwnd)
{
    ...
    if ((tmp_str = read_version_file_field("version"), tmp_str))
    {
        ...
        if (tmp_str[0] != 'v') {
            ...
            snprintf(buffer, sizeof(buffer), "Bongosec %s", prefixed_version);
        } else {
            snprintf(buffer, sizeof(buffer), "Bongosec %s", tmp_str);
        }
