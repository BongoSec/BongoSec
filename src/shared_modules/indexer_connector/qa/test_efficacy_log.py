...
def test_initialize_indexer_connector(opensearch):
    ...
    while counter < 10:
        ...
        response = requests.get(url)
        if response.status_code == 200 and 'bongosec-states-vulnerabilities-default' in response.text:
...
def test_add_bulk_indexer_connector(opensearch):
    ...
    counter = 0
    while counter < 10:
        url = 'http://'+GLOBAL_URL+'/bongosec-states-vulnerabilities-default/_search'
        query = {
            "query": {
                "match_all": {}
            }
    ...
    url = 'http://'+GLOBAL_URL+'/bongosec-states-vulnerabilities-default/_delete_by_query?refresh=true'
    query = {
        "query": {
            "match_all": {}
        }
    ...
    url = 'http://'+GLOBAL_URL+'/bongosec-states-vulnerabilities-cluster/_doc/000_pkghash_CVE-2022-123456?refresh=true'
    query = """{
      "agent": {
        "build": {
...
def test_update_mappings_connector(opensearch):
    ...
    # -------------------------------------------
    # Create the index and try to insert a document with old mapping, this should fail and the elemente queue in the persistent rocksdb queue.
    # -------------------------------------------
    args = ["-c", "shared_modules/indexer_connector/qa/test_data/" + test_name + "/config.json",
            "-t", "shared_modules/indexer_connector/qa/test_data/" + test_name + "/template.json",
    ...
    url = 'http://' + GLOBAL_URL + '/bongosec-states-vulnerabilities-default/'
    response = requests.get(url)
    if response.status_code == 200:
