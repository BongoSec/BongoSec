...
find_library(BONGOSECEXT NAMES bongosecext HINTS "${SRC_FOLDER}")
set(uname "Win32")

if(NOT BONGOSECEXT)
  message(FATAL_ERROR "BONGOSECEXT is set to '${BONGOSECEXT}', but did not find any file matching ${SRC_FOLDER}/${CMAKE_FIND_LIBRARY_PREFIXES}bongosecext${CMAKE_FIND_LIBRARY_SUFFIXES}")
  message(FATAL_ERROR "libbongosecext not found in ${SRC_FOLDER} Aborting...")
endif()

# Find the bongosec sysinfo library
...
add_definitions(-DTEST_WINAGENT -DDEBUG -DENABLE_AUDIT -D_WIN32_WINNT=0x600 -DBONGOSEC_UNIT_TESTING)
...
target_link_libraries(DEPENDENCIES_O ${BONGOSECLIB} ${BONGOSECEXT} ${PTHREAD} ${SYSINFO} ${STATIC_CMOCKA} wsock32 wevtapi shlwapi comctl32 advapi32 kernel32 psapi gdi32 iphlpapi ws2_32 crypt32 wintrust)

# Set tests dependencies
# Use --start-group and --end-group to handle circular dependencies
set(TEST_DEPS -Wl,--start-group ${BONGOSECLIB} ${BONGOSECEXT} ${SYSINFO} DEPENDENCIES_O -Wl,--end-group ${PTHREAD} ${STATIC_CMOCKA} wsock32 wevtapi shlwapi comctl32 advapi32 kernel32 psapi gdi32 iphlpapi ws2_32 crypt32 -fprofile-arcs -ftest-coverage)
set(TEST_EVENT_DEPS -Wl,--start-group ${BONGOSECLIB} ${BONGOSECEXT} ${SYSINFO} DEPENDENCIES_O -Wl,--end-group ${PTHREAD} ${STATIC_CMOCKA} wsock32 wevtapi shlwapi comctl32 advapi32 kernel32 psapi gdi32 iphlpapi ws2_32 crypt32 -fprofile-arcs -ftest-coverage)
