...
Each `test suite` uses a [regex_matching](https://github.com/bongosec/bongosec/blob/v4.3.5/src/os_regex/os_regex.h#L45-L49) structure, which is initially empty and is shared between its `unit tests`, allowing to test the memory usage. These suites are JSON objects with the following structure:
...
```json
{
  ...
  "log": "Some pattern in a Bongosec log.",
  "end_match": " log.",
  "captured_groups": [
    "Bongosec"
  ]
