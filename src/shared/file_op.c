...
#ifndef WIN32
/**
 * @brief Get the Bongosec installation directory
 *
 * It is obtained from the /proc directory, argv[0], or the env variable BONGOSEC_HOME
 *
 * @param arg ARGV0 - Program name
 * @return Pointer to the Bongosec installation path on success
 */
char *w_homedir(char *arg) {
    ...
    if (realpath("/proc/self/exe", buff) != NULL) {
        ...
        // The path was not found so read BONGOSEC_HOME env var
        char * home_env = NULL;
        if (home_env = getenv(BONGOSEC_HOME_ENV), home_env) {
