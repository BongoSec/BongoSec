...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Pytest Coverage (Bongosec)')
    parser.add_argument('-p', '--path', dest='bongosec_path', action='store', required=True,
                        help='Path to the Bongosec repository.')
...
def main():
    ...
    bongosec_path = arguments.bongosec_path

    if not exists(bongosec_path):
        print(f'Base Bongosec path is not valid: {bongosec_path}')
        exit(1)

    for module in PYTHON_MODULES:
        current_module_path = join(bongosec_path, module)

        if not exists(current_module_path):
            print(f'Bongosec module path is not valid: {current_module_path}')
            exit(1)
