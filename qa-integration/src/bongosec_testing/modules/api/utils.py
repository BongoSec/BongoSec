...
from bongosec_testing.constants.api import BONGOSEC_API_PROTOCOL, BONGOSEC_API_HOST, BONGOSEC_API_PORT, BONGOSEC_API_USER, \
                                        BONGOSEC_API_PASSWORD, LOGIN_ROUTE, USERS_ROUTE, RESOURCE_ROUTE_MAP, \
                                        TARGET_ROUTE_MAP
from bongosec_testing.constants.paths.api import BONGOSEC_API_CERTIFICATE
from bongosec_testing.utils.file import read_json_file
...
def get_base_url(protocol: str = BONGOSEC_API_PROTOCOL, host: str = BONGOSEC_API_HOST, port: str = BONGOSEC_API_PORT) -> str:
    """Get Bongosec API's base URL.
    ...
    Returns:
        base_url (str): Bongosec API's base URL
    """
...
def set_authorization_header(user: str = None, password: str = None) -> dict:
    ...
    """
    user = BONGOSEC_API_USER if user is None else user
    password = BONGOSEC_API_PASSWORD if password is None else password
...
def login(user: str = BONGOSEC_API_USER, password: str = BONGOSEC_API_PASSWORD,
          timeout: int = session_parameters.default_timeout, login_attempts: int = 3, backoff_factor: float = 0.5,
          host: str = BONGOSEC_API_HOST, port: str = BONGOSEC_API_PORT, protocol: str = BONGOSEC_API_PROTOCOL
    ...
    response = session.post(url, headers=set_authorization_header(user, password), verify=BONGOSEC_API_CERTIFICATE,
                                timeout=timeout)
...
def allow_user_to_authenticate(user_id: str = None) -> requests.Response:
    ...
    response = requests.put(url + params, headers=authentication_headers, verify=BONGOSEC_API_CERTIFICATE)
...
def manage_security_resources(method: str = 'get', resource: Union[dict, str] = None,
    ...
    authentication_headers = login()[0]
    response = getattr(requests, method)(url, headers=authentication_headers, verify=BONGOSEC_API_CERTIFICATE, json=payload)
...
def relate_resources(test_metadata: dict) -> None:
    ...
    for origin in relationships:
        ...
        for idx, target_value in enumerate(target_values):
            ...
            response = requests.post(url, headers=login()[0], verify=BONGOSEC_API_CERTIFICATE)
            if response.status_code != 200 or response.json()['error'] != 0:
...
def remove_resources_relationship(origin_resource: dict = None, target_resource: dict = None) -> dict:
    ...
    response = requests.delete(url, headers=login()[0], verify=BONGOSEC_API_CERTIFICATE)
    if response.status_code != 200 or response.json()['error'] != 0:
...
def compare_config_api_response(configuration, section):
    ...
    Args:
        configuration (dict): Dictionary with Bongosec manager configuration.
...
def get_manager_configuration(section=None, field=None):
    ...
    Args:
        section (str): bongosec configuration section, E.g: "active-response", "ruleset"...
        field   (str): section child. E.g, fields for ruleset section are: decoder_dir, rule_dir, etc

    Returns:
        `obj`(str or map): active configuration indicated by Bongosec API. If section and field are selected, it will
    ...
    assert response.json()['error'] == 0, f"Bongosec API response status different from 0: {response.json()}"
...
def get_api_details_dict(protocol=BONGOSEC_API_PROTOCOL, host=BONGOSEC_API_HOST, port=BONGOSEC_API_PORT, user=BONGOSEC_API_USER, password=BONGOSEC_API_PASSWORD,
