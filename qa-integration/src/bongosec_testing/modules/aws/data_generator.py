...
from bongosec_testing.constants.aws import *
from bongosec_testing.utils.random import get_random_ip, get_random_port, get_random_string
...
class ServerAccessDataGenerator(DataGenerator):
    ...
    def get_data_sample(self):
        ...
        for _ in range(5):
            data.append(
                [
                    str(uuid4()), 'bongosec-server-access-integration-tests',
                    self.date.strftime('[%d/%b/%Y:%H:%M:%S %z]'), get_random_ip(),
                    f"arn:aws:iam::{RANDOM_ACCOUNT_ID}:user/fake.user", get_random_string(16).upper(),
                    'REST.GET.WEBSITE', '-', 'GET, /bongosec-server-access-integration-tests?website= HTTP/1.1',
