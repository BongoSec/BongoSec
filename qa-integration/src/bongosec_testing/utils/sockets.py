...
from bongosec_testing.constants.paths.sockets import QUEUE_SOCKETS_PATH, BONGOSEC_DB_SOCKET_PATH, \
                                                  MODULESD_C_INTERNAL_SOCKET_PATH, \
                                                  ACTIVE_RESPONSE_SOCKET_PATH
from bongosec_testing.utils.network import UDP


def delete_sockets(path=None):
    """Delete a list of Bongosec socket files or all of them if None is specified.
    ...
    try:
        if path is None:
            ...
            if os.path.exists(BONGOSEC_DB_SOCKET_PATH):
                os.remove(BONGOSEC_DB_SOCKET_PATH)
...
def send_request_socket(query, socket_path=BONGOSEC_DB_SOCKET_PATH):
...
def send_message_to_syslog_socket(message, port, protocol, manager_address="127.0.0.1"):
    """Send a message to the syslog server of bongosec-remoted.
