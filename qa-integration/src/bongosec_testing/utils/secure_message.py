...
def pack(data, format_: str = "<I") -> bytes:
    """Pack data with a given header. Using Bongosec header by default.
...
def unpack(data, format_: str = "<I") -> bytes:
    """Unpack data with a given header. Using Bongosec header by default.
...
def get_agent_id(message: bytes) -> Union[str, None]:
    """
    Extracts the agent ID from a given Bongosec message.

    In Bongosec, the agent ID is sent within the message, enclosed between
...
def get_payload(message: bytes, algorithm: str) -> bytes:
    """
    Extracts the payload from a given Bongosec message.
...
def set_algorithm_header(message: bytes, algorithm: str) -> bytes:
    """
    Adds an encryption algorithm header to a given Bongosec message.
