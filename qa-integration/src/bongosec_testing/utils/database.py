...
from bongosec_testing.constants.paths.sockets import QUEUE_DB_PATH, BONGOSEC_DB_SOCKET_PATH
from bongosec_testing.tools.db_administrator import DatabaseAdministrator
from bongosec_testing.utils import services, secure_message


def delete_dbs():
    """Delete all bongosec-db databases."""
...
def query_wdb(command, response_only=True, multiple_responses=False) -> List[str]:
    """Make queries to bongosec-db using the wdb socket.

    Args:
        command (str): bongosec-db command alias. For example `global get-agent-info 000`.

    Returns:
        list: Query response data.
    """
    # If the wdb socket is not yet up, then wait or restart bongosec-db
    if not os.path.exists(BONGOSEC_DB_SOCKET_PATH):
        max_retries = 6
        for _ in range(2):
            retry = 0
            # Wait if the wdb socket is not still alive (due to bongosec-db restarts). Max 3 seconds
            while not os.path.exists(BONGOSEC_DB_SOCKET_PATH) and retry < max_retries:
                time.sleep(0.5)
                retry += 1

            # Restart bongosec-db in case of wdb socket is not yet up.
            if not os.path.exists(BONGOSEC_DB_SOCKET_PATH):
                services.control_service('restart', daemon=BONGOSEC_DB_DAEMON)

        # Raise custom exception if the socket is not up in the expected time, even restarting bongosec-db
        if not os.path.exists(BONGOSEC_DB_SOCKET_PATH):
            raise Exception('The wdb socket is not up. bongosec-db was restarted but the socket was not found')

    # Create and open the socket connection with bongosec-db socket
    sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    sock.connect(BONGOSEC_DB_SOCKET_PATH)
    data = []
...
def make_sqlite_query(db_path: str, query_list: List[str]) -> None:
    ...
    """
    services.control_service('stop', daemon=BONGOSEC_DB_DAEMON)
    ...
    try:
        ...
        services.control_service('start', daemon=BONGOSEC_DB_DAEMON)
