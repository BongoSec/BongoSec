...
from bongosec_testing.constants.daemons import CLUSTER_DAEMON, API_DAEMON, BONGOSEC_AGENT, BONGOSEC_MANAGER, BONGOSEC_AGENT_WIN
from bongosec_testing.constants.paths.binaries import BIN_PATH, BONGOSEC_CONTROL_PATH
from bongosec_testing.constants.paths.sockets import BONGOSEC_SOCKETS, BONGOSEC_OPTIONAL_SOCKETS
from bongosec_testing.constants.paths.variables import VAR_RUN_PATH, VERSION_FILE
from bongosec_testing.constants.platforms import MACOS, SOLARIS, WINDOWS
...
def get_service() -> str:
    """
    Retrieves the name of the Bongosec service running on the current platform.

    Returns:
        str: The name of the Bongosec service. It can be either 'bongosec-manager'
             or 'bongosec-agent'.
    ...
    if platform.system() in ['Windows', WINDOWS]:
        return BONGOSEC_AGENT

    else:  # Linux, sunos5, darwin, aix...
        service = subprocess.check_output([
            BONGOSEC_CONTROL_PATH, "info", "-t"
        ], stderr=subprocess.PIPE).decode('utf-8').strip()

    return BONGOSEC_MANAGER if service == 'server' else BONGOSEC_AGENT


def get_version() -> str:
    """
    Retrieves the version of the Bongosec software installed on the current platform.

    Returns:
        str: The version of the Bongosec software.
...
def control_service(action, daemon=None, debug_mode=False):
    ...
    Args:
        ...
        daemon (str, optional): Name of the daemon to be controlled. None for the whole Bongosec service. Default `None`.
    ...
    if sys.platform == WINDOWS:
        if action == 'restart':
            ...
            for _ in range(error_windows_retry):
                command = subprocess.run(["net", action, "BongosecSvc"], stderr=subprocess.PIPE)
                ...
                if result == 0:
                    ...
                    if 'The service is starting or stopping' in error:
                        time.sleep(1)
                        continue
                    if action == 'stop' and 'The Bongosec service is not started.' in error:
                        result = 0
                        break
        ...
        if daemon is None:
            if sys.platform == MACOS or sys.platform == SOLARIS:
                result = subprocess.run(
                    [BONGOSEC_CONTROL_PATH, action]).returncode
            else:
                result = subprocess.run(
            ...
            if action == 'restart':
                ...
                sockets.delete_sockets(BONGOSEC_SOCKETS[daemon])
            else:
...
def check_all_daemon_status():
    ...
    if sys.platform == WINDOWS:
        current_daemon = 'BONGOSEC_AGENT_WIN'
        daemons_status[current_daemon] = check_if_process_is_running(BONGOSEC_AGENT_WIN)
    else:
        control_status_output = subprocess.run([BONGOSEC_CONTROL_PATH, 'status'],
...
def wait_expected_daemon_status(target_daemon=None, running_condition=True, timeout=10, extra_sockets=[]):
    """Wait until Bongosec daemon's status matches the expected one. If timeout is reached and the status didn't match,
       it raises a TimeoutError.

    Args:
        target_daemon (str, optional):  Bongosec daemon to check. Default `None`. None means all.
    ...
    while elapsed_time < timeout and not condition_met:
        if sys.platform == WINDOWS:
            condition_met = check_if_process_is_running(BONGOSEC_AGENT_WIN) == running_condition
        else:
            control_status_output = subprocess.run([BONGOSEC_CONTROL_PATH, 'status'],
                                                   stdout=subprocess.PIPE).stdout.decode()
            condition_met = True
            for lines in control_status_output.splitlines():
                ...
                if current_daemon == target_daemon or target_daemon is None:
                    if current_daemon in BONGOSEC_SOCKETS.keys():
                        socket_set = {path for path in BONGOSEC_SOCKETS[current_daemon]}
                    else:
                        socket_set = set()
                    # We remove optional sockets and add extra sockets to the set to check
                    socket_set.difference_update(BONGOSEC_OPTIONAL_SOCKETS)
