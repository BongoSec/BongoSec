...
from bongosec_testing import DATA_PATH
from bongosec_testing.constants.paths.configurations import BONGOSEC_CONF_PATH, BONGOSEC_LOCAL_INTERNAL_OPTIONS

from . import file


def get_minimal_configuration():
    """Get the bongosec minimal configuration data.

    Returns:
        List of str: Bongosec minimal configuration data.
    """
    return file.read_file_lines(os.path.join(DATA_PATH, 'configuration_template', 'all_disabled_ossec.conf'))


def get_bongosec_conf() -> List[str]:
    """
    ...
    """
    return file.read_file_lines(BONGOSEC_CONF_PATH)


def write_bongosec_conf(bongosec_conf: List[str]) -> None:
    """
    Write a new configuration in 'ossec.conf' file.

    Args:
        bongosec_conf (list or str): Lines to be written in the ossec.conf file.
    """
    file.write_file(BONGOSEC_CONF_PATH, bongosec_conf)


def set_section_bongosec_conf(sections: List[dict], template: List[str] = None) -> List[str]:
    """
    Set a configuration in a section of Bongosec. It replaces the content if it exists.

    Args:
        sections (list): List of dicts with section and new elements
        section (str, optional): Section of Bongosec configuration to replace. Default `'syscheck'`
    ...
    def find_module_config(bongosec_conf: ET.ElementTree, section: str, attributes: List[dict]) -> ET.ElementTree:
        r"""
        ...
        Args:
            bongosec_conf (ElementTree): An ElementTree object with all the data of the ossec.conf
        ...
        if attributes is None:
            return bongosec_conf.find(section)
        else:
            ...
            try:
                return bongosec_conf.find(query)
            except AttributeError:
                return None

    # Get Bongosec configuration as a list of str
    raw_bongosec_conf = get_bongosec_conf() if template is None else template
    # Generate a ElementTree representation of the previous list to work with its sections
    bongosec_conf = to_elementTree(purge_multiple_root_elements(raw_bongosec_conf))
    for section in sections:
        attributes = section.get('attributes')
        section_conf = find_module_config(
            bongosec_conf, section['section'], attributes)
        # Create section if it does not exist, clean otherwise
        if not section_conf:
            section_conf = ET.SubElement(
                bongosec_conf.getroot(), section['section'])
            section_conf.text = '\n    '
            section_conf.tail = '\n\n  '
        else:
    ...
    return to_str_list(bongosec_conf)
...
def get_local_internal_options_dict():
    ...
    with open(BONGOSEC_LOCAL_INTERNAL_OPTIONS, 'r') as local_internal_option_file:
...
def set_local_internal_options_dict(dict_local_internal_options):
    ...
    """
    with open(BONGOSEC_LOCAL_INTERNAL_OPTIONS, 'w') as local_internal_option_file:
...
def load_configuration_template(data_file_path, configuration_parameters=[], configuration_metadata=[]):
    """Load different configurations of Bongosec from a YAML file.
    ...
    Returns:
        list(dict): List containing bongosec configurations in dictionary form.
