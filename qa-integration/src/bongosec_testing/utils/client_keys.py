...
def add_client_keys_entry(agent_id: str, agent_name: str, agent_ip: str = 'any', agent_key: str = None) -> None:
    ...
    with open(BONGOSEC_CLIENT_KEYS_PATH, 'r') as client_keys:
    ...
    with open(BONGOSEC_CLIENT_KEYS_PATH, 'w') as client_keys:
...
def get_client_keys(path: str = BONGOSEC_CLIENT_KEYS_PATH) -> List[dict]:
    """Get client keys from a file.

    Args:
        path (str, optional): Path to the file containing the client keys.
            Defaults to BONGOSEC_CLIENT_KEYS_PATH.
...
def check_client_keys(id, expected):
    ...
    found = False
    try:
        with open(BONGOSEC_CLIENT_KEYS_PATH) as client_file:
