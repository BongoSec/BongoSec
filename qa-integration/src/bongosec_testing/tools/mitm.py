...
from bongosec_testing.constants.users import BONGOSEC_UNIX_USER, BONGOSEC_UNIX_GROUP
from bongosec_testing.utils import secure_message
from bongosec_testing.utils.cluster import CLUSTER_DATA_HEADER_SIZE, cluster_msg_build
...
class StreamHandler(socketserver.BaseRequestHandler):
    ...
    def default_bongosec_handler(self):
        """Default bongosec daemons TCP handler method for MITM server."""
...
class DatagramHandler(socketserver.BaseRequestHandler):
    ...
    def default_bongosec_handler(self):
        """Default bongosec daemons UDP handler method for MITM server."""
        data = self.request[0]
...
class ManInTheMiddle:
    ...
    def run(self, *args):
        ...
        if isinstance(self.listener_socket_address, str):
            uid = pwd.getpwnam(BONGOSEC_UNIX_USER).pw_uid
            gid = grp.getgrnam(BONGOSEC_UNIX_GROUP).gr_gid
