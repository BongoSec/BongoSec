...
from bongosec.core.agent import get_agents_info, get_groups, expand_group
from bongosec.core.common import rbac, broadcast, cluster_nodes
from bongosec.core.exception import BongosecPermissionError
from bongosec.core.results import AffectedItemsBongosecResult
from bongosec.core.utils import expand_rules, expand_lists, expand_decoders
from bongosec.rbac.orm import RolesManager, PoliciesManager, AuthenticationManager, RulesManager
...
def list_handler(result: AffectedItemsBongosecResult, original: dict = None, allowed: dict = None, target: dict = None,
                 add_denied: bool = False, **post_proc_kwargs: dict) -> AffectedItemsBongosecResult:
        ...
        for res_id, target_param in target.items():
            ...
            for denied_item in denied:
                result.add_failed_item(id_=denied_item,
                                       error=BongosecPermissionError(4000, extra_message=f'Resource type: {res_id}',
                                                                  ids=denied))
    if not add_denied or post_proc_kwargs.get('force'):
        ...
        if 'default_result_kwargs' in post_proc_kwargs and result is None:
            return AffectedItemsBongosecResult(**post_proc_kwargs['default_result_kwargs'])
...
def expose_resources(actions: list = None, resources: list = None, post_proc_func: callable = list_handler,
    ...
    """Decorator to apply user permissions on a Bongosec framework function based on exposed action:resource pairs.
            ...
            for res_id, target_param in target_params.items():
                try:
                    ...
                    if add_denied:
                        ...
                        if res_id in integer_resources:
                            denied = {int(i) if i.isdigit() else i for i in denied}
                        raise BongosecPermissionError(4000,
