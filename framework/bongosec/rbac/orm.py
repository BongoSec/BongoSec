...
from bongosec.core.common import bongosec_uid, bongosec_gid, DEFAULT_RBAC_RESOURCES
from bongosec.core.utils import get_utc_now, safe_move
from bongosec.rbac.utils import clear_cache

logger = logging.getLogger("bongosec-api")

# Max reserved ID value
BONGOSEC_USER_ID = 1
BONGOSEC_WUI_USER_ID = 2
MAX_ID_RESERVED = 99
    ...
    def migrate_data(self, source: str, target: str, from_id: int = None, to_id: int = None):
        ...
        with AuthenticationManager(self.sessions[target]) as auth_manager:
            for user in old_users:
                if user.id in (BONGOSEC_USER_ID, BONGOSEC_WUI_USER_ID):
        ...
        if from_id == BONGOSEC_USER_ID and to_id == BONGOSEC_WUI_USER_ID:
            return
...
def check_database_integrity():
    ...
    def _set_permissions_and_ownership(database: str):
        """Set Bongosec ownership and permissions.
        ...
        """
        chown(database, bongosec_uid(), bongosec_gid())
    ...
    try:
        ...
        if os.path.exists(DB_FILE):
            ...
            if current_version < expected_version:
                ...
                db_manager.migrate_data(source=DB_FILE, target=DB_FILE_TMP, from_id=BONGOSEC_USER_ID,
                                        to_id=BONGOSEC_WUI_USER_ID)
                ...
                safe_move(DB_FILE_TMP, DB_FILE,
                          ownership=(bongosec_uid(), bongosec_gid()),
                          permissions=0o640)
        ...
        logger.error("Error retrieving the current Bongosec RBAC database version. Aborting database integrity check")
        db_manager.close_sessions()
        raise e
