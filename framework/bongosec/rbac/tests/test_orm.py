...
from bongosec.rbac.tests.utils import init_db
from bongosec.rbac.orm import BONGOSEC_USER_ID, BONGOSEC_WUI_USER_ID, MAX_ID_RESERVED, User
...
def db_setup():
    with patch('bongosec.core.common.bongosec_uid'), patch('bongosec.core.common.bongosec_gid'):
        with patch('sqlalchemy.create_engine', return_value=create_engine("sqlite://")):
            with patch('shutil.chown'), patch('os.chmod'):
                with patch('api.constants.SECURITY_PATH', new=test_data_path):
                    import bongosec.rbac.orm as rbac
                    # Clear mappers
...
def fresh_in_memory_db():
    ...
    with patch('bongosec.core.common.bongosec_uid'), patch('bongosec.core.common.bongosec_gid'):
        import bongosec.rbac.orm as orm
        reload(orm)
...
def test_database_init(db_setup):
    ...
    with db_setup.RolesManager() as rm:
        assert rm.get_role('bongosec') != db_setup.SecurityError.ROLE_NOT_EXIST
...
def test_delete_all_expired_rules(db_setup):
    """Check that rules are correctly deleted"""
    with patch('bongosec.rbac.orm.time', return_value=0):
        add_token(db_setup)
    with db_setup.TokenManager() as tm:
...
@patch("bongosec.rbac.orm.create_engine")
@patch("bongosec.rbac.orm.sessionmaker")
...
@patch("bongosec.rbac.orm._Base.metadata.create_all")
...
def test_databasemanager_get_table(fresh_in_memory_db):
    ...
    with patch("bongosec.rbac.orm._new_columns", new={"new_column"}):
...
@patch("bongosec.rbac.orm.safe_move")
@patch("bongosec.rbac.orm.os.remove")
@patch("bongosec.rbac.orm.chown")
@patch("bongosec.rbac.orm.os.chmod")
def test_check_database_integrity(chmod_mock, chown_mock, remove_mock, safe_move_mock, fresh_in_memory_db):
    ...
    db_mock = MagicMock()
    with patch("bongosec.rbac.orm.db_manager", new=db_mock):
        with patch("bongosec.rbac.orm.os.path.exists", return_value=True):
            with patch("bongosec.rbac.orm.CURRENT_ORM_VERSION", new=99999):
                ...
                db_mock.assert_has_calls([
                    ...
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
                                      from_id=fresh_in_memory_db.BONGOSEC_USER_ID,
                                      to_id=fresh_in_memory_db.BONGOSEC_WUI_USER_ID),
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
        ...
        safe_move_mock.assert_called_once_with(fresh_in_memory_db.DB_FILE_TMP, fresh_in_memory_db.DB_FILE,
                                               ownership=(fresh_in_memory_db.bongosec_uid(),
                                                          fresh_in_memory_db.bongosec_gid()),
                                               permissions=0o640)
...
@patch("bongosec.rbac.orm.DatabaseManager.close_sessions")
@patch("bongosec.rbac.orm.os.remove")
def test_check_database_integrity_exceptions(remove_mock, close_sessions_mock, exception, fresh_in_memory_db):
    ...
    with patch("bongosec.rbac.orm.os.path.exists", side_effect=mocked_exists) as mock_exists:
        with patch("bongosec.rbac.orm.DatabaseManager.connect", side_effect=exception) as db_manager_mock:
...
@pytest.mark.parametrize('from_id, to_id, users', [
    (BONGOSEC_USER_ID, BONGOSEC_WUI_USER_ID, [
        User('bongosec', 'test', user_id=BONGOSEC_USER_ID),
        User('bongosec-wui', 'test2', user_id=BONGOSEC_WUI_USER_ID)
    ]),
...
def test_migrate_data(db_setup, from_id, to_id, users):
    ...
    if to_id == BONGOSEC_WUI_USER_ID:
        with patch("bongosec.rbac.orm.db_manager.get_data", return_value=users):
            with patch("bongosec.rbac.orm.AuthenticationManager.update_user") as mock_update_user:
        ...
        empty_list = []
        user_exists = False
        with patch("bongosec.rbac.orm.AuthenticationManager.add_user",
                   side_effect=[not user_exists, user_exists, not user_exists]) as mock_add_user:
            with patch("bongosec.rbac.orm.db_manager.get_data",
