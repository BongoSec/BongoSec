...
import bongosec.core.results as results
        ...
        if match := re.match(regex, pid_file):
            try:
                ...
                if daemon.replace('-', '_') in command:
                    ...
                    print(f"{daemon}: Process {pid} does not belong to {daemon}, removing from {common.BONGOSEC_PATH}/var/run...")

            except (OSError, psutil.NoSuchProcess):
                print(f'{daemon}: Non existent process {pid}, removing from {common.BONGOSEC_PATH}/var/run...')
            finally:
...
def process_array(array: list, search_text: str = None, complementary_search: bool = False,
    ...
    """Process a Bongosec framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise BongosecError(1405, extra_message=str(limit))
        elif limit == 0:
            raise BongosecError(1406)
    ...
    if offset < 0:
        raise BongosecError(1400)
    elif limit < 1:
        raise BongosecError(1401)
    else:
...
def sort_array(array: list, sort_by: list = None, sort_ascending: bool = True,
    ...
    def check_sort_fields(allowed_sort_fields, sort_by):
        ...
        if not sort_by.issubset(allowed_sort_fields):
            ...
            raise BongosecError(1403, extra_remediation='Allowed sort fields: {0}. '
    ...
    if sort_by:  # array should be a dictionary or a Class
        ...
        if type(array) is set or (type(array[0]) is not dict and 'class \'bongosec' not in str(type(array[0]))):
...
def select_array(array: list, select: list = None, required_fields: set = None,
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise BongosecError(1724, "{}".format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise BongosecError(1724, "{}".format(', '.join(select)))
...
def delete_bongosec_file(full_path: str) -> bool:
    """Delete a Bongosec file.
    ...
    bool
        True if success.
    """
    if not full_path.startswith(common.BONGOSEC_PATH) or '..' in full_path:
        raise BongosecError(1907)
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise BongosecError(1723, "Available algorithms are {0}.".format(', '.join(algorithm_list)))
...
def check_bongosec_limits_unchanged(new_conf, original_conf):
    """Check if Bongosec limits remain unchanged.
    ...
    for disabled_limit in [conf for conf, allowed in limits_configuration.items() if not allowed['allow']]:
        ...
        if len(new_limits) != len(original_limits) or any(x != y for x, y in zip(new_limits, original_limits)):
            raise BongosecError(1127, extra_message=f"global > limits > {disabled_limit}")
...
def check_indexer(new_conf: str, original_conf: str):
    ...
    if not upload_configuration['indexer']['allow']:
        ...
        if len(new_indexer) != len(original_indexer) or any(x != y for x, y in zip(new_indexer, original_indexer)):
            raise BongosecError(1127, extra_message='indexer')
    ...
    if not blocked_configurations['public_key']['allow']:
        ...
        for api_key in api_keys:
            ...
            try:
                ...
                raise BongosecError(1131, extra_message=f'{extra_msg}')
            if response_minimum_quota == minimum_quota:
                raise BongosecError(1130, extra_message='integrations > virustotal')


def load_bongosec_xml(xml_path, data=None):
    if not data:
        with open(xml_path) as f:
            try:
                data = f.read()
            except Exception as e:
                raise BongosecError(1113, extra_message=str(e))
...
class BongosecVersion:

    def __init__(self, version):

        pattern = r"(?:Bongosec )?v?(\d+)\.(\d+)\.(\d+)\-?(alpha|beta|rc)?(\d*)"
...
def get_timeframe_in_seconds(timeframe: str) -> int:
    ...
    if not timeframe.isdigit():
        if 'h' not in timeframe and 'd' not in timeframe and 'm' not in timeframe and 's' not in timeframe:
            raise BongosecError(1411, timeframe)
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise BongosecError(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class BongosecDBBackend(AbstractDatabaseBackend):
    """
    This class describes a bongosec db backend that executes database queries.
    """
    ...
    def __init__(self, agent_id=None, query_format='agent', request_slice=500):
        if query_format == 'agent' and not path.exists(path.join(common.WDB_PATH, f"{agent_id}.db")):
            raise BongosecError(2007, extra_message=f"There is no database for agent {agent_id}. "
    ...
    def connect_to_db(self):
        return BongosecDBConnection(request_slice=self.request_slice)
    ...
    def execute(self, query, request, count=False):
        """Execute SQL query through BongosecDB socket."""
        query = self._substitute_params(query, request)
        return self.conn.execute(query=self._render_query(query), count=count)


class BongosecDBQuery(object):
    """This class describes a database query for bongosec."""
    ...
    def __init__(self, offset: int, limit: int, table: str, sort: dict, search: dict, select: list, query: str,
        ...
        # For example, the following regex -> (name!=bongosec;id>5),group=webserver <- would return 3 different matches:
        #   (name != bongosec ;
        #    id   > 5      ),
        #    group=webserver
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, BongosecDBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise BongosecError(1405, extra_message=str(self.limit))
        ...
        if self.sort:
            if self.sort['fields']:
                ...
                if not set(sort_fields).issubset(allowed_sort_fields):
                    raise BongosecError(1403, "Allowed sort fields: {}. Fields: {}".format(
    ...
    def _parse_select_filter(self, select_fields):
        if select_fields:
            ...
            if not set_select_fields or not set_select_fields.issubset(set_fields_keys):
                raise BongosecError(1724, "Allowed select fields: {0}. Fields {1}". \
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise BongosecError(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'BongosecDBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'BongosecDBQueryGroups':
            resource = 'name'
        else:
            raise BongosecInternalError(1123)
...
class BongosecDBQueryDistinct(BongosecDBQuery):
    ...
    def _add_filters_to_query(self):
        BongosecDBQuery._add_filters_to_query(self)
        self.query += ' WHERE ' if not self.q and 'WHERE' not in self.query else ' AND '
        self.query += ' AND '.join(
            ["{0} IS NOT null AND {0} != ''".format(self.fields[field]) for field in self.select])
    ...
    def _add_select_to_query(self):
        if len(self.select) > 1:
            raise BongosecError(1410)

        BongosecDBQuery._add_select_to_query(self)
...
def validate_bongosec_xml(content: str, config_file: bool = False):
    """Validate Bongosec XML files (rules, decoders and ossec.conf)
    ...
    try:
        ...
        if config_file:
            ...
            check_bongosec_limits_unchanged(final_xml, current_xml)
        # Check xml format
        load_bongosec_xml(xml_path='', data=final_xml)
    except ExpatError:
        raise BongosecError(1113)
    except BongosecError as e:
        raise e
    except Exception as e:
        raise BongosecError(1113, str(e))
...
def upload_file(content: str, file_path: str, check_xml_formula_values: bool = True):
    ...
    Raises
    ------
    BongosecInternalError(1005)
        Error reading file.
    BongosecInternalError(1016)
        Error moving file.
    BongosecError(1006)
    ...
    try:
        ...
        raise BongosecInternalError(1005) from exc

    # Move temporary file to group folder
    try:
        new_conf_path = path.join(common.BONGOSEC_PATH, file_path)
        safe_move(tmp_file_path, new_conf_path, ownership=(common.bongosec_uid(), common.bongosec_gid()), permissions=0o660)
    except PermissionError as exc:
        raise BongosecError(1006) from exc
    except Error as exc:
        raise BongosecInternalError(1016) from exc

    return results.BongosecResult({'message': 'File was successfully updated'})
...
def to_relative_path(full_path: str, prefix: str = common.BONGOSEC_PATH) -> str:
    """Return a relative path from the Bongosec base directory.
    ...
    full_path : str
        Absolute path.
    prefix : str, opt
        Prefix to strip from the absolute path. Default `common.BONGOSEC_PATH`
...
def check_if_bongosec_agent_version(version_str: str) -> bool:
    """Check if the string has the expected bongosec agent version format.

    Parameters
    ----------
    version_str : str
        The bongosec version string.

    Returns
    -------
    bool
        True if the string has the expected bongosec version format.

    """
    if not isinstance(version_str, str):
        return False

    return bool(re.match(r'^Bongosec v(\d+)\.(\d+)\.(\d+)', version_str))


def parse_bongosec_agent_version(version_str: str) -> tuple:
    ...
    -------
    tuple
        The tuple of the bongosec version string.
    """
