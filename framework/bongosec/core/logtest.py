...
from bongosec.core.common import LOGTEST_SOCKET, DECIMALS_DATE_FORMAT, origin_module
from bongosec.core.bongosec_socket import BongosecSocketJSON, create_bongosec_socket_message
...
def send_logtest_msg(command: str = None, parameters: dict = None) -> dict:
    ...
    """
    full_message = create_bongosec_socket_message(origin={'name': 'Logtest', 'module': origin_module.get()},
                                               command=command,
                                               parameters=parameters)
    logtest_socket = BongosecSocketJSON(LOGTEST_SOCKET)
