...
from bongosec.core.bongosec_queue import BaseQueue, BongosecAnalysisdQueue, BongosecQueue


@patch('bongosec.core.bongosec_queue.BaseQueue._connect')
...
@patch('bongosec.core.bongosec_queue.BaseQueue.close')
@patch('bongosec.core.bongosec_queue.socket.socket.connect')
...
@patch('bongosec.core.bongosec_queue.socket.socket.connect')
@patch('bongosec.core.bongosec_queue.socket.socket.setsockopt')
def test_BaseQueue_protected_connect(mock_set, mock_conn):
    """Test BaseQueue._connect function."""

    BaseQueue('test_path')

    with patch('bongosec.core.bongosec_queue.socket.socket.getsockopt', return_value=1):
        BaseQueue('test_path')
...
@patch('bongosec.core.bongosec_queue.socket.socket.connect', side_effect=Exception)
def test_BaseQueue_protected_connect_ko(mock_conn):
    """Test BaseQueue._connect function exceptions."""

    with pytest.raises(BongosecException, match=".* 1010 .*"):
        BaseQueue('test_path')
...
@patch('bongosec.core.bongosec_queue.socket.socket.connect')
@patch('bongosec.core.bongosec_queue.BaseQueue.MAX_MSG_SIZE', new=0)
def test_BaseQueue_protected_send(mock_conn, send_response, error):
    ...
    error : bool
        Indicates whether a BongosecException will be raised or not.
    """
    ...
    with patch('socket.socket.send', return_value=send_response):
        if error:
            with pytest.raises(BongosecException, match=".* 1011 .*"):
                queue._send('msg')
        else:
            queue._send('msg')
...
def test_BaseQueue_protected_send_ko(mock_send, mock_conn, errno, match):
    ...
    queue = BaseQueue('test_path')
    with pytest.raises(BongosecException, match=match):
...
@patch('bongosec.core.bongosec_queue.socket.socket.connect')
@patch('bongosec.core.bongosec_queue.socket.socket.close')
...
@patch('bongosec.core.bongosec_queue.socket.socket.connect')
@patch('bongosec.core.bongosec_queue.BongosecQueue._send')
def test_BongosecQueue_send_msg_to_agent(mock_send, mock_conn, msg, agent_id, msg_type):
    """Test BongosecQueue.send_msg_to_agent function.
...
@patch('bongosec.core.bongosec_queue.socket.socket.connect')
@patch('bongosec.core.bongosec_queue.BongosecQueue._send', side_effect=Exception)
def test_BongosecQueue_send_msg_to_agent_ko(mock_send, mock_conn, msg, agent_id, msg_type, expected_exception):
    """Test BongosecQueue.send_msg_to_agent function exceptions.
...
@patch('bongosec.core.bongosec_queue.socket.socket.connect')
@patch('bongosec.core.bongosec_queue.BongosecAnalysisdQueue._send')
def test_BongosecAnalysisdQueue_send_msg(mock_send, mock_conn):
    """Test BongosecAnalysisdQueue.send_msg function."""

    queue = BongosecAnalysisdQueue('test_path')
...
@patch('bongosec.core.bongosec_queue.socket.socket.connect')
@patch('bongosec.core.bongosec_queue.BongosecAnalysisdQueue._send', side_effect=Exception)
def test_BongosecAnalysisdQueue_send_msg_ko(mock_send, mock_conn, max_msg_size, expected_error_code):
    """Test BongosecAnalysisdQueue.send_msg function exceptions."""

    queue = BongosecAnalysisdQueue('test_path')
    queue.MAX_MSG_SIZE = max_msg_size

    with pytest.raises(BongosecException, match=f'.* {expected_error_code} .*'):
