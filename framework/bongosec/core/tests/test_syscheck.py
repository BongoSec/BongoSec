...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        from bongosec.core import syscheck
        from bongosec.core.utils import get_date_from_timestamp


@pytest.mark.parametrize('agent', ['002', '080'])
@patch("bongosec.core.syscheck.BongosecDBBackend")
@patch("bongosec.core.syscheck.BongosecDBQuery.__init__")
def test_bongosec_db_query_syscheck__init__(mock_wdbquery, mock_backend, agent):
    """Test if BongosecDBQuery and BongosecDBBackend are called with the expected parameters.
    ...
    agent: str
        The agent to test.
    """
    syscheck.BongosecDBQuerySyscheck(agent)
...
@patch("bongosec.core.syscheck.BongosecDBBackend")
def test_bongosec_db_syscheck_format_data_into_dictionary(mock_backend, data, is_json):
    """Test if _format_data_into_dictionary() returns the expected element."""
    test = syscheck.BongosecDBQuerySyscheck('002', offset=0, limit=1000, sort=None, search='test',
