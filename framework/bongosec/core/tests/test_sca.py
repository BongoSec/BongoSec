...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        from bongosec.core import sca as core_sca
...
@patch('bongosec.core.utils.BongosecDBBackend.__init__', return_value=None)
@patch('bongosec.core.utils.BongosecDBQuery.__init__')
def test_BongosecDBQuerySCA__init__(mock_wdbq, mock_backend, mock_get_basic_info, distinct, agent_id, offset, limit, sort,
                                 search, query, count, get_data, select):
    """Test if method __init__ of BongosecDBQuerySCA works properly.
    ...
    """
    wdbq_sca = core_sca.BongosecDBQuerySCA(agent_id=agent_id, offset=offset, limit=limit, sort=sort, search=search,
    ...
    mock_wdbq.assert_called_once_with(ANY, offset=offset, limit=limit, table='sca_policy', sort=sort, search=search,
                                      select=select, fields=core_sca.BongosecDBQuerySCA.DB_FIELDS,
...
def test_BongosecDBQuerySCA__format_data_into_dictionary(agent_id, offset, limit, sort, search, select, query, count,
                                                      get_data):
    """Check if BongosecDBQuerySCA's method _format_data_into_dictionary works properly.
...
@patch('bongosec.core.sca.BongosecDBQuerySCA.__init__')
def test_BongosecDBQuerySCACheck__init__(mock_wdbqsca, select, sca_checks_test_list, expected_default_query):
    """Test if method __init__ of BongosecDBQuerySCACheck works properly.
    ...
    """
    core_sca.BongosecDBQuerySCACheck(agent_id='000', select=select, sort={'fields': ['title'], 'order': 'asc'},
...
def test_BongosecDBQuerySCACheck_parse_select_filter(mock_exists, mock_get_basic_info, mock_backend):
    """Test if method _parse_select_filter of BongosecDBQuerySCACheck works properly."""
    wdbq_sca_check = core_sca.BongosecDBQuerySCACheck(agent_id='000', sort={'value': 'test'},
...
@patch('bongosec.core.sca.BongosecDBQuerySCA.__init__')
def test_BongosecDBQuerySCACheckIDs__init__(mock_wdbqsca, query):
    """Test if method __init__ of BongosecDBQuerySCACheckIDs works properly.

    Parameters
    ----------
    query : str
        Query used to initialize the BongosecDBQuerySCACheckIDs object.
    """
    ...
    core_sca.BongosecDBQuerySCACheckIDs(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
                                     search={'value': 'test'},
...
@patch('bongosec.core.utils.BongosecDBBackend.__init__', return_value=None)
@patch('bongosec.core.agent.Agent.get_basic_information')
def test_BongosecDBQuerySCACheckIDs_protected_pass_filter(mock_get_basic_info, mock_backend, field, value, expected):
    """Test BongosecDBQuerySCACheckIDs._pass_filter function."""
    query = core_sca.BongosecDBQuerySCACheckIDs(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
...
@patch('bongosec.core.sca.BongosecDBQuerySCA.__init__')
def test_BongosecDBQuerySCACheckRelational__init__(mock_wdbqsca, select, table, sca_checks_test_list):
    """Test if method __init__ of BongosecDBQuerySCACheckRelational works properly.
    ...
    select : list or None
        Fields to select.
    table : str
        Table used to initialize the BongosecDBQuerySCACheckRelational object.
    sca_checks_test_list : list
        List of SCA checks IDs.
    """
    query_sca_check_relational = core_sca.BongosecDBQuerySCACheckRelational(agent_id='000', table=table,
...
@patch('bongosec.core.sca.BongosecDBQuerySCA.__enter__')
@patch('bongosec.core.sca.BongosecDBQuerySCA.__init__', return_value=None)
@patch('bongosec.core.sca.BongosecDBQuery.__exit__')
def test_BongosecDBQueryDistinctSCACheck__init__(mock_exit, mock_wdbqsca, mock_enter, select):
    """Test if method __init__ of BongosecDBQueryDistinctSCACheck works properly."""
    ...
    core_sca.BongosecDBQueryDistinctSCACheck(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
    ...
    mock_wdbqsca.assert_has_calls(
        [call(agent_id='000', offset=0, limit=None, sort={'fields': ['title'], 'order': 'asc'},
              ...
              default_query=core_sca.BongosecDBQueryDistinctSCACheck.INNER_QUERY_PATTERN, search={'value': 'test'}),
