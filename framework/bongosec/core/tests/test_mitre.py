...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        from bongosec.core.mitre import *


@patch('bongosec.core.utils.BongosecDBConnection', return_value=InitWDBSocketMock(sql_schema_file='schema_mitre_test.sql'))
def test_BongosecDBQueryMitreMetadata(mock_wdb):
    """Verify that the method connects correctly to the database and returns the correct type."""
    db_query = BongosecDBQueryMitreMetadata()
    data = db_query.run()

    assert isinstance(db_query, BongosecDBQueryMitre) and isinstance(data, dict)


@pytest.mark.parametrize('wdb_query_class', [
    BongosecDBQueryMitreGroups,
    BongosecDBQueryMitreMitigations,
    BongosecDBQueryMitreReferences,
    BongosecDBQueryMitreTactics,
    BongosecDBQueryMitreTechniques,
    BongosecDBQueryMitreSoftware

])
@patch('bongosec.core.utils.BongosecDBConnection', return_value=InitWDBSocketMock(sql_schema_file='schema_mitre_test.sql'))
def test_BongosecDBQueryMitre_classes(mock_wdb, wdb_query_class):
...
@patch('bongosec.core.utils.BongosecDBConnection')
