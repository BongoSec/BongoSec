...
@patch("bongosec.core.rootcheck.BongosecDBBackend")
@patch("bongosec.core.rootcheck.BongosecDBQuery.__init__")
@patch('bongosec.core.agent.Agent.get_basic_information')
def test_BongosecDBQueryRootcheck_init(mock_info, mock_bongosecDBQuery, mock_backend):
    """Test if BongosecDBQuery and BongosecDBBackend are called with expected parameters"""
    test = rootcheck.BongosecDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test', select=['log'],
                                           query='test', count=True, get_data=True, distinct=False, filters=None,
                                           fields={})
    mock_backend.assert_called_with('100')
    mock_bongosecDBQuery.assert_called_with(ANY, offset=1, limit=1, table='pm_event', sort=None, search='test',
...
@patch("bongosec.core.rootcheck.BongosecDBBackend")
@patch('bongosec.core.agent.Agent.get_basic_information')
def test_BongosecDBQueryRootcheck_default_query(mock_info, mock_backend, distinct):
...
@patch("bongosec.core.rootcheck.BongosecDBBackend")
@patch('bongosec.core.agent.Agent.get_basic_information')
def test_BongosecDBQueryRootcheck_parse_filters(mock_info, mock_backend):
...
@patch("bongosec.core.rootcheck.BongosecDBBackend")
@patch('bongosec.core.agent.Agent.get_basic_information')
def test_BongosecDBQueryRootcheck_filter_status(mock_info, mock_backend, status, expected_items):
...
@patch("bongosec.core.rootcheck.BongosecDBBackend")
@patch('bongosec.core.agent.Agent.get_basic_information')
def test_BongosecDBQueryRootcheck_filter_status_ko(mock_info, mock_backend):
    """Test if expected exception is raised when status does not exist"""
    test = rootcheck.BongosecDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test', select=['log'],
                                           query='', count=True, get_data=True, distinct=False, fields={},
                                           filters={'status': 'all', 'pci_dss': None, 'cis': None})

    with pytest.raises(BongosecException, match=".* 1603 .*"):
        test._filter_status({'value': 'test'})


@patch("bongosec.core.rootcheck.BongosecDBBackend")
@patch('bongosec.core.agent.Agent.get_basic_information')
def test_BongosecDBQueryRootcheck_format_data_into_dictionary(mock_info, mock_backend):
    """Test if format_data_into_dictionary() returns expected element"""
    test = rootcheck.BongosecDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test',
                                           select=['log', 'date_first', 'status', 'date_last', 'cis', 'pci_dss'],
                                           query='', count=True, get_data=True, distinct=False,
                                           fields=rootcheck.BongosecDBQueryRootcheck.fields,
                                           filters={'status': 'all', 'pci_dss': None, 'cis': None})
...
@patch('bongosec.core.wdb.BongosecDBConnection')
def test_rootcheck_delete_agent(mock_db_conn, agent):
    """Test if proper parameters are being sent to the bongosecdb socket.

    Parameters
    ----------
    agent : str
        Agent whose information is being deleted from the db.
    mock_db_conn : BongosecDBConnection
        Object used to send the delete message to the bongosecdb socket.
    """
