...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        ...
        from bongosec.core import decoder
...
@pytest.mark.parametrize("detail, value, details, expected_result", [
    ("parent", "bongosec", {'regex': ['regex00']}, {'parent': 'bongosec', 'regex': ['regex00']}),
...
@pytest.mark.parametrize("filename, relative_dirname, status, permissions, exception", [
    ...
    ('wrong_decoders.xml', 'decoders', "all", 777, BongosecInternalError(1501)),
    ('non_existing.xml', 'decoders', "disabled", 777, BongosecError(1502)),
    ('test1_decoders.xml', 'decoders', "all", 000, BongosecError(1502)),
])
@patch('bongosec.core.common.BONGOSEC_PATH', new=test_data_path)
