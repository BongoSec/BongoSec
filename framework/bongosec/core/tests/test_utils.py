...
from bongosec.core.common import OSSEC_TMP_PATH
...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        from bongosec import BongosecException
        from bongosec.core.agent import BongosecDBQueryAgents
        from bongosec.core import utils, exception
        from bongosec.core.common import BONGOSEC_PATH, AGENT_NAME_LEN_LIMIT
...
@patch('bongosec.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
...
def test_cut_array_ko(limit, offset, expected_exception):
    ...
    """
    with pytest.raises(exception.BongosecException, match=f'.* {expected_exception} .*'):
...
@patch('bongosec.core.utils.len', return_value=1)
@patch('bongosec.core.utils.cut_array')
@patch('bongosec.core.utils.select_array', return_value=ANY)
@patch('bongosec.core.utils.filter_array_by_query', return_value=ANY)
@patch('bongosec.core.utils.search_array', return_value=ANY)
@patch('bongosec.core.utils.sort_array', return_value=ANY)
...
@patch('bongosec.core.utils.chmod')
...
@patch('bongosec.core.utils.chown')
...
@patch('bongosec.core.utils.common.BONGOSEC_PATH', new='/test/path')
@patch('bongosec.core.utils.path.exists', return_value=True)
@patch('bongosec.core.utils.remove')
def test_delete_bongosec_file(mock_remove, mock_exists):
    """Check delete_file calls functions with expected params"""
    assert utils.delete_bongosec_file('/test/path/etc/file')
    mock_remove.assert_called_once_with('/test/path/etc/file')


@patch('bongosec.core.utils.common.BONGOSEC_PATH', new='/test/path')
def test_delete_bongosec_file_ko():
    """Check delete_file calls functions with expected params"""
    with pytest.raises(utils.BongosecError, match=r'\b1907\b'):
        utils.delete_bongosec_file('/test/different_path/etc/file')

    with pytest.raises(utils.BongosecError, match=r'\b1907\b'):
        utils.delete_bongosec_file('/test/path/file/../../home')

    with patch('bongosec.core.utils.path.exists', return_value=False):
        with pytest.raises(utils.BongosecError, match=r'\b1906\b'):
            utils.delete_bongosec_file('/test/path/etc/file')

    with patch('bongosec.core.utils.path.exists', return_value=True):
        with pytest.raises(utils.BongosecError, match=r'\b1907\b'):
            utils.delete_bongosec_file('/test/path/etc/file')
...
@patch('bongosec.core.utils.chown')
@patch('bongosec.core.utils.chmod')
@patch('bongosec.core.utils.utime')
...
def test_safe_move_exception(mock_utime, mock_chmod, mock_chown):
    ...
    with TemporaryDirectory() as tmpdirname:
        tmp_file = NamedTemporaryFile(dir=tmpdirname, delete=False)
        target_file = os.path.join(tmpdirname, 'target')
        with patch('bongosec.core.utils.rename', side_effect=OSError(1)):
...
@patch('bongosec.core.utils.chmod')
@patch('bongosec.core.utils.mkdir')
@patch('bongosec.core.utils.curdir', new='var')
def test_mkdir_with_mode(mock_mkdir, mock_chmod, dir_name, path_exists):
    """Test mkdir_with_mode function."""
    with patch('bongosec.core.utils.path.exists', return_value=path_exists):
...
@patch('bongosec.core.utils.mkdir', side_effect=OSError)
def test_mkdir_with_mode_ko(mock_mkdir, dir_name, exists):
    """Test mkdir_with_mode function errors work."""
    with patch('bongosec.core.utils.path.exists', return_value=exists):
        with pytest.raises(OSError):
            utils.mkdir_with_mode(dir_name)


@patch('bongosec.core.utils.open')
@patch('bongosec.core.utils.iter', return_value=['1', '2'])
def test_md5(mock_iter, mock_open):
    """Test md5 function."""
    with patch('bongosec.core.utils.hashlib.md5') as md:
...
def test_blake2b(mock_iter, mock_open):
    """Test md5 function."""
    with patch('bongosec.core.utils.hashlib.blake2b') as blake2b_mock:
...
def test_protected_get_hashing_algorithm_ko():
    ...
    with pytest.raises(exception.BongosecException, match=".* 1723 .*"):
...
def test_get_hash(mock_open):
    ...
    with patch('bongosec.core.utils.iter', return_value=['1', '2']):
        with patch('bongosec.core.utils.hashlib.new') as md:
            md.return_value.update.side_effect = None
    ...
    with patch('bongosec.core.utils.iter', return_value=[]):
...
def test_check_remote_commands(data, allow_config, exceptions, should_raise):
    ...
    with patch('bongosec.core.utils.configuration.api_conf', new=api_conf):
        if should_raise:
            with pytest.raises(exception.BongosecError, match=r'.* 1124 .*'):
                utils.check_remote_commands(data)
        else:
            utils.check_remote_commands(data)


@patch('bongosec.core.utils.compile', return_value='Something')
def test_basic_load_bongosec_xml(mock_compile):
    """Test basic load_bongosec_xml functionality."""
    with patch('bongosec.core.utils.open') as f:
        f.return_value.__enter__.return_value = StringIO(test_xml)
        result = utils.load_bongosec_xml('test_file')

        assert isinstance(result, Element)


def test_load_bongosec_xml():
    """Test load_bongosec_xml function."""
    ...
    for rule_file in os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_bongosec_xml')):
        path = os.path.join(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_bongosec_xml'),
                            rule_file)
        original = parse(path).getroot()
        result = utils.load_bongosec_xml(path)
...
def test_load_bongosec_xml_ko(expected_exception):
    """Test load_bongosec_xml fails gracefully when reading an invalid utf-8 character sequence"""
    file_path = os.path.join(test_data_path, 'test_load_bongosec_xml_ko/invalid_utf8.xml')
    with pytest.raises(BongosecException, match=f'.* {expected_exception} .*'):
        utils.load_bongosec_xml(file_path)

@pytest.mark.parametrize('version1, version2', [
    ('Bongosec v3.5.0', 'Bongosec v3.5.2'),
    ('Bongosec v3.6.1', 'Bongosec v3.6.3'),
    ('Bongosec v3.7.2', 'Bongosec v3.8.0'),
    ('Bongosec v3.8.0', 'Bongosec v3.8.1'),
    ('Bongosec v3.9.0', 'Bongosec v3.9.2'),
    ('Bongosec v3.9.10', 'Bongosec v3.9.14'),
    ('Bongosec v3.10.1', 'Bongosec v3.10.10'),
    ('Bongosec v4.10.10', 'Bongosec v4.11.0'),
    ('Bongosec v5.1.15', 'Bongosec v5.2.0'),
    ('v3.6.0', 'v3.6.1'),
    ('v3.9.1', 'v3.9.2'),
    ('v4.0.0', 'v4.0.1'),
    ('3.6.0', '3.6.1'),
...
def test_version_ok(version1, version2):
    """Test BongosecVersion class."""
    current_version = utils.BongosecVersion(version1)
    new_version = utils.BongosecVersion(version2)
...
@pytest.mark.parametrize('version1, version2', [
    ('v3.6.0', 'v.3.6.1'),
    ('Bongosec v4', 'Bongosec v5'),
    ('Bongosec v3.9', 'Bongosec v3.10'),
    ('ABC v3.10.1', 'ABC v3.10.12'),
    ('Bongosecv3.9.0', 'Bongosecv3.9.2'),
    ('3.9', '3.10'),
    ('3.9.0', '3.10'),
    ('3.10', '4.2'),
    ('3', '3.9.1')
...
def test_version_ko(version1, version2):
    """Test BongosecVersion class."""
    try:
        utils.BongosecVersion(version1)
        utils.BongosecVersion(version2)
    except ValueError:
        return


@pytest.mark.parametrize('version1, version2', [
    ('Bongosec v3.10.10', 'Bongosec v3.10.10'),
    ('Bongosec v5.1.15', 'Bongosec v5.1.15'),
    ('v3.6.0', 'v3.6.0'),
    ('v3.9.2', 'v3.9.2')
])
...
def test_BongosecVersion_to_array():
    """Test BongosecVersion.to_array function."""
    version = utils.BongosecVersion('Bongosec v3.10.0-alpha4')

    assert isinstance(version.to_array(), list)


def test_BongosecVersion__str__():
    """Test BongosecVersion.__str__ function."""
    version = utils.BongosecVersion('Bongosec v3.10.0-alpha4')
...
@pytest.mark.parametrize('version1, version2', [
    ('Bongosec v3.5.2', 'Bongosec v4.0.0'),
    ('Bongosec v3.10.0-alpha', 'Bongosec v3.10.0'),
    ('Bongosec v3.10.0-alpha4', 'Bongosec v3.10.0-beta4'),
    ('Bongosec v3.10.0-alpha3', 'Bongosec v3.10.0-alpha4'),
])
def test_BongosecVersion__ge__(version1, version2):
    """Test BongosecVersion.__ge__ function."""
    current_version = utils.BongosecVersion(version1)
    new_version = utils.BongosecVersion(version2)
...
def test_failed_test_get_timeframe_in_seconds():
    ...
    with pytest.raises(exception.BongosecException, match=".* 1411 .*"):
...
@patch('bongosec.core.utils.path.exists', return_value=True)
@patch('bongosec.core.utils.glob.glob', return_value=True)
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('bongosec.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_BongosecDBQuery_protected_clean_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                             query_filter, expected_query_filter, expected_wef):
    """Test BongosecDBQuery._clean_filter function."""
    query = utils.BongosecDBQuery(offset=0, limit=500, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.BongosecDBBackend(agent_id=1),
...
@patch('socket.socket.connect')
@patch('bongosec.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_BongosecDBQuery_protected_add_limit_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                   limit, error, expected_exception):
    """Test BongosecDBQuery._add_limit_to_query function."""
    query = utils.BongosecDBQuery(offset=0, limit=limit, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.BongosecDBBackend(agent_id=1),
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_sort_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests BongosecDBQuery._sort_query function works"""

    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.BongosecDBBackend(agent_id=1), count=5,
                               get_data=None)
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_add_sort_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                  sort, expected_exception):
    """Test BongosecDBQuery._add_sort_to_query function."""
    fields = {'1': 'one', '2': 'two', '3': 'three', '4': 'four'}
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=sort,
                               search=None, select=None, filters=None,
                               fields=fields,
                               default_sort_field=None, query=None,
                               backend=utils.BongosecDBBackend(agent_id=1),
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_add_search_to_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test BongosecDBQuery._add_search_to_query function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('bongosec.core.utils.glob.glob', return_value=True)
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_parse_select_filter(mock_socket_conn, mock_glob, mock_conn_db, mock_exists,
                                                    selector_fields, error, expected_exception):
    """Test BongosecDBQuery._parse_select_filter function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('bongosec.core.utils.BongosecDBQuery._parse_select_filter')
def test_BongosecDBQuery_protected_add_select_to_query(mock_parse, mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test BongosecDBQuery._add_select_to_query function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('bongosec.core.utils.path.exists', return_value=True)
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
def test_BongosecDBQuery_protected_parse_query_regex(mock_backend_connect, mock_exists, q, expected_query_filters):
    """Test BongosecDBQuery._parse_query function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_parse_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_parse_legacy_filters(mock_socket_conn, mock_conn_db, mock_glob,
                                                     mock_exists, filter_):
    """Test BongosecDBQuery._parse_legacy_filters function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=filter_,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.BongosecDBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('bongosec.core.utils.BongosecDBQuery._parse_legacy_filters')
@patch('bongosec.core.utils.BongosecDBQuery._parse_query')
def test_BongosecDBQuery_parse_filters(mock_query, mock_filter, mock_socket_conn, mock_conn_db, mock_glob,
                                    mock_exists, filter, q):
    """Test BongosecDBQuery._parse_filters function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search={"negation": True, "value": "1"}, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=q,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('bongosec.core.utils.BongosecDBQuery._filter_status')
@patch('bongosec.core.utils.BongosecDBQuery._filter_date')
def test_BongosecDBQuery_protected_process_filter(mock_date, mock_status, mock_socket_conn, mock_conn_db,
                                               mock_glob, mock_exists, field_name, field_filter, q_filter):
    """Tests BongosecDBQuery._process_filter."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04', 'status': 'active'},
                               default_sort_field=None, query=None,
                               backend=utils.BongosecDBBackend(agent_id=0), count=5,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('bongosec.core.utils.BongosecDBQuery._process_filter')
def test_BongosecDBQuery_protected_add_filters_to_query(mock_process, mock_socket_conn,
                                                     mock_conn_db, mock_glob, mock_exists):
    """Test BongosecDBQuery._add_filters_to_query function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=['os.name'],
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query='os.name=ubuntu',
                               backend=utils.BongosecDBBackend(agent_id=0), distinct=True,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('bongosec.core.utils.path.exists', return_value=True)
def test_BongosecDBQuery_protected_add_filters_to_query_final_query(mock_conn_db, mock_file_exists,
                                                                 filters, expected_query):
    """Test BongosecDBQuery._add_filters_to_query final query."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None, search=None, select=None,
                               fields={'id': 'id', 'status': 'status', 'group': 'group'},
                               default_sort_field=None, query='', backend=utils.BongosecDBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_get_total_items(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test BongosecDBQuery._get_total_items function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_get_total_items_mitre(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.BongosecDBBackend(agent_id=0, query_format='mitre'), count=5,
                               get_data=None, date_fields=['date1', 'date2'])
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_substitute_params(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.BongosecDBQuery._get_total_items function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.BongosecDBQuery._format_data_into_dictionary."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_filter_status(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.BongosecDBQuery._filter_status function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_filter_date(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            date_filter, filter_db_name, time, error):
    """Test utils.BongosecDBQuery._filter_date function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
    ...
    with patch('bongosec.core.utils.get_timeframe_in_seconds', return_value=time):
        if error:
            with pytest.raises(exception.BongosecException, match=".* 1412 .*"):
...
@patch('socket.socket.connect')
def test_BongosecDBQuery_general_run(mock_socket_conn, execute_value, expected_result):
    """Test utils.BongosecDBQuery.general_run function."""
    with patch('bongosec.core.utils.BongosecDBBackend.execute', return_value=execute_value):
        query = BongosecDBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('socket.socket.connect')
def test_BongosecDBQuery_oversized_run(mock_socket_conn, execute_value, rbac_ids, negate,
                                    final_rbac_ids, expected_result):
    """Test utils.BongosecDBQuery.oversized_run function."""
    with patch('bongosec.core.utils.BongosecDBBackend.execute', side_effect=[execute_value, final_rbac_ids]):
        query = BongosecDBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('bongosec.core.utils.BongosecDBQuery._default_query')
def test_BongosecDBQuery_reset(mock_query, mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.BongosecDBQuery.reset function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'os.name'},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.BongosecDBQuery._default_query function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test utils.BongosecDBQuery._default_count_query function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQuery_protected_pass_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            value):
    """Test utils.BongosecDBQuery._pass_filter function."""
    query = utils.BongosecDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQueryDistinct_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    """Test utils.BongosecDBQueryDistinct._default_query function."""
    query = utils.BongosecDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQueryDistinct_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.BongosecDBQueryDistinct._default_count_query function."""
    query = utils.BongosecDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('bongosec.core.utils.BongosecDBQuery._add_filters_to_query')
def test_BongosecDBQueryDistinct_protected_add_filters_to_query(mock_add, mock_socket_conn, mock_conn_db,
                                                             mock_glob, mock_exists):
    """Test utils.BongosecDBQueryDistinct._add_filters_to_query function."""
    query = utils.BongosecDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
                                       query=None, select={'name'},
                                       fields={'name': '`group`'}, count=True,
                                       get_data=True, default_sort_field='`group`',
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('bongosec.core.utils.BongosecDBQuery._add_select_to_query')
def test_BongosecDBQueryDistinct_protected_add_select_to_query(mock_add, mock_socket_conn, mock_conn_db,
                                                            mock_glob, mock_exists, select):
    """Test utils.BongosecDBQueryDistinct._add_select_to_query function."""
    query = utils.BongosecDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
    ...
    if len(select) > 1:
        with pytest.raises(exception.BongosecException, match=".* 1410 .*"):
            query._add_select_to_query()
    else:
        query._add_select_to_query()
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQueryDistinct_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db,
                                                                    mock_glob, mock_exists):
    """Test utils.BongosecDBQueryDistinct._format_data_into_dictionary function."""
    query = utils.BongosecDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
                                       query=None, select={'fields': ['name']},
                                       fields={'name': '`group`'}, count=True,
                                       get_data=True,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BongosecDBQueryGroupBy__init__(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests utils.BongosecDBQueryGroupBy.__init__ function works"""
    utils.BongosecDBQueryGroupBy(filter_fields=None, offset=0, limit=1, table='agent',
                              ...
                              backend=utils.BongosecDBBackend(agent_id=0), get_data=None,
...
@patch('bongosec.core.utils.BongosecDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('bongosec.core.utils.BongosecDBQuery._get_total_items')
def test_BongosecDBQueryGroupBy_protected_get_total_items(mock_total, mock_socket_conn, mock_conn_db,
                                                       mock_glob, mock_exists):
    """Test utils.BongosecDBQueryGroupBy._get_total_items function."""
    query = utils.BongosecDBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
...
@patch('bongosec.core.utils.BongosecDBQuery._add_select_to_query')
@patch('bongosec.core.utils.BongosecDBQuery._parse_select_filter')
def test_BongosecDBQueryGroupBy_protected_add_select_to_query(mock_parse, mock_add, mock_socket_conn,
                                                           mock_conn_db, mock_glob, mock_exists):
    """Test utils.BongosecDBQueryGroupBy._add_select_to_query function."""
    query = utils.BongosecDBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
                                      limit=1, table='agent', sort=None, search=None,
                                      select={'name'}, filters=None,
                                      fields={'name': '`group`'}, query=None,
...
def test_filter_array_by_query(q, return_length):
    ...
    if return_length == -1:
        with pytest.raises(exception.BongosecError, match='.* 1407 .*'):
...
def test_select_array(select, required_fields, expected_result):
    ...
    except utils.BongosecError as e:
        assert e.code == 1724
...
@patch('bongosec.core.utils.check_bongosec_limits_unchanged')
@patch('bongosec.core.utils.check_remote_commands')
@patch('bongosec.core.utils.check_agents_allow_higher_versions')
@patch('bongosec.core.utils.check_virustotal_integration')
@patch('bongosec.core.utils.check_indexer')
@patch('bongosec.core.manager.common.BONGOSEC_PATH', new=test_files_path)
def test_validate_bongosec_xml(mock_check_indexer, mock_virus_total_integration,
                            mock_agents_versions, mock_remote_commands, mock_unchanged_limits):
    """Test validate_bongosec_xml method works and methods inside are called with expected parameters"""
    ...
    with patch('builtins.open', m):
        utils.validate_bongosec_xml(xml_file)
    ...
    with patch('builtins.open', m):
        utils.validate_bongosec_xml(xml_file, config_file=True)
...
def test_validate_bongosec_xml_ko(effect, expected_exception):
    """Tests validate_bongosec_xml function works when open function raises an exception.
    Parameters
    ----------
    effect : Exception
    ...
    with patch('bongosec.core.utils.load_bongosec_xml', side_effect=effect):
        with pytest.raises(BongosecException, match=f'.* {expected_exception} .*'):
            utils.validate_bongosec_xml(input_file)


@patch('bongosec.core.utils.full_copy')
...
@patch('bongosec.core.utils.full_copy', side_effect=IOError)
def test_delete_file_with_backup_ko(mock_copyfile):
    """Test delete_file_with_backup function exceptions."""
    with pytest.raises(utils.BongosecError, match='.* 1019 .*'):
...
def test_to_relative_path():
    ...
    path = 'etc/ossec.conf'
    assert utils.to_relative_path(os.path.join(BONGOSEC_PATH, path)) == path

    assert utils.to_relative_path(path, prefix='etc') == os.path.basename(path)


@patch('bongosec.core.utils.common.RULES_PATH', new=test_files_path)
@patch('bongosec.core.utils.common.USER_RULES_PATH', new=test_files_path)
...
@patch('bongosec.core.utils.common.DECODERS_PATH', new=test_files_path)
@patch('bongosec.core.utils.common.USER_DECODERS_PATH', new=test_files_path)
...
@patch('bongosec.core.utils.common.LISTS_PATH', new=test_files_path)
@patch('bongosec.core.utils.common.USER_LISTS_PATH', new=test_files_path)
...
@patch('bongosec.core.utils.copy2', new=copyfile)
...
def test_check_bongosec_limits_unchanged(new_conf, unchanged_limits_conf, original_conf, limits_conf):
    ...
    with patch('bongosec.core.utils.configuration.api_conf', new=api_conf):
        if limits_conf['eps']['allow'] or unchanged_limits_conf:
            utils.check_bongosec_limits_unchanged(new_conf, original_conf)
        else:
            with pytest.raises(exception.BongosecError, match=".* 1127 .*"):
                utils.check_bongosec_limits_unchanged(new_conf, original_conf)
...
def test_agents_allow_higher_versions(new_conf, agents_conf):
    ...
    with patch('bongosec.core.utils.configuration.api_conf', new=api_conf):
        if agents_conf['allow_higher_versions']['allow'] or new_conf.find('no') != -1:
            ...
            with pytest.raises(exception.BongosecError, match=".* 1129 .*"):
                utils.check_agents_allow_higher_versions(new_conf)
...
@patch('bongosec.core.utils.tempfile.mkstemp',
        return_value=('handle', os.path.join(OSSEC_TMP_PATH, 'file.tmp')))
@patch('bongosec.core.utils.chmod')
@patch('bongosec.core.common.bongosec_gid')
@patch('bongosec.core.common.bongosec_uid')
def test_upload_file(mock_uid, mock_gid, 
    ...
    ----------
    mock_uid: Mock
        mock of the bongosec.core.common.bongosec_uid function.
    mock_gid: Mock
        mock of the bongosec.core.common.bongosec_gid function.
    mock_chmod: Mock
        mock of the bongosec.core.utils.chmod function.
    mock_mks
        Mock of the bongosec.core.utils.tempfile.mkstemp function.
    chk_xml: bool
    ...
    with patch('bongosec.core.utils.open', mko):
        with patch('bongosec.core.utils.safe_move') as mock_safe_move:
            result = utils.upload_file(content, file_path=filename,
                                        check_xml_formula_values=chk_xml)
            assert isinstance(result, BongosecResult)
            handle.write.assert_called_once_with(content)
            tmp_path = os.path.join(OSSEC_TMP_PATH, 'file.tmp')
            file_path = os.path.join(BONGOSEC_PATH, filename)
...
@pytest.mark.parametrize(
    'sm_side_effect, w_side_effect, upload_error', [
        ...
        (None, True, (utils.BongosecInternalError, 1005)),
        # safe_move raises an Error()
        (Error(), False, (utils.BongosecInternalError, 1016)),
        # safe_move raises a PermissionError()
        (PermissionError(), False, (utils.BongosecError, 1006))]
)
@patch('bongosec.core.utils.tempfile.mkstemp',
        return_value=('handle', os.path.join(OSSEC_TMP_PATH, 'file.tmp')))
@patch('bongosec.core.utils.chmod')
...
def test_upload_file_ko(mock_uid, mock_gid, mock_chmod, mock_mks,
    ...
    with patch('bongosec.core.utils.open', mko):
        with patch('bongosec.core.utils.safe_move', side_effect=sm_side_effect) as mock_safe_move:
...
def test_check_virustotal_integration(integrations_conf, new_conf):
    ...
    with patch('bongosec.core.utils.get') as mock_requests_get:
        if not integrations_conf['virustotal']['public_key']['allow']:
            with pytest.raises(exception.BongosecError, match=".* 1130 .*"):
                mock_response = Mock()
            ...
            with pytest.raises(exception.BongosecError, match=".* 1131 .*"):
...
@pytest.mark.parametrize(
    ...
    [
        ("Bongosec v4.3.10", True),
        ("Bongosec v3.0.0", True),
        ("Bongosec v1.2.3", True),
        ("Bongosecv4.3.10", False),
        ("bongosec v4.3.10", False),
        ("Bongosec v4.3", False),
        ("Bongosec 4.3.10", False),
        ("Not a version", False),
    ]
)
def test_check_if_bongosec_agent_version(version_str, expected_result):
    """
    Test the check_if_bongosec_agent_version function.
    ...
    """
    assert utils.check_if_bongosec_agent_version(version_str) == expected_result


@pytest.mark.parametrize(
    "version_str, expected_tuple",
    [
        ("Bongosec v4.3.10", (4, 3, 10)),
        ("Bongosec v0.0.1", (0, 0, 1)),
    ]
)
def test_parse_bongosec_agent_version(version_str, expected_tuple):
    """
    Test the parse_bongosec_agent_version function.
    ...
    """
    assert utils.parse_bongosec_agent_version(version_str) == expected_tuple
