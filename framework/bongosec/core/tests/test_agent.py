...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        from bongosec.core.agent import *
        from bongosec.core.exception import BongosecException
        from api.util import remove_nones_to_dict
        from bongosec.core.common import reset_context_cache
...
def get_BongosecDBQuery_params(wdb_class):
    """Get default parameters for the specified BongosecDBQuery class.

    Parameters
    ----------
    wdb_class : str
        Suffix of the BongosecDBQuery class. Example: 'Agents' to get default parameters from `BongosecDBQueryAgents` class.
    ...
    """
    with patch('bongosec.core.agent.BongosecDBQuery.__init__') as wdbquery_mock:
        getattr(sys.modules[__name__], f'BongosecDBQuery{wdb_class}')()
        return wdbquery_mock.call_args.kwargs


# list with Bongosec packages availables with their hash
...
@patch("bongosec.core.agent.BongosecDBBackend")
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BongosecDBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    """Tests if method __init__ of BongosecDBQueryAgents works properly.
    ...
    if value:
        BongosecDBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(BongosecException, match=".* 2005 .*"):
            BongosecDBQueryAgents()


@patch('socket.socket.connect')
def test_BongosecDBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of BongosecDBQueryAgents returns expected query"""
    query_agent = BongosecDBQueryAgents()
...
@patch('socket.socket.connect')
def test_BongosecDBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of BongosecDBQueryAgents returns expected result
    ...
    """
    query_agent = BongosecDBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_BongosecDBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of BongosecDBQueryAgents returns expected query"""
    query_agent = BongosecDBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_BongosecDBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of BongosecDBQueryAgents returns expected data"""
    query_agent = BongosecDBQueryAgents(offset=0, limit=1, sort=None,
...
@patch('socket.socket.connect')
def test_BongosecDBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of BongosecDBQueryAgents returns expected query"""
    query_agent = BongosecDBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_BongosecDBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of BongosecDBQueryAgents returns expected query
...
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BongosecDBQueryGroup__init__(socket_mock, send_mock, backend_mock, value):
    """Test if method __init__ of BongosecDBQueryGroup works properly.
...
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BongosecDBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of BongosecDBQueryGroup works properly.
        ...
        """
    query_group = BongosecDBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_BongosecDBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of BongosecDBQueryGroupByAgents works properly."""
    query_group = BongosecDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
...
@patch('socket.socket.connect')
def test_BongosecDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of BongosecDBQueryGroupByAgents works properly."""
    query_group = BongosecDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
                                            search={'value': 'test', 'negation': True},
                                            select={'os.name'}, query=None, count=5, get_data=None)

    query_group.filter_fields = {'fields': set(query_group.filter_fields)}
    query_group._data = [{'count': 1, 'name': 'bongosec-master'},
                         {'count': 1, 'name': 'bongosec-agent1'}]
...
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BongosecDBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    """Tests if BongosecDBQueryGroupByAgents works properly."""
    query_group = BongosecDBQueryGroupByAgents(filter_fields=filter_fields, offset=0, limit=None, sort=None,
...
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BongosecDBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    """Tests if _add_sort_to_query method of BongosecDBQueryGroup works properly"""
    query_group = BongosecDBQueryGroup()
...
@patch('socket.socket.connect')
def test_BongosecDBQueryMultigroups__init__(mock_socket_conn):
    """Tests if method __init__ of BongosecDBQueryMultigroups works properly."""
    query_multigroups = BongosecDBQueryMultigroups(group_id='test')
...
@patch('socket.socket.connect')
def test_BongosecDBQueryMultigroups_default_query(mock_socket_conn, group_id):
    """Tests if method _default_query of BongosecDBQueryMultigroups works properly.
    ...
    """
    query_multigroups = BongosecDBQueryMultigroups(group_id=group_id)
...
@patch('socket.socket.connect')
def test_BongosecDBQueryMultigroups_default_count_query(mock_socket_conn):
    """Tests if method _default_count_query of BongosecDBQueryMultigroups works properly."""
    query_multigroups = BongosecDBQueryMultigroups(group_id='test')
...
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BongosecDBQueryMultigroups_get_total_items(mock_socket_conn, send_mock):
    """Tests if method _get_total_items of BongosecDBQueryMultigroups works properly."""
    query_multigroups = BongosecDBQueryMultigroups(group_id='test')
...
@patch('bongosec.core.agent.Agent._add')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(BongosecResourceNotFound, match='.* 1701 .*'):
...
def test_agent_get_key_ko(socket_mock, send_mock):
    ...
    with pytest.raises(BongosecError, match='.* 1703 .*'):
        agent = Agent('000')
        agent.get_key()


@patch('bongosec.core.agent.BongosecQueue.send_msg_to_agent')
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(BongosecQueue(common.AR_SOCKET))

    # Assert send_msg method is called with correct params
    mock_send_msg.assert_called_with(BongosecQueue.HC_FORCE_RECONNECT, agent_id)


@patch('bongosec.core.agent.BongosecQueue')
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(BongosecError, match='.* 1707 .*'):
        agent = Agent('003')
        agent.reconnect(mock_queue)


@patch('bongosec.core.agent.Agent._remove_authd', return_value='Agent was successfully deleted')
def test_agent_remove(mock_remove_authd):
    """Tests if method remove() works as expected."""

    with patch('bongosec.core.agent.get_manager_status', return_value={'bongosec-authd': 'running'}):
        agent = Agent('000')
        result = agent.remove()
...
def test_agent_remove_ko(mock_remove_authd):
    ...
    with pytest.raises(BongosecError, match='.* 1726 .*'):
        agent = Agent('000')
        agent.remove()


@patch('bongosec.core.agent.BongosecSocketJSON')
def test_agent_remove_authd(mock_bongosec_socket):
    """Tests if method remove_authd() works as expected"""
    agent = Agent('000')
    agent._remove_authd(purge=True)
    mock_bongosec_socket.return_value.send.assert_called_once_with(
        {"function": "remove", "arguments": {"id": str(0).zfill(3), "purge": True}})
    mock_bongosec_socket.return_value.receive.assert_called_once()
    mock_bongosec_socket.return_value.close.assert_called_once()
...
@patch('bongosec.core.agent.Agent._add_authd')
...
@patch('bongosec.core.agent.get_manager_status', return_value={'bongosec-authd': 'stopped'})
def test_agent_add_ko(mock_maganer_status):
    """Test if _add() method raises expected exception."""
    agent = Agent('001')

    with pytest.raises(BongosecError, match='.* 1706 .*'):
...
@patch('bongosec.core.agent.BongosecSocketJSON')
def test_agent_add_authd(mock_bongosec_socket, name, ip, id, key, force):
    ...
    mock_bongosec_socket.return_value.send.assert_called_once_with(socket_msg)


@pytest.mark.parametrize("mocked_exception, expected_exception", [
    (None, None),
    (BongosecError(9008, cmd_error=True), ".* 1705 .*"),
    (BongosecError(9007, cmd_error=True), ".* 1706 .*"),
    (BongosecError(9012, cmd_error=True), ".* 1708 .*"),
    (BongosecError(9000, cmd_error=True), ".* None")
])
@patch('bongosec.core.agent.BongosecSocketJSON')
def test_agent_add_authd_ko(mock_bongosec_socket, mocked_exception, expected_exception):
    """Tests if method _add_authd() raises expected exception"""
    agent = Agent('001')

    if not mocked_exception:
        with pytest.raises(BongosecError, match=".* 1709 .*"):
            agent._add_authd('test_add', '192.168.0.1', '2', 'adsiojew')
    else:
        mock_bongosec_socket.return_value.receive.side_effect = mocked_exception
        with pytest.raises(BongosecError, match=expected_exception):
...
@patch('bongosec.core.agent.rmtree')
@patch('bongosec.core.agent.path.exists', return_value=True)
@patch('bongosec.core.common.SHARED_PATH', new=os.path.join(test_data_path, 'etc', 'shared'))
...
@patch('bongosec.core.agent.Agent.get_agent_groups', return_value=['default'])
@patch('bongosec.core.agent.Agent.set_agent_group_relationship')
...
def test_agent_add_group_to_agent_ko(agent_groups_mock):
    ...
    with patch('bongosec.core.agent.Agent.get_agent_groups', side_effect=BongosecError(2003)):
        with pytest.raises(BongosecInternalError, match='.* 2007 .*'):
...
def test_agent_group_exists_ko():
    ...
    with pytest.raises(BongosecError, match='.* 1722 .*'):
        Agent.group_exists('default**')


@patch('bongosec.core.agent.BongosecDBConnection.send', return_value=('ok', '["payload"]'))
@patch('socket.socket.connect')
...
@patch('bongosec.core.agent.BongosecDBConnection.send')
@patch('socket.socket.connect')
...
def test_agent_set_agent_group_relationship_ko(socket_connect_mock):
    ...
    with pytest.raises(BongosecInternalError, match='.* 2005 .*'):
...
@patch('bongosec.core.agent.Agent.set_agent_group_relationship')
@patch('bongosec.core.agent.Agent.group_exists', return_value=True)
@patch('bongosec.core.agent.Agent.get_basic_information')
def test_agent_unset_single_group_agent(agent_info_mock, group_exists_mock, set_agent_group_mock, agent_id, group_id,
    ...
    """
    with patch('bongosec.core.agent.Agent.get_agent_groups', return_value=previous_groups):
...
def test_agent_unset_single_group_agent_ko(socket_mock, agent_information_mock):
    ...
    with patch('bongosec.core.agent.Agent.group_exists', return_value=False):
        with pytest.raises(BongosecResourceNotFound, match='.* 1710 .*'):
            Agent.unset_single_group_agent('002', 'test_group')

    # Agent does not belong to group
    with patch('bongosec.core.agent.Agent.get_agent_groups', return_value=['new_group', 'new_group2']):
        # Group_id is not within group_list
        with pytest.raises(BongosecError, match='.* 1734 .*'):
            Agent.unset_single_group_agent('002', 'test_group', force=True)

    # Group ID is 'default' and it is the last only one remaining
    with patch('bongosec.core.agent.Agent.get_agent_groups', return_value=['default']):
        # Agent file does not exists
        with pytest.raises(BongosecError, match='.* 1745 .*'):
            Agent.unset_single_group_agent('002', 'default', force=True)


@patch('bongosec.core.bongosec_socket.BongosecSocket')
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config(socket_mock, send_mock, mock_bongosec_socket):
    """Test getconfig method returns expected message."""
    agent = Agent('001')
    mock_bongosec_socket.return_value.receive.return_value = b'ok {"test": "conf"}'
    result = agent.get_config('com', 'active-response', 'Bongosec v4.0.0')
...
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config_ko(socket_mock, send_mock, mock_bongosec_socket):
    """Test getconfig method raises expected exceptions."""
    # Invalid component
    agent = Agent('003')
    with pytest.raises(BongosecError, match=".* 1101 .*"):
        agent.get_config('invalid_component', 'active-response', 'Bongosec v4.0.0')

    # Component or config is none
    agent = Agent('003')
    with pytest.raises(BongosecError, match=".* 1307 .*"):
        agent.get_config('com', None, 'Bongosec v4.0.0')
        agent.get_config(None, 'active-response', 'Bongosec v4.0.0')

    # Agent Bongosec version is lower than ACTIVE_CONFIG_VERSION
    agent = Agent('002')
    with pytest.raises(BongosecInternalError, match=".* 1735 .*"):
        agent.get_config('com', 'active-response', 'Bongosec v3.6.0')


@patch('bongosec.core.bongosec_socket.BongosecSocket')
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats(socket_mock, send_mock, mock_bongosec_socket):
    """Test get_stats method returns expected message."""
    agent = Agent('001')
    mock_bongosec_socket.return_value.receive.return_value = b'{"error":0, "data":{"global":{}, "interval":{}}}'
...
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats_ko(socket_mock, send_mock, mock_bongosec_socket):
    """Test get_stats method raises expected exception when the agent's version is lower than required."""
    agent = Agent('002')
    with pytest.raises(BongosecInternalError, match=r'\b1735\b'):
        agent.get_stats('logcollector')


@pytest.mark.parametrize('agents_list, versions_list', [
    (['001', '002', '003', '004'],
     [{'version': ver} for ver in ['Bongosec v4.2.0', 'Bongosec v4.0.0', 'Bongosec v4.2.1', 'Bongosec v3.13.2']])
])
@patch('bongosec.core.agent.BongosecQueue.send_msg_to_agent')
@patch('bongosec.core.agent.BongosecQueue.__init__', return_value=None)
def test_send_restart_command(wq_mock, wq_send_msg, agents_list, versions_list):
    ...
    """
    with patch('bongosec.core.agent.Agent.get_basic_information', side_effect=versions_list):
        for agent_id, agent_version in zip(agents_list, versions_list):
            wq = BongosecQueue(common.AR_SOCKET)
            send_restart_command(agent_id, agent_version['version'], wq)
            expected_msg = BongosecQueue.RESTART_AGENTS_JSON if BongosecVersion(
                agent_version['version']) >= BongosecVersion(common.AR_LEGACY_VERSION) else BongosecQueue.RESTART_AGENTS
...
def test_get_agents_info():
    ...
    with patch('bongosec.core.agent.open', mock_open(read_data=client_keys)) as m:
...
def test_get_groups():
    ...
    with patch('bongosec.core.common.SHARED_PATH', new=shared):
        try:
...
def test_expand_group(socket_mock, group, wdb_response, expected_agents):
    ...
    wdb_response: list
        Mock return values for the `BongosecDBConnection.send` method.
    expected_agents : set
    ...
    with patch('bongosec.core.wdb.BongosecDBConnection.send', side_effect=wdb_response):
...
@patch('bongosec.core.agent.core_upgrade_agents')
