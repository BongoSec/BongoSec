...
from bongosec.core.exception import BongosecException, BongosecError
import pytest
...
@pytest.mark.parametrize('code, extra_message, extra_remediation, cmd_error, dapi_errors, title, type, exc_string', [
        ...
        (1017, {'node_name': 'Node Name', 'not_ready_daemons': 'not ready daemons'}, None, None, None, None, None, 
            'Error 1017 - Some Bongosec daemons are not ready yet in node "Node Name" (not ready daemons)'),        
    ])
def test_bongosec_exception_to_string(code, extra_message, extra_remediation, cmd_error, dapi_errors, title , type, exc_string):
    """Check object constructor """
    exc = BongosecException(code, extra_message, extra_remediation, cmd_error, dapi_errors, title, type)
    assert str(exc) == exc_string


def test_bongosec_exception__or__():
    """Check that BongosecException's | operator performs the join of dapi errors properly."""
    excp1 = BongosecException(1308)
    excp1._dapi_errors = {'test1': 'test error'}
    excp2 = BongosecException(1308)
    excp2._dapi_errors = {'test2': 'test error'}
    excp3 = excp2 | excp1
    assert excp3._dapi_errors == {'test1': 'test error', 'test2': 'test error'}


def test_bongosec_exception__deepcopy__():
    """Check that BongosecException's __deepcopy__ magic method works properly."""
    excp1 = BongosecException(1308)
    excp2 = excp1.__deepcopy__()
    assert excp1 == excp2 and excp1 is not excp2


def test_bongosec_error__or__():
    """Check that BongosecError's | operator performs the union of id sets properly."""
    error1 = BongosecError(1309, ids={1, 2, 3})
    error2 = BongosecError(1309, ids={4, 5, 6})
