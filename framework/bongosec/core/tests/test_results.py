...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        from bongosec.core.results import BongosecResult, AffectedItemsBongosecResult, _goes_before_than, nested_itemgetter, merge
        from bongosec import BongosecException, BongosecError

param_name = ['affected_items', 'total_affected_items', 'sort_fields', 'sort_casting', 'sort_ascending',
              'all_msg', 'some_msg', 'none_msg']
BONGOSEC_EXCEPTION_CODE = 1725
FAILED_AGENT_ID = '999'


@pytest.fixture(scope='function')
def get_bongosec_result():
    return BongosecResult(dct={"data": {"items": [{"item1": "data1"}, {"item2": "OK"}], "message": "Everything ok"}},
                       str_priority=['KO', 'OK'])


@pytest.fixture(scope='function')
def get_bongosec_affected_item():
    def _get_affected(params=None):
        kwargs = {p_name: param for p_name, param in zip(param_name, params)}
        return AffectedItemsBongosecResult(**kwargs)

    return _get_affected


@pytest.fixture(scope='function')
def get_bongosec_failed_item():
    item = AffectedItemsBongosecResult()
    item.add_failed_item(id_=FAILED_AGENT_ID, error=BongosecException(BONGOSEC_EXCEPTION_CODE))
    return item
...
def test_results_BongosecResult__merge_str(dikt, priority, get_bongosec_affected_item):
    """Test method `_merge_str` from `BongosecResult`.
        ...
        priority : list
            Used to set the BongosecResult priority.
        """
    bongosec_result = BongosecResult(deepcopy(dikt), str_priority=priority)
    assert isinstance(bongosec_result, BongosecResult)
    item2 = bongosec_result.dikt['data']['items'][1]['item2']
    merge_result = bongosec_result._merge_str(item2, 'KO')
    assert merge_result == priority[0] if priority else '{}|{}'.format(item2, 'KO')


def test_results_BongosecResult_to_dict(get_bongosec_result):
    """Test method `to_dict` from `BongosecResult`."""
    dict_result = get_bongosec_result.to_dict()
    assert isinstance(dict_result, dict)
    assert (key == result_key for key, result_key in zip(['str_priority', 'result'], dict_result.keys()))


def test_results_BongosecResult_render(get_bongosec_result):
    """Test method `render` from `BongosecResult`."""
    render_result = get_bongosec_result.render()
    assert isinstance(render_result, dict)
    assert render_result == get_bongosec_result.dikt


def test_results_BongosecResult_decode_json(get_bongosec_result):
    """Test class method `decode_json` from `BongosecResult`."""
    bongosec_result = get_bongosec_result
    decoded_result = BongosecResult.decode_json(bongosec_result.to_dict())
    assert decoded_result == bongosec_result
...
def test_results_AffectedItemsBongosecResult(get_bongosec_affected_item, param_value):
    """Test class `AffectedItemsBongosecResult`.
    ...
    """
    affected_result = get_bongosec_affected_item(param_value)
    assert isinstance(affected_result, AffectedItemsBongosecResult)
    for value, dikt_value in zip(param_value, affected_result.dikt.values()):
        assert value == dikt_value


def test_results_AffectedItemsBongosecResult_add_failed_item():
    """Test method `add_failed_item` from class `AffectedItemsBongosecResult`."""
    affected_result = AffectedItemsBongosecResult()
    id_list = ['001', '002']
    # Add two failed items with different id but same exception
    for agent_id in id_list:
        affected_result.add_failed_item(id_=agent_id, error=BongosecException(BONGOSEC_EXCEPTION_CODE))

    assert affected_result.failed_items
    assert set(id_list) == next(iter(affected_result.failed_items.values()))


def test_results_AffectedItemsBongosecResult_add_failed_items_from(get_bongosec_failed_item):
    """Test method `add_failed_items_from` from class `AffectedItemsBongosecResult`."""
    affected_result = AffectedItemsBongosecResult()
    failed_result = get_bongosec_failed_item
    affected_result.add_failed_items_from(failed_result)
    assert affected_result.failed_items == failed_result.failed_items


def test_results_AffectedItemsBongosecResult_add_failed_items_from_exception():
    """Test raised exception from method `add_failed_items_from` from class `AffectedItemsBongosecResult`."""
    affected_result = AffectedItemsBongosecResult()
    with pytest.raises(BongosecException, match='.* 1000 .*'):
        affected_result.add_failed_items_from('Invalid type')


def test_results_AffectedItemsBongosecResult_remove_failed_items(get_bongosec_failed_item):
    """Test method `remove_failed_items` from class `AffectedItemsBongosecResult`."""
    failed_result = get_bongosec_failed_item
    failed_result.remove_failed_items(code={BONGOSEC_EXCEPTION_CODE})
    assert not failed_result.failed_items


def test_results_AffectedItemsBongosecResult___or__(get_bongosec_failed_item):
    """Test method `__or__` from class `AffectedItemsBongosecResult`."""
    agent_list_1 = ['001', '002']
    agent_list_2 = ['004', '003']
    affected_item_1 = AffectedItemsBongosecResult(affected_items=deepcopy(agent_list_1))
    affected_item_2 = AffectedItemsBongosecResult(affected_items=deepcopy(agent_list_2))
    failed_item = get_bongosec_failed_item
...
@pytest.mark.parametrize('or_item, expected_result', [
    (BongosecError(BONGOSEC_EXCEPTION_CODE, ids=['001']), AffectedItemsBongosecResult),
    (BongosecError(BONGOSEC_EXCEPTION_CODE), BongosecException),
    (BongosecException(BONGOSEC_EXCEPTION_CODE), BongosecException),
    ({'Invalid type': None}, None)
])
def test_results_AffectedItemsBongosecResult___or___exceptions(or_item, expected_result):
    """Test raised exceptions from method `__or__` from class `AffectedItemsBongosecResult`."""
    affected_result = AffectedItemsBongosecResult()
...
def test_results_AffectedItemsBongosecResult_to_dict():
    """Test method `to_dict` from class `AffectedItemsBongosecResult`."""
    affected_result = AffectedItemsBongosecResult()
...
def test_results_AffectedItemsBongosecResult_properties():
    """Test getters and setters from class `AffectedItemsBongosecResult`."""
    affected_result = AffectedItemsBongosecResult()
...
def test_results_AffectedItemsBongosecResult_failed_items_properties(get_bongosec_failed_item):
    """Test `failed_items` properties from class `AffectedItemsBongosecResult`."""
    fail_items = get_bongosec_failed_item.failed_items
    total_fail_items = get_bongosec_failed_item.total_failed_items
    assert isinstance(fail_items, dict)
    assert total_fail_items == 1


def test_results_AffectedItemsBongosecResult_message_property():
    """Test `message` property from class `AffectedItemsBongosecResult`."""
    ...
    none_msg_result = AffectedItemsBongosecResult(**messages).message
    all_msg_result = AffectedItemsBongosecResult(**messages, affected_items=['001']).message
    aux_result = AffectedItemsBongosecResult(**messages, affected_items=['001'])
    aux_result.add_failed_item(BongosecException(BONGOSEC_EXCEPTION_CODE))
...
def test_results_AffectedItemsBongosecResult__merge_str(self_field, other_field, key, expected_result):
    """Test method `_merge_str` from class `AffectedItemsBongosecResult`."""
    affected_result = AffectedItemsBongosecResult()
    merge_result = affected_result._merge_str(self_field, other_field, key=key)
    assert merge_result == expected_result


def test_results_AffectedItemsBongosecResult_encode_decode_json(get_bongosec_affected_item):
    """Test methods `encode_json` and `decode_json` from class `AffectedItemsBongosecResult`."""
    param_list = [['001', '002'], 2, None, ['int'], [True, True], 'Sample message', 'Sample message', 'Sample message']
    affected_result = get_bongosec_affected_item(param_list)
    affected_result.add_failed_item(id_=FAILED_AGENT_ID, error=BongosecException(BONGOSEC_EXCEPTION_CODE))
    # Use a complete AffectedIemsBongosecResult to encode a json and then decode it
    json_item = affected_result.encode_json()
    decoded_json = AffectedItemsBongosecResult.decode_json(json_item)
    assert affected_result == decoded_json


def test_results_AffectedItemsBongosecResult_render(get_bongosec_affected_item):
    """Test method `render` from class `AffectedItemsBongosecResult`."""
    param_list = [['001', '002'], 2, None, ['int'], [True, True], 'Sample message', 'Sample message', 'Sample message']
    affected_result = get_bongosec_affected_item(param_list)
    for agent_id in [FAILED_AGENT_ID, 'Invalid ID']:
        affected_result.add_failed_item(id_=agent_id, error=BongosecException(BONGOSEC_EXCEPTION_CODE))
    # Render a valid AffectedItemsBongosecResult and check it has all the expected fields
