...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        ...
        from bongosec.core import rule
...
@pytest.mark.parametrize('rule_file, rule_path, rule_status, exception', [
    ...
    ('noexists.xml', 'tests/data/rules', 'enabled', BongosecError(1201))
])
@patch("bongosec.core.common.BONGOSEC_PATH", new=parent_directory)
@patch("bongosec.core.common.RULES_PATH", new=data_path)
...
@patch("bongosec.core.rule.load_bongosec_xml", side_effect=OSError(13, 'Error', 'Permissions'))
def test_load_rules_from_file_permissions(mock_load):
    """Test set_groups rule core function."""
    with pytest.raises(BongosecError, match='.* 1207 .*'):
        rule.load_rules_from_file('nopermissions.xml', 'tests/data/rules', 'disabled')


@patch("bongosec.core.rule.load_bongosec_xml", side_effect=OSError(8, 'Error', 'Unknown'))
