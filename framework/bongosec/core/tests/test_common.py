...
from bongosec.core.common import find_bongosec_path, bongosec_uid, bongosec_gid, context_cached, reset_context_cache, \
    get_context_cache


@pytest.mark.parametrize('fake_path, expected', [
    ('/var/ossec/framework/python/lib/python3.7/site-packages/bongosec-3.10.0-py3.7.egg/bongosec', '/var/ossec'),
    ('/my/custom/path/framework/python/lib/python3.7/site-packages/bongosec-3.10.0-py3.7.egg/bongosec', '/my/custom/path'),
    ('/my/fake/path', '')
])
def test_find_bongosec_path(fake_path, expected):
    with patch('bongosec.core.common.__file__', new=fake_path):
        assert (find_bongosec_path.__wrapped__() == expected)


def test_find_bongosec_path_relative_path():
    with patch('os.path.abspath', return_value='~/framework'):
        assert (find_bongosec_path.__wrapped__() == '~')


def test_bongosec_uid():
    with patch('bongosec.core.common.getpwnam', return_value=getpwnam("root")):
        bongosec_uid()


def test_bongosec_gid():
    with patch('bongosec.core.common.getgrnam', return_value=getgrnam("root")):
        bongosec_gid()
...
@patch('bongosec.core.logtest.create_bongosec_socket_message', side_effect=SystemExit)
...
async def test_origin_module_context_var_api(mock_check_bongosec_status, mock_create_socket_msg):
    ...
    # side_effect used to avoid mocking the rest of functions
    with pytest.raises(SystemExit):
        d = dapi.DistributedAPI(f=logtest.run_logtest, logger=logging.getLogger('bongosec'), is_async=True)
