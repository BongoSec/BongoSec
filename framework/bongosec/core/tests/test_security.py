...
from bongosec.tests.test_security import db_setup  # noqa
...
def test_revoke_tokens(db_setup):
    """Checks that the return value of revoke_tokens is a BongosecResult.
    ...
    db_setup: callable
        This function creates the rbac.db file.
    """
    with patch('bongosec.core.security.change_keypair', side_effect=None):
        security, BongosecResult, _ = db_setup
        mock_current_user = ContextVar('current_user', default='bongosec')
        with patch("bongosec.sca.common.current_user", new=mock_current_user):
...
def test_invalid_roles_tokens(db_setup, role_list, expected_roles):
    ...
    expected_roles : set
        Expected roles.
    """
    with patch('bongosec.core.security.TokenManager.add_user_roles_rules') as TM_mock:
...
@patch('bongosec.core.security.TokenManager.add_user_roles_rules')
...
@patch("bongosec.core.security.revoke_tokens")
@patch("bongosec.core.security.check_database_integrity")
@patch("bongosec.core.security.os.remove")
