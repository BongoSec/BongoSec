...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        from bongosec.core import wlogging
...
def test_sizebasedfilerotatinghandler_compute_log_directory(mock_mkdir):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("bongosec.core.wlogging.os.path.exists", new=_mock_exists):
...
@patch('bongosec.core.wlogging.SizeBasedFileRotatingHandler')
@patch('bongosec.core.wlogging.TimeBasedFileRotatingHandler')
def test_bongosec_logger_setup_logger(mock_time_handler, mock_size_handler, mock_add_handler, mock_add_level_name,
                                   max_size):
    """Test if method setup_logger of BongosecLogger setups the logger attribute properly.
    ...
    # To bypass the checking of the existence of a valid Bongosec install
    # Check time handler
    with patch('os.path.join', return_value=tmp_dir.name):
        w_logger = wlogging.BongosecLogger(foreground_mode=True, log_path=tmp_dir.name,
...
@patch.object(wlogging.BongosecLogger, 'log_path', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BongosecLogger, 'tag', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BongosecLogger, 'logger', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BongosecLogger, 'foreground_mode', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BongosecLogger, 'debug_level', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BongosecLogger, 'logger_name', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BongosecLogger, 'custom_formatter', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BongosecLogger, 'max_size', create=True, new_callable=PropertyMock)
@patch('logging.Formatter')
def test_bongosec_logger__init__(mock_lformatter, mock_max_size, mock_formatter, mock_logger_name, mock_debug_level,
                              mock_foreground_mode, mock_logger, mock_tag, mock_log_path):
    """Test if BongosecLogger __init__ method initialize all attributes properly."""
    # To bypass the checking of the existence of a valid Bongosec install
    with patch('os.path.join'):
        wlogging.BongosecLogger(foreground_mode=mock_foreground_mode, log_path=mock_log_path, tag=mock_tag,
...
@patch('bongosec.core.wlogging.SizeBasedFileRotatingHandler')
@patch('bongosec.core.wlogging.TimeBasedFileRotatingHandler')
def test_bongosec_logger_getattr(mock_time_handler, mock_size_handler, attribute, expected_exception, expected_value):
    """Test if BongosecLogger __getattr__ method works properly."""
    tmp_dir = tempfile.TemporaryDirectory()
    # To bypass the checking of the existence of a valid Bongosec install
    with patch('os.path.join'):
        w_logger = wlogging.BongosecLogger(foreground_mode=True, log_path=tmp_dir.name, tag='%(test)s %(test)s: %(test)s',
