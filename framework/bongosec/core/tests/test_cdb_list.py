...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        ...
        from bongosec.core.cdb_list import check_path, get_list_from_file, iterate_lists, \
...
RELATIVE_PATH = os.path.join("framework", "bongosec", "core", "tests", "data", "test_cdb_list")
PATH_FILE = os.path.join(RELATIVE_PATH, "test_lists")

CONTENT_FILE = {'test-bongosec-w': 'write',
                'test-bongosec-r': 'read',
                'test-bongosec-a': 'attribute',
                'test-bongosec-x': 'execute',
                'test-bongosec-c': 'command',
                'test-key': 'value:1',
                'test-key:1': 'value',
                'test-key:2': 'value:2',
                'test-key::::::3': 'value3',
...
def test_split_key_value_with_quotes(line, expected_key, expected_value):
    ...
    if expected_key and expected_value:
        ...
        with pytest.raises(BongosecError) as e:
...
def test_get_list_from_file(raw):
    ...
    """
    full_path = os.path.join(common.BONGOSEC_PATH, PATH_FILE)
    if raw:
...
@pytest.mark.parametrize("error_to_raise, bongosec_error_code", [
...
def test_get_list_from_file_with_errors(error_to_raise, bongosec_error_code):
    """Test `get_list_from_file` core functionality when using invalid files or paths as parameter.

    `get_list_from_file` must raise the proper BongosecError when facing certain scenarios like a Permission Denied error
    when opening a file.
    ...
    bongosec_error_code : int
        Error code of the `BongosecError` that must be raised by `get_list_from_file` when the specified `OSError` occurrs.
    """
    with patch("builtins.open", mock_open()) as mock:
        ...
        try:
            ...
            assert e.code == bongosec_error_code
...
def test_validate_cdb_list():
    ...
    with open(os.path.join(common.BONGOSEC_PATH, PATH_FILE)) as f:
        bongosec_cdb_list = f.read()

    try:
        validate_cdb_list(bongosec_cdb_list)
    except BongosecError:
...
def test_validate_cdb_list_ko():
    ...
    with pytest.raises(BongosecError, match=r'\b1112\b'):
        validate_cdb_list('')

    # Raise exeption when using invalid CDB list
    with pytest.raises(BongosecError, match=r'\b1800\b'):
        validate_cdb_list("test:key:testvalue\n")


@patch('bongosec.core.cdb_list.chmod')
...
def test_create_list_file(mock_delete, mock_chmod):
    ...
    with open(os.path.join(common.BONGOSEC_PATH, PATH_FILE)) as f:
        with patch('bongosec.core.cdb_list.common.BONGOSEC_PATH', new='/var/ossec'):
            with patch('builtins.open') as mock_open:
                bongosec_cdb_list = f.read()
                result = create_list_file('/test/path', bongosec_cdb_list, permissions=0o660)
                assert mock_open.return_value.__enter__().write.call_count == len(bongosec_cdb_list.split('\n'))-1
...
def test_create_list_file_ko():
    ...
    with pytest.raises(BongosecError, match=r'\b1806\b'):
        create_list_file(full_path='/test/path', content=' ')


@patch('bongosec.core.cdb_list.remove')
@patch('bongosec.core.cdb_list.delete_bongosec_file')
def test_delete_list(mock_delete_bongosec_file, mock_remove):
    """Check that delete_list function uses expected params."""
    path = 'etc/list/test_list'
    delete_list(path)
    mock_delete_bongosec_file.assert_called_once_with(os.path.join(common.BONGOSEC_PATH, path))
    mock_remove.assert_called_once_with(os.path.join(common.BONGOSEC_PATH, path + '.cdb'))
