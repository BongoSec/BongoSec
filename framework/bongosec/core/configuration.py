...
from bongosec.core import common, bongosec_socket
from bongosec.core.exception import BongosecError, BongosecInternalError, BongosecResourceNotFound
from bongosec.core.utils import cut_array, load_bongosec_xml, safe_move
...
DEFAULT_CTI_URL = 'https://cti.bongosec.github.io'
...
def _replace_custom_values(opt_value: Union[list, dict, str]) -> Union[list, dict, str]:
    """Replace custom values introduced by 'load_bongosec_xml' with their real values.
...
def _rcl2json(filepath: str) -> dict:
    ...
    try:
        ...
        raise BongosecError(1101, str(e))
...
def get_ossec_conf(section: str = None, field: str = None, conf_file: str = common.OSSEC_CONF,
    ...
    try:
        # Read XML
        xml_data = load_bongosec_xml(conf_file)

        # Parse XML to JSON
        data = _ossecconf2json(xml_data)
    except Exception as e:
        if not from_import:
            raise BongosecError(1101, extra_message=str(e))
        else:
            print(f"bongosec-apid: There is an error in the ossec.conf file: {str(e)}")
            sys.exit(0)
    ...
    if section:
        try:
            ...
            if section not in CONF_SECTIONS.keys():
                raise BongosecError(1102, extra_message=e.args[0])
            else:
                raise BongosecError(1106, extra_message=e.args[0])
...
def get_agent_conf(group_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if not os_path.exists(os_path.join(common.SHARED_PATH, group_id)):
        raise BongosecResourceNotFound(1710, group_id)
    ...
    if not os_path.exists(agent_conf):
        raise BongosecError(1006, agent_conf)
    ...
    try:
        ...
        # Parse XML to JSON
            ...
            # Read XML
            xml_data = load_bongosec_xml(agent_conf)
...
def get_agent_conf_multigroup(multigroup_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if multigroup_id and not os_path.exists(os_path.join(common.MULTI_GROUPS_PATH, multigroup_id)) or not multigroup_id:
        raise BongosecResourceNotFound(1710, extra_message=multigroup_id if multigroup_id else "No multigroup provided")
    ...
    if not os_path.exists(agent_conf):
        raise BongosecError(1006, extra_message=os_path.join("BONGOSEC_PATH", "var", "multigroups", agent_conf))
...
def get_file_conf(filename: str, group_id: str = None, type_conf: str = None, raw: bool = False) -> dict | str:
    ...
    if not os_path.exists(file_path):
        raise BongosecError(1006, file_path)
    ...
    if type_conf:
        if type_conf in types:
            ...
            raise BongosecError(1104, f'{type_conf}. Valid types: {types.keys()}')
    else:
...
def parse_internal_options(high_name: str, low_name: str) -> str:
    ...
    Raises
    ------
    BongosecInternalError(1107)
        Internal options file not found.
    BongosecInternalError(1108)
    ...
    if not os_path.exists(common.INTERNAL_OPTIONS_CONF):
        raise BongosecInternalError(1107)
    ...
    try:
        ...
        raise BongosecInternalError(1108, e.args[0])
...
def get_internal_options_value(high_name: str, low_name: str, max_: int, min_: int) -> int:
    ...
    if not option.isdigit():
        raise BongosecError(1109, f'Option: {high_name}.{low_name}. Value: {option}')

    option = int(option)
    if option < min_ or option > max_:
        raise BongosecError(1110, f'Max value: {max_}. Min value: {min_}. Found: {option}.')
...
def upload_group_configuration(group_id: str, file_content: str) -> str:
    ...
    BongosecError(1113)
        XML syntax error.
    BongosecError(1114)
        Bongosec syntax error.
    BongosecError(1115)
        Error executing verify-agent-conf.
    BongosecInternalError(1743)
        Error running Bongosec syntax validator.
    ...
    try:
        # check Bongosec xml format
        try:
            subprocess.check_output([os_path.join(common.BONGOSEC_PATH, "bin", "verify-agent-conf"), '-f', tmp_file_path],
            ...
            if output_regex:
                raise BongosecError(1114, ' '.join(output_regex))
            else:
                raise BongosecError(1115, e.output.decode())
        except Exception as e:
            raise BongosecInternalError(1743, str(e))

        # move temporary file to group folder
        try:
            new_conf_path = os_path.join(common.SHARED_PATH, group_id, "agent.conf")
            safe_move(tmp_file_path, new_conf_path, ownership=(common.bongosec_uid(), common.bongosec_gid()),
                      permissions=0o660)
        except Exception as e:
            raise BongosecInternalError(1016, extra_message=str(e))
...
def get_active_configuration(agent_id: str, component: str, configuration: str) -> dict:
    ...
    BongosecInternalError(1121)
        If the socket cant be created.
    BongosecInternalError(1118)
    ...
    component_socket_mapping = {'agent': 'analysis', 'agentless': 'agentless', 'analysis': 'analysis', 'auth': 'auth',
                                ...
                                'request': 'remote', 'syscheck': 'syscheck', 'bongosec-db': 'wdb', 'wmodules': 'wmodules'}
    component_socket_dir_mapping = {'agent': 'sockets', 'agentless': 'sockets', 'analysis': 'sockets',
                                    ...
                                    'request': 'sockets', 'syscheck': 'sockets', 'bongosec-db': 'db',
                                    'wmodules': 'sockets'}

    if not component or not configuration:
        raise BongosecError(1307)

    # Check if the component is correct
    components = component_socket_mapping.keys()
    if component not in components:
        raise BongosecError(1101, f'Valid components: {", ".join(components)}')
    ...
    def get_active_configuration_manager():
        ...
        dest_socket = os_path.join(common.BONGOSEC_PATH, "queue", component_socket_dir_mapping[component],
                                   component_socket_mapping[component])

        # Verify component configuration
        if not os.path.exists(dest_socket):
            raise BongosecError(1121,
        ...
        if component_socket_mapping[component] not in sockets_json_protocol:
            ...
            # Socket connection
            try:
                s = bongosec_socket.BongosecSocket(dest_socket)
            except BongosecInternalError:
                raise
            except Exception as unhandled_exc:
                raise BongosecInternalError(1121, extra_message=str(unhandled_exc))
            ...
            try:
                ...
                raise BongosecInternalError(1118, extra_message="Data could not be received")
            finally:
                s.close()
        ...
        # JSON socket message
            ...
            msg = bongosec_socket.create_bongosec_socket_message(origin={'module': common.origin_module.get()},
                                                           command=GETCONFIG_COMMAND,
                                                           parameters={'section': configuration})

            # Socket connection
            try:
                s = bongosec_socket.BongosecSocketJSON(dest_socket)
            except BongosecInternalError:
                raise
            except Exception as unhandled_exc:
                raise BongosecInternalError(1121, extra_message=str(unhandled_exc))
    ...
    if rec_error == 'ok' or rec_error == 0:
        ...
        if data.get('auth', {}).get('use_password') == 'yes':
            try:
                with open(os_path.join(common.BONGOSEC_PATH, "etc", "authd.pass"), 'r') as f:
        ...
        raise BongosecError(1117 if "No such file or directory" in rec_data or "Cannot send request" in rec_data else 1116,
                         extra_message=f'{component}:{configuration}')


def write_ossec_conf(new_conf: str):
    """Replace the current bongosec configuration (ossec.conf) with the provided configuration.
    ...
    try:
        ...
        raise BongosecError(1126, extra_message=str(e))
