...
import bongosec.core.cluster.cluster
import bongosec.core.cluster.utils
import bongosec.core.manager
import bongosec.core.results as wresults
from bongosec import agent
from bongosec.cluster import get_node_wrapper, get_nodes_info
from bongosec.core import common, exception
from bongosec.core.cluster import local_client, common as c_common
from bongosec.core.cluster.cluster import check_cluster_status
from bongosec.core.exception import BongosecException, BongosecClusterError, BongosecError
from bongosec.core.pyDaemonModule import spawn_process_pool_worker, API_AUTHENTICATION_PROCESS
from bongosec.core.bongosec_socket import bongosec_sendsync


authentication_funcs = {'check_token', 'check_user_master', 'get_permissions', 'get_security_conf'}
events_funcs = {'send_event_to_analysisd'}

node_info = bongosec.core.cluster.cluster.get_node()
pools = common.mp_pools.get()
if node_info['type'] == 'master':
    # Suppress exception when the user running Bongosec cannot access /dev/shm.
...
class DistributedAPI:
    ...
    def __init__(self, f: Callable, logger: logging.getLogger, f_kwargs: Dict = None, node: c_common.Handler = None,
        ...
        self.cluster_items = bongosec.core.cluster.utils.get_cluster_items() if node is None else node.cluster_items
        self.debug = debug
        ...
        if not basic_services:
            self.basic_services = ('bongosec-modulesd', 'bongosec-analysisd', 'bongosec-execd', 'bongosec-db', 'bongosec-remoted')
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'bongosec-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.BongosecException]:
        """
        Distribute an API call.

        Returns
        -------
        dict or BongosecException
            Dictionary with API response or BongosecException in case of error.
        """
        try:
            ...
            try:
                response = json.loads(response, object_hook=c_common.as_bongosec_object) \
                    if isinstance(response, str) else response
            except json.decoder.JSONDecodeError:
                response = {'message': response}

            return response if isinstance(response, (wresults.AbstractBongosecResult, exception.BongosecException)) \
                else wresults.BongosecResult(response)

        except json.decoder.JSONDecodeError:
            e = exception.BongosecInternalError(3036)
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.BongosecInternalError as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.BongosecClusterError))
            return e
        except exception.BongosecError as e:
            e.dapi_errors = self.get_error_info(e)
            return e
            ...
            return exception.BongosecInternalError(1000,
                                                dapi_errors=self.get_error_info(e))

    def check_bongosec_status(self):
        """
        There are some services that are required for bongosec to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * Bongosec must be started before using the API is the services are stopped.

        The basic services bongosec needs to be running are: bongosec-modulesd, bongosec-remoted, bongosec-analysisd, bongosec-execd
        and bongosec-db
        """
        if self.f == bongosec.core.manager.status:
            return

        status = bongosec.core.manager.status()
        ...
        if not_ready_daemons:
            ...
            raise exception.BongosecInternalError(1017, extra_message=extra_info)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise exception.BongosecInternalError(3021)
                except OperationalError as exc:
                    raise exception.BongosecInternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.BongosecInternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.BongosecInternalError(3036)
            except process.BrokenProcessPool:
                raise exception.BongosecInternalError(900)
            ...
            # BongosecClusterError
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913, 1017} and not isinstance(e,
                                                                                           exception.BongosecClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.BongosecJSONEncoder)
        except (exception.BongosecError, exception.BongosecResourceNotFound) as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.BongosecJSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.BongosecInternalError(1000,
                                                           dapi_errors=self.get_error_info(e)),
                              cls=c_common.BongosecJSONEncoder)
    ...
    def get_error_info(self, e) -> Dict:
        ...
        except exception.BongosecException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.BongosecException) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of BongosecInternalError
        if isinstance(e, exception.BongosecInternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('BONGOSEC_HOME', os.path.relpath(h.baseFilename, start=common.BONGOSEC_PATH))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        res = json.loads(await client.send_file(os.path.join(common.BONGOSEC_PATH,
                                                             self.f_kwargs['tmp_file']),
                                                node_name),
                         object_hook=c_common.as_bongosec_object)
        os.remove(os.path.join(common.BONGOSEC_PATH, self.f_kwargs['tmp_file']))
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(command=b'dapi',
                                             data=json.dumps(self.to_dict(),
                                                             cls=c_common.BongosecJSONEncoder).encode())
        return json.loads(node_response,
                          object_hook=c_common.as_bongosec_object)

    async def forward_request(self) -> [wresults.AbstractBongosecResult, exception.BongosecException]:
        ...
        Returns
        -------
        wresults.AbstractBongosecResult or exception.BongosecException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractBongosecResult, exception.BongosecException]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a BongosecError exception
                if isinstance(result, BongosecClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = BongosecError.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractBongosecResult, exception.BongosecException)) \
                else wresults.BongosecResult(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsBongosecResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractBongosecResult):
        ...
        # It might be a BongosecError after reducing
        if isinstance(response, wresults.AffectedItemsBongosecResult):
...
class BongosecRequestQueue:
    """Represents a queue of Bongosec requests"""
...
class APIRequestQueue(BongosecRequestQueue):
    """
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('bongosec').getChild('dapi')
        self.logger.addFilter(bongosec.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_bongosec_object)
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.BongosecJSONEncoder).encode())
            except Exception as e:
...
class SendSyncRequestQueue(BongosecRequestQueue):
    """
    ...
    def __init__(self, server):
        super().__init__(server)
        self.logger = logging.getLogger('bongosec').getChild('sendsync')
        self.logger.addFilter(bongosec.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='SendSync'))
    ...
    async def run(self):
        while True:
            ...
            try:
                ...
                result = await bongosec_sendsync(**request)
