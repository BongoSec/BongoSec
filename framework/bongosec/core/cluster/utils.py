...
from bongosec.core.exception import BongosecError, BongosecException, BongosecInternalError, BongosecHAPHelperError
from bongosec.core.results import BongosecResult
from bongosec.core.utils import temporary_cache
from bongosec.core.bongosec_socket import create_bongosec_socket_message
from bongosec.core.wlogging import BongosecLogger
...
execq_lockfile = os.path.join(common.BONGOSEC_PATH, "var", "run", ".api_execq_lock")

HELPER_DEFAULTS = {
    HAPROXY_PORT: 5555,
    HAPROXY_PROTOCOL: 'http',
    HAPROXY_BACKEND: 'bongosec_reporting',
    ...
    for field in [
        ...
        if helper_config.get(field):
            try:
                ...
                raise BongosecError(3004, extra_message=f"HAProxy Helper {field} must be an integer.")
    ...
    for field in [IMBALANCE_TOLERANCE]:
        if helper_config.get(field):
            try:
                ...
                raise BongosecError(3004, extra_message=f"HAProxy Helper {field} must be a float.")
    return helper_config
...
def parse_haproxy_helper_config(helper_config: dict) -> dict:
    ...
    if helper_config[HAPROXY_PROTOCOL].lower() == 'https' and type(helper_config[HAPROXY_CERT]) == bool:
        raise BongosecHAPHelperError(3042, extra_message='HAProxy certificate file required in the haproxy_cert parameter')
...
def read_cluster_config(config_file=common.OSSEC_CONF, from_import=False) -> typing.Dict:
    ...
    try:
        ...
        if e.code == 1106:
            ...
            raise BongosecError(3006, extra_message=e.message)
    except Exception as e:
        raise BongosecError(3006, extra_message=str(e))
    ...
    if isinstance(config_cluster['port'], str) and not config_cluster['port'].isdigit():
        raise BongosecError(3004, extra_message="Cluster port must be an integer.")
...
def get_manager_status(cache=False) -> typing.Dict:
    ...
    try:
        ...
        raise BongosecInternalError(1913, extra_message=str(e))

    processes = ['bongosec-agentlessd', 'bongosec-analysisd', 'bongosec-authd', 'bongosec-csyslogd', 'bongosec-dbd', 'bongosec-monitord',
                 'bongosec-execd', 'bongosec-integratord', 'bongosec-logcollector', 'bongosec-maild', 'bongosec-remoted',
                 'bongosec-reportd', 'bongosec-syscheckd', 'bongosec-clusterd', 'bongosec-modulesd', 'bongosec-db', 'bongosec-apid']

    data, pidfile_regex, run_dir = {}, re.compile(r'.+\-(\d+)\.pid$'), os.path.join(common.BONGOSEC_PATH, "var", "run")
...
def get_cluster_status() -> typing.Dict:
    ...
    try:
        cluster_status |= {"running": "yes" if get_manager_status()['bongosec-clusterd'] == 'running' else "no"}
...
def manager_restart() -> BongosecResult:
    """Restart Bongosec manager.

    Send JSON message with the 'restart-bongosec' command to common.EXECQ_SOCKET socket.

    Raises
    ------
    BongosecInternalError(1901)
        If the socket path doesn't exist.
    BongosecInternalError(1902)
        If there is a socket connection error.
    BongosecInternalError(1014)
    ...
    try:
        ...
        # json msg for restarting Bongosec manager
        msg = json.dumps(create_bongosec_socket_message(origin={'module': common.origin_module.get()},
                                                     command=common.RESTART_BONGOSEC_COMMAND,
        ...
        if os.path.exists(socket_path):
            try:
                ...
                raise BongosecInternalError(1902)
        else:
            raise BongosecInternalError(1901)

        try:
            conn.send(msg.encode())
            conn.close()
        except socket.error as e:
            raise BongosecInternalError(1014, extra_message=str(e))
    finally:
        fcntl.lockf(lock_file, fcntl.LOCK_UN)
        lock_file.close()
        read_config.cache_clear()

    return BongosecResult({'message': 'Restart request sent'})
...
def get_cluster_items():
    ...
    try:
        ...
        with open(os.path.join(common.BONGOSEC_PATH, here, 'cluster.json')) as f:
        ...
        raise BongosecError(3005, str(e))
...
class ClusterLogger(BongosecLogger):
    """
    Define the logger used by bongosec-clusterd.
    """
    ...
    def setup_logger(self):
        ...
        Set ups cluster logger. In addition to super().setup_logger() this method adds:
            ...
            * Sets log level based on the "debug_level" parameter received from bongosec-clusterd binary.
        """
        super().setup_logger()
...
def process_spawn_sleep(child):
    ...
    """
    pid = os.getpid()
    pyDaemonModule.create_pid(f'bongosec-clusterd_child_{child}', pid)
...
async def forward_function(func: callable, f_kwargs: dict = None, request_type: str = 'local_master',
    ...
    Returns
    -------
    Return either a dict or `BongosecResult` instance in case the execution did not fail. Return an exception otherwise.
    """

    import concurrent
    from asyncio import run

    from bongosec.core.cluster.dapi.dapi import DistributedAPI
