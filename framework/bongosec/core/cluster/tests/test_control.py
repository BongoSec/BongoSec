...
with patch('bongosec.github.iomon.getgrnam'):
    with patch('bongosec.github.iomon.getpwnam'):
        with patch('bongosec.github.iomon.bongosec_uid'):
            with patch('bongosec.github.iomon.bongosec_gid'):
                ...
                from bongosec.core.cluster import control
                from bongosec.core.cluster.local_client import LocalClient
                from bongosec import BongosecInternalError, BongosecError
...
async def test_get_nodes():
    ...
    with patch('bongosec.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
    ...
    with patch('bongosec.core.cluster.local_client.LocalClient.execute', side_effect=[BongosecClusterError(3020), 'error']):
        with pytest.raises(BongosecClusterError):
            await control.get_nodes(lc=local_client)
...
async def test_get_system_nodes():
    ...
    with patch('bongosec.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
        ...
        for expected in expected_result:
            with patch('bongosec.core.cluster.control.get_nodes', return_value=expected):
                result = await control.get_system_nodes()
                assert result == [expected['items'][0]['name']]

        with patch('bongosec.core.cluster.control.get_nodes', side_effect=BongosecInternalError(3012)):
            result = await control.get_system_nodes()
            assert result == BongosecError(3013)
...
async def test_get_node_ruleset_integrity():
    ...
    local_client = LocalClient()
    with patch('bongosec.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client) as execute_mock:
