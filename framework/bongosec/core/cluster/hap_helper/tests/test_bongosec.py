...
import pytest
from bongosec.agent import get_agents, reconnect_agents
from bongosec.cluster import get_nodes_info
from bongosec.core.results import AffectedItemsBongosecResult

from framework.bongosec.core.cluster.hap_helper.bongosec import BongosecAgent, BongosecDAPI


class TestBongosecAgent:
    ...
    def test_can_reconnect(self, version: str, expected: bool):
        ...
        assert BongosecAgent.can_reconnect(version) == expected
    ...
    def test_get_agents_able_to_reconnect(self):
        ...
        assert BongosecAgent.get_agents_able_to_reconnect(agents_list=agents) == [2, 3]


@mock.patch('framework.bongosec.core.cluster.hap_helper.bongosec.DistributedAPI', autospec=True)
class TestBongosecDAPI:
    bongosec_dapi = BongosecDAPI(tag='test')

    @pytest.fixture
    def fixture_affected_items_result(self):
        return AffectedItemsBongosecResult()
    ...
    @mock.patch('framework.bongosec.core.cluster.hap_helper.bongosec.get_system_nodes', return_value={})
    async def test_get_cluster_nodes(
        self,
        get_system_nodes_mock: mock.AsyncMock,
        dapi_mock: mock.MagicMock,
        fixture_affected_items_result: AffectedItemsBongosecResult,
        nodes_data: list,
        excluded_nodes: list,
    ):
        """Check the correct output of `get_cluster_nodes` function."""

        self.bongosec_dapi.excluded_nodes = excluded_nodes
        ...
        ret_val = await self.bongosec_dapi.get_cluster_nodes()

        dapi_mock.assert_called_once_with(
            f=get_nodes_info,
            f_kwargs=None,
            logger=self.bongosec_dapi.logger,
    ...
    async def test_reconnect_agents(
        ...
        ret_val = await self.bongosec_dapi.reconnect_agents(agent_list=agent_list)
    ...
    async def test_get_agents_node_distribution(
        ...
        ret_val = await self.bongosec_dapi.get_agents_node_distribution()
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'q': 'id!=000',
                'limit': self.bongosec_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.bongosec_dapi.logger,
            request_type='local_master',
        )
    ...
    async def test_get_agents_belonging_to_node(
        self, dapi_mock: mock.MagicMock, fixture_affected_items_result: AffectedItemsBongosecResult, limit: Optional[int]
    ):
        ...
        ret_val = await self.bongosec_dapi.get_agents_belonging_to_node(node_name=node_name, limit=limit)
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'q': 'id!=000',
                'limit': limit or self.bongosec_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.bongosec_dapi.logger,
            request_type='local_master',
        )
