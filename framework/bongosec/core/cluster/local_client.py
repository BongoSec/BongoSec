...
from bongosec.core.cluster import client
...
class LocalClient(client.AbstractClientManager):
    ...
    def __init__(self):
        """Class constructor"""
        super().__init__(configuration=bongosec.core.cluster.utils.read_config(), enable_ssl=False, performance_test=0,
                         concurrency_test=0, file='', string=0, logger=logging.getLogger(), tag="Local Client",
                         cluster_items=bongosec.core.cluster.utils.get_cluster_items())
    ...
    async def start(self):
        ...
        try:
            self.transport, self.protocol = await loop.create_unix_connection(
                                             ...
                                             path=os.path.join(common.BONGOSEC_PATH, 'queue', 'cluster', 'c-internal.sock'))
        except (ConnectionRefusedError, FileNotFoundError):
            raise exception.BongosecInternalError(3012)
        except MemoryError:
            raise exception.BongosecInternalError(1119)
        except Exception as e:
            raise exception.BongosecInternalError(3009, str(e))
        ...
        while True:
            ...
            try:
                ...
                if min_timeout < self.cluster_items['intervals']['worker']['keep_alive']:
                    raise exception.BongosecInternalError(3020)
                else:
                    try:
