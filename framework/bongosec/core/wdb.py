...
class AsyncBongosecDBConnection:
    """
    ...
    async def _send(self, msg, raw=False):
        """Format and send message to bongosec-db socket without blocking event loop.

        Parameters
        ----------
        msg : str
            Message to be sent to bongosec-db.
        raw : bool
            If `True`, the status message from bongosec-db is included in the response.

        Returns
        -------
        str, list
            Result for the request sent to bongosec-db.
        """
        try:
            ...
            try:
                ...
                raise BongosecInternalError(2010, extra_message=e)

            if raw:
                return data
            elif data[0] == "err":
                raise BongosecError(2003, data[1])
            else:
                return json.loads(data[1], object_hook=BongosecDBConnection.json_decoder)
        except (FileNotFoundError, ConnectionError) as e:
            with contextlib.suppress(Exception):
                await self.open_connection()
            raise BongosecInternalError(2005, extra_message=e)
    ...
    async def run_wdb_command(self, command):
        ...
        ----------
        command : str
            Command to be executed inside bongosec-db
        ...
        if len(result) > 1:
            if result[0] == 'err':
                raise BongosecInternalError(2007, extra_message=result[1])

        else:
            if result[0] != 'ok':
                raise BongosecInternalError(2007)

        return result


class BongosecDBConnection:
    """
    ...
    def __init__(self, request_slice=500):
        ...
        ----------
        request_slice : int
            Maximum number of items to request from bongosec-db on the first call.
        """
        self.socket_path = common.WDB_SOCKET
        ...
        try:
            ...
            raise BongosecInternalError(2005, e)
    ...
    def __query_input_validation(self, query: str):
        ...
        for check, error_text in input_val_errors:
            if not check:
                raise BongosecError(2004, error_text)
    ...
    def _send(self, msg: str, raw: bool = False) -> dict:
        ...
        Raises
        ------
        BongosecInternalError(2009)
            Pagination error. Response from bongosec-db was over the maximum socket buffer size.
        BongosecError(2003)
        ...
        if data_size >= MAX_SOCKET_BUFFER_SIZE:
            raise BongosecInternalError(2009)
        ...
        if data[0] == "err":
            ...
            return BongosecDBConnection.loads(data[1])
    ...
    def loads(string: str) -> dict:
        ...
        ----------
        string : str
            String response from `bongosec-db`. It must be a dumped JSON.
        ...
        dict
            JSON object.
        """
        data = json.loads(string, object_hook=BongosecDBConnection.json_decoder)
        if '"(null)"' in string:
    ...
    def delete_agents_db(self, agents_id: List[str]) -> dict:
        """Delete agents db through bongosec-db service.
        ...
        -------
        dict
            Dict received from bongosec db in the form: {"agents": {"ID": "MESSAGE"}}, where MESSAGE may be one of the
            following:
                - Ok
                - Invalid agent ID
                ...
                - DB not found
        """
        return self._send(f"bongosecdb remove {' '.join(agents_id)}")
    ...
    def send(self, query: str, raw: bool = True) -> Union[str, dict]:
        ...
        ----------
        query : str
            Query to be executed in bongosec-db.
        raw : bool
    ...
    def execute(self, query, count=False, delete=False, update=False):
        ...
        def send_request_to_wdb(query_lower, step, off, response):
            try:
                ...
                response.extend(BongosecDBConnection.loads(request_response))
        ...
        if delete:
            ...
            if regex.match(query_lower) is None:
                raise BongosecError(2004, "Delete query is wrong")
        ...
        if update:
            regex = re.compile(r"\w+ \d+? sql update ([\w\d,*_ ]+) set value = '([\w\d,*_ ]+)' where key (=|like)?"
                               r" '([a-z0-9,*_%\- ]+)'")
            if regex.match(query_lower) is None:
                raise BongosecError(2004, "Update query is wrong")
            return self._send(query_lower)
        ...
        if not re.search(r'.?select count\([\w \*]+\)( as [^,]+)? from', query_without_where):
            ...
            try:
                ...
                raise BongosecError(2006, str(e))
            except (BongosecError, BongosecInternalError) as e:
                raise e
            except Exception as e:
                raise BongosecInternalError(2007, str(e))
