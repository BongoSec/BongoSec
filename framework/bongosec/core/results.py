...
import bongosec.core.exception as wexception
from bongosec.core import utils
from bongosec.core.common import DATABASE_LIMIT

current_module = sys.modules[__name__]


class AbstractBongosecResult(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.BongosecInternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractBongosecResult):
            self.dikt = dct.dikt
        else:
            raise wexception.BongosecInternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractBongosecResult subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractBongosecResult objects. When merged with a BongosecException, the result is
        always a BongosecException

        Parameters
        ----------
        other : AbstractBongosecResult or wexception.BongosecException

        Raises
        ------
        BongosecInternalError(1000)
            Bongosec internal error when two object could not be merged.

        Returns
        -------
        AbstractBongosecResult or wexception.BongosecException
            Resultant object.
        """
        if isinstance(other, wexception.BongosecException):
            return other
        elif not isinstance(other, (dict, AbstractBongosecResult)):
            raise wexception.BongosecInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractBongosecResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in BongosecResult subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in bongosec.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.

        Returns
        -------
        AbstractBongosecResult
            Filtered AbstractBongosecResult
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in BongosecResult subclasses.
...
class BongosecResult(AbstractBongosecResult):
    """
...
class AffectedItemsBongosecResult(AbstractBongosecResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.BongosecException = None):
        ...
        error : wexception.BongosecException
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsBongosecResult
        ...
        wexception.BongosecInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsBongosecResult):
            raise wexception.BongosecInternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsBongosecResult with self.

        Parameters
        ----------
        other : AffectedItemsBongosecResult
            AffectedItemsBongosecResult object to merge.
        ...
        if isinstance(other, wexception.BongosecError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.BongosecException):
            return result
        elif not isinstance(other, AffectedItemsBongosecResult):
            raise wexception.BongosecInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsBongosecResult as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractBongosecResult
            Instance of AbstractBongosecResult.
        """
        result = cls()
    ...
    def render(self) -> dict:
        """Render AffectedItemsBongosecResult object.

        Returns
        -------
        dict
            Return AffectedItemsBongosecResult as a dictionary with fields data, message and error.
        """
