...
from bongosec.core.utils import load_bongosec_xml, add_dynamic_detail
...
def load_decoders_from_file(decoder_file: str, decoder_path: str, decoder_status: str) -> list:
    ...
    try:
        decoders = list()
        position = 0
        root = load_bongosec_xml(os.path.join(common.BONGOSEC_PATH, decoder_path, decoder_file))
        ...
        raise BongosecError(1502, extra_message=os.path.join('BONGOSEC_HOME', decoder_path, decoder_file))
    except Exception:
        raise BongosecInternalError(1501, extra_message=os.path.join('BONGOSEC_HOME', decoder_path, decoder_file))
