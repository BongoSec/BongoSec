...
from bongosec.core import common, configuration, stats
from bongosec.core.InputValidator import InputValidator
from bongosec.core.cluster.utils import get_manager_status
from bongosec.core.common import AGENT_COMPONENT_STATS_REQUIRED_VERSION, DATE_FORMAT
from bongosec.core.exception import BongosecException, BongosecError, BongosecInternalError, BongosecResourceNotFound
from bongosec.core.utils import BongosecVersion, plain_dict_to_nested_dict, get_fields_to_nest, BongosecDBQuery, \
    BongosecDBQueryDistinct, BongosecDBQueryGroupBy, BongosecDBBackend, get_utc_now, get_utc_strptime, \
    get_date_from_timestamp
from bongosec.core.bongosec_queue import BongosecQueue
from bongosec.core.bongosec_socket import BongosecSocket, BongosecSocketJSON, create_bongosec_socket_message
from bongosec.core.wdb import BongosecDBConnection
...
class BongosecDBQueryAgents(BongosecDBQuery):
    """Class used to query Bongosec agents."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        if filters is None:
            filters = {}
        unify_bongosec_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = BongosecDBBackend(query_format='global')
        BongosecDBQuery.__init__(self, offset=offset, limit=limit, table='agent', sort=sort, search=search, select=select,
    ...
    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the Bongosec query."""
        BongosecDBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _sort_query(self, field: str) -> str:
        ...
        return BongosecDBQuery._sort_query(self, field)

    def _add_search_to_query(self):
        """Add search to the Bongosec query with the specific implications.

        Since IDs are stored in the database as integers, id searches must be turned into integers to work as expected.
        """
        if self.search:
            del self.fields['id']
            BongosecDBQuery._add_search_to_query(self)
            self.fields['id'] = 'id'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            if q_filter['operator'] == '=':
                ...
                raise BongosecError(1409, f"Valid operators for 'group' field: {', '.join(valid_group_operators)}. "
                                       f"Used operator: {q_filter['operator']}")
        else:
            BongosecDBQuery._process_filter(self, field_name, field_filter, q_filter)


class BongosecDBQueryGroup(BongosecDBQuery):
    """Class used to query Bongosec groups."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        backend = BongosecDBBackend(query_format='global')
        BongosecDBQuery.__init__(self, offset=offset, limit=limit, table='`group`', sort=sort, search=search,
                              select=select,
...
class BongosecDBQueryDistinctAgents(BongosecDBQueryDistinct, BongosecDBQueryAgents):
    pass


class BongosecDBQueryGroupByAgents(BongosecDBQueryGroupBy, BongosecDBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        BongosecDBQueryAgents.__init__(self, *args, **kwargs)
        BongosecDBQueryGroupBy.__init__(self, *args, table=self.table, fields=self.fields, filter_fields=filter_fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return BongosecDBQuery._format_data_into_dictionary(self)


class BongosecDBQueryMultigroups(BongosecDBQueryAgents):
    ...
    def __init__(self, group_id: str, query: str = '', *args: dict, **kwargs: dict):
        ...
        BongosecDBQueryAgents.__init__(self, query=query, *args, **kwargs)
...
class Agent:
    ...
    def load_info_from_db(self, select: list = None):
        ...
        Raises
        ------
        BongosecResourceNotFound(1701)
            Agent does not exist.
        """
        with BongosecDBQueryAgents(offset=0, limit=None, sort=None, search=None, select=select,
    ...
    def reconnect(self, wq: BongosecQueue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : BongosecQueue
            BongosecQueue used for the active response message.
        ...
        -------
        str
            Message generated by Bongosec.
        """
        # Check if agent is active
        self.get_basic_information()
        if self.status.lower() != 'active':
            raise BongosecError(1707)

        # Send force reconnect message to the BongosecQueue
        ret_msg = wq.send_msg_to_agent(BongosecQueue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        str
            Message generated by Bongosec.
        """
        # Check that bongosec-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except BongosecInternalError as e:
            # bongosec-authd is not running due to a problem with /proc availability
            raise BongosecError(1726, extra_message=str(e))

        if manager_status.get('bongosec-authd') != 'running':
            # bongosec-authd is not running
            raise BongosecError(1726)
        ...
        try:
            ...
            raise BongosecInternalError(1757, extra_message=str(e))
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = BongosecSocketJSON(common.AUTHD_SOCKET)
    ...
    def _add(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise BongosecError(1706, extra_message=ip)
            else:
                try:
        ...
        try:
            ...
            raise BongosecInternalError(1725, extra_message=str(e))

    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to Bongosec using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        try:
            ...
            if e.code == 9008:
                raise BongosecError(1705, extra_message=name)
            elif e.code == 9007:
                raise BongosecError(1706, extra_message=ip)
            elif e.code == 9012:
                raise BongosecError(1708, extra_message=id)
            raise e
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = BongosecDBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: str = None,
        ...
        db_query = BongosecDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
    ...
    def unset_single_group_agent(agent_id: str, group_id: str, force: bool = False) -> str:
        ...
        if not force:
            ...
            if not Agent.group_exists(group_id):
                raise BongosecResourceNotFound(1710)
        ...
        if group_id not in group_list:
            raise BongosecError(1734)
        elif len(group_list) == 1:
            if group_id == 'default':
                raise BongosecError(1745)
            else:
                set_default = True
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or Bongosec vX.Y.Z.
        ...
        """
        if BongosecVersion(agent_version) < BongosecVersion(common.ACTIVE_CONFIG_VERSION):
            raise BongosecInternalError(1735, extra_message=f"Minimum required version is {common.ACTIVE_CONFIG_VERSION}")
    ...
    def get_stats(self, component: str) -> dict:
        ...
        Raises
        ------
        BongosecInternalError(1015)
            Agent version is null.
        BongosecInternalError(1735)
        ...
        if self.version is None:
            raise BongosecInternalError(1015)
        agent_version = BongosecVersion(self.version.split(" ")[1])
        required_version = BongosecVersion(AGENT_COMPONENT_STATS_REQUIRED_VERSION.get(component))
        if agent_version < required_version:
            raise BongosecInternalError(1735, extra_message="Minimum required version is " + str(required_version))

        return stats.get_daemons_stats_from_socket(self.id, component)


def unify_bongosec_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_bongosec_version_format(filters: dict):
    """Verify and format the specified bongosec version into the 'bongosec vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f"bongosec {'v' if 'v' not in wv else ''}{wv}"
        elif re.match(r'^bongosec \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def send_restart_command(agent_id: str = '', agent_version: str = '', wq: BongosecQueue = None) -> str:
    ...
    wq : BongosecQueue
        BongosecQueue used for the active response messages.
    ...
    str
        Message generated by Bongosec.
    """
    # If the Bongosec agent version is newer or equal to the AR legacy version,
    # the message sent will have JSON format
    if BongosecVersion(agent_version) >= BongosecVersion(common.AR_LEGACY_VERSION):
        ret_msg = wq.send_msg_to_agent(BongosecQueue.RESTART_AGENTS_JSON, agent_id)
    else:
        ret_msg = wq.send_msg_to_agent(BongosecQueue.RESTART_AGENTS, agent_id)
...
def core_upgrade_agents(agents_chunk: list, command: str = 'upgrade_result', wpk_repo: str = None, version: str = None,
    ...
    """
    msg = create_bongosec_socket_message(origin={'module': 'api'},
                                      command=command,
                                      parameters={
                                          'agents': agents_chunk,
                                          'version': unify_bongosec_upgrade_version_format(version),
                                          'force_upgrade': force,
                                          'use_http': use_http,
    ...
    s = BongosecSocket(common.UPGRADE_SOCKET)
