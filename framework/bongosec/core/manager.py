...
from bongosec.core.configuration import get_active_configuration, get_cti_url
from bongosec.core.utils import get_utc_now, get_utc_strptime, tail
from bongosec.core.bongosec_socket import BongosecSocket
...
BONGOSEC_UID_KEY = 'bongosec-uid'
BONGOSEC_TAG_KEY = 'bongosec-tag'
...
def get_ossec_log_fields(log: str, log_format: LoggingFormat = LoggingFormat.plain) -> Union[tuple, None]:
    ...
    if "rootcheck" in tag:  # Unify rootcheck category
        tag = "bongosec-rootcheck"

    return get_utc_strptime(date, '%Y/%m/%d %H:%M:%S'), tag, level.lower(), description


def get_bongosec_active_logging_format() -> LoggingFormat:
    """Obtain the Bongosec active logging format.

    Returns
    -------
    LoggingFormat
        Bongosec active log format. Can either be `plain` or `json`. If it has both types, `plain` will be returned.
    """
...
def get_ossec_logs(limit: int = 2000) -> list:
    ...
    log_format = get_bongosec_active_logging_format()
    if log_format == LoggingFormat.plain and exists(common.BONGOSEC_LOG):
        bongosec_log_content = tail(common.BONGOSEC_LOG, limit)
    elif log_format == LoggingFormat.json and exists(common.BONGOSEC_LOG_JSON):
        bongosec_log_content = tail(common.BONGOSEC_LOG_JSON, limit)
    else:
        raise BongosecInternalError(1020)

    for line in bongosec_log_content:
...
def validate_ossec_conf() -> str:
    ...
    # Message for checking Bongosec configuration
    wcom_msg = common.CHECK_CONFIG_COMMAND

    # Connect to wcom socket
    if exists(wcom_socket_path):
        try:
            wcom_socket = BongosecSocket(wcom_socket_path)
        except BongosecException as e:
            extra_msg = f'Socket: BONGOSEC_PATH/queue/sockets/com. Error {e.message}'
            raise BongosecInternalError(1013, extra_message=extra_msg)
    else:
    ...
    try:
        ...
        raise BongosecInternalError(1904, extra_message=str(e))
...
def parse_execd_output(output: str) -> Dict:
    ...
    if error_flag != 0:
        ...
        raise BongosecError(1908, extra_message=', '.join(errors))
    else:
        response = {'status': 'OK'}
...
def get_update_information_template(
        uuid: str,
        update_check: bool,
        current_version: str = f"v{bongosec.__version__}",
    ...
    current_version : str, optional
        Indicates the current version of Bongosec, by default bongosec.__version__.
...
async def query_update_check_service(installation_uid: str) -> dict:
    ...
    installation_uid : str
        Bongosec UID to include in the query.
    ...
    update_information : dict
        Updates information.
    """
    current_version = f'v{bongosec.__version__}'
    headers = {
        BONGOSEC_UID_KEY: installation_uid,
        BONGOSEC_TAG_KEY: current_version,
        USER_AGENT_KEY: f'Bongosec UpdateCheckService/{current_version}'
    }
