...
from bongosec.core.common import origin_module
...
def create_bongosec_queue_socket_msg(flag: str, str_agent_id: str, msg: str, is_restart: bool = False) -> str:
    """Create message that will be sent to the BongosecQueue socket.
    ...
    -------
    str
        Message that will be sent to the BongosecQueue socket.
    """
...
class BaseQueue:
    ...
    def _connect(self):
        try:
            ...
            if length_send_buffer < BongosecQueue.MAX_MSG_SIZE:
                self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, BongosecQueue.MAX_MSG_SIZE)
        except Exception:
            raise BongosecInternalError(1010, self.path)
    ...
    def _send(self, msg: bytes) -> None:
        ...
        try:
            ...
            if sent == 0:
                raise BongosecInternalError(1011, self.path)
        except socket.error:
            raise BongosecInternalError(1011, self.path)
...
class BongosecQueue(BaseQueue):
    """
    BongosecQueue Object.
    """
    ...
    RESTART_AGENTS_JSON = json.dumps(create_bongosec_socket_message(origin={'module': origin_module.get()},
                                                                 command="restart-bongosec0",
    ...
    def send_msg_to_agent(self, msg: str = '', agent_id: str = '', msg_type: str = '') -> str:
        ...
        msg_is_no_ar = msg in [BongosecQueue.HC_SK_RESTART, BongosecQueue.HC_FORCE_RECONNECT]
        msg_is_restart = msg in [BongosecQueue.RESTART_AGENTS, BongosecQueue.RESTART_AGENTS_JSON]
        ...
        # AR
        if msg_type == BongosecQueue.AR_TYPE:
            socket_msg = create_bongosec_queue_socket_msg(flag, str_agent_id, msg) if agent_id != '000' else msg
            # Return message
        ...
        # Restart agents
            ...
            # If msg is not a non active-response command and not a restart command, raises BongosecInternalError
            if not msg_is_no_ar and not msg_is_restart:
                raise BongosecInternalError(1012, msg)
            socket_msg = create_bongosec_queue_socket_msg(flag, str_agent_id, msg, is_restart=msg_is_restart)
            # Return message
            if msg == BongosecQueue.HC_SK_RESTART:
                ret_msg = "Restarting Syscheck on agent" if agent_id else "Restarting Syscheck on all agents"
            elif msg == BongosecQueue.HC_FORCE_RECONNECT:
                ret_msg = "Reconnecting agent" if agent_id else "Reconnecting all agents"
            else:  # msg == BongosecQueue.RESTART_AGENTS or msg == BongosecQueue.RESTART_AGENTS_JSON
        ...
        try:
            ...
            raise BongosecError(1014, extra_message=f": BongosecQueue socket with path {self.path}")

        return ret_msg


class BongosecAnalysisdQueue(BaseQueue):
    """
    BongosecAnalysisdQueue Object.
    """
    ...
    def send_msg(self, msg_header: str, msg: str):
        ...
        try:
            ...
            raise BongosecError(1014, extra_message=f': BongosecAnalysisdQueue socket with path {self.path}. {str(e)}')
