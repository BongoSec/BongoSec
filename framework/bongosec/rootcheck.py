...
from bongosec.core.agent import get_agents_info, get_rbac_filters, BongosecDBQueryAgents
...
from bongosec.core.rootcheck import BongosecDBQueryRootcheck, last_scan, rootcheck_delete_agent
...
def run(agent_list: list = None) -> AffectedItemsBongosecResult:
    ...
    """
    result = AffectedItemsBongosecResult(all_msg='Rootcheck scan was restarted on returned agents',
    ...
    with BongosecDBQueryAgents(limit=None, select=["id", "status"], query=f'status!=active', **rbac_filters) as db_query:
...
def clear(agent_list: list = None) -> AffectedItemsBongosecResult:
    ...
    AffectedItemsBongosecResult
        JSON containing the affected agents.
    """
    result = AffectedItemsBongosecResult(all_msg='Rootcheck database was cleared on returned agents',
    ...
    not_found_agents = agent_list - system_agents
    # Add non existent agents to failed_items
    [result.add_failed_item(id_=agent_id, error=BongosecResourceNotFound(1701)) for agent_id in not_found_agents]
...
def get_last_scan(agent_list: list) -> AffectedItemsBongosecResult:
    ...
    """
    result = AffectedItemsBongosecResult(all_msg='Last rootcheck scan of the agent was returned',
...
def get_rootcheck_agent(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: str = None,
    ...
    if filters is None:
        filters = {}
    result = AffectedItemsBongosecResult(all_msg='All selected rootcheck information was returned',
                                      some_msg='Some rootcheck information was not returned',
                                      none_msg='No rootcheck information was returned'
                                      )

    with BongosecDBQueryRootcheck(agent_id=agent_list[0], offset=offset, limit=limit, sort=sort, search=search,
