...
from bongosec.core.task import BongosecDBQueryTask
...
def get_task_status(filters: dict = None, select: list = None, search: dict = None, offset: int = 0,
                    limit: int = DATABASE_LIMIT, sort: dict = None, q: str = None, ) -> AffectedItemsBongosecResult:
    ...
    AffectedItemsBongosecResult
        Tasks's status.
    """
    result = AffectedItemsBongosecResult(all_msg='All specified task\'s status were returned',
                                      some_msg='Some status were not returned',
                                      none_msg='No status was returned')

    with BongosecDBQueryTask(filters=filters, offset=offset, limit=limit, query=q, sort=sort, search=search,
