...
from bongosec.core.exception import BongosecException, BongosecResourceNotFound
from bongosec.core.results import AffectedItemsBongosecResult
from bongosec.core.stats import get_daemons_stats_, get_daemons_stats_socket, hourly_, totals_, weekly_
...
def totals(date: datetime.date) -> AffectedItemsBongosecResult:
    ...
    """
    result = AffectedItemsBongosecResult(all_msg='Statistical information for each node was successfully read',
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def hourly() -> AffectedItemsBongosecResult:
    ...
    """
    result = AffectedItemsBongosecResult(all_msg='Statistical information per hour for each node was successfully read',
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def weekly() -> AffectedItemsBongosecResult:
    ...
    """
    result = AffectedItemsBongosecResult(all_msg='Statistical information per week for each node was successfully read',
...
def get_daemons_stats_agents(daemons_list: list = None, agent_list: list = None):
    ...
    daemon_socket_mapping = {'bongosec-remoted': common.REMOTED_SOCKET,
                             'bongosec-analysisd': common.ANALYSISD_SOCKET}
    result = AffectedItemsBongosecResult(all_msg='Statistical information for each daemon was successfully read',
    ...
    if agent_list:
        if 'all' not in agent_list:
            ...
            with BongosecDBQueryAgents(limit=None, select=["id", "status"], **rbac_filters) as db_query:
                data = db_query.run()
            ...
            with contextlib.suppress(KeyError):
                ...
                result.add_failed_item('000', exception.BongosecError(1703))

            # Add non-existent agents to failed_items
            not_found_agents = agent_list - system_agents
            [result.add_failed_item(id_=agent, error=exception.BongosecResourceNotFound(1701)) for agent in
             not_found_agents]

            # Add non-active agents to failed_items
            non_active_agents = [agent['id'] for agent in data['items'] if agent['status'] != 'active']
            [result.add_failed_item(id_=agent, error=exception.BongosecError(1707)) for agent in non_active_agents]
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def get_daemons_stats(daemons_list: list = None) -> AffectedItemsBongosecResult:
...
def get_agents_component_stats_json(agent_list: list = None, component: str = None) -> AffectedItemsBongosecResult:
    ...
    AffectedItemsBongosecResult
        Component stats.
    """
    result = AffectedItemsBongosecResult(all_msg='Statistical information for each agent was successfully read',
    ...
    for agent_id in agent_list:
        try:
            if agent_id not in system_agents:
                raise exception.BongosecResourceNotFound(1701)
