...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        ...
        from bongosec import cluster
        from bongosec.core import common
        from bongosec.core.exception import BongosecError, BongosecResourceNotFound
        from bongosec.core.cluster.local_client import LocalClient
        from bongosec.core.results import BongosecResult

default_config = {'disabled': True, 'node_type': 'master', 'name': 'bongosec', 'node_name': 'node01',
                  'key': '', 'port': 1516, 'bind_addr': '0.0.0.0', 'nodes': ['NODE_IP'], 'hidden': 'no'}


@patch('bongosec.cluster.read_config', return_value=default_config)
...
@patch('bongosec.cluster.read_config', side_effect=BongosecError(1001))
def test_read_config_wrapper_exception(mock_read_config):
    """Verify the exceptions raised in read_config_wrapper."""
    result = cluster.read_config_wrapper()
    assert list(result.failed_items.keys())[0] == BongosecError(1001)
...
@patch('bongosec.cluster.get_node', side_effect=BongosecError(1001))
...
def test_get_status_json():
    ...
    expected = BongosecResult({'data': {"enabled": "no" if default_config['disabled'] else "yes", "running": "no"}})
    assert result == expected


@pytest.mark.asyncio
@patch('bongosec.core.cluster.local_client.LocalClient.start', side_effect=None)
async def test_get_health_nodes(mock_unix_connection):
    ...
    with patch('bongosec.cluster.get_health', side_effect=async_mock):
...
async def test_get_nodes_info():
    ...
    with patch('bongosec.cluster.get_nodes', side_effect=valid_node):
    ...
    assert result.failed_items[BongosecResourceNotFound(1730)] == {'noexists'}
...
@patch("bongosec.cluster.node_id", new="testing_node")
@pytest.mark.asyncio
async def test_get_ruleset_sync_status(ruleset_integrity):
    ...
    with patch("bongosec.cluster.get_node_ruleset_integrity",
...
async def test_get_ruleset_sync_status_ko():
    ...
    exc = BongosecError(1000)
    with patch("bongosec.cluster.get_node_ruleset_integrity", side_effect=exc):
