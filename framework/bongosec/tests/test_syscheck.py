...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        ...
        from bongosec.syscheck import run, clear, last_scan, files
        from bongosec.syscheck import AffectedItemsBongosecResult
        from bongosec import BongosecError, BongosecInternalError
...
@patch('bongosec.core.common.CLIENT_KEYS', new=os.path.join(test_agent_data_path, 'client.keys'))
@patch('bongosec.syscheck.BongosecDBQueryAgents.__exit__')
@patch('bongosec.syscheck.BongosecDBQueryAgents.__init__', return_value=None)
@patch('bongosec.syscheck.BongosecQueue._connect')
@patch('bongosec.syscheck.BongosecQueue.send_msg_to_agent', side_effect=set_callable_list)
@patch('bongosec.syscheck.BongosecQueue.close')
def test_syscheck_run(close_mock, send_mock, connect_mock, agent_init_mock, agent_exit_mock,
    ...
    """
    with patch('bongosec.syscheck.BongosecDBQueryAgents.run', return_value=failed_items[0]):
...
@patch('bongosec.core.wdb.BongosecDBConnection.__init__', return_value=None)
@patch('bongosec.core.wdb.BongosecDBConnection.execute', return_value=None)
@patch('bongosec.core.wdb.BongosecDBConnection.close')
def test_syscheck_clear(wdb_close_mock, wdb_execute_mock, wdb_init_mock, agent_list, expected_result, agent_info_list,
    ...
    """
    with patch('bongosec.syscheck.get_agents_info', return_value=set(agent_info_list)), \
            patch('bongosec.syscheck.BongosecDBQueryAgents') as mock_wdbqa:
...
@patch('bongosec.core.wdb.BongosecDBConnection.__init__', return_value=None)
@patch('bongosec.core.wdb.BongosecDBConnection.execute', side_effect=BongosecError(1000))
@patch('bongosec.core.wdb.BongosecDBConnection.close')
...
@pytest.mark.parametrize('agent_id, bongosec_version', [
    (['001'], {'version': 'Bongosec v3.6.0'}),
    (['002'], {'version': 'Bongosec v3.8.3'}),
    (['005'], {'version': 'Bongosec v3.5.3'}),
    (['006'], {'version': 'Bongosec v3.9.4'}),
    (['004'], {}),
])
...
@patch("bongosec.syscheck.BongosecDBConnection.execute", return_value=[{'end': '', 'start': ''}])
@patch('socket.socket.connect')
def test_syscheck_last_scan(socket_mock, wdb_conn_mock, db_mock, exists_mock, agent_id, bongosec_version):
    ...
    agent_id : list
        Agent ID.
    bongosec_version : dict
        Dict with the Bongosec version to be applied.
    """
    with patch('bongosec.syscheck.Agent.get_basic_information', return_value=bongosec_version):
...
@patch('bongosec.core.utils.path.exists', return_value=True)
@patch('socket.socket.connect')
@patch('bongosec.core.common.WDB_PATH', new=test_data_path)
