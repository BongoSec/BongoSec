...
with patch('bongosec.core.common.bongosec_uid'):
    with patch('bongosec.core.common.bongosec_gid'):
        ...
        from bongosec.agent import add_agent, assign_agents_to_group, create_group, delete_agents, delete_groups, \
        ...
        from bongosec.core.agent import Agent
        from bongosec import BongosecError, BongosecException, BongosecInternalError
        from bongosec.core.results import BongosecResult, AffectedItemsBongosecResult
        from bongosec.core.tests.test_agent import InitAgent
        from api.util import remove_nones_to_dict
        from bongosec.core.exception import BongosecResourceNotFound
...
@pytest.mark.parametrize('fields, expected_items', [
    ...
    (
            ['version'],
            [{'version': 'Bongosec v3.9.0', 'count': 1}, {'version': 'Bongosec v3.8.2', 'count': 2},
             {'version': 'Bongosec v3.6.2', 'count': 1}, {'version': 'N/A', 'count': 2}]
    ),
    (
...
@patch('bongosec.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
...
def test_agent_get_distinct_agents(socket_mock, send_mock, fields, expected_items):
    ...
    assert isinstance(distinct, AffectedItemsBongosecResult), 'The returned object is not an "AffectedItemsBongosecResult".'
...
def test_agent_sort_order(socket_mock, send_mock, fields, order, expected_items):
    ...
    assert isinstance(sorted_agents, AffectedItemsBongosecResult), 'The returned object is not an ' \
...
def test_agent_get_agents_summary_status(socket_mock, send_mock):
    ...
    assert isinstance(summary, BongosecResult), 'The returned object is not an "BongosecResult" instance.'
...
def test_agent_get_agents_summary_os(connect_mock, send_mock):
    ...
    assert isinstance(summary, AffectedItemsBongosecResult), 'The returned object is not an "BongosecResult" instance.'
...
@patch('bongosec.core.agent.Agent.reconnect')
@patch('bongosec.agent.get_agents_info', return_value=short_agent_list)
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_agents(socket_mock, send_mock, agents_info_mock, reconnect_mock, agent_list, expected_items,
    ...
    assert isinstance(result, AffectedItemsBongosecResult), 'The returned object is not an "AffectedItemsBongosecResult".'
...
@patch('bongosec.agent.send_restart_command')
@patch('bongosec.agent.get_agents_info', return_value=set(short_agent_list))
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('bongosec.agent.get_agents')
@patch('bongosec.agent.get_groups')
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('group, q, expected_q', [
    ('default', '(name~bongosec,status~active)', 'group=default;((name~bongosec,status~active))'),
    ('default', 'name~bongosec,status~active', 'group=default;(name~bongosec,status~active)')
])
@patch('bongosec.agent.get_agents')
@patch('bongosec.agent.get_groups')
...
@pytest.mark.parametrize('agent_list, filters, q, error_code, expected_items', [
    ...
    (full_agent_list[1:], {'status': 'all', 'older_than': '1s', 'version': 'Bongosec v3.6.2'}, None, 1731, ['002']),
    ...
    (['001', '002'], {'status': 'all', 'older_than': '1s'}, None, BongosecError(1726), None),
])
@patch('bongosec.agent.Agent.remove')
@patch('bongosec.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_delete_agents(socket_mock, send_mock, mock_remove, agent_list, filters, q, error_code, expected_items):
    ...
    """
    if not isinstance(error_code, BongosecException):
...
@patch('bongosec.core.agent.get_manager_status', return_value={'bongosec-authd': 'running'})
...
@patch('bongosec.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('bongosec.core.common.SHARED_PATH', new=test_shared_path)
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
def test_agent_get_agent_groups_exceptions(socket_mock, send_mock, mock_get_groups, db_global, system_groups,
    ...
    with patch('bongosec.core.common.DATABASE_PATH_GLOBAL', new=db_global):
        try:
...
@patch('bongosec.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('bongosec.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('bongosec.core.common.SHARED_PATH', new=test_shared_path)
...
@pytest.mark.parametrize('shared_path, group_list, group_exists, side_effect, expected_exception', [
    (test_shared_path, ['none'], False, None, BongosecResourceNotFound(1710)),
    ('invalid-path', ['default'], True, None, BongosecError(1006)),
    (test_shared_path, ['default'], True, BongosecError(1405), BongosecError(1405)),
    (test_shared_path, ['default'], True, BongosecException(1400), BongosecInternalError(1727))
])
@patch('bongosec.agent.process_array')
@patch('bongosec.core.agent.Agent.group_exists')
def test_agent_get_group_files_exceptions(mock_group_exists, mock_process_array, shared_path, group_list, group_exists,
    ...
    """
    with patch('bongosec.core.common.SHARED_PATH', new=shared_path):
        ...
        except (BongosecError, BongosecInternalError) as e:
...
@patch('bongosec.core.common.SHARED_PATH', new=test_shared_path)
@patch('bongosec.core.common.bongosec_gid', return_value=getgrnam('root'))
@patch('bongosec.core.common.bongosec_uid', return_value=getpwnam('root'))
@patch('bongosec.agent.chown_r')
def test_create_group(chown_mock, uid_mock, gid_mock, group_id):
    ...
    try:
        ...
        assert isinstance(result, BongosecResult), 'The returned object is not an "BongosecResult" instance.'
...
@pytest.mark.parametrize('group_id, exception, exception_code', [
    ('default', BongosecError, 1711),
    ('group-1', BongosecError, 1711),
    ('invalid!', BongosecError, 1722),
    ('delete-me', BongosecInternalError, 1005),
    ('ar.conf', BongosecError, 1713),
    ('agent-template.conf', BongosecError, 1713)
])
@patch('bongosec.core.common.SHARED_PATH', new=test_shared_path)
def test_create_group_exceptions(group_id, exception, exception_code):
    ...
    exception_code : int
        Expected error code for the Bongosec Exception object raised by `get_group_files` with the given parameters.
    """
    try:
...
@patch('bongosec.agent.get_groups')
@patch('bongosec.agent.Agent.delete_single_group')
...
@pytest.mark.parametrize('group_list, expected_errors', [
    (['none-1'], [BongosecResourceNotFound(1710)]),
    (['default'], [BongosecError(1712)]),
    (['none-1', 'none-2'], [BongosecResourceNotFound(1710)]),
    (['default', 'none-1'], [BongosecError(1712), BongosecResourceNotFound(1710)]),
])
@patch('bongosec.agent.get_groups')
def test_agent_delete_groups_other_exceptions(mock_get_groups, group_list, expected_errors):
    ...
    group_list : List of str
        List of groups to be deleted.
    expected_errors : list of BongosecError
        List of expected BongosecError to be raised by delete_groups if a group is not valid. An exception will be returned
...
@patch('bongosec.agent.Agent.add_group_to_agent')
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['none-1'], ['001'], BongosecResourceNotFound(1710), True),
    (['group-1'], ['100'], BongosecResourceNotFound(1701), False),
    (['default'], ['000'], BongosecError(1703), False)
])
@patch('bongosec.agent.Agent.group_exists')
@patch('bongosec.agent.Agent.add_group_to_agent')
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_assign_agents_to_group_exceptions(socket_mock, send_mock, mock_add_group, mock_group_exists, group_list,
    ...
    catch_exception : bool
        ...
        `AffectedItemsBongosecResult` containing the exceptions in its 'failed_items'.
    """
    ...
    except BongosecException as ex:
...
@patch('bongosec.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('bongosec.core.agent.Agent.unset_single_group_agent')
@patch('bongosec.agent.get_groups')
@patch('bongosec.agent.get_agents_info')
...
@pytest.mark.parametrize('group_id, agent_id, expected_error', [
    ('any-group', '100', BongosecResourceNotFound(1701)),
    ('any-group', '000', BongosecError(1703)),
    ('group-1', '005', BongosecResourceNotFound(1710)),
])
@patch('bongosec.agent.get_agents_info', return_value=short_agent_list)
@patch('bongosec.agent.get_groups', side_effect={'default'})
def test_agent_remove_agent_from_group_exceptions(group_mock, agents_info_mock, group_id, agent_id, expected_error):
    ...
    expected_error : BongosecError
        The BongosecError object expected to be raised by remove_agent_from_group with the given parameters.
    """
    try:
    ...
    except (BongosecError, BongosecResourceNotFound) as error:
...
@patch('bongosec.core.agent.Agent.unset_single_group_agent')
@patch('bongosec.agent.get_agents_info', return_value=short_agent_list)
@patch('bongosec.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['any-group'], ['100'], BongosecResourceNotFound(1701), True),
    (['any-group'], ['000'], BongosecError(1703), True),
    (['any-group'], ['005'], BongosecResourceNotFound(1710), False),
])
@patch('bongosec.core.agent.Agent.unset_single_group_agent')
@patch('bongosec.agent.get_agents_info', return_value=short_agent_list)
@patch('bongosec.agent.get_groups', return_value={'group-1'})
def test_agent_remove_agent_from_groups_exceptions(mock_get_groups, mock_get_agents, mock_unset, group_list, agent_list,
    ...
    try:
        ...
        assert not catch_exception, \
            f'An "BongosecError" exception was expected but was not raised.'
        # Check Typing
        assert isinstance(result, AffectedItemsBongosecResult), 'The returned object is not an "AffectedItemsBongosecResult".'
        assert isinstance(result.failed_items, dict), \
        ...
        assert catch_exception, \
            f'No exception should be raised at this point. An AffectedItemsBongosecResult object with at least one ' \
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['non-group'], ['000'], BongosecResourceNotFound(1710), True),
    (['group-1'], ['000'], BongosecError(1703), False),
    (['group-1'], ['100'], BongosecResourceNotFound(1701), False),
])
@patch('bongosec.agent.get_agents_info', return_value=short_agent_list)
@patch('bongosec.agent.get_groups', return_value={'group-1'})
...
def test_agent_get_outdated_agents(socket_mock, send_mock):
    ...
    # Check typing
    assert isinstance(result, AffectedItemsBongosecResult)
    assert isinstance(result.affected_items, list)
    # Check affected items
...
@pytest.mark.parametrize('agent_set, expected_errors_and_items, result_from_socket, filters, raise_error', [
    ...
    (
            ...
            {'error': 1,
             'data': [{'error': 18,
                       'message': 'Error from socket indicating BongosecInternalError',
                       'agent': 1}
                      ],
             'message': 'Error'},
            None,
...
@patch('bongosec.agent.get_agents_info', return_value=set(full_agent_list))
@patch('bongosec.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_upgrade_agents(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    """
    with patch('bongosec.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        core_upgrade_agents_mock.return_value = result_from_socket

        if raise_error:
            # Upgrade expecting a Bongosec Exception
            for error in expected_errors_and_items.keys():
                if int(error) in ERROR_CODES_UPGRADE_SOCKET_BAD_REQUEST:
                    with pytest.raises(BongosecError, match=f".* {error} .*"):
                    ...
                    with pytest.raises(BongosecInternalError, match=f".* {error} .*"):
                        upgrade_agents(agent_list=list(agent_set), filters=filters)
        else:
...
def test_agent_get_upgrade_result(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    with patch('bongosec.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        ...
        if raise_error:
            # Get upgrade result expecting a Bongosec Exception
            for error in expected_errors_and_items.keys():
                with pytest.raises(BongosecInternalError, match=f".* {error} .*"):
...
def test_agent_get_agent_config(mock_exists, socket_mock, send_mock, bongosec_socket_mock, agent_list, component, configuration):
    ...
    """
    bongosec_socket_mock.return_value.receive.return_value = b'ok {"test": "conf"}'
...
def test_agent_get_agent_config_exceptions(socket_mock, send_mock, agent_list):
    ...
    try:
        ...
        assert error == BongosecError(1740)
...
@patch('bongosec.core.common.SHARED_PATH', new=test_shared_path)
@patch('bongosec.core.common.MULTI_GROUPS_PATH', new=test_multigroup_path)
@patch('bongosec.agent.get_agents_info', return_value=full_agent_list)
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('agent_list, expected_error', [
    (['000'], BongosecError(1703)),
    (['100'], BongosecResourceNotFound(1701))
])
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_agents_sync_group_exceptions(socket_mock, send_mock, agent_list, expected_error):
    ...
    agent_list : List of str
        List of agent ID's.
    expected_error : BongosecError
        Expected BongosecError to be returned (not raised) by the function.
    """
    result = get_agents_sync_group(agent_list=agent_list)
    assert isinstance(result, AffectedItemsBongosecResult), 'The returned object is not an "AffectedItemsBongosecResult".'
    # Check failed items
...
@patch('bongosec.core.common.SHARED_PATH', new=test_shared_path)
@patch('bongosec.core.configuration.upload_group_configuration')
...
@patch('bongosec.core.common.SHARED_PATH', new=test_shared_path)
@patch('bongosec.agent.get_distinct_agents')
@patch('bongosec.agent.get_agent_groups')
@patch('bongosec.agent.get_agents_summary_status')
@patch('bongosec.agent.get_agents')
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('bongosec.agent.get_agents_info', return_value=['test', 'test2'])
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_get_agents_big_env(mock_conn, mock_send, mock_get_agents, insert_agents_db, agent_list, params, expected_ids):
    ...
    with patch('bongosec.agent.get_agents_info', return_value=set(agent_ids_format(range(100000)))):
...
@patch('bongosec.core.agent.Agent.get_agent_groups')
@patch('bongosec.core.agent.Agent.set_agent_group_file')
@patch('bongosec.core.agent.Agent')
...
@patch('bongosec.core.agent.Agent.group_exists', return_value=False)
...
def test_unset_single_group_agent_ko(agent_basic_mock, group_exists_mock, get_groups_mock, agent_id, group_id,
    ...
    expected_exc: int
        Expected BongosecException code error.
    """
    with pytest.raises(BongosecException, match=f".* {expected_exc} .*"):
...
def test_check_uninstall_permission():
    ...
    expected = BongosecResult({'message': 'User has permission to uninstall agents'})
