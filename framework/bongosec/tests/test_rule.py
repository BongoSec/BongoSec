...
from bongosec.core.common import USER_RULES_PATH
import pytest
...
def mock_bongosec_paths():
    with patch('bongosec.core.common.RULES_PATH', new=os.path.join(parent_directory, core_data_path)):
        with patch('bongosec.core.common.USER_RULES_PATH', new=os.path.join(parent_directory, tests_data_path)):
            with patch('bongosec.core.common.BONGOSEC_PATH', new=parent_directory):
                with patch('bongosec.rule.to_relative_path', side_effect=lambda x: os.path.relpath(x, parent_directory)):
                    yield
...
@patch("bongosec.rule.configuration.get_ossec_conf", return_value=rule_ossec_conf)
def test_get_rules_files_status_include(mock_ossec, status, func):
    ...
    if status == 'random':
        ...
        with pytest.raises(BongosecError, match='.* 1202 .*'):
            func(status=status)
    else:
...
@patch('bongosec.core.configuration.get_ossec_conf', return_value=rule_ossec_conf)
...
@patch('bongosec.core.configuration.get_ossec_conf', return_value=other_rule_ossec_conf)
...
def test_failed_get_rules():
    ...
    with pytest.raises(BongosecError, match=".* 1203 .*"):
...
def test_get_rule_file_path(filename, relative_dirname, result, mock_bongosec_paths):
    """Test get_rule_file_path function."""
    with patch('bongosec.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        res = rule.get_rule_file_path(filename=filename, 
                                            relative_dirname=relative_dirname)
        assert res == os.path.join(bongosec.core.common.BONGOSEC_PATH, result) if result else not res
...
def test_get_rule_file(filename, raw, relative_dirname, contains):
    ...
    with patch('bongosec.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        if raw:
            ...
            assert isinstance(d_files, AffectedItemsBongosecResult)
...
@patch('bongosec.rule.delete_rule_file')
@patch('bongosec.rule.full_copy')
@patch('bongosec.rule.upload_file')
@patch('bongosec.rule.remove')
@patch('bongosec.rule.safe_move')
@patch('bongosec.rule.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_xml, mock_full_copy,
    ...
    with patch('bongosec.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('bongosec.rule.validate_upload_delete_dir', return_value=ret_validation):
            with patch('bongosec.rule.exists', return_value=overwrite):
                ...
                if overwrite:
                    full_path = os.path.join(bongosec.github.iomon.BONGOSEC_PATH, rule_path)
...
@patch('bongosec.rule.delete_rule_file', side_effect=BongosecError(1019))
@patch('bongosec.rule.upload_file')
@patch('bongosec.rule.safe_move')
...
def test_delete_rule_file(file, relative_dirname):
    ...
    with patch('bongosec.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        with patch('bongosec.rule.exists', return_value=True):
            with patch('bongosec.rule.remove'):
...
def test_delete_rule_file_ko():
    ...
    with patch('bongosec.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('bongosec.rule.remove', side_effect=IOError()):
