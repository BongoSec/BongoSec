...
import pytest
from bongosec.core.common import USER_DECODERS_PATH


with patch('bongosec.core.common.getgrnam'):
    with patch('bongosec.core.common.getpwnam'):
...
def mock_bongosec_paths():
    """Mock bongosec paths."""
    with patch('bongosec.core.common.BONGOSEC_PATH', new=test_data_path):
        with patch('bongosec.core.configuration.get_ossec_conf', return_value=decoder_ossec_conf):
            with patch('bongosec.core.common.DECODERS_PATH',
                       new=os.path.join(test_data_path, "tests", "data", "decoders")):
                with patch('bongosec.core.common.USER_DECODERS_PATH',
                           new=os.path.join(test_data_path, "tests", "data", "etc", "decoders")):
                    with patch('bongosec.decoder.to_relative_path',
                               side_effect=lambda x: os.path.relpath(x, bongosec.core.common.BONGOSEC_PATH)):
                        yield
...
@pytest.mark.parametrize('names, status, filename, relative_dirname, parents, expected_names, expected_total_failed', [
    (None, None, None, None, False, {'agent-buffer', 'json', 'agent-upgrade', 'bongosec', 'agent-restart'}, 0),
    (['agent-buffer'], None, None, None, False, {'agent-buffer'}, 0),
    (['agent-buffer', 'non_existing'], None, None, None, False, {'agent-buffer'}, 1),
    (None, 'enabled', None, None, False, {'agent-buffer', 'agent-upgrade', 'bongosec', 'agent-restart'}, 0),
    (None, 'disabled', None, None, False, {'json'}, 0),
    (['agent-upgrade', 'non_existing', 'json'], 'enabled', None, None, False, {'agent-upgrade'}, 1),
    (None, None, 'test1_decoders.xml', None, False, {'agent-buffer', 'agent-upgrade', 'bongosec', 'agent-restart'}, 0),
    (None, None, 'test2_decoders.xml', 'tests/data/decoders', False, {'json'}, 0),
    (None, 'all', None, 'tests/data/decoders', True, {'bongosec', 'json'}, 0),
...
@pytest.mark.parametrize('conf, exception', [
    ...
    ({'ruleset': None}, BongosecInternalError(1500))
])
def test_get_decoders_files(conf, exception):
    """Test get_decoders_files function"""
    with patch('bongosec.core.configuration.get_ossec_conf', return_value=conf):
        try:
            # UUT call
        ...
        except BongosecInternalError as exc:
...
def test_get_decoder_file_path(filename, relative_dirname, result, mock_bongosec_paths):
    """Test get_decoder_file_path function."""
    res = decoder.get_decoder_file_path(filename=filename, 
                                         relative_dirname=relative_dirname)
    assert res == os.path.join(bongosec.core.common.BONGOSEC_PATH, result) if result else not res and isinstance(res, str)
...
def test_get_decoder_file(filename, raw, relative_dirname, contains, mock_bongosec_paths):
    ...
    if raw:
        ...
        # Assert the result is an AffectedItemsBongosecResult
        assert isinstance(result, AffectedItemsBongosecResult)
...
@patch('bongosec.decoder.delete_decoder_file')
@patch('bongosec.decoder.full_copy')
@patch('bongosec.decoder.validate_bongosec_xml')
@patch('bongosec.decoder.upload_file')
@patch('bongosec.decoder.remove')
@patch('bongosec.decoder.safe_move')
@patch('bongosec.decoder.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_upload_file,
                     mock_xml, mock_full_copy, mock_delete, mock_bongosec_paths,
    ...
    with patch('bongosec.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('bongosec.decoder.exists', return_value=overwrite):
            with patch('bongosec.decoder.to_relative_path',
                    side_effect=lambda x: os.path.relpath(x, bongosec.core.common.BONGOSEC_PATH)):
            ...
            if overwrite:
                full_path = os.path.join(bongosec.github.iomon.BONGOSEC_PATH, decoder_path)
...
@patch('bongosec.decoder.delete_decoder_file', side_effect=BongosecError(1019))
@patch('bongosec.decoder.upload_file')
@patch('bongosec.decoder.safe_move')
...
def test_upload_file_ko(*_):
    ...
    with patch('bongosec.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('bongosec.decoder.exists'):
    ...
    # clean backup files
    search_pattern = os.path.join(bongosec.core.common.BONGOSEC_PATH, "**", "*.backup")
...
def test_delete_decoder_file(filename, relative_dirname):
    ...
    with patch('bongosec.decoder.exists', return_value=True):
        # Assert returned type is AffectedItemsBongosecResult when everything is correct
        with patch('bongosec.decoder.remove'):
...
def test_delete_decoder_file_ko():
    ...
    with patch('bongosec.decoder.exists', return_value=True):
        with patch('bongosec.decoder.remove', side_effect=IOError()):
