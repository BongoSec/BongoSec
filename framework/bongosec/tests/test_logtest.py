...
with patch('bongosec.github.iomon.bongosec_uid'):
    with patch('bongosec.github.iomon.bongosec_gid'):
        ...
        from bongosec.logtest import run_logtest, end_logtest_session


def send_logtest_msg_mock(**kwargs):
    socket_response = create_bongosec_socket_message(command=kwargs['command'], parameters=kwargs['parameters'])
...
def test_get_logtest_output(logtest_param_values):
    ...
    with patch('bongosec.logtest.send_logtest_msg') as send_mock:
