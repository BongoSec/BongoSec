...
SOCKET_PATH_DAEMONS_MAPPING = {'/var/ossec/queue/sockets/remote': 'bongosec-remoted',
                               '/var/ossec/queue/sockets/analysis': 'bongosec-analysisd'}
DAEMON_SOCKET_PATHS_MAPPING = {'bongosec-remoted': '/var/ossec/queue/sockets/remote',
                               'bongosec-analysisd': '/var/ossec/queue/sockets/analysis'}
...
def test_totals():
    ...
    with patch('bongosec.stats.totals_', return_value=({})):
        response = stats.totals(date(2019, 8, 13))
        assert response.total_affected_items == len(response.affected_items)
        assert isinstance(response, AffectedItemsBongosecResult), 'The result is not BongosecResult type'
...
@patch('bongosec.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('bongosec.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('bongosec.core.common.WDB_SOCKET', '/var/ossec/queue/db/wdb')
@patch('bongosec.stats.get_daemons_stats_socket')
def test_get_daemons_stats(mock_get_daemons_stats_socket):
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['bongosec-remoted', 'bongosec-analysisd', 'bongosec-db'])
    ...
    assert isinstance(response, AffectedItemsBongosecResult), 'The result is not AffectedItemsBongosecResult type'
    assert response.total_affected_items == len(response.affected_items)


@patch('bongosec.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/wrong_socket_name')
def test_get_daemons_stats_ko():
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['bongosec-remoted'])
...
@pytest.mark.parametrize('daemons_list, expected_daemons_list', [
    ([], ['bongosec-remoted', 'bongosec-analysisd']),
    (['bongosec-remoted'], ['bongosec-remoted']),
    (['bongosec-remoted', 'bongosec-analysisd'], ['bongosec-remoted', 'bongosec-analysisd'])
])
@patch('bongosec.core.wdb.BongosecDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
@patch('bongosec.stats.get_agents_info', return_value={'000', '001', '002', '003', '004', '005'})
@patch('bongosec.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('bongosec.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('bongosec.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats)
def test_get_daemons_stats_agents(mock_get_daemons_stats_socket, mock_get_agents_info, mock_socket_connect,
    ...
    assert isinstance(result, AffectedItemsBongosecResult), 'The result is not an AffectedItemsBongosecResult object'
...
@patch('bongosec.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('bongosec.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('bongosec.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats_all)
...
@patch('bongosec.stats.get_daemons_stats_', return_value=[{"events_decoded": 1.0}])
...
@patch('bongosec.core.agent.Agent.get_stats')
@patch('bongosec.stats.get_agents_info', return_value=['000', '001'])
