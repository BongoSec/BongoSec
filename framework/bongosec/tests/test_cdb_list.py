...
from bongosec.core.exception import BongosecInternalError
...
with patch('bongosec.core.common.getgrnam'):
    with patch('bongosec.core.common.getpwnam'):
        ...
        from bongosec.cdb_list import get_lists, get_path_lists, iterate_lists, get_list_file, upload_list_file,\
            delete_list_file
        from bongosec.core import common
        from bongosec.core.results import AffectedItemsBongosecResult

RELATIVE_PATH = os.path.join("framework", "bongosec", "tests", "data", "test_cdb_list")
NAME_FILE_1 = "test_lists_1"
NAME_FILE_2 = "test_lists_2"
NAME_FILES = [NAME_FILE_1, NAME_FILE_2]

RESULT_GET_LIST_FILE_1 = [{'items': [{'key': 'test-bongosec-w', 'value': 'write'},
                                     {'key': 'test-bongosec-r', 'value': 'read'},
                                     {'key': 'test-bongosec-a', 'value': 'attribute'},
                                     {'key': 'test-bongosec-x', 'value': 'execute'},
                                     {'key': 'test-bongosec-c', 'value': 'command'}
                                     ],
...
@patch('bongosec.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
@pytest.mark.parametrize("search_text, complementary_search, search_in_fields, paths, expected_result", [
    ...
    ("test-bongosec-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-ossec-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-bongosec-w", False, "items", [NAME_FILE_2], []),
    ...
    ("test-bongosec-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-ossec-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-bongosec-w", True, "items", [NAME_FILE_2], RESULT_GET_LIST_FILE_2),
])
@patch('bongosec.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
def test_get_lists_sort():
    ...
    assert isinstance(result_a, AffectedItemsBongosecResult)
    assert isinstance(result_b, AffectedItemsBongosecResult)
...
@patch('bongosec.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
@patch('bongosec.cdb_list.iterate_lists', side_effect=lists_path_mock)
...
def test_get_list_file(filename, raw, expected_result, total_failed_items):
    ...
    """
    with patch('bongosec.cdb_list.get_filenames_paths', return_value=[os.path.join(DATA_PATH, 'test_lists_2')]):
        result = get_list_file([filename], raw)
        if raw:
            assert result == expected_result
        else:
            isinstance(result, AffectedItemsBongosecResult)
...
@patch('bongosec.cdb_list.safe_move')
@patch('bongosec.cdb_list.delete_file_with_backup')
@patch('bongosec.cdb_list.upload_file')
@patch('bongosec.cdb_list.delete_list_file')
@patch('bongosec.cdb_list.remove')
@patch('bongosec.cdb_list.exists', return_value=True)
...
@patch('bongosec.cdb_list.common.USER_LISTS_PATH', return_value='/test/path')
@patch('bongosec.cdb_list.remove')
def test_upload_list_file_ko(mock_remove, mock_lists_path):
    ...
    with patch('bongosec.cdb_list.safe_move') as mock_safe_move:
        with patch('bongosec.cdb_list.exists', return_value=True):
            ...
            with patch('bongosec.cdb_list.get_filenames_paths', return_value=['/test']):
...
def test_delete_list_file(mock_delete_file):
    ...
    try:
        ...
        with patch('bongosec.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH):
            result = delete_list_file(['test_file'])
            assert result.render()['data']['affected_items'][0] ==\
                   'framework/bongosec/tests/data/test_cdb_list/test_file'
    finally:
        try:
            os.remove(test_file)
        except Exception:
