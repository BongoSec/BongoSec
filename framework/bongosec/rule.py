...
import bongosec.core.configuration as configuration
from bongosec.core import common
from bongosec.core.cluster.cluster import get_node
from bongosec.core.cluster.utils import read_cluster_config
from bongosec.core.exception import BongosecError
from bongosec.core.results import AffectedItemsBongosecResult
from bongosec.core.rule import check_status, load_rules_from_file, format_rule_decoder_file, REQUIRED_FIELDS, \
    RULE_REQUIREMENTS, SORT_FIELDS, RULE_FIELDS, RULE_FILES_FIELDS, RULE_FILES_REQUIRED_FIELDS
from bongosec.core.utils import process_array, safe_move, \
    validate_bongosec_xml, upload_file, full_copy, to_relative_path
from bongosec.core.logtest import validate_dummy_logtest
...
def get_rules(rule_ids: list = None, status: str = None, group: str = None, pci_dss: str = None, gpg13: str = None,
    ...
    """
    result = AffectedItemsBongosecResult(none_msg='No rule was returned',
    ...
    for rule_id in no_existent_ids:
        result.add_failed_item(id_=rule_id, error=BongosecError(1208))
...
def get_rules_files(status: str = None, relative_dirname: str = None, filename: list = None, offset: int = 0,
                    ...
                    distinct: bool = False) -> AffectedItemsBongosecResult:
    ...
    ------
    BongosecError(1200)
        Error reading rules from `BONGOSEC_HOME/etc/ossec.conf`.
    ...
    AffectedItemsBongosecResult
        Affected items.
    """
    result = AffectedItemsBongosecResult(none_msg='No rule files were returned',
...
def get_groups(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None, sort_ascending: bool = True,
               search_text: str = None, complementary_search: bool = False,
               search_in_fields: list = None) -> AffectedItemsBongosecResult:
    ...
    AffectedItemsBongosecResult
        Dictionary: {'items': array of items, 'totalItems': Number of items (without applying the limit)}
    """
    result = AffectedItemsBongosecResult(none_msg='No groups in rules were returned',
...
def get_requirement(requirement: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    if requirement not in RULE_REQUIREMENTS:
        result.add_failed_item(id_=requirement,
                               error=BongosecError(1205, extra_message=requirement,
...
def get_rule_file_path(filename: str = None, relative_dirname: str = None) -> str:
    ...
    if len(rules) == 0:
        ...
        return join(common.BONGOSEC_PATH, rule['relative_dirname'], filename)
    else:
        return normpath(join(common.BONGOSEC_PATH, rules[0]['relative_dirname'], filename))


def get_rule_file(filename: str = None, raw: bool = False,
                  relative_dirname: str = None) -> Union[str, AffectedItemsBongosecResult]:
    ...
    Returns
    -------
    str or AffectedItemsBongosecResult
        Content of the file. AffectedItemsBongosecResult format if `raw=False`.
    """
    result = AffectedItemsBongosecResult(none_msg='No rule was returned',
    ...
    if not full_path:
        result.add_failed_item(id_=filename,
                               error=BongosecError(1415, extra_message=f"{filename}"))
        return result
    ...
    try:
        ...
        result.add_failed_item(id_=filename,
                               error=BongosecError(1413, extra_message=f"{filename}: {str(exc)}"))
    except OSError:
        result.add_failed_item(id_=filename,
                               error=BongosecError(1414, extra_message=f"{filename}"))

    return result


def validate_upload_delete_dir(relative_dirname: Union[str, None]) -> Tuple[str, BongosecError]:
    ...
    Tuple (str, BongosecError)
        ...
        The second element of the tuple is a BongosecError exception
            If relative_dirname has no 'rule_dir' tag in ruleset return BongosecError(1505).
            If relative_dirname is inside the default RULES_PATH return BongosecError(1506).
            If relative_dirname has a 'rule_dir' tag in ruleset but it doesn't exists return BongosecError(1507).
    ...
    if not relative_dirname in ruleset_conf['rule_dir']:
        bongosec_error = BongosecError(1209)
    elif commonpath([join(common.BONGOSEC_PATH, relative_dirname), common.RULES_PATH]) == common.RULES_PATH:
        bongosec_error = BongosecError(1210)
    elif not exists(join(common.BONGOSEC_PATH, relative_dirname)):
        bongosec_error = BongosecError(1211)
    return relative_dirname, bongosec_error


@expose_resources(actions=['rules:update'], resources=['*:*:*'])
def upload_rule_file(filename: str, content: str, relative_dirname: str = None, 
                     overwrite: bool = False) -> AffectedItemsBongosecResult:
    ...
    AffectedItemsBongosecResult
        Affected items.
    """
    result = AffectedItemsBongosecResult(all_msg='Rule was successfully uploaded',
                                      none_msg='Could not upload rule')

    backup_file = ''
    try:
        relative_dirname, bongosec_error = validate_upload_delete_dir(
            relative_dirname=relative_dirname)
        full_path = join(common.BONGOSEC_PATH, relative_dirname, filename)
        if bongosec_error:
            raise bongosec_error
        if len(content) == 0:
            raise BongosecError(1112)

        validate_bongosec_xml(content)

        # If file already exists and overwrite is False, raise exception
        if not overwrite and exists(full_path):
            raise BongosecError(1905)
        elif overwrite and exists(full_path):
            backup_file = f'{full_path}.backup'
            try:
                full_copy(full_path, backup_file)
            except IOError as exc:
                raise BongosecError(1019) from exc
...
def delete_rule_file(filename: Union[str, list], relative_dirname: str = None) -> AffectedItemsBongosecResult:
    """Delete a rule file.
    ...
    result = AffectedItemsBongosecResult(all_msg='Rule was successfully deleted',
                                      none_msg='Could not delete rule')

    try:
        relative_dirname, bongosec_error = validate_upload_delete_dir(
            relative_dirname=relative_dirname)
        full_path = join(common.BONGOSEC_PATH, relative_dirname, file)
        if bongosec_error:
            raise bongosec_error
        ...
        if exists(full_path):
            try:
                ...
                raise BongosecError(1907) from exc
        else:
