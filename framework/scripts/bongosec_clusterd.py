...
from bongosec.core.utils import clean_pid_files
...
def set_logging(foreground_mode=False, debug_mode=0) -> BongosecLogger:
    """Set cluster logger.
...
def print_version():
    """Print Bongosec metadata."""
    from bongosec.core.cluster import __author__, __licence__, __version__, __bongosec_name__
    print(f"\n{__bongosec_name__} {__version__} - {__author__}\n\n{__licence__}")
...
def exit_handler(signum, frame):
    ...
    pyDaemonModule.delete_child_pids('bongosec-clusterd', cluster_pid, main_logger)

    # Remove cluster's pidfile
    pyDaemonModule.delete_pid('bongosec-clusterd', cluster_pid)
...
# Master main
#
async def master_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: BongosecLogger):
    ...
    logger : BongosecLogger
        Cluster logger.
    """
    from bongosec.core.cluster import local_server, master
    from bongosec.core.cluster.hap_helper.hap_helper import HAPHelper
...
# Worker main
#
async def worker_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: BongosecLogger):
    ...
    from bongosec.core.cluster import local_server, worker
...
def main():
    """Main function of the bongosec-clusterd script in charge of starting the cluster process."""
    import bongosec.core.cluster.cluster

    # Set correct permissions on cluster.log file
    if os.path.exists(f'{common.BONGOSEC_PATH}/logs/cluster.log'):
        os.chown(f'{common.BONGOSEC_PATH}/logs/cluster.log', common.bongosec_uid(), common.bongosec_gid())
        os.chmod(f'{common.BONGOSEC_PATH}/logs/cluster.log', 0o660)
    ...
    if cluster_configuration['disabled']:
        sys.exit(0)
    try:
        bongosec.core.cluster.cluster.check_cluster_config(cluster_configuration)
    except Exception as e:
        main_logger.error(e)
        sys.exit(1)
    ...
    bongosec.core.cluster.cluster.clean_up()

    # Check for unused PID files
    clean_pid_files('bongosec-clusterd')

    # Foreground/Daemon
    if not args.foreground:
        pyDaemonModule.pyDaemon()

    # Drop privileges to bongosec
    if not args.root:
        os.setgid(common.bongosec_gid())
        os.setuid(common.bongosec_uid())

    pid = os.getpid()
    pyDaemonModule.create_pid('bongosec-clusterd', pid)
    if args.foreground:
    ...
    try:
        ...
        pyDaemonModule.delete_child_pids('bongosec-clusterd', pid, main_logger)
        pyDaemonModule.delete_pid('bongosec-clusterd', pid)


if __name__ == '__main__':
    import bongosec.core.cluster.utils as cluster_utils
    from bongosec.core import common, configuration, pyDaemonModule
    ...
    # Set logger
    try:
        debug_mode_ = configuration.get_internal_options_value('bongosec_clusterd', 'debug', 2, 0) or args.debug_level
    except Exception:
        debug_mode_ = 0
