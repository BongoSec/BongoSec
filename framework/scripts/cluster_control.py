...
from bongosec.core.cluster import control, local_client
from bongosec.core.common import DECIMALS_DATE_FORMAT
from bongosec.core.utils import get_utc_strptime
...
def main():
    ...
    cluster_status = bongosec.core.cluster.utils.get_cluster_status()
    if cluster_status['enabled'] == 'no' or cluster_status['running'] == 'no':
        logging.error("Cluster is not running.")
        sys.exit(1)

    cluster_config = bongosec.core.cluster.utils.read_config()
    bongosec.core.cluster.cluster.check_cluster_config(config=cluster_config)
    try:
