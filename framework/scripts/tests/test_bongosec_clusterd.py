...
import pytest
import scripts.bongosec_clusterd as bongosec_clusterd
from bongosec.core.cluster.utils import HAPROXY_DISABLED, HAPROXY_HELPER


def test_set_logging():
    """Check and set the behavior of set_logging function."""
    import bongosec.core.cluster.utils as cluster_utils

    bongosec_clusterd.cluster_utils = cluster_utils
    with patch.object(cluster_utils, 'ClusterLogger') as clusterlogger_mock:
        assert bongosec_clusterd.set_logging(foreground_mode=False, debug_mode=0)
...
@patch('scripts.bongosec_clusterd.os.kill')
@patch('scripts.bongosec_clusterd.os.getpid', return_value=1001)
def test_exit_handler(os_getpid_mock, os_kill_mock):
    """Set the behavior when exiting the script."""
    from bongosec.core import pyDaemonModule
    ...
    bongosec_clusterd.main_logger = LoggerMock()
    bongosec_clusterd.pyDaemonModule = pyDaemonModule
    bongosec_clusterd.original_sig_handler = original_sig_handler
    with patch('scripts.bongosec_clusterd.signal', SignalMock):
        with patch.object(bongosec_clusterd, 'main_logger') as main_logger_mock:
            with patch.object(bongosec_clusterd.pyDaemonModule, 'delete_child_pids') as delete_child_pids_mock:
                with patch.object(bongosec_clusterd.pyDaemonModule, 'delete_pid') as delete_pid_mock:
                    with patch.object(bongosec_clusterd, 'original_sig_handler') as original_sig_handler_mock:
                        bongosec_clusterd.exit_handler(9, 99)
                        main_logger_mock.assert_has_calls([call.info('SIGNAL [(9)-(9)] received. Exit...')])
                        delete_child_pids_mock.assert_called_once_with(
                            'bongosec-clusterd', os_getpid_mock.return_value, main_logger_mock)
                        delete_pid_mock.assert_called_once_with('bongosec-clusterd', os_getpid_mock.return_value)
                        ...
                        bongosec_clusterd.original_sig_handler = original_sig_handler_not_callable
                        bongosec_clusterd.exit_handler(9, 99)
                        main_logger_mock.assert_has_calls([call.info('SIGNAL [(9)-(9)] received. Exit...')])
                        delete_child_pids_mock.assert_called_once_with('bongosec-clusterd', 1001, main_logger_mock)
                        delete_pid_mock.assert_called_once_with('bongosec-clusterd', 1001)
...
async def test_master_main(helper_disabled: bool):
    ...
    with patch('scripts.bongosec_clusterd.asyncio.gather', gather):
        with patch('bongosec.core.cluster.master.Master', MasterMock):
            with patch('bongosec.core.cluster.local_server.LocalServerMaster', LocalServerMasterMock):
                with patch('bongosec.core.cluster.hap_helper.hap_helper.HAPHelper', HAPHElperMock):
                    await bongosec_clusterd.master_main(
                        args=args,
...
async def test_worker_main(asyncio_sleep_mock):
    ...
    with patch.object(bongosec_clusterd, 'main_logger') as main_logger_mock:
        with patch('concurrent.futures.ProcessPoolExecutor', side_effect=FileNotFoundError) as processpoolexecutor_mock:
            with patch('scripts.bongosec_clusterd.asyncio.gather', gather):
                with patch('scripts.bongosec_clusterd.logging.info') as logging_info_mock:
                    with patch('bongosec.core.cluster.worker.Worker', WorkerMock):
                        with patch('bongosec.core.cluster.local_server.LocalServerWorker', LocalServerWorkerMock):
                            with pytest.raises(IndexError):
                                await bongosec_clusterd.worker_main(
                            ...
                            main_logger_mock.assert_has_calls([
                                call.warning(
                                    "In order to take advantage of Bongosec 4.3.0 cluster improvements, the directory "
                                ...
                                call.warning(
                                    'The Bongosec cluster will be run without the improvements added in Bongosec 4.3.0 and '
                                    'higher versions.')
                            ])
...
@patch('scripts.bongosec_clusterd.argparse.ArgumentParser')
def test_get_script_arguments(argument_parser_mock):
    """Set the bongosec_clusterd script parameters."""
    from bongosec.core import common

    bongosec_clusterd.common = common
    with patch.object(bongosec_clusterd.common, 'OSSEC_CONF', 'testing/path'):
        bongosec_clusterd.get_script_arguments()
...
@patch('scripts.bongosec_clusterd.sys.exit', side_effect=sys.exit)
@patch('scripts.bongosec_clusterd.os.getpid', return_value=543)
@patch('scripts.bongosec_clusterd.os.setgid')
@patch('scripts.bongosec_clusterd.os.setuid')
@patch('scripts.bongosec_clusterd.os.chmod')
@patch('scripts.bongosec_clusterd.os.chown')
@patch('scripts.bongosec_clusterd.os.path.exists', return_value=True)
@patch('builtins.print')
def test_main(print_mock, path_exists_mock, chown_mock, chmod_mock, setuid_mock, setgid_mock, getpid_mock, exit_mock):
    """Check and set the behavior of bongosec_clusterd main function."""
    import bongosec.core.cluster.utils as cluster_utils
    from bongosec.core import common, pyDaemonModule
    ...
    bongosec_clusterd.main_logger = LoggerMock()
    bongosec_clusterd.args = args
    bongosec_clusterd.common = common
    bongosec_clusterd.pyDaemonModule = pyDaemonModule
    bongosec_clusterd.cluster_utils = cluster_utils
    with patch.object(common, 'bongosec_uid', return_value='uid_test'):
        with patch.object(common, 'bongosec_gid', return_value='gid_test'):

            with patch.object(bongosec_clusterd.cluster_utils, 'read_config',
                              return_value={'disabled': False, 'node_type': 'master'}):
                with patch.object(bongosec_clusterd.main_logger, 'error') as main_logger_mock:
                    with patch.object(bongosec_clusterd.main_logger, 'info') as main_logger_info_mock:
                        with patch.object(bongosec_clusterd.cluster_utils, 'read_config', side_effect=Exception):
                            with pytest.raises(SystemExit):
                                bongosec_clusterd.main()
                            main_logger_mock.assert_called_once()
                            main_logger_mock.reset_mock()
                            path_exists_mock.assert_any_call(f'{common.BONGOSEC_PATH}/logs/cluster.log')
                            chown_mock.assert_called_with(f'{common.BONGOSEC_PATH}/logs/cluster.log', 'uid_test',
                                                          'gid_test')
                            chmod_mock.assert_called_with(f'{common.BONGOSEC_PATH}/logs/cluster.log', 432)
                            exit_mock.assert_called_once_with(1)
                            exit_mock.reset_mock()

                        with patch.object(bongosec_clusterd.cluster_utils, 'read_config', return_value={'disabled': True}):
                            with pytest.raises(SystemExit):
                                bongosec_clusterd.main()
                            exit_mock.assert_called_once_with(0)
                            exit_mock.reset_mock()

                        with patch('bongosec.core.cluster.cluster.check_cluster_config', side_effect=IndexError):
                            with pytest.raises(SystemExit):
                                bongosec_clusterd.main()
                            main_logger_mock.assert_called_once()
                            exit_mock.assert_called_once_with(1)
                        ...
                        with patch('bongosec.core.cluster.cluster.check_cluster_config', return_value=None):
                            with pytest.raises(SystemExit):
                                bongosec_clusterd.main()
                            main_logger_mock.assert_called_once()
                            exit_mock.assert_called_once_with(0)
                            ...
                            bongosec_clusterd.args = args
                            with patch('bongosec.core.cluster.cluster.clean_up') as clean_up_mock:
                                with patch('scripts.bongosec_clusterd.clean_pid_files') as clean_pid_files_mock:
                                    with patch.object(bongosec_clusterd.pyDaemonModule, 'pyDaemon') as pyDaemon_mock:
                                        with patch.object(bongosec_clusterd.pyDaemonModule,
                                                          'create_pid') as create_pid_mock:
                                            with patch.object(bongosec_clusterd.pyDaemonModule, 'delete_child_pids'):
                                                with patch.object(bongosec_clusterd.pyDaemonModule,
                                                    ...
                                                    clean_pid_files_mock.assert_called_once_with('bongosec-clusterd')
                                                    ...
                                                    create_pid_mock.assert_called_once_with('bongosec-clusterd', 543)
                                                    delete_pid_mock.assert_called_once_with('bongosec-clusterd', 543)
                                                    ...
                                                    bongosec_clusterd.cluster_items = {}
                                                    with patch('scripts.bongosec_clusterd.master_main',
