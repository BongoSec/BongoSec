...
import scripts.bongosec_logtest as bongosec_logtest


class BongosecSocketMock:
    """Auxiliary mock."""
...
class BongosecDeamonProtocolMock:
    """Auxiliary mock."""
...
def test_init_argparse(argument_parser_mock):
    ...
    bongosec_logtest.init_argparse()
...
@patch('scripts.bongosec_logtest.init_logger')
@patch('scripts.bongosec_logtest.BongosecLogtest')
@patch('scripts.bongosec_logtest.init_argparse')
@patch('builtins.input', return_value='mock')
def test_main(input_mock, argparse_mock, bongosec_logtest_class_mock, init_logger_mock, register_mock, logger_warning_mock,
    ...
    bongosec_logtest_class_mock.return_value = BongosecLogtestMock()

    # Test the first 'try' present in the 'while'
    try:
        bongosec_logtest.main()
    ...
    logger_info_mock.assert_has_calls([call('%s', 'Bongosec ERROR - Bongosec.'),
                                       ...
                                       call('Starting bongosec-logtest %s', 'ERROR'),
                                       call('Type one log per line')])
    logger_error_mock.assert_called_once_with('Unit test configuration wrong syntax: %s',
                                              bongosec_logtest_class_mock.return_value.get_last_ut())
    logger_warning_mock.assert_has_calls([call('** Bongosec-Logtest: %s', 'WARNING'), call('')])

    assert bongosec_logtest_class_mock.return_value.show_last_ut_result_called is True
    assert bongosec_logtest_class_mock.return_value.remove_last_session_called is False
    assert bongosec_logtest_class_mock.return_value.get_last_ut_called is True
    assert bongosec_logtest_class_mock.return_value.process_log_called is True
    ...
    bongosec_logtest_class_mock.return_value = BongosecLogtestMock(True)
    ...
    bongosec_logtest_class_mock.return_value = BongosecLogtestMock(process_log_exception='ValueError')
    ...
    logger_error_mock.assert_called_once_with('** Bongosec-logtest error ')

    # Test the third exception
    logger_error_mock.reset_mock()
    bongosec_logtest_class_mock.return_value = BongosecLogtestMock(process_log_exception='ConnectionError')
    logger_error_mock.side_effect = Exception()
    ...
    logger_error_mock.assert_called_once_with('** Bongosec-logtest error when connecting with bongosec-analysisd')


# Test BongosecDaemonProtocol class methods

def create_bongosec_daemon_protocol_class():
    """Create new BongosecDaemonProtocol class."""
    return bongosec_logtest.BongosecDeamonProtocol()


def test_wdp_init():
    """Test the init method, checking the initial status of its attributes."""
    wdp = create_bongosec_daemon_protocol_class()

    assert isinstance(wdp.protocol, dict)
    assert wdp.protocol['version'] == 1
    assert isinstance(wdp.protocol['origin'], dict)
    assert wdp.protocol['origin']['name'] == 'bongosec-logtest'
    assert wdp.protocol['origin']['module'] == 'bongosec-logtest'


@patch('json.dumps', return_value='')
def test_wdp_wrap(json_dumps_mock):
    """Test if the data is being properly wrapped with bongosec daemon protocol information."""
    wdp = create_bongosec_daemon_protocol_class()
    assert wdp.wrap(command='command', parameters={'parameters': 'parameters'}) == json_dumps_mock.return_value
    json_dumps_mock.assert_called_once_with({'version': 1, 'origin': {'name': 'bongosec-logtest', 'module':
        'bongosec-logtest'}, 'command': 'command', 'parameters':
                                                 {'parameters': 'parameters'}})


@patch('json.loads', return_value={'error': 'error', 'message': 'message'})
def test_wdp_unwrap(json_loads_mock):
    """Test if the data is being properly unwrapped from the bongosec daemon protocol."""
    wdp = create_bongosec_daemon_protocol_class()
    msg = {}
...
# Test BongosecSocket methods

def create_bongosec_socket_class(file):
    """Create a new BongosecSocket class."""
    return bongosec_logtest.BongosecSocket(file=file)


def test_ws_init():
    """Test the correct start of the BongosecSocket class."""
    file = ''
    assert create_bongosec_socket_class(file).file == file
...
def test_ws_sent(unpack_mock, pack_mock, msg_waitall_mock, stream_mock, unix_mock, socket_socket_mock):
    ...
    ws = create_bongosec_socket_class(file=file)
...
# Test BongosecLogtest class methods

@patch('scripts.bongosec_logtest.BongosecSocket', return_value=BongosecSocketMock())
@patch('scripts.bongosec_logtest.LOGTEST_SOCKET')
@patch('scripts.bongosec_logtest.BongosecDeamonProtocol', return_value=BongosecDeamonProtocolMock())
def create_bongosec_logtest_class(bongosec_deamon_mock, logtest_socket_mock, bongosec_socket_mock):
    """Create new BongosecLogtest class."""
    return bongosec_logtest.BongosecLogtest()


def test_wl_init():
    """Test the correct initialization of BongosecLogtest class."""
    wl = create_bongosec_logtest_class()

    assert isinstance(wl.protocol, BongosecDeamonProtocolMock)
    assert isinstance(wl.socket, BongosecSocketMock)
...
@patch('scripts.bongosec_logtest.BongosecLogtest.remove_session')
...
@patch('logging.debug')
@patch('json.dumps', return_value='')
@patch('scripts.bongosec_logtest.BongosecLogtest.show_ossec_logtest_like')
...
@patch('logging.info')
@patch('scripts.bongosec_logtest.BongosecLogtest.show_phase_info')
def test_wl_show_ossec_logtest_like(show_phase_info_mock, info_mock):
    """Test if bongosec-logtest output is being shown as ossec-logtest output."""
    ...
    # Test the third 'if'
    bongosec_logtest.BongosecLogtest.show_ossec_logtest_like(output)
...
def test_wl_show_phase_info(info_mock):
    """Check if bongosec-logtest is processing phase information."""
    phase_data = {'key': 'value', 'key2': 'value2', 'key3': {'1': '2'}}
    show_first = ['key']

    bongosec_logtest.BongosecLogtest.show_phase_info(phase_data=phase_data, show_first=show_first)
    info_mock.assert_has_calls([call("\t%s: '%s'", 'key', 'value'), call("\t%s: '%s'", 'key2', 'value2')])


@patch('logging.info')
@patch('scripts.bongosec_logtest.BongosecLogtest.get_last_ut', return_value='mock')
...
def create_bongosec_class():
    """Auxiliary function to create a Bongosec class."""
    return bongosec_logtest.Bongosec


@patch('scripts.bongosec_logtest.common.find_bongosec_path', return_value='')
def test_bongosec_get_install_path(find_bongosec_path_mock):
    """Test is we can get the installation path correctly."""
    assert create_bongosec_class().get_install_path() == find_bongosec_path_mock.return_value


@patch('subprocess.PIPE')
@patch('subprocess.Popen')
@patch('os.path.join', return_value='')
@patch('scripts.bongosec_logtest.Bongosec.get_install_path', return_value='')
def test_bongosec_get_info(get_install_mock, join_mock, popen_mock, pipe_mock):
    """Check if we can properly obtain information from bongosec-control."""
    ...
    bongosec = create_bongosec_class()
    popen_mock.return_value = ProcMock()
    field = 'key'

    # Test the 'try'
    assert bongosec.get_info(field=field) == field
    get_install_mock.assert_called_once_with()
    join_mock.assert_called_once_with(get_install_mock.return_value, 'bin', 'bongosec-control')
    ...
    assert bongosec.get_info(field=field) == 'ERROR'


@patch('scripts.bongosec_logtest.Bongosec.get_info', return_value='')
def test_bongosec_get_version_str(get_info_mock):
    """Test if the version is being properly retrieved."""
    assert create_bongosec_class().get_version_str() == get_info_mock.return_value
    get_info_mock.assert_called_once_with('BONGOSEC_VERSION')
...
def test_bongosec_get_license(dedent_mock):
    """Test that the license was not changed."""
    assert create_bongosec_class().get_license() == dedent_mock.return_value
...
def test_init_logger(logging_mock):
    ...
    bongosec_logtest.init_logger(ArgsMock())
