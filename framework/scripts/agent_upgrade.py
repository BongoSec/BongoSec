...
# Set framework path
path.append(dirname(argv[0]) + '/../framework')  # It is necessary to import Bongosec package

# Import framework
try:
    import bongosec.agent
    from bongosec.agent import upgrade_agents, get_upgrade_result, get_agents
    from bongosec.core import common
    from bongosec.core.exception import BongosecError
    from bongosec.core.cluster import utils as cluster_utils
except Exception as e:
    print("Error importing 'Bongosec' package.\n\n{0}\n".format(e))
    exit()
...
def get_script_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("-v", "--version", type=str, help="Version to upgrade. [Default: latest Bongosec version]")
...
def list_outdated():
    ...
    agents = bongosec.agent.get_outdated_agents()
...
async def main():
    ...
    except BongosecError as bongosec_err:
        print(f"Error {bongosec_err.code}: {bongosec_err.message}")
        if args.debug:
            raise
