...
from bongosec.core.common import LOGTEST_SOCKET
...
def main():
    """bongosec-logtest main function."""
    # Parse cmdline args
    ...
    if args.version:
        logging.info('%s', Bongosec.get_description())
        logging.info('%s', Bongosec.get_license())
        sys.exit(0)
    ...
    # Initialize bongosec-logtest component
    w_logtest = BongosecLogtest(location=args.location)
    logging.info('Starting bongosec-logtest %s', Bongosec.get_version_str())
    ...
    while True:
        ...
        try:
            ...
            logging.error('** Bongosec-logtest error ' + str(error))
            continue
        except ConnectionError:
            logging.error('** Bongosec-logtest error when connecting with bongosec-analysisd')
            continue
        ...
        if 'messages' in output.keys():
            ...
            for message in output['messages']:
                if message.startswith("WARNING"):
                    logging.warning('** Bongosec-Logtest: %s', message)
        ...
        # Show bongosec-logtest output
        BongosecLogtest.show_output(output)
...
class BongosecDeamonProtocol:
    """Encapsulate logic communication aspects between Bongosec daemons."""

    def __init__(self, version: int = 1, origin_module: str = "bongosec-logtest", module_name: str = "bongosec-logtest"):
        """Class constructor.
        ...
        origin_module : str
            Origin source module. Default: "bongosec-logtest"
        module_name : str
            Source module name. Default: "bongosec-logtest"
        """
        self.protocol = dict()
    ...
    def wrap(self, command: str, parameters: dict) -> str:
        """Wrap data with Bongosec daemon protocol information.
    ...
    def unwrap(self, msg: dict) -> dict:
        """Unwrap data from Bongosec daemon protocol information.
...
class BongosecSocket:
    """Encapsulate bongosec-socket communication (header with message size)."""
    ...
    def send(self, msg: str) -> bytes:
        """Send and receive data to bongosec-socket (header with message size).
...
class BongosecLogtest:
    """Top level class to interact with bongosec-logtest feature, part of bongosec-analysisd."""
    ...
    def __init__(self, location: str = "stdin", log_format: str = "syslog"):
        ...
        """
        self.protocol = BongosecDeamonProtocol()
        self.socket = BongosecSocket(LOGTEST_SOCKET)
    ...
    def process_log(self, log, token: str = None, options: str = None) -> dict:
        """Send log event to bongosec-logtest and receive the outcome.
    ...
    def show_output(output: dict):
        ...
        BongosecLogtest.show_ossec_logtest_like(output)

    def show_ossec_logtest_like(output: dict):
        """Show bongosec-logtest output like ossec-logtest.

        Parameters
        ----------
        output : dict
            Bongosec-logtest outcome.
        """
        ...
        if 'predecoder' in output_data:
            BongosecLogtest.show_phase_info(output_data['predecoder'], ['timestamp', 'hostname', 'program_name'])
        # Decoding phase
        logging.info('')
        logging.info('**Phase 2: Completed decoding.')
        if 'decoder' in output_data and output_data['decoder']:
            BongosecLogtest.show_phase_info(output_data['decoder'], ['name', 'parent'])
            if 'data' in output_data:
                BongosecLogtest.show_phase_info(output_data['data'])
        else:
        ...
        if 'rule' in output_data:
            ...
            BongosecLogtest.show_phase_info(output_data['rule'], ['id', 'level', 'description', 'groups', 'firedtimes'])
        if output['alert']:
            logging.info('**Alert to be generated.')

    def show_phase_info(phase_data: dict, show_first: list = None, prefix: str = ""):
        """Show bongosec-logtest processing phase information.
        ...
        for field in sorted(phase_data.keys()):
            if isinstance(phase_data.get(field), dict):
                BongosecLogtest.show_phase_info(phase_data.pop(field), [], prefix + field + '.')
            else:
...
class Bongosec:
    def get_install_path() -> str:
        """Get Bongosec installation path, obtained relative to the path of this file.

        Returns
        -------
        str
            Bongosec installation path.
        """
        return common.find_bongosec_path()

    def get_info(field: str) -> str:
        """Get Bongosec information from bongosec-control.
        ...
        str
            Field value.
        """
        bongosec_control = os.path.join(Bongosec.get_install_path(), "bin", "bongosec-control")
        bongosec_env_vars = dict()
        try:
            proc = subprocess.Popen([bongosec_control, "info"], stdout=subprocess.PIPE)
    ...
    def get_version_str() -> str:
        ...
        str
            Bongosec version.
        """
        return Bongosec.get_info('BONGOSEC_VERSION')

    def get_description() -> str:
        """Get Bongosec description, contact info and version.
...
def init_logger(args: argparse.Namespace):
    """Initialize bongosec-logtest logger.
