...
main() {
...

    {
        if [ ! -d $folder ]
        then
            >&2 echo "INFO: Cloning bongosec/$branch"
            git clone --quiet --depth 1 https://github.com/bongosec/bongosec.git -b $branch $folder > /dev/null || exit
        fi
    ...
    declare -A components=( \
        ...
        [syscollector]=bongosec_modules/syscollector\
        [fim]=syscheckd\
    )
