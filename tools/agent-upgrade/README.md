...
```
openssl req -new -nodes -newkey rsa:2048 -keyout wpkcert.key -out wpkcert.csr -subj '/C=US/ST=CA/O=Bongosec'
```

- `/C=US` is the country.
- `/ST=CA` is the state.
- `/O=Bongosec` is the organization's name.
...
```
curl -Lo bongosec-3.0.zip https://github.com/bongosec/bongosec/archive/3.0.zip
unzip bongosec-3.0.zip
```

2. Compile the project:

```
make -C bongosec-3.0/src TARGET=agent
```
...
In this particular case, the Bongosec Project's root directory contains the proper `upgrade.sh` file.
...
1. Overwrite the shipped root CA with your certificate. This will prevent your agent from upgrading using WPK packages from Bongosec.
...
Get the WPK package into the Bongosec manager and run:
...
> bongosec_agent_v3.0.0-beta7_centos_7_x86_64.wpk
