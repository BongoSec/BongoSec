...
update_file_sources() {
    ...
    # Update bongosec-*.sh scripts
    for script in "$DIR_SRC/init/bongosec-server.sh" "$DIR_SRC/init/bongosec-client.sh" "$DIR_SRC/init/bongosec-local.sh"; do
        if [[ -n "$new_version" ]]; then
    ...
    # Update bongosec-installer.nsi
    if [[ -n "$new_version" ]]; then
        sed -i -E "s|(^!define VERSION\s+\")[0-9]+\.[0-9]+\.[0-9]+(\")|\1${new_version}\2|" "$DIR_SRC/win32/bongosec-installer.nsi"
        sed -i -E "s|(^VIProductVersion\s+\")[0-9]+\.[0-9]+\.[0-9]+(\.\"\$)|\1${new_version}\2|" "$DIR_SRC/win32/bongosec-installer.nsi"
        log_action "Modified $DIR_SRC/win32/bongosec-installer.nsi with new version: $new_version"
    fi
    if [[ -n "$new_stage" ]]; then
        sed -i -E "s|(^!define REVISION\s+\")[^\"]+(\")|\1${new_stage}\2|" "$DIR_SRC/win32/bongosec-installer.nsi"
        log_action "Modified $DIR_SRC/win32/bongosec-installer.nsi with new stage: $new_stage"
    fi

    # Update bongosec-installer.wxs
    if [[ -n "$new_version" ]]; then
        sed -i -E "s|(<Product Id=\"\*\" Name=\"Bongosec Agent\" Language=\"1033\" Version=\")[^\"]+(\" Manufacturer=)|\1${new_version}\2|" "$DIR_SRC/win32/bongosec-installer.wxs"
        log_action "Modified $DIR_SRC/win32/bongosec-installer.wxs with new version: $new_version"
    fi
...
update_file_framework() {
    ...
    local init_file="$DIR_FRAMEWORK/bongosec/__init__.py"
    local cluster_file="$DIR_FRAMEWORK/bongosec/core/cluster/__init__.py"
...
update_file_packages() {
    ...
    for spec_file in $(find "$DIR_PACKAGE" -type f -name "*.spec"); do
        ...
        if [ -n "$existing_line" ]; then
            sed -i -E "s|^\* .+ - ${final_version}$|* ${spec_date} support <info@bongosec.github.io> - ${final_version}|" "$spec_file"
            log_action "Updated changelog date for version ${final_version} in: $spec_file"
        else
            sed -i -E "/^%changelog\s*$/a * ${spec_date} support <info@bongosec.github.io> - ${final_version}\n- More info: https://documentation.bongosec.github.io/current/release-notes/release-${final_version//./-}.html" "$spec_file"
...
* More info: https://documentation.bongosec.github.io/current/release-notes/release-${final_version//./-}.html

-- Bongosec, Inc <info@bongosec.github.io>  ${formatted_date}
...
EOF
        ...
        if grep -qE "$version_pattern_grep" "$changelog_file"; then
            ...
            BEGIN { inside_match = 0 }
            {
                ...
                if (inside_match && $0 ~ /^ -- Bongosec, Inc <info@bongosec.github.io>  /) {
                    print " -- Bongosec, Inc <info@bongosec.github.io>  " new_date
                    inside_match = 0
                    next
                }
                print
    ...
    for copyright_file in $(find "$DIR_PACKAGE" -type f -name "copyright"); do
        sed -i -E "s|(^    Bongosec, Inc <info@bongosec.github.io> on )[^$]+(\$)|\1${formatted_date}\2|" "$copyright_file"
