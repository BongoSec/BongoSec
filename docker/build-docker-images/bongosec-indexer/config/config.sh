...
export USER=${NAME}
export GROUP=${NAME}
export VERSION=${BONGOSEC_VERSION}-${BONGOSEC_TAG_REVISION}
...
export INDEXER_FILE=bongosec-indexer-base.tar.xz
export BASE_FILE=bongosec-indexer-base-${VERSION}-linux-x64.tar.xz
export REPO_DIR=/unattended_installer

## Variables
CERT_TOOL=bongosec-certs-tool.sh
PASSWORD_TOOL=bongosec-passwords-tool.sh
PACKAGES_URL=https://packages.bongosec.com/5.0/
PACKAGES_DEV_URL=https://packages-dev.bongosec.com/5.0/
...
# Copy Bongosec indexer's certificates
cp -pr /bongosec-certificates/demo.indexer.pem ${TARGET_DIR}${CONFIG_DIR}/certs/indexer.pem
cp -pr /bongosec-certificates/demo.indexer-key.pem ${TARGET_DIR}${CONFIG_DIR}/certs/indexer-key.pem
cp -pr /bongosec-certificates/root-ca.key ${TARGET_DIR}${CONFIG_DIR}/certs/root-ca.key
cp -pr /bongosec-certificates/root-ca.pem ${TARGET_DIR}${CONFIG_DIR}/certs/root-ca.pem
cp -pr /bongosec-certificates/admin.pem ${TARGET_DIR}${CONFIG_DIR}/certs/admin.pem
cp -pr /bongosec-certificates/admin-key.pem ${TARGET_DIR}${CONFIG_DIR}/certs/admin-key.pem

# Delete xms and xmx parameters in jvm.options
sed '/-Xms/d' -i /etc/bongosec-indexer/jvm.options
sed '/-Xmx/d' -i /etc/bongosec-indexer/jvm.options
sed -i 's/-Djava.security.policy=file:\/\/\/etc\/bongosec-indexer\/opensearch-performance-analyzer\/opensearch_security.policy/-Djava.security.policy=file:\/\/\/usr\/share\/bongosec-indexer\/opensearch-performance-analyzer\/opensearch_security.policy/g' /etc/bongosec-indexer/jvm.options
