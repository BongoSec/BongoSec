## variables
APT_KEY=https://packages-dev.bongosec.com/key/GPG-KEY-BONGOSEC
GPG_SIGN="gpgcheck=1\ngpgkey=${APT_KEY}]"
REPOSITORY="[bongosec]\n${GPG_SIGN}\nenabled=1\nname=EL-\$releasever - Bongosec\nbaseurl=https://packages-dev.bongosec.com/pre-release/yum/\nprotect=1"
...
if [[ -n "${BONGOSEC_TAG}" ]]; then
  APT_KEY=https://packages.bongosec.com/key/GPG-KEY-BONGOSEC
  GPG_SIGN="gpgcheck=1\ngpgkey=${APT_KEY}]"
  REPOSITORY="[bongosec]\n${GPG_SIGN}\nenabled=1\nname=EL-\$releasever - Bongosec\nbaseurl=https://packages.bongosec.com/5.x/yum/\nprotect=1"
fi
