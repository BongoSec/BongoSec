...
BONGOSEC_DEFAULT_RETRY_CONFIGURATION = {aws_tools.RETRY_ATTEMPTS_KEY: 10, aws_tools.RETRY_MODE_BOTO_KEY: 'standard'}
MESSAGE_HEADER = "1:Bongosec-AWS:"


class BongosecIntegration:
    """
    ...
    def __init__(self, access_key, secret_key, profile, iam_role_arn, service_name=None, region=None,
        ...
        self.bongosec_path = utils.find_bongosec_path()
        self.bongosec_version = utils.get_bongosec_version()
        self.bongosec_queue = path.join(self.bongosec_path, "queue", "sockets", "queue")
        self.bongosec_wodle = path.join(self.bongosec_path, "wodles", "aws")
    ...
    def default_config(profile: str) -> dict:
        ...
        This method is called when Bongosec Integration is instantiated and sets a default config using .aws/config file
        ...
        if path.exists(aws_tools.DEFAULT_AWS_CONFIG_PATH):
            ...
            try:
                ...
                if aws_tools.RETRY_ATTEMPTS_KEY in profile_config \
                    ...
                    # Set retry config
                    retries = copy.deepcopy(BONGOSEC_DEFAULT_RETRY_CONFIGURATION)
                    aws_tools.debug(
            ...
            args['config'] = config.Config(retries=copy.deepcopy(BONGOSEC_DEFAULT_RETRY_CONFIGURATION))
            aws_tools.debug(
    ...
    def send_msg(self, msg, dump_json=True):
        """
        Sends an AWS event to the Bongosec Queue
        ...
        try:
            ...
            if e.errno == 111:
                aws_tools.error("Bongosec must be running.")
                sys.exit(11)
            elif e.errno == 90:
                aws_tools.error("Message too long to send to Bongosec.  Skipping message...")
                aws_tools.debug('+++ ERROR: Message longer than buffer socket for Bongosec. Consider increasing rmem_max. '
                                'Skipping message...', 1)
            else:
                aws_tools.error("Error sending message to bongosec: {}".format(e))
                sys.exit(13)
        except Exception as e:
            aws_tools.error("Error sending message to bongosec: {}".format(e))
            sys.exit(13)
...
class BongosecAWSDatabase(BongosecIntegration):
    """
    ...
    def __init__(self, access_key, secret_key, profile, iam_role_arn, db_name,
        ...
        BongosecIntegration.__init__(self, service_name=service_name,
        ...
        self.db_path = "{0}/{1}.db".format(self.bongosec_wodle, db_name)
        ...
        try:
            if self.db_cursor.execute(self.sql_find_table, {'name': 'metadata'}).fetchone():
                ...
                try:
                    ...
                    if metadata_version != self.bongosec_version:
                        self.db_cursor.execute(self.sql_update_version_metadata, {'bongosec_version': self.bongosec_version})
                ...
                try:
                    ...
                    self.db_cursor.execute(self.sql_insert_version_metadata, {'bongosec_version': self.bongosec_version})
