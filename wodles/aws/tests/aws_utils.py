...
BONGOSEC_VERSION = "4.5.0"

TEST_BONGOSEC_PATH = "/var/ossec"
TEST_DATABASE = "test"
...
def get_bongosec_integration_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
    ...
    """Return a dict containing every parameter supported by BongosecIntegration. Used to simulate different ossec.conf
    configurations.
...
def get_bongosec_aws_database_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
                                      db_name: str = TEST_DATABASE, access_key: str = None, secret_key: str = None,
                                      iam_role_arn: str = None, region: str = None, discard_field: str = None,
                                      discard_regex: str = None, sts_endpoint: str = None, service_endpoint: str = None,
                                      iam_role_duration: str = None, external_id: str = None):
    """Return a dict containing every parameter supported by BongosecAWSDatabase.
...
def get_mocked_bongosec_integration(**kwargs):
    with patch('bongosec_integration.BongosecIntegration.get_client'), \
            patch('bongosec_integration.sqlite3.connect'), \
            patch('bongosec_integration.utils.find_bongosec_path', return_value=TEST_BONGOSEC_PATH), \
            patch('bongosec_integration.utils.get_bongosec_version', return_value=BONGOSEC_VERSION):
        return bongosec_integration.BongosecIntegration(**get_bongosec_integration_parameters(**kwargs))


def get_mocked_bongosec_aws_database(**kwargs):
    with patch('bongosec_integration.BongosecAWSDatabase.check_metadata_version'), \
            patch('bongosec_integration.BongosecIntegration.get_client'), \
            patch('bongosec_integration.sqlite3.connect'), \
            patch('bongosec_integration.utils.find_bongosec_path', return_value=TEST_BONGOSEC_PATH), \
            patch('bongosec_integration.utils.get_bongosec_version', return_value=BONGOSEC_VERSION):
        return bongosec_integration.BongosecAWSDatabase(**get_bongosec_aws_database_parameters(**kwargs))
