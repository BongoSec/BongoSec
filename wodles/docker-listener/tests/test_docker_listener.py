...
def test_DockerListener__init__():
    ...
    for attribute in ['bongosec_path', 'bongosec_queue', 'msg_header', 'client', 'thread1', 'thread2']:
...
def test_DockerListener_process(mock_send):
    """Test process function sends the decoded events to the Bongosec socket."""
    event = b"test"
    dl = docker_listener.DockerListener()
    dl.process(event)
...
def test_DockerListener_send_msg(mock_socket):
    """Test send_msg sends the messages with the expected contents to the Bongosec socket."""
    msg = '{"test": "value"}'
    ...
    m.connect.assert_called_with(dl.bongosec_queue)
