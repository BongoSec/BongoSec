...
def main():
    ...
    try:
        ...
        if arguments.integration_type == "pubsub":
            ...
            with ThreadPoolExecutor() as executor:
                ...
                # check permissions
                subscriber_client = BongosecGCloudSubscriber(credentials_file, project, logger, subscription_id)
                ...
                if messages_per_thread > 0:
                    for _ in range(n_threads - 1):
                        client = BongosecGCloudSubscriber(credentials_file, project, logger, subscription_id)
    ...
    except exceptions.BongosecIntegrationException as gcloud_exception:
        logging_func = logger.critical if \
            isinstance(gcloud_exception, exceptions.BongosecIntegrationInternalError) else \
            logger.error
