...
def append_bongosec_prefixes(dictionary: Dict, path_fields: Dict[Any, List[Tuple[str, str]]]) -> None:
    """Append Bongosec prefix to all path fields in a dictionary.
    Parameters
    ----------
    dictionary : dict
...
def read_yaml_config(config_file: str = CONFIG_FILE_PATH, default_conf: dict = None) -> Dict:
    ...
    # Append Bongosec prefixes to all relative paths in configuration
    append_bongosec_prefixes(configuration, {API_SSL_PATH: [('https', 'key'), ('https', 'cert'), ('https', 'ca')]})
