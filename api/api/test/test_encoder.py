...
def custom_hook(dct):
    if 'key' in dct:
        ...
        return BongosecResult.decode_json({'result': dct, 'str_priority': 'v2'})
    else:
        return dct


@pytest.mark.parametrize('o', [{'key': 'v1'},
                               BongosecResult({'k1': 'v1'}, str_priority='v2')
                               ]
                         )
def test_encoder_dumps(o):
    """Test dumps method from API encoder using BongosecAPIJSONEncoder."""
    encoded = dumps(o)
...
def test_encoder_prettify():
    """Test prettify method from API encoder using BongosecAPIJSONEncoder."""
