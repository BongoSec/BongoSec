...
from bongosec.core.exception import BongosecError, BongosecPermissionError, BongosecResourceNotFound, \
    BongosecInternalError
...
@pytest.mark.parametrize('exception_type, code, extra_fields, returned_code, returned_exception', [
    ...
    (BongosecError, 1000, ['remediation', 'code'], 400, ProblemException),
    (BongosecPermissionError, 4000, ['remediation', 'code'], 403, ProblemException),
    (BongosecResourceNotFound, 1710, ['remediation', 'code'], 404, ProblemException),
    (BongosecInternalError, 1000, ['remediation', 'code'], 500, ProblemException)
])
...
@pytest.mark.parametrize('obj, code', [
    ((BongosecError(6001), ['value0', 'value1']), 429),
    ((BongosecInternalError(1000), ['value0', 'value1']), None),
    ((BongosecPermissionError(4000), ['value0', 'value1']), None),
    ((BongosecResourceNotFound(1710), ['value0', 'value1']), None)
])
...
async def test_only_master_endpoint(mock_exc):
    ...
    with patch('api.util.running_in_master_node', return_value=False):
        await func_()
        mock_exc.assert_called_once_with(BongosecResourceNotFound(902))
