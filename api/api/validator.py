...
_bongosec_key = re.compile(r'[a-zA-Z0-9]+$')
_bongosec_version = re.compile(r'^(?:bongosec |)v?\d+\.\d+\.\d+$', re.IGNORECASE)
...
BONGOSEC_COMPONENT_CONFIGURATION_MAPPING = MappingProxyType(
    {
...
def is_safe_path(path: str, basedir: str = common.BONGOSEC_PATH, relative: bool = True) -> bool:
    ...
    path : str
        Path to be checked.
    basedir : str
        Bongosec installation directory.
    relative : bool
...
def check_component_configuration_pair(component: str, configuration: str) -> BongosecError:
    """

    Parameters
    ----------
    component : str
        Bongosec component name.
    configuration : str
    ...
    -------
    BongosecError
        It can either return a `BongosecError` or `None`, depending on the given component and configuration. The exception
        is returned and not raised because we use the object to create a problem on API level.
    """
    if configuration not in BONGOSEC_COMPONENT_CONFIGURATION_MAPPING[component]:
        return BongosecError(1128, extra_message=f"Valid configuration values for '{component}': "
                                              f"{BONGOSEC_COMPONENT_CONFIGURATION_MAPPING[component]}")
...
@Draft4Validator.FORMAT_CHECKER.checks("bongosec_key")
def format_bongosec_key(value):
    return check_exp(value, _bongosec_key)


@Draft4Validator.FORMAT_CHECKER.checks("bongosec_version")
def format_bongosec_version(value):
    return check_exp(value, _bongosec_version)
