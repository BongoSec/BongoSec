...
from bongosec.core.cluster.utils import running_in_master_node
...
def to_relative_path(full_path: str) -> str:
    """Return a relative path from Bongosec base directory.
    ...
    -------
    str
        Relative path from Bongosec base directory.
    """
    return os.path.relpath(full_path, common.BONGOSEC_PATH)
...
def _create_problem(exc: Exception, code: int = None):
    ...
    ----------
    exc : Exception
        If `exc` is an instance of `BongosecException` it will be casted into a ProblemException,
...
def only_master_endpoint(func):
    ...
    async def wrapper(*args, **kwargs):
        if not running_in_master_node():
            raise_if_exc(exception.BongosecResourceNotFound(902))
        else:
