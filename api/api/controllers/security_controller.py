...
from bongosec import security, __version__
from bongosec.core.cluster.control import get_system_nodes
from bongosec.core.cluster.dapi.dapi import DistributedAPI
from bongosec.core.exception import BongosecException, BongosecPermissionError
from bongosec.core.results import AffectedItemsBongosecResult, BongosecResult
from bongosec.core.security import revoke_tokens
from bongosec.rbac import preprocessor

logger = logging.getLogger('bongosec-api')
auth_re = re.compile(r'basic (.*)', re.IGNORECASE)


@deprecate_endpoint(link=f'https://documentation.bongosec.github.io/{__version__}/user-manual/api/reference.html#'
...
async def deprecated_login_user(user: str, raw: bool = False) -> ConnexionResponse:
    ...
    return ConnexionResponse(body=token,
           ...
           ConnexionResponse(body=dumps(BongosecResult({'data': TokenResponseModel(token=token)})),
...
async def get_user_me_policies(pretty: bool = False, wait_for_complete: bool = False) -> ConnexionResponse:
    ...
    """
    data = BongosecResult({'data': request.context['token_info']['rbac_policies'],
...
async def revoke_all_tokens(pretty: bool = False) -> ConnexionResponse:
    ...
    data = raise_if_exc(await dapi.distribute_function())
    if type(data) == AffectedItemsBongosecResult and len(data.affected_items) == 0:
        raise_if_exc(BongosecPermissionError(4000, data.message))
