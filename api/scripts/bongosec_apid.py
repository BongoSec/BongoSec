...
def assign_bongosec_ownership(filepath: str):
    ...
    if not os.path.isfile(filepath):
        f = open(filepath, "w")
        f.close()
    if os.stat(filepath).st_gid != common.bongosec_gid() or \
        os.stat(filepath).st_uid != common.bongosec_uid():
        os.chown(filepath, common.bongosec_uid(), common.bongosec_gid())
...
def configure_ssl(params):
    ...
    try:
        ...
        if not os.path.exists(api_conf['https']['key']) \
            ...
            logger.info(
                f"Generated private key file in BONGOSEC_PATH/{to_relative_path(api_conf['https']['key'])}")
            generate_self_signed_certificate(private_key, api_conf['https']['cert'])
            logger.info(
                f"Generated certificate file in BONGOSEC_PATH/{to_relative_path(api_conf['https']['cert'])}")
        ...
        # Check and assign ownership to bongosec user for server.key and server.crt files
        assign_bongosec_ownership(api_conf['https']['key'])
        assign_bongosec_ownership(api_conf['https']['cert'])
        ...
        if exc.errno == 22:
            ...
            msg = f'Bongosec API SSL ERROR. Please, ensure ' \
                    f'if path to certificates is correct in the configuration ' \
                    f'file BONGOSEC_PATH/{to_relative_path(CONFIG_FILE_PATH)}'
            print(msg)
            logger.error(msg)
            raise exc from exc
...
def start(params: dict):
    """Run the Bongosec API.

    If another Bongosec API is running, this function will fail because uvicorn server will
    ...
    app.add_middleware(BongosecAccessLoggerMiddleware, MiddlewarePosition.BEFORE_EXCEPTION)
...
def print_version():
    ...
    print('\n{} {} - {}\n\n{}'.format(__bongosec_name__, __version__, __author__, __licence__))
...
def add_debug2_log_level_and_error():
    """Add a new debug level used by bongosec api and framework."""
...
if __name__ == '__main__':
    ...
    from bongosec.core import common, pyDaemonModule, utils
    from bongosec.rbac.orm import check_database_integrity
    ...
    from api.middlewares import (
        ...
        BongosecAccessLoggerMiddleware,
    ...
    try:
        ...
        print(f"Error when trying to start the Bongosec API. {e}")
        sys.exit(1)
    ...
    try:
        ...
        print(f'Internal error when trying to start the Bongosec API. {e}')
        sys.exit(1)
    finally:
