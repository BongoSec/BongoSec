...
cp -rf /tmp_volume/config/* /var/ossec/ && chown -R bongosec:bongosec /var/ossec/api
...
if [ $4 == "standalone" ]; then
  ...
  sed -i "s:validate_responses=False:validate_responses=True:g" /var/ossec/api/scripts/bongosec_apid.py
fi
...
chown root:bongosec /var/ossec/etc/client.keys
chown -R bongosec:bongosec /var/ossec/queue/db
chown -R bongosec:bongosec /var/ossec/etc/shared
chmod --reference=/var/ossec/etc/shared/default /var/ossec/etc/shared/group*
cd /var/ossec/etc/shared && find -name merged.mg -exec chown bongosec:bongosec {} \; && cd /
chown root:bongosec /var/ossec/etc/shared/ar.conf
...
if [ "$3" == "master" ]; then
  ...
  # Wait until Bongosec API is ready
  elapsed_time=0
