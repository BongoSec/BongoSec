...
framework = path.join(base, 'framework', 'bongosec')
...
def extract_module_from_path(file_path):
    ...
    bongosec_modules = parts[-1]
    return bongosec_modules
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        mapping_list = list()
        for module in bongosec_modules:
            chdir(module)
    ...
    # Calculate mappings for a given file
        ...
        if not path.exists(path.join(base, file_rel_path)):
            print('The relative path does not exist. Example: "framework/bongosec/agent.py"')
            exit(0)
