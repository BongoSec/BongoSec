...
The API integration tests are used to verify that the API is working properly in a complete Bongosec environment.
This environment is built using [`docker`](https://www.docker.com/).

The `bongosec/api/test/integration` directory contains all the API integration tests files and directories used for the
...
The Bongosec environment used to perform the API integration tests is built using `docker`.

This environment is composed of **12 docker containers**. These containers have the following components installed: 3
Bongosec managers, that compose a Bongosec cluster (1 master, 2 workers); 4 Bongosec agents with the same version as the managers
forming the cluster; 4 Bongosec agents with version 3.13.2 (old); and 1 HAProxy load balancer.

The Bongosec version used for the managers and non-old agents is the one specified by the branch used to perform the API
integration tests.

The `docker-compose.yml` file used to deploy the environment is at `bongosec/api/test/integration/env`. The `Dockerfile`,
...
The Bongosec docker environment will have a different configuration depending on
the [`pytest` mark](https://docs.pytest.org/en/6.2.x/mark.html) used to run the tests with.

- If the `standalone` mark is specified, a Bongosec environment with **1 manager and 12 agents** will be built (no cluster)
  .

- If the `cluster` mark is specified, a Bongosec cluster setup with **3 managers and 12 agents** will be built.

- If **no mark** is specified, a Bongosec cluster setup with **3 managers and 12 agents** will be built.

The following table shows how these marks must be used with the `pytest` command and the environment they build:

| Command                          | Environment                                          |  
|----------------------------------|------------------------------------------------------|
| `pytest TEST_NAME`               | Bongosec cluster environment                            |  
| `pytest -m cluster TEST_NAME`    | Bongosec cluster environment                            |
| `pytest -m standalone TEST_NAME` | Bongosec environment with cluster disabled (standalone) | 
...
standalone** can only be passed in a Bongosec environment with cluster disabled and cases with **only cluster** mark can
only be passed in a Bongosec cluster environment.
...
These tests are used to check the proper functioning of a Bongosec environment with RBAC configurations. The `conftest.py`
...
Every time a pull request is created in GitHub for the `bongosec` repository, a battery of checks is performed in the CI
...
The `bongosec/api/test/integration/mapping` directory contains the `integration_test_api_endpoints.json` file that
...
To perform a Bongosec API integration test, we need a specific `python3` environment. This python environment includes the 
...
rootdir: /home/user/git/bongosec/api/test/integration, inifile: pytest.ini
...
We can also use the `bongosec/api/test/integration/run_tests.py` script. This script includes the possibility to collect a 
...
optional arguments:
  ...
  -m {both,standalone,cluster}, --mode {both,standalone,cluster}
                        Specify where to pass API integration tests. Run tests in both environments, standalone environment or Bongosec cluster environment. Default "both".
...
at `bongosec/api/test/integration/_test_results`. Containers' logs (`ossec.log`, `api.log` and `cluster.log`) are stored 
