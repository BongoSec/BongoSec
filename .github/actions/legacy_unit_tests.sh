...
build_bongosec_test_flags() {
    local target=$1
    echo "Building Bongosec for target: $target"
    cd "$GITHUB_WORKSPACE"
    make deps -C src TARGET=${target} -j$(nproc)
    make -C src TARGET=${target} DEBUG=1 TEST=1 -j$(nproc)
}

build_bongosec_unit_tests() {
    local target=$1
    echo "Building Bongosec Unit Tests for target: $target"
...
run_bongosec_unit_tests() {
    local target=$1
    echo "Running Bongosec Unit Tests for target: $target"
    cd "$GITHUB_WORKSPACE/src/unit_tests/build"
    if [[ $target == "agent" ]]; then
...
main() {
    ...
    clean
    build_bongosec_test_flags $target
    build_bongosec_unit_tests $target
    run_bongosec_unit_tests $target
    format_display_test_results
