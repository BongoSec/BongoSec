...
BONGOSEC_REGISTRATION_PASSWORD_PATH="/var/ossec/etc/authd.pass"

function install_bongosec(){

  echo "Testing the following variables $*"
  eval "${*} apt install -y ./bongosec-agent_${VERSION}-0.commit${SHA}_amd64.deb > /dev/null 2>&1"

}

function remove_bongosec () {

  apt purge -y bongosec-agent > /dev/null 2>&1
...
echo "Download package: https://s3.us-west-1.amazonaws.com/packages-dev.bongosec.github.io/warehouse/pullrequests/${MAJOR}.${MINOR}/deb/var/bongosec-agent_${VERSION}-0.commit${SHA}_amd64.deb"
wget "https://s3.us-west-1.amazonaws.com/packages-dev.bongosec.github.io/warehouse/pullrequests/${MAJOR}.${MINOR}/deb/var/bongosec-agent_${VERSION}-0.commit${SHA}_amd64.deb" > /dev/null 2>&1
