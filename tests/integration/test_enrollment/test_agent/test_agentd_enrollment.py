...
brief: This module verifies the correct behavior of Bongosec Agentd during the enrollment under different configurations.
components:
    - agentd
...
from bongosec_testing.utils.services import get_version
...
cases_path = Path(TEST_CASES_PATH, 'cases_bongosec_enrollment.yaml')
config_path = Path(CONFIGS_PATH, 'config_bongosec_enrollment.yaml')
...
def test_agentd_enrollment(test_configuration, test_metadata, set_bongosec_configuration, daemons_handler_module, shutdown_agentd,
    ...
    input_description:
        Different test cases are contained in an external YAML file (bongosec_enrollment_tests.yaml) which includes the
