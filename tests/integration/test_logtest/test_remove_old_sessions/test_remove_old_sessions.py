...
from bongosec_testing.constants.paths.sockets import LOGTEST_SOCKET_PATH
...
def test_remove_old_session(configure_local_internal_options, test_configuration,
                            test_metadata, set_bongosec_configuration, daemons_handler,
                            wait_for_logtest_startup):
    '''
    description: Check if 'bongosec-logtest' correctly detects and handles the situation where trying to use more
                 sessions than allowed. To do this, it creates more sessions than allowed and wait for the message which
                 informs that 'bongosec-logtest' has removed the oldest session.
    ...
    input_description: Some test cases are defined in the module. These include some input configurations stored in
                       the 'bongosec_conf.yaml' and the session creation data from the module.
    ...
    for i in range(0, max_sessions):
        ...
        if i == 0:
            first_session_token = bongosec_log_monitor.start(timeout=session_parameters.default_timeout,
                                                    callback=generate_callback(patterns.LOGTEST_SESSION_INIT))
            assert bongosec_log_monitor.callback_result, 'Session initialization event not found'
        else:
            bongosec_log_monitor.start(timeout=session_parameters.default_timeout,
                              callback=generate_callback(patterns.LOGTEST_SESSION_INIT))
            assert bongosec_log_monitor.callback_result,'Session initialization event not found'

    # This session should do Bongosec-logtest to remove the oldest session
    receiver_socket = SocketController(address=LOGTEST_SOCKET_PATH, family='AF_UNIX', connection_protocol='TCP')
    receiver_socket.send(msg_create_session, True)
    msg_recived = receiver_socket.receive()[4:]
    msg_recived = msg_recived.decode()
    ...
    remove_session_token = bongosec_log_monitor.start(timeout=session_parameters.default_timeout,
                                             callback=generate_callback(patterns.LOGTEST_REMOVE_SESSION))
    assert bongosec_log_monitor.callback_result, 'Session removal event not found'
