...
from bongosec_testing.tools.simulators.agent_simulator import connect
from bongosec_testing.utils.configuration import get_test_cases_data, load_configuration_template
from bongosec_testing.modules.remoted.configuration import REMOTED_DEBUG
...
from bongosec_testing.modules.remoted import patterns
from bongosec_testing.tools.monitors import queue_monitor
from bongosec_testing.utils.agent_groups import create_group, delete_group, add_agent_to_group
...
def test_shared_configuration(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                            set_bongosec_configuration, daemons_handler, simulate_agents):
    ...
    description: Check if the manager pushes shared configuration to agents as expected.
                 ...
                 completely pushed.
                 For example, if Bongosec Manager sends new shared files from group shared folder when the merged.mg
    ...
    parameters:
        ...
        - configure_local_internal_options:
            type: fixture
            brief: Configure the Bongosec local internal options using the values from `local_internal_options`.
        - daemons_handler:
            type: fixture
    ...
    bongosec_log_monitor.start(callback=generate_callback(regex=patterns.START_UP, replacement={"agent_name": agent.name, "agent_ip": '127.0.0.1'}))
    ...
    bongosec_log_monitor.start(callback=generate_callback(regex=patterns.MERGED_NEW_SHARED_END_SEND))
    assert bongosec_log_monitor.callback_result
