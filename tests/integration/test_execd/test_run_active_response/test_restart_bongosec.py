...
brief: Active responses execute a script in response to the triggering of specific alerts based
       ...
       which are executed by the 'bongosec-execd' daemon via scripts, run correctly.
...
references:
    - https://documentation.bongosec.github.io/current/user-manual/capabilities/active-response/#active-response
'''
import sys
import pytest
...
from bongosec_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, BONGOSEC_LOG_PATH
from bongosec_testing.constants.platforms import WINDOWS
from bongosec_testing.modules.execd.active_response.patterns import ACTIVE_RESPONSE_RESTART_BONGOSEC
from bongosec_testing.modules.agentd.configuration import AGENTD_WINDOWS_DEBUG
from bongosec_testing.modules.execd.configuration import EXECD_DEBUG
from bongosec_testing.modules.execd.patterns import EXECD_SHUTDOWN_RECEIVED
...
ar_conf = 'restart-bongosec0 - restart-bongosec - 0\nrestart-bongosec0 - restart-bongosec.exe - 0'
...
def test_execd_restart(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                       set_bongosec_configuration, configure_ar_conf, remoted_simulator, authd_simulator,
                       daemons_handler, send_execd_message):
    '''
    description: Check if 'restart-bongosec' command of 'active response' is executed correctly.
                 For this purpose, a simulated server is used, from which the active response is sent.
                 This response includes the order to restart the Bongosec agent, which must restart after
    ...
    assertions:
        ...
        - Check the restart-bongosec program is used.
    input_description:
    ...
    if error_message := test_metadata.get('expected_error'):
        ...
        assert ar_monitor.callback_result, 'AR `bongosec-restart` did not fail.'
        return

    bongosec_log_monitor.start(callback=generate_callback(EXECD_SHUTDOWN_RECEIVED))
    assert bongosec_log_monitor.callback_result, 'Execd `shutdown` log not raised.'

    ar_monitor.start(callback=generate_callback(ACTIVE_RESPONSE_RESTART_BONGOSEC))
    assert ar_monitor.callback_result, 'AR `restart-bongosec` program not used.'
