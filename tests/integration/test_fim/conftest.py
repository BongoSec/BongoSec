...
from bongosec_testing.constants.platforms import WINDOWS, MACOS, CENTOS, UBUNTU, DEBIAN
from bongosec_testing.modules.fim.patterns import MONITORING_PATH, EVENT_TYPE_SCAN_END
from bongosec_testing.modules.fim.utils import create_registry, delete_registry
...
from bongosec_testing.utils.services import get_service
...
def start_monitoring() -> None:
    FileMonitor(BONGOSEC_LOG_PATH).start(generate_callback(MONITORING_PATH))
...
def detect_end_scan(test_metadata: dict) -> None:
    ...
    bongosec_log_monitor.start(timeout=60, callback=generate_callback(EVENT_TYPE_SCAN_END))
