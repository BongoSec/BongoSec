...
from bongosec_testing.modules.fim.patterns import SENDING_FIM_EVENT, PATH_MONITORED_REALTIME, PATH_MONITORED_WHODATA, PATH_MONITORED_WHODATA_WINDOWS
...
def test_max_eps(test_configuration, test_metadata, configure_local_internal_options,
    ...
    '''
    description: Check if the 'bongosec-syscheckd' daemon applies the limit set in the 'max_eps' tag when
    ...
    input_description: A test case (max_eps) is contained in external YAML file (cases_max_eps.yaml) which
                       includes configuration settings for the 'bongosec-syscheckd' daemon and, these are
    ...
    bongosec_log_monitor.start(callback=generate_callback(regex))
    ...
    bongosec_log_monitor.start(accumulations=n_results, callback=generate_callback(SENDING_FIM_EVENT))
    assert bongosec_log_monitor.callback_result

    result = parse_integrity_message(bongosec_log_monitor.callback_result)
    date_time_count = {}
