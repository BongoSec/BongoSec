...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/syscheck.html#synchronization
...
from bongosec_testing.modules.fim.patterns import EVENT_TYPE_DELETED, EVENT_TYPE_ADDED
...
def test_moving_file_to_whodata(test_configuration, test_metadata, set_bongosec_configuration, truncate_monitored_files,
                                configure_local_internal_options, create_paths_files, daemons_handler, start_monitoring):
    '''
    description: Check if the 'bongosec-syscheckd' daemon detects events when moving files from a directory
    ...
    # Check event 'delete'
    bongosec_log_monitor.start(generate_callback(EVENT_TYPE_DELETED), timeout=60)
    ...
    # Check event 'add'
    bongosec_log_monitor.start(generate_callback(EVENT_TYPE_ADDED), timeout=60)
    callback_result = bongosec_log_monitor.callback_result
    assert callback_result
