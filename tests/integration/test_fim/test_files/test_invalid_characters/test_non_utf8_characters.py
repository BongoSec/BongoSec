...
from bongosec_testing.modules.fim import configuration
from bongosec_testing.modules.fim.patterns import (IGNORING_DUE_TO_INVALID_NAME,
...
def test_valid_utf8_filenames_do_not_trigger_warning(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options,
                                                     truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring) -> None:
    '''
    description: Check if the 'bongosec-syscheckd' correctly processes valid UTF-8 file names without triggering warnings.
    '''
...
@pytest.mark.parametrize('test_configuration, test_metadata', zip(test_configuration, test_metadata), ids=cases_ids)
def test_surrogate_range_should_trigger_warning_1(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options,
                                                   truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring) -> None:
    '''
    description: Check if the 'bongosec-syscheckd' logs a warning for non-UTF-8 sequences in file names.
    '''
    monitor = FileMonitor(BONGOSEC_LOG_PATH)
...
@pytest.mark.skipif(sys.platform == "win32",
                    reason="The test fails not to due to UTF-8 validation, but to an unknown reason. The files with this invalid sequence are edge cases that is hard to reproduce in real world scenarios.")
@pytest.mark.parametrize('test_configuration, test_metadata', zip(test_configuration, test_metadata), ids=cases_ids)
def test_surrogate_range_should_trigger_warning_2(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options,
                                                      truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring) -> None:
    '''
    description: Check if the 'bongosec-syscheckd' logs a warning for non-UTF-8 sequences in file names.
    '''
...
@pytest.mark.skipif(sys.platform == "win32",
                    reason="The test fails not to due to UTF-8 validation, but to an unknown reason. The files with this invalid sequence are edge cases that is hard to reproduce in real world scenarios.")
@pytest.mark.parametrize('test_configuration, test_metadata', zip(test_configuration, test_metadata), ids=cases_ids)
def test_surrogate_range_should_trigger_warning_3(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options,
                                                        truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring) -> None:
    '''
    description: Check if the 'bongosec-syscheckd' logs a warning for non-UTF-8 sequences in file names.
    '''
...
@pytest.mark.parametrize('test_configuration, test_metadata', zip(test_configuration, test_metadata), ids=cases_ids)
def test_invalid_lead_byte_should_trigger_warning_posix(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options,
                                                            truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring) -> None:
    '''
    description: Check if the 'bongosec-syscheckd' logs a warning for non-UTF-8 sequences in file names.
    '''
...
@pytest.mark.skipif(sys.platform == "win32", reason="POSIX-specific test")
@pytest.mark.parametrize('test_configuration, test_metadata', zip(test_configuration, test_metadata), ids=cases_ids)
def test_out_of_range_sequence_should_trigger_warning_posix(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options,
                                                            truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring) -> None:
    '''
    description: Check if the 'bongosec-syscheckd' logs a warning for non-UTF-8 sequences in file names.
    '''
...
@pytest.mark.parametrize('test_configuration, test_metadata', zip(test_configuration, test_metadata), ids=cases_ids)
def test_5_byte_sequence_should_trigger_warning_posix(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options,
                                                                truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring) -> None:
    '''
    description: Check if the 'bongosec-syscheckd' logs a warning for non-UTF-8 sequences in file names.
    '''
...
@pytest.mark.skipif(sys.platform == "win32", reason="POSIX-specific test")
@pytest.mark.parametrize('test_configuration, test_metadata', zip(test_configuration, test_metadata), ids=cases_ids)
def test_6_byte_sequence_should_trigger_warning_posix(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options,
                                                          truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring) -> None:
    '''
    description: Check if the 'bongosec-syscheckd' logs a warning for non-UTF-8 sequences in file names.
    '''
