...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       ...
       The FIM capability is managed by the 'bongosec-syscheckd' daemon, which checks
...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/syscheck.html#ignore
...
import pytest
from bongosec_testing import global_parameters
...
from bongosec_testing.modules.fim.patterns import EVENT_TYPE_ADDED, IGNORING_DUE_TO_SREGEX, IGNORING_DUE_TO_PATTERN
...
# Skipped on Windows due Issue https://github.com/bongosec/bongosec/issues/9298"
...
def test_ignore_subdirectory(test_configuration, test_metadata, configure_local_internal_options,
                             truncate_monitored_files, set_bongosec_configuration, folder_to_monitor, daemons_handler,
                             file_to_monitor):
    '''
    description: Check if the 'bongosec-syscheckd' daemon ignores the files that are in a monitored subdirectory
    ...
    input_description: Different test cases are contained in external YAML files
                       ...
                       for the 'bongosec-syscheckd' daemon and testing directories to monitor.
    ...
    if test_metadata.setdefault('xfail', False):
        pytest.xfail(reason="Expected fail - Issue https://github.com/bongosec/bongosec/issues/22186.")

    bongosec_log_monitor = FileMonitor(BONGOSEC_LOG_PATH)

    if test_metadata.setdefault('triggers_event', False) == True:
        bongosec_log_monitor.start(timeout=60, callback=generate_callback(EVENT_TYPE_ADDED))
        callback_result = bongosec_log_monitor.callback_result
        ...
        if test_metadata['is_pattern'] == False:
            bongosec_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_SREGEX))
            assert bongosec_log_monitor.callback_result
        else:
            bongosec_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_PATTERN))
            assert bongosec_log_monitor.callback_result
