...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       ...
       the 'queue/diff/local' folder, where Bongosec stores the compressed files used to perform
...
from bongosec_testing.modules.fim.patterns import DIFF_DISK_QUOTA_LIMIT, ERROR_MSG_DISK_QUOTA_LIMIT
...
def test_disk_quota_default(test_configuration, test_metadata, configure_local_internal_options,
                                    truncate_monitored_files, set_bongosec_configuration, daemons_handler):
    '''
    description: Check if the 'bongosec-syscheckd' daemon limits the size of the folder where the data used to perform
    ...
    bongosec_log_monitor.start(generate_callback(DIFF_DISK_QUOTA_LIMIT), timeout=30)
    callback_result = bongosec_log_monitor.callback_result
    assert callback_result, ERROR_MSG_DISK_QUOTA_LIMIT
    assert str(bongosec_log_monitor.callback_result[0]) == str(DISK_QUOTA_DEFAULT_VALUE), 'Wrong value for disk_quota'
