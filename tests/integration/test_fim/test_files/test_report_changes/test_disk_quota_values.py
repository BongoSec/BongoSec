...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       ...
       the size of the 'queue/diff/local' folder where Bongosec stores the compressed files used
...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/syscheck.html#disk-quota
...
from bongosec_testing.modules.fim.patterns import DISK_QUOTA_LIMIT_REACHED, EVENT_TYPE_REPORT_CHANGES, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED, \
...
from bongosec_testing.utils.random import get_random_string
...
def test_disk_quota_values(test_configuration, test_metadata, configure_local_internal_options,
    ...
    '''
    description: Check if the 'bongosec-syscheckd' daemon limits the size of the diff folder to generate
    ...
    bongosec_log_monitor.start(callback=generate_callback(DISK_QUOTA_LIMIT_REACHED), timeout=30)
    ...
    assert bongosec_log_monitor.callback_result, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
    assert 'Unable to calculate diff due to \'disk_quota\' limit has been reached.' in bongosec_log_monitor.callback_result, 'Wrong content_changes field'
