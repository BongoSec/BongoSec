...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/syscheck.html#diff
...
from bongosec_testing.modules.fim.patterns import EVENT_TYPE_MODIFIED, EVENT_TYPE_ADDED, ERROR_MSG_FIM_EVENT_NOT_DETECTED, \
                                               EVENT_TYPE_DELETED, EVENT_TYPE_REPORT_CHANGES, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
from bongosec_testing.modules.fim.utils import make_diff_file_path, get_fim_event_data
from bongosec_testing.tools.monitors.file_monitor import FileMonitor
from bongosec_testing.utils.file import write_file_write, delete_files_in_folder, truncate_file
...
def test_reports_file_and_nodiff(test_configuration, test_metadata, configure_local_internal_options,
                        truncate_monitored_files, set_bongosec_configuration, create_paths_files, daemons_handler, detect_end_scan):
    '''
    description: Check if the 'bongosec-syscheckd' daemon reports the file changes (or truncates if required)
    ...
    input_description: A test case is contained in external YAML files (configuration_report_changes_and_diff.yaml, cases_report_changes_and_diff.yaml)
                       which includes configuration settings for the 'bongosec-syscheckd' daemon and, these are
    ...
    bongosec_log_monitor.start(generate_callback(EVENT_TYPE_ADDED), timeout=30)
    assert bongosec_log_monitor.callback_result, ERROR_MSG_FIM_EVENT_NOT_DETECTED
    ...
    bongosec_log_monitor = FileMonitor(BONGOSEC_LOG_PATH)
    bongosec_log_monitor.start(generate_callback(EVENT_TYPE_REPORT_CHANGES), timeout=30)
    assert bongosec_log_monitor.callback_result, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
    assert 'No content changes were found for this file.' in str(bongosec_log_monitor.callback_result[0]), 'Wrong content_changes field'
    ...
    bongosec_log_monitor.start(generate_callback(EVENT_TYPE_MODIFIED), timeout=20)
    assert bongosec_log_monitor.callback_result
    event = get_fim_event_data(bongosec_log_monitor.callback_result)
    ...
    assert get_fim_event_data(bongosec_log_monitor.callback_result)['mode'] == test_metadata.get('fim_mode')
