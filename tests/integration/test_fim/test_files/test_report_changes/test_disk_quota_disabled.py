...
from bongosec_testing.modules.fim.configuration import SYSCHECK_DEBUG
...
from bongosec_testing.utils.file import write_file
...
def test_disk_quota_disabled(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                             set_bongosec_configuration, folder_to_monitor, file_to_monitor, daemons_handler, detect_end_scan):
    '''
    description: Check if the 'bongosec-syscheckd' daemon limits the size of the folder where the data used
    ...
    bongosec_log_monitor.start(generate_callback(DISK_QUOTA_LIMIT_REACHED), timeout=30)
    assert (bongosec_log_monitor.callback_result == None), f'Error, unexpected disk_quota limit event.'
    ...
    assert 'More changes...' in str(bongosec_log_monitor.callback_result[0]), 'Wrong content_changes field'
