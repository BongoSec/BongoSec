...
from bongosec_testing.modules.fim.patterns import SENDING_FIM_EVENT
...
def test_fim_disabled(test_configuration, test_metadata, set_bongosec_configuration, truncate_monitored_files,
                      configure_local_internal_options, folder_to_monitor, daemons_handler):

    '''
    description: Check if the 'bongosec-syscheckd' daemon generates FIM events when it is disabled
    ...
    assertions:
        - Verify that when the 'bongosec-syscheckd' daemon is disabled, no FIM events are generated.
    ...
    bongosec_log_monitor.start(generate_callback(SENDING_FIM_EVENT))
