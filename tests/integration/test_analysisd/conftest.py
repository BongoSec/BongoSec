...
from bongosec_testing.constants.paths.configurations import CUSTOM_RULES_PATH, CUSTOM_RULES_FILE, BONGOSEC_CONF_PATH
from bongosec_testing.constants.paths.logs import ALERTS_JSON_PATH, BONGOSEC_LOG_PATH
from bongosec_testing.constants.users import BONGOSEC_UNIX_GROUP, BONGOSEC_UNIX_USER
from bongosec_testing.modules.analysisd import patterns
from bongosec_testing.tools.monitors import file_monitor
from bongosec_testing.utils import callbacks, file


@pytest.fixture()
def prepare_custom_rules_file(request, test_metadata):
    """Configure a syscollector custom rules for testing.
    Restarting bongosec-analysisd is required to apply this changes.
    """
    ...
    shutil.chown(target_rule, BONGOSEC_UNIX_USER, BONGOSEC_UNIX_GROUP)
...
def configure_local_rules(request, test_configuration):
    """Configure a custom rule for testing. Restart Bongosec is needed for applying the configuration. """
...
def configure_remove_tags(request, test_metadata):
    ...
    file.replace_regex_in_file(test_metadata['remove_tags'], [''] * len(test_metadata['remove_tags']), BONGOSEC_CONF_PATH)
