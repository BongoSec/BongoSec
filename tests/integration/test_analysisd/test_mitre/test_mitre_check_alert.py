...
brief: The 'bongosec-analysisd' daemon receives the log messages and compares them to the rules.
       ...
       Specifically, these tests will check if the 'bongosec-analysisd' daemon generates alerts
...
from bongosec_testing.constants.paths.logs import ALERTS_JSON_PATH
from bongosec_testing.modules.analysisd import patterns, utils
...
def test_mitre_check_alert(test_configuration, truncate_monitored_files, configure_local_rules, daemons_handler):
    ...
    tags:
        ...
        - wdb_socket
    '''
    bongosec_alert_monitor = file_monitor.FileMonitor(ALERTS_JSON_PATH)

    # Wait until Mitre's event is detected
    if test_configuration not in invalid_configurations:
        bongosec_alert_monitor.start(timeout=30, callback=callbacks.generate_callback(patterns.ANALYSISD_ALERT_STARTED))
        utils.validate_mitre_event(json.loads(bongosec_alert_monitor.callback_result))
    else:
