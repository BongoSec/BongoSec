...
brief: The 'bongosec-analysisd' daemon uses a series of decoders and rules to analyze and interpret logs and events and
...
references:
    - https://documentation.bongosec.github.io/current/user-manual/ruleset/ruleset-xml-syntax/rules.html#if-sid
'''
import pytest
import time
import re
...
from bongosec_testing.constants.paths.logs import ARCHIVES_LOG_PATH
from bongosec_testing.modules.analysisd import utils, configuration as analysisd_config
from bongosec_testing.modules.remoted import configuration as remoted_config
from bongosec_testing.scripts.syslog_simulator import DEFAULT_MESSAGE_SIZE
from bongosec_testing.tools import thread_executor
from bongosec_testing.tools.simulators import run_syslog_simulator
from bongosec_testing.utils import configuration, file
...
def test_limitation(test_configuration, test_metadata, load_bongosec_basic_configuration, set_bongosec_configuration,
    ...
    test_phases:
        - setup:
            - Load Bongosec light configuration.
            - Apply ossec.conf configuration changes according to the configuration template and use case.
            - Apply custom settings in local_internal_options.conf.
            - Truncate bongosec logs.
            - Restart bongosec-manager service to apply configuration changes.
        - test:
    ...
    parameters:
        ...
        - load_bongosec_basic_configuration:
            type: fixture
            brief: Load basic bongosec configuration.
        - set_bongosec_configuration:
            type: fixture
            brief: Apply changes to the ossec.conf configuration.
        - configure_local_internal_options:
            type: fixture
            brief: Configure the Bongosec local internal options.
        - truncate_monitored_files:
            type: fixture
            brief: Truncate bongosec logs.
        - daemons_handler:
            type: fixture
    ...
    # Check that bongosec-manager is processing syslog events
    ...
    # Check that the bongosec-manager is receiving events but it is not processing them due to the limitation
...
def test_queueing_events_after_limitation(test_configuration, test_metadata, load_bongosec_basic_configuration, set_bongosec_configuration,
                                          configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
...
def test_dropping_events_when_queue_is_full(test_configuration, test_metadata, load_bongosec_basic_configuration, set_bongosec_configuration,
                                            configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
...
def test_event_processing_in_order_single_thread(test_configuration, test_metadata, load_bongosec_basic_configuration, set_bongosec_configuration,
                                                 configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
...
def test_event_processing_in_order_multi_thread(test_configuration, test_metadata, load_bongosec_basic_configuration, set_bongosec_configuration,
                                                configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
    description: Check that events are processed in order according to the position within the queue, and
        that events that are being received during the blocking phase are being added to the end of the queue when
