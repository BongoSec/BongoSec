...
from bongosec_testing.utils import configuration, callbacks
...
def test_accepted_values(test_configuration, test_metadata, load_bongosec_basic_configuration, set_bongosec_configuration,
    ...
    test_phases:
        ...
        - test:
            ...
            - Check that bongosec-analysisd is running (it has not been crashed).
        - teardown:
            - Truncate bongosec logs.
    ...
    """
    # Start monitor
    monitor_enabled = file_monitor.FileMonitor(BONGOSEC_LOG_PATH)
...
def test_invalid_values(test_configuration, test_metadata, load_bongosec_basic_configuration, set_bongosec_configuration,
                        truncate_monitored_files, daemons_handler):
    """
    description: Check for configuration error and bongosec-analysisd if the EPS limiting configuration has unaccepted
    ...
    test_phases:
        ...
        - test:
            - Restart bongosec-manager service to apply configuration changes.
            - Check that a configuration error is raised when trying to start bongosec-manager.
            - Check that bongosec-analysisd is not running (due to configuration error).
        - teardown:
            - Truncate bongosec logs.
            - Restore initial configuration.
    ...
    """
    # Start monitor
    monitor_error = file_monitor.FileMonitor(BONGOSEC_LOG_PATH)
...
def test_missing_configuration(test_configuration, test_metadata, load_bongosec_basic_configuration, set_bongosec_configuration,
    ...
    test_phases:
        ...
        - test:
            ...
            - Check whether the EPS limitation is activated, deactivated or generates a configuration error due to a
              missing label.
            - Check if bongosec-analysisd is running or not (according to the expected behavior).
        - teardown:
            - Truncate bongosec logs.
            - Restore initial configuration.
