...
brief: The 'bongosec-analysisd' daemon receives the log messages and compares them to the rules.
       ...
       Specifically, these tests will check if the 'bongosec-analysisd' daemon correctly handles
...
def test_scan_messages(test_metadata, configure_local_internal_options, configure_sockets_environment_module,
    ...
    '''
    description: Check if when the 'bongosec-analysisd' daemon socket receives a message with
                 a file scanning-related event, it generates the corresponding alert
                 that sends to the 'bongosec-db' daemon socket.
