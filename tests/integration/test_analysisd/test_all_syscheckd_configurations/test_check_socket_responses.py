...
brief: The 'bongosec-analysisd' daemon receives the log messages and compares them to the rules.
       It then creates an alert when a log message matches an applicable rule.
       Specifically, these tests will verify if the 'bongosec-analysisd' daemon correctly handles
...
from bongosec_testing.constants.daemons import BONGOSEC_DB_DAEMON, ANALYSISD_DAEMON
from bongosec_testing.constants.paths.sockets import BONGOSEC_DB_SOCKET_PATH, ANALYSISD_QUEUE_SOCKET_PATH
from bongosec_testing.modules.analysisd import patterns, configuration as analysisd_config
from bongosec_testing.modules.monitord import configuration as monitord_config
from bongosec_testing.tools import mitm
...
mitm_wdb = mitm.ManInTheMiddle(address=BONGOSEC_DB_SOCKET_PATH, family='AF_UNIX', connection_protocol='TCP')
mitm_analysisd = mitm.ManInTheMiddle(address=ANALYSISD_QUEUE_SOCKET_PATH, family='AF_UNIX', connection_protocol='UDP')
monitored_sockets_params = [(BONGOSEC_DB_DAEMON, mitm_wdb, True), (ANALYSISD_DAEMON, mitm_analysisd, True)]
...
def test_validate_socket_responses(test_metadata, configure_local_internal_options, configure_sockets_environment_module,
    ...
    '''
    description: Validate every response from the 'bongosec-analysisd' daemon socket
                 to the 'bongosec-db' daemon socket using 'syscheck' common events.
    ...
    parameters:
        ...
        - wait_for_analysisd_startup:
            type: fixture
            brief: Wait until the 'bongosec-analysisd' has begun and the 'alerts.json' file is created.
        - test_case:
            type: list
