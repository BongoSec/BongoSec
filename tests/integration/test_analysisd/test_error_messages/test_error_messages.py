...
brief: The 'bongosec-analysisd' daemon receives the log messages and compares them to the rules.
       ...
       Specifically, these tests will check if the 'bongosec-analysisd' daemon handles correctly
...
from bongosec_testing.constants.daemons import ANALYSISD_DAEMON
from bongosec_testing.constants.paths.logs import BONGOSEC_LOG_PATH
from bongosec_testing.constants.paths.sockets import ANALYSISD_QUEUE_SOCKET_PATH
...
def test_error_messages(test_metadata, configure_local_internal_options, configure_sockets_environment_module,
    ...
    '''
    description: Check if when the 'bongosec-analysisd' daemon socket receives a message with an invalid event,
                 it generates the corresponding error that sends to the 'bongosec-db' daemon socket.
    ...
    # Start monitor
    monitor_log = file_monitor.FileMonitor(BONGOSEC_LOG_PATH)
