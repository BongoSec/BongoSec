"""
copyright: Copyright (C) 2025, BongoSec
...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/api/configuration.html
...
from bongosec_testing.constants.api import CONFIGURATION_TYPES, MANAGER_CONFIGURATION_ROUTE
from bongosec_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from bongosec_testing.modules.api.utils import login, get_base_url
from bongosec_testing.utils.configuration import get_test_cases_data, load_configuration_template, get_bongosec_conf
...
def test_limit_eps(test_configuration, test_metadata, set_bongosec_configuration, add_configuration,
    ...
    bongosec_config = " ".join(get_bongosec_conf())
    assert f"<maximum>{test_metadata['expected_max_value']}</maximum>" in bongosec_config
    assert f"<timeframe>{test_metadata['expected_timeframe_value']}</timeframe>" in bongosec_config
