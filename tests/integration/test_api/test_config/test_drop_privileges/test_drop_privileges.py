...
brief: These tests will check if the 'drop_privileges' setting of the API is working properly.
       This setting allows the user who starts the 'bongosec-apid' daemon to be different from
       the 'root' user. The Bongosec API is an open source 'RESTful' API that allows for interaction
       with the Bongosec manager from a web browser, command line tool like 'cURL' or any script
...
references:
    - https://documentation.bongosec.github.io/current/user-manual/api/getting-started.html
    - https://documentation.bongosec.github.io/current/user-manual/api/configuration.html#drop-privileges
...
from bongosec_testing.constants.api import CONFIGURATION_TYPES
from bongosec_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from bongosec_testing.constants.paths.api import BONGOSEC_API_SCRIPT
from bongosec_testing.utils.configuration import get_test_cases_data, load_configuration_template
from bongosec_testing.utils.services import search_process_by_command
...
def test_drop_privileges(test_configuration, test_metadata, add_configuration, truncate_monitored_files,
    ...
    description: Check if 'drop_privileges' affects the user of the API process. In this test, the 'PID' of the API
                 process is obtained. After that, it gets the user ('root' or 'bongosec') and checks if it matches the
    ...
    test_phases:
        ...
        - test:
            - Search bongosec-apid process and verify that it is present
    ...
    parameters:
        ...
        - add_configuration:
            type: fixture
            brief: Add configuration to the Bongosec API configuration files.
        - truncate_monitored_files:
            type: fixture
            brief: Truncate all the log files and json alerts files before and after the test execution.
        - daemons_handler:
            type: fixture
            brief: Wrapper of a helper function to handle Bongosec daemons.
        - wait_for_api_start:
            type: fixture
            brief: Monitor the API log file to detect whether it has been started or not.

    assertions:
        - Verify that when 'drop_privileges' is enabled the user who has started the 'bongosec-apid' daemon is 'bongosec'.
        - Verify that when 'drop_privileges' is disabled the user who has started the 'bongosec-apid' daemon is 'root'.
    ...
    expected_output:
        - PID of the 'bongosec-apid' process.
        - bongosec (if drop_privileges is enabled)
        - root (if drop_privileges is disabled)
    """
    expected_user = test_metadata['expected_user']

    # Get bongosec-apid process info
    api_process = search_process_by_command(BONGOSEC_API_SCRIPT)
    assert api_process is not None, f"The process {BONGOSEC_API_SCRIPT} could not be found"
