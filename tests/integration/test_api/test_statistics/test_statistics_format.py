...
brief: These tests will check if the stats can be obtained from the API and if they follow the expected schema.
       The Bongosec API is an open source 'RESTful' API that allows for interaction with the Bongosec manager from
...
references:
    - https://documentation.bongosec.github.io/current/user-manual/api/reference.html (Get Bongosec daemon stats)
    - https://documentation.bongosec.github.io/current/user-manual/api/reference.html (Get Bongosec daemon stats from an agent)
...
from bongosec_testing.constants.api import DAEMONS_STATS_ROUTE
from bongosec_testing.modules.api.utils import get_base_url, login, validate_statistics
...
@pytest.mark.parametrize('test_configuration,test_metadata', zip(test1_configuration, test1_metadata),
                         ids=test1_cases_ids)
def test_manager_statistics_format(test_configuration, test_metadata, load_bongosec_basic_configuration,
                                   set_bongosec_configuration, daemons_handler):
    """
