...
from bongosec_testing.constants.api import BONGOSEC_API_PORT, CONFIGURATION_TYPES
from bongosec_testing.modules.api.configuration import get_configuration, append_configuration, delete_configuration_file
...
def add_configuration(test_configuration: list[dict], request: pytest.FixtureRequest) -> None:
    """Add configuration to the Bongosec API configuration files.
...
def wait_for_api_start(test_configuration: dict) -> None:
    ...
    if test_configuration is not None:
        if test_configuration.get('blocks') is not None:
            ...
            port = test_configuration['blocks'].get('port', BONGOSEC_API_PORT)
