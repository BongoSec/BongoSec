...
brief: These tests will check if the response_postprocessing middleware of the API handled by the 'bongosec-apid' daemon is
       working properly. The Bongosec API is an open source 'RESTful' API that allows the interaction with the Bongosec
...
from bongosec_testing.constants.api import BONGOSEC_API_PROTOCOL
from bongosec_testing.modules.api.utils import login, get_base_url, set_authorization_header
...
def test_response_postprocessing(test_configuration, test_metadata, truncate_monitored_files, daemons_handler,
    ...
    if use_login_token:
        ...
        session.mount(f"{BONGOSEC_API_PROTOCOL}://", adapter)
