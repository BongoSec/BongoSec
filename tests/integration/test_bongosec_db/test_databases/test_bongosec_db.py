...
from bongosec_testing.utils.callbacks import make_callback
from bongosec_testing.modules.bongosec_db import BONGOSEC_DB_PREFIX
from bongosec_testing.utils import configuration
from bongosec_testing.utils.database import query_wdb
from bongosec_testing.utils.db_queries.agent_db import agent_integrity_check
...
# List items -> (bongosec_daemon: str,(
#                mitm: ManInTheMiddle
#                daemon_first: bool))
# Example1 -> ('bongosec-clusterd', None)              Only start bongosec-clusterd with no MITM
# Example2 -> ('bongosec-clusterd', (my_mitm, True))   Start MITM and then bongosec-clusterd
...
def validate_bongosec_db_response(expected_output, response):
    """ Method to validate the Bongosec-DB response.
...
def test_bongosec_db_messages_agent(test_metadata, configure_sockets_environment_module, connect_to_sockets_module,
                                 clean_databases, clean_registered_agents, insert_agents_test):
    '''
    description: Check that every input agent message in bongosec-db socket generates the proper output to bongosec-db
    ...
    parameters:
        ...
        - clean_registered_agents:
            type: fixture
            brief: Remove all agents of bongosecdb.
        - insert_agents_test:
            type: fixture
    ...
    for index, stage in enumerate(test_metadata['test_case']):
        ...
        if 'use_regex' in stage and stage['use_regex'] == 'yes':
            ...
            match = validate_bongosec_db_response(expected_output, response)
...
def test_bongosec_db_messages_global(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in bongosec-db socket generates the proper output to bongosec-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
                 stages (test_case, input field) and compare the result with the input list of stages (test_case,
                 output field).
...
def test_bongosec_db_range_checksum(configure_sockets_environment_module, connect_to_sockets_module,
    ...
    log_monitor.start(callback=make_callback('range checksum: Time: ', prefix=BONGOSEC_DB_PREFIX,
                                             escape=True), timeout=BONGOSEC_DB_CHECKSUM_CALCULUS_TIMEOUT)
    ...
    agent_integrity_check()
    log_monitor.start(callback=make_callback('range checksum avoided', prefix=BONGOSEC_DB_PREFIX,
                                             escape=True), timeout=BONGOSEC_DB_CHECKSUM_CALCULUS_TIMEOUT)
...
def test_bongosec_db_messages_tasks(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in bongosec-db socket generates the proper output to bongosec-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
