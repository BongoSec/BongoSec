...
brief: The 'bongosec-agentd' program is the client-side daemon that communicates with the server.
       ...
       the agent successfully enrolls after losing connection with the 'bongosec-remoted' daemon.
...
from bongosec_testing.modules.agentd.patterns import AGENTD_REQUESTING_KEY
...
def test_agentd_initial_enrollment_retries(test_metadata, set_bongosec_configuration, configure_local_internal_options, truncate_monitored_files, remove_keys_file, daemons_handler):
    '''
    description: Check how the agent behaves when it makes multiple enrollment attempts
                 ...
                 performs multiple enrollment requests to the 'bongosec-authd' daemon before
                 getting the new key to communicate with the 'bongosec-remoted' daemon.
    ...
    bongosec_log_monitor.start(callback=callbacks.generate_callback(AGENTD_REQUESTING_KEY,{'IP':''}), timeout = 300, accumulations = 4)
    assert (bongosec_log_monitor.callback_result != None), f'Enrollment retries was not sent'
    ...
    # Check if no Bongosec module stopped due to Agentd Initialization
    check_module_stop()
