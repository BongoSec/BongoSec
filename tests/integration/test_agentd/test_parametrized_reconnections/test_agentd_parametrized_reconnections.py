...
brief: The 'bongosec-agentd' program is the client-side daemon that communicates with the server.
       The objective is to check how the 'bongosec-agentd' daemon behaves when there are delays
       between connection attempts to the 'bongosec-remoted' daemon using TCP and UDP protocols.
       The 'bongosec-remoted' program is the server side daemon that communicates with the agents.
...
references:
    - https://documentation.bongosec.github.io/current/user-manual/registering/index.html
...
from bongosec_testing.modules.agentd.configuration import AGENTD_DEBUG, AGENTD_WINDOWS_DEBUG, AGENTD_TIMEOUT
from bongosec_testing.modules.agentd.patterns import AGENTD_TRYING_CONNECT, AGENTD_CONNECTED_TO_SERVER
...
configs_path = Path(CONFIGS_PATH, 'bongosec_conf.yaml')
...
def test_agentd_parametrized_reconnections(test_metadata, set_bongosec_configuration, configure_local_internal_options,
    ...
    assertions:
        - Verify that when the 'bongosec-agentd' daemon initializes, it connects to
          the 'bongosec-remoted' daemon of the manager before reaching the maximum number of attempts.
    ...
    input_description: An external YAML file (bongosec_conf.yaml) includes configuration settings for the agent.
    ...
    bongosec_log_monitor.start(accumulations = test_metadata['MAX_RETRIES'], callback=callbacks.generate_callback(AGENTD_TRYING_CONNECT,{'IP':'','PORT':''}))
    assert (bongosec_log_monitor.callback_result != None), f'Trying to connect to server message not found expected times'

    #Check number of connected message is the expected
    if test_metadata['ENROLL'] == 'yes':
        bongosec_log_monitor.start(callback=callbacks.generate_callback(AGENTD_CONNECTED_TO_SERVER))
