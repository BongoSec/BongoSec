...
brief: The Bongosec 'github' module allows you to collect all the 'audit logs' from GitHub using its API.
...
references:
    - https://github.com/bongosec/bongosec-documentation/blob/develop/source/github/monitoring-github-activity.rst
...
def test_invalid(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options,
    ...
    input_description: A configuration template (github_integration) is contained in an external YAML file
                       (bongosec_conf.yaml). That template is combined with different test cases defined in
    ...
    bongosec_log_monitor.start(callback=callbacks.generate_callback(patterns.MODULESD_CONFIGURATION_ERROR, {
