...
brief: Integratord manages Bongosec integrations with other applications such as Yara or Slack, by feeding
...
references:
    - https://documentation.bongosec.github.io/current/user-manual/manager/manual-integration.html#slack
    - https://documentation.bongosec.github.io/current/user-manual/reference/daemons/bongosec-integratord.html
...
from bongosec_testing.modules.integratord.patterns import INTEGRATORD_THIRD_PARTY_RESPONSE, INTEGRATORD_INODE_CHANGED, \
...
from bongosec_testing.utils.file import copy, remove_file
...
def test_integratord_change_json_inode(test_configuration, test_metadata, set_bongosec_configuration, truncate_monitored_files,
                                       configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    description: Check that bongosec-integratord detects a change in the inode of the alerts.json and continues reading
                 alerts.

    test_phases:
        - setup:
            - Apply ossec.conf configuration changes according to the configuration template and use case.
            - Truncate Bongosec's logs.
        ...
        - test:
            ...
            - Replace the `alerts.json` file while bongosec-integratord is reading it.
            - Wait for the inode change to be detected by bongosec-integratord.
            - Check if bongosec-integratord detects that the file's inode has changed.
            - Insert an alert in the `alerts.json` file.
    ...
    expected_output:
        - r'.+bongosec-integratord.*DEBUG: jqueue_next.*Alert file inode changed.*'
        - r'.+bongosec-integratord.*Processing alert.*'
        - r'.+bongosec-integratord.*<Response [200]>'
    '''
    ...
    # Start monitor
    bongosec_monitor.start(callback=generate_callback(INTEGRATORD_INODE_CHANGED), timeout=session_parameters.default_timeout)
...
def test_integratord_read_valid_alerts(test_configuration, test_metadata, set_bongosec_configuration, truncate_monitored_files,
                                       configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    ...
    expected_output:
        - r'.+bongosec-integratord.*alert_id.*\"integration\": \"slack\".*'
    '''
...
def test_integratord_read_invalid_alerts(test_configuration, test_metadata, set_bongosec_configuration, truncate_monitored_files,
                                         configure_local_internal_options, daemons_handler, wait_for_integratord_start):
    '''
    ...
    test_phases:
        ...
        - test:
            ...
            - Check if bongosec-integratord process the alert and report an error.
        - teardown:
            - Truncate Bongosec's logs.
            - Restore initial configuration, both `ossec.conf` and `local_internal_options.conf`.
    ...
    expected_output:
        - r'.+bongosec-integratord.*WARNING: Invalid JSON alert read.*'
        - r'.+bongosec-integratord.*WARNING: Overlong JSON alert read.*'
    ...
    # Start monitor
    bongosec_monitor.start(callback=generate_callback(callback), timeout=session_parameters.default_timeout)
