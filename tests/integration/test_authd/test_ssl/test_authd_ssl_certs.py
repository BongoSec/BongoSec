...
brief: These tests will check if the 'bongosec-authd' daemon is able to handle secure connections using
       the 'SSL' (Secure Socket Layer) protocol. The 'bongosec-authd' daemon can automatically add
...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/registering/host-verification-registration.html
...
def test_authd_ssl_certs(test_configuration, test_metadata, set_bongosec_configuration,
                         generate_ca_certificate, truncate_monitored_files, daemons_handler,
                         wait_for_authd_startup):
    '''
    description:
        Checks if the 'bongosec-authd' daemon can manage 'SSL' connections with agents
    ...
    assertions:
        - Verify that the agent can only connect to the 'bongosec-authd' daemon socket using a valid certificate.
