...
brief: These tests will check if the 'SSL' (Secure Socket Layer) protocol-related settings of
       the 'bongosec-authd' daemon are working correctly. The 'bongosec-authd' daemon can
       automatically add a Bongosec agent to a Bongosec manager and provide the key
...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/auth.html
...
def test_ossec_auth_configurations(test_configuration, test_metadata, set_bongosec_configuration,
                                   truncate_monitored_files, daemons_handler,
                                   configure_sockets_environment, wait_for_authd_startup):
    '''
    description:
        Checks if the 'SSL' settings of the 'bongosec-authd' daemon work correctly by enrolling agents
