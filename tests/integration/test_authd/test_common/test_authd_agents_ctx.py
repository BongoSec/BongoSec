...
brief: These tests will check if the 'bongosec-authd' daemon correctly handles the enrollment requests
       from agents with pre-existing IP addresses or names. The 'bongosec-authd' daemon can automatically
       add a Bongosec agent to a Bongosec manager and provide the key to the agent. It is used along with
...
references:
    - https://documentation.bongosec.github.io/current/user-manual/reference/daemons/bongosec-authd.html
    - https://documentation.bongosec.github.io/current/user-manual/reference/tools/agent_groups.html
...
import pytest
import time
from bongosec_testing.constants.paths.sockets import AUTHD_SOCKET_PATH
from bongosec_testing.constants.ports import DEFAULT_SSL_REMOTE_ENROLLMENT_PORT
from bongosec_testing.utils.agent_groups import check_agent_groups
from bongosec_testing.utils.db_queries.global_db import delete_agent
from bongosec_testing.utils.client_keys import check_client_keys
...
def test_ossec_authd_agents_ctx(test_configuration, test_metadata, set_bongosec_configuration, truncate_monitored_files,
                                clean_agents_ctx, daemons_handler, wait_for_authd_startup, connect_to_sockets, set_up_groups):
    '''
    description:
        Check if when the 'bongosec-authd' daemon receives an enrollment request from an agent
    ...
    input_description:
        Different test cases are contained in an external YAML file (bongosec_conf.yaml)
        which includes configuration settings for the 'bongosec-authd' daemon.

    expected_output:
        - r'Accepting connections on port 1515' (When the 'bongosec-authd' daemon is ready to accept enrollments)
