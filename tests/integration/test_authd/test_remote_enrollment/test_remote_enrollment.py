...
brief: These tests will check if the 'remote enrollment' option of the 'bongosec-authd' daemon
       settings is working properly. The 'bongosec-authd' daemon can automatically add
       a Bongosec agent to a Bongosec manager and provide the key to the agent.
...
references:
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/auth.html#remote-enrollment
...
from bongosec_testing.constants.ports import DEFAULT_SSL_CLUSTER_PORT
from bongosec_testing.utils import callbacks
from bongosec_testing.modules.authd import PREFIX
from bongosec_testing.tools.socket_controller import SocketController
from bongosec_testing.tools.monitors import file_monitor
from bongosec_testing.constants.ports import DEFAULT_SSL_REMOTE_ENROLLMENT_PORT
from bongosec_testing.constants.daemons import AUTHD_DAEMON, BONGOSEC_DB_DAEMON, MODULES_DAEMON
from bongosec_testing.utils.configuration import load_configuration_template, get_test_cases_data
...
monitored_sockets_params = [(MODULES_DAEMON, None, True), (BONGOSEC_DB_DAEMON, None, True), (AUTHD_DAEMON, None, True)]
...
def test_remote_enrollment(test_configuration, test_metadata, set_bongosec_configuration,
                           truncate_monitored_files, daemons_handler):
    '''
    description:
        Checks if the 'bongosec-authd' daemon remote enrollment is enabled/disabled according
    ...
    parameters:
        ...
        - daemons_handler:
            type: fixture
            brief: Restarts bongosec or a specific daemon passed.
    ...
    expected_output:
        - r'Accepting connections on port 1515. No password required.' (When the 'bongosec-authd' daemon)
    ...
    file_monitor.FileMonitor(BONGOSEC_LOG_PATH).start(timeout=5,
