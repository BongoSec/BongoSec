...
brief: The 'bongosec-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       correctly executed by the logcollector, and the Bongosec API returns the same values for
...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/localfile.html#command
...
config_path = Path(CONFIGURATIONS_PATH, 'bongosec_basic_configuration_command.yaml')
...
def test_configuration_command(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'bongosec-logcollector' daemon can monitor commands that use multiple parameters.
    ...
    bongosec_log_monitor.start(callback=callback)
