...
brief: The 'bongosec-logcollector' daemon monitors configured files and commands for new log messages.
       Specifically, these tests will check if the logcollector detects invalid values for
       the 'log_format' tag and the Bongosec API returns the same values for the configured
...
references:
    - https://documentation.bongosec.github.io/current/user-manual/capabilities/log-data-collection/index.html
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/localfile.html#log-format
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/localfile.html#location
...
from bongosec_testing.constants.paths.logs import BONGOSEC_LOG_PATH, MACOS_LOG_COMMAND_PATH
...
from bongosec_testing.modules.logcollector import configuration as logcollector_configuration
from bongosec_testing.modules.logcollector import patterns, PREFIX
...
default_config_path = Path(CONFIGURATIONS_PATH, 'bongosec_basic_configuration_log_format.yaml')
default_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format.yaml')

macos_duplicated_config_path = Path(CONFIGURATIONS_PATH, 'bongosec_duplicated_macos_configuration.yaml')
macos_duplicated_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_duplicated.yaml')

macos_no_defined_config_path = Path(CONFIGURATIONS_PATH, 'bongosec_no_defined_location_macos_configuration.yaml')
macos_no_defined_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_no_defined.yaml')

win_config_path = Path(CONFIGURATIONS_PATH, 'bongosec_basic_configuration_log_format_location.yaml')
...
def check_log_format_valid(test_configuration, test_metadata):
    """Check if Bongosec run correctly with the specified log formats.
    ...
    if test_metadata['log_format'] not in log_format_not_print_analyzing_info:
        bongosec_log_monitor.start(timeout=5,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE,
                                                   {'file': test_metadata['location']}))
        assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_FILE
        ...
        if test_metadata['log_format'] == 'full_command':
            ...
            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MONITORING_COMMAND, {
                                'command': test_metadata['command']})
        bongosec_log_monitor.start(timeout=5, callback=callback)
        assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_COMMAND_MONITORING
        ...
        assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_DJB_MULTILOG_NOT_PRODUCED
        ...
        if 'location' in test_metadata and test_metadata['location'] != 'macos':
            ...
            assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_INVALID_MACOS_VALUE
        if 'location' not in test_metadata:
            bongosec_log_monitor.start(timeout=5,
                                    callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MISSING_LOCATION))
            assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_MISSING_LOCATION_VALUE

        bongosec_log_monitor.start(timeout=5,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                   {'command_path': MACOS_LOG_COMMAND_PATH}))
        assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_MACOS_LOG_NOT_PRODUCED


def check_log_format_invalid(test_metadata):
    """Check if Bongosec fails because a invalid frequency configuration value.
    ...
    bongosec_log_monitor.start(timeout=5, callback=log_callback)
    assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_GENERIC_MESSAGE
...
def check_log_file_duplicated():
    """Check if Bongosec shows a warning message when the configuration is duplicated."""
    bongosec_log_monitor = file_monitor.FileMonitor(BONGOSEC_LOG_PATH)
    bongosec_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_LOG_FILE_DUPLICATED))
    assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_LOG_FILE_DUPLICATED
    bongosec_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                                 {'command_path': MACOS_LOG_COMMAND_PATH}))
    assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_MACOS
...
def test_log_format(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                    set_bongosec_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'bongosec-logcollector' daemon detects invalid configurations for the 'log_format' tag.
                 ...
                 Finally, the test will verify that the Bongosec API returns the same values for the 'localfile' section
    ...
    assertions:
        ...
        - Verify that the Bongosec API returns the same values for the 'localfile' section as the configured one.


    input_description: A configuration templates (test_basic_configuration_log_format) are contained in externals
                       YAML files (bongosec_basic_configuration.yaml, bongosec_duplicated_macos_configuration.yaml, and
                       bongosec_no_defined_location_macos_configuration.yaml). Those templates are combined with
                       different test cases defined in the module. Those include configuration settings for
                       the 'bongosec-logcollector' daemon.
    ...
    if test_metadata['valid_value']:
        ...
        if sys.platform == WINDOWS:
            pytest.xfail("Windows agent allows invalid localfile configuration:\
                          https://github.com/bongosec/bongosec/issues/10890")
