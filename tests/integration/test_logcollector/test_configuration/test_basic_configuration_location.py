...
brief: The 'bongosec-logcollector' daemon monitors configured files and commands for new log messages.
       Specifically, these tests will check if the Bongosec component (agent or manager) starts when
       the 'location' tag is set in the configuration, and the Bongosec API returns the same values for
...
from bongosec_testing.modules.agentd import configuration as agentd_configuration
from bongosec_testing.modules.logcollector import patterns
from bongosec_testing.modules.logcollector import utils
...
# Configuration
default_config_path = Path(CONFIGURATIONS_PATH, 'bongosec_basic_configuration_log_format_location.yaml')
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                set_bongosec_configuration, daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'bongosec-logcollector' daemon starts properly when the 'location' tag is used.
                 ...
                 Bongosec component is started by checking its process, and the Bongosec API returns the same
    ...
    assertions:
        - Verify that the Bongosec component (agent or manager) can start when the 'location' tag is used.
        - Verify that the Bongosec API returns the same value for the 'localfile' section as the configured one.
    ...
    input_description: A configuration template (test_basic_configuration_location) is contained in an external
                       YAML file (bongosec_basic_configuration.yaml). That template is combined with different
                       test cases defined in the module. Those include configuration settings for
                       the 'bongosec-logcollector' daemon.

    expected_output:
        - Boolean values to indicate the state of the Bongosec component.
    ...
    if sys.platform == WINDOWS:
        if test_metadata['location'] == 'invalidchannel':
            ...
            bongosec_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT, callback=callback)
            assert (bongosec_log_monitor.callback_result != None), patterns.ERROR_EVENTCHANNEL
    else:
