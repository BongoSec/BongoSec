...
brief: The 'bongosec-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       specified in the 'alias' tag when monitoring a command, and the Bongosec API returns the same
...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/localfile.html#alias
...
config_path = Path(CONFIGURATIONS_PATH, 'bongosec_basic_configuration.yaml')
...
def test_configuration_alias(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'bongosec-logcollector' daemon changes a command name in the log messages by
                 ...
                 will verify that the Bongosec API returns the same values for the 'localfile' section that
                 the configured one.
    ...
    input_description: A configuration template (test_basic_configuration_alias) is contained in an external YAML file
                       (bongosec_basic_configuration.yaml). That template is combined with two test cases defined
                       in the module. Those include configuration settings for the 'bongosec-logcollector' daemon.
    ...
    bongosec_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_READING_COMMAND_ALIAS, {
