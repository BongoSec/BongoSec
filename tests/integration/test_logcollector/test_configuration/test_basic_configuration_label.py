...
brief: The 'bongosec-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'label' tag is set in the configuration, and the Bongosec API returns the same values for
...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/localfile.html#label
...
config_path = Path(CONFIGURATIONS_PATH, 'bongosec_basic_configuration_label.yaml')
...
def test_configuration_label(test_configuration, test_metadata, set_bongosec_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'bongosec-logcollector' daemon can monitor log files configured to use labels.
                 ...
                 Finally, the test will verify that the Bongosec API returns the same values for
    ...
    bongosec_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE, {
