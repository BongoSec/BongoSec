...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/localfile.html#out-format
...
config_path = Path(CONFIGURATIONS_PATH, 'bongosec_basic_configuration_target.yaml')
...
def test_configuration_target(test_configuration, test_metadata, set_bongosec_configuration, configure_local_internal_options, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'bongosec-logcollector' daemon detects invalid configurations for the 'target' attribute
                 ...
                 the Bongosec API returns the same values for the 'localfile' section that the configured one.
    ...
    input_description: A configuration template (test_basic_configuration_target) is contained in an external
                       ...
                       for the 'bongosec-logcollector' daemon.
    ...
    bongosec_log_monitor.start(callback, timeout=10)
    assert bongosec_log_monitor.callback_result, assert_error
