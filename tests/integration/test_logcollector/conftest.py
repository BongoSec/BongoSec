...
from bongosec_testing.constants.paths.configurations import BONGOSEC_CONF_PATH
from bongosec_testing.constants.daemons import LOGCOLLECTOR_DAEMON
from bongosec_testing.modules.logcollector.patterns import LOGCOLLECTOR_MODULE_START
...
from bongosec_testing.utils.file import truncate_file, replace_regex_in_file, write_json_file

# Logcollector internal paths
LOGCOLLECTOR_OFE_PATH = path_join(BONGOSEC_PATH, 'queue', 'logcollector', 'file_status.json')

@pytest.fixture()
def stop_logcollector(request):
    """Stop bongosec-logcollector and truncate logs file."""
...
@pytest.fixture()
def remove_all_localfiles_bongosec_config(request):
    """Configure a custom settting for testing. Restart Bongosec is needed for applying the configuration. """
    ...
    replace_regex_in_file(list_tags, [''] * len(list_tags), BONGOSEC_CONF_PATH, True)
