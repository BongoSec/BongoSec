...
references:
    ...
    - https://documentation.bongosec.github.io/current/user-manual/reference/ossec-conf/localfile.html
...
from bongosec_testing.utils.file import write_json_file
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                reset_ofe_status, remove_all_localfiles_bongosec_config, set_bongosec_configuration, pre_send_journal_logs,
                                daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'bongosec-logcollector' daemon starts properly when the 'journald' tag is used 
                 ...
                 Finally, the test will verify that the Bongosec-logcollector read the logs, and the Bongosec API returns the correct values
                 for the 'localfile' section.

    bongosec_min_version: 4.9.0
    ...
    parameters:
        ...
        - remove_all_localfiles_bongosec_config:
            type: fixture
            brief: Remove all 'localfile' sections from the Bongosec configuration.
        - reset_ofe_status:
            type: fixture
    ...
    assertions:
        - Verify that the Bongosec component (agent or manager) can start when the 'jouranld' tag is used.
        - Verify that the Bongosec component (agent or manager) can read the logs from the 'journald' log format and the logs older than the current logcollector start time.

    input_description: A configuration file with journal block settings and the expected log messages.
                       Those include configuration settings for `journal` configuration in 'bongosec-logcollector'.
