#!/bin/bash
#
# Bongosec Security Dashboards Plugin repository bumper (Pure Shell Version)
...
REVISION="00"
CURRENT_VERSION=""
BONGOSEC_DASHBOARD_PLUGINS_WORKFLOW_FILE="${REPO_PATH}/.github/workflows/build_bongosec_dashboard_with_plugins.yml"
...
pre_update_checks() {
  ...
  CURRENT_REVISION=$(sed -n '/"bongosec": {/,/}/ s/^\s*"revision"\s*:\s*"\([^"]*\)".*$/\1/p' "$PACKAGE_JSON" | head -n 1)
...
update_package_json() {
  if [ -f "$PACKAGE_JSON" ]; then
    ...
    # Use sed with address range to target lines within the "bongosec": { ... } block
    # Update version in package.json
    if [[ "$CURRENT_VERSION" != "$VERSION" ]]; then
      log "Attempting to update version to $VERSION within 'bongosec' object in $PACKAGE_JSON"
      # Note: This sed command assumes a specific formatting and might be fragile.
      # It looks for the block starting with a line containing "bongosec": { and ending with the next line containing only }
      # Within that block, it replaces the value on the line starting with "version":
      sed -i "/\"bongosec\": {/,/}/ s/^\(\s*\"version\"\s*:\s*\)\"[^\"]*\"/\1\"$VERSION\"/" "$PACKAGE_JSON"
      modified=true
    fi

    # Update revision in package.json
    if [[ "$CURRENT_REVISION" != "$REVISION" ]]; then
      log "Attempting to update revision to $REVISION within 'bongosec' object in $PACKAGE_JSON"
      # Similar sed command for the revision line within the same block
      sed -i "/\"bongosec\": {/,/}/ s/^\(\s*\"revision\"\s*:\s*\)\"[^\"]*\"/\1\"$REVISION\"/" "$PACKAGE_JSON"
      modified=true
    fi
...
update_changelog() {
  ...
  local changelog_header="## Bongosec dashboard v${VERSION} - OpenSearch Dashboards ${OPENSEARCH_VERSION} - Revision "
...
update_build_workflow() {
  log "Updating $(basename $BONGOSEC_DASHBOARD_PLUGINS_WORKFLOW_FILE)..."

  if [ -f "$BONGOSEC_DASHBOARD_PLUGINS_WORKFLOW_FILE" ]; then
    local modified=false
    ...
    if grep -qE "$workflow_version_pattern" "$BONGOSEC_DASHBOARD_PLUGINS_WORKFLOW_FILE"; then
      log "Pattern found in $(basename $BONGOSEC_DASHBOARD_PLUGINS_WORKFLOW_FILE). Attempting update..."
      # If the pattern exists, perform the substitution
      sed -i -E "s/${workflow_version_pattern}/\1v${VERSION}-${STAGE}/g" "$BONGOSEC_DASHBOARD_PLUGINS_WORKFLOW_FILE"
      modified=true
    else
      log "Pattern not found in $(basename $BONGOSEC_DASHBOARD_PLUGINS_WORKFLOW_FILE). Skipping update."
    fi

    if [[ $modified == true ]]; then
      log "Successfully updated yml@v${VERSION}-${STAGE} in $(basename $BONGOSEC_DASHBOARD_PLUGINS_WORKFLOW_FILE)"
    fi
  fi
}
...
update_base_package_dockerfile() {
  ...
  if [ -f "$DOCKERFILE_FOR_BASE_PACKAGES" ]; then
    ...
    # Update all occurrences of bongosec-packages-to-base:x.y.z with bongosec-packages-to-base:$VERSION
    sed -i -E "s/(bongosec-packages-to-base:)$VERSION_PATTERN/\1${VERSION}/g" "$DOCKERFILE_FOR_BASE_PACKAGES" && modified=true
...
update_rendering_service_test_snap() {
  ...
  if [ -f "$rendering_service_test_snap" ]; then
    ...
    # Update all occurrences of ("bongosecVersion": "x.y.z",) with ("bongosecVersion": "$VERSION",)
    # Define the pattern regex
    local pattern_regex="(\"bongosecVersion\": \")$VERSION_PATTERN"
    # Check if the pattern exists in the file
