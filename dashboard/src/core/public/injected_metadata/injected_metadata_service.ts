...
export class InjectedMetadataService {
  ...
  public setup(): InjectedMetadataSetup {
    return {
      ...
      getBongosecVersion: () => {
        return this.state.bongosecVersion;
      },

      getBongosecDocVersion: () => {
        return this.state.bongosecVersion?.split('.').slice(0, 2).join('.') || 'current';
      },
    };
  }
}
...
export interface InjectedMetadataSetup {
  ...
  getBongosecVersion: () => string;
  getBongosecDocVersion: () => string;
}
