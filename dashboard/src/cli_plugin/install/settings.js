...
const LATEST_BONGOSEC_PLUGIN_BASE_URL = 'https://bongosec.github.io/packages/pre-release/ui/dashboard';

function generateUrls({ version, plugin, bongosecVersion }) {
  return plugin.includes('bongosec')
    ? [plugin, generateBongosecPluginUrl(bongosecVersion, plugin)]
...
function generateBongosecPluginUrl(version, plugin) {
  return `${LATEST_BONGOSEC_PLUGIN_BASE_URL}/${plugin}-${version}-1.zip`;
}
...
export function parse(command, options, osdPackage) {
  const settings = {
    ...
    bongosecVersion: osdPackage.bongosec.version,
  ...
  // Clean up.
  delete settings.bongosecVersion;
