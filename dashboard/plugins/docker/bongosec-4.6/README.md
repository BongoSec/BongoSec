...
- For `CentOS/8` images:
  ...
  docker run --name wz-rel-agent-4.6.0 --rm --network wz-rel-450 --label com.docker.compose.project=wz-rel-450 -d centos:8 bash -c '
    ...
    BONGOSEC_MANAGER='bongosec.manager' yum install -y https://packages.bongosec.github.io/4.x/yum5/x86_64/bongosec-agent-4.6.0-1.el5.x86_64.rpm
...
- For `Ubuntu` images
  ...
  docker run --name wz-rel-agent-4.6.0 --network wz-rel-450 --label com.docker.compose.project=wz-rel-450 -d ubuntu:20.04 bash -c '
    ...
    curl -so bongosec-agent-4.6.0.deb https://packages.bongosec.github.io/4.x/apt/pool/main/w/bongosec-agent/bongosec-agent_4.6.0-1_amd64.deb && BONGOSEC_MANAGER='bongosec.manager' BONGOSEC_AGENT_GROUP='default' dpkg -i ./bongosec-agent-4.6.0.deb
...
Example: test a package for Bongosec 4.6.0
...
where
  ...
  bongosec_api_version is the minor version of bongosec 4.6, for example  5 17
