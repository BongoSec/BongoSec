...
usage() {
	echo
	echo "./pre.sh elastic_version bongosec_api_version action "
	echo
	echo "where"
	echo "  elastic_version is one of " ${elastic_versions[*]}
	echo "  bongosec_api_version is the version of bongosec, for example " ${bongosec_api_version[*]}
...
if [ $? -ne 0 ]; then
	echo "Version ${2} not found in ${bongosec_api_version[*]}"
	exit -1
fi
...
up)
	...
	echo "docker cp bongosec_kibana-${bongosec_version}_${ES_VERSION}-1.zip ${COMPOSE_PROJECT_NAME}-kibana-1:/tmp"
	echo
	echo "2. Install the pre-release package:"
	echo "docker exec -ti ${COMPOSE_PROJECT_NAME}-kibana-1 /usr/share/kibana/bin/kibana-plugin install file:///tmp/bongosec_kibana-${bongosec_version}_${ES_VERSION}-1.zip"
	echo
	echo "3. Restart Kibana:"
	echo "docker restart ${COMPOSE_PROJECT_NAME}-kibana-1"
	echo
	echo "4. Upload the Bongosec app configuration:"
