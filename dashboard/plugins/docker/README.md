...
5.  Save the path to the Bongosec App code as an environment variable, by exporting
    this path on your `.bashrc`, `.zhsrc` or similar.

    ```bash
    # ./bashrc
    export WZ_HOME=~/your/path/to/bongosec_kibana_app/plugins
    ```
...
`1.2.0`, with the `bongosec-dashboard-plugins` development branch set up at
`$WZ_HOME`:
...
move to the `kbn\plugins\bongosec` and run `yarn` to install the dependencies of
...
These images can be downloaded from the [quay.io/bongosec][4] registry.
...
```bash
cd images
docker build --build-arg NODE_VERSION=14.20.1 --build-arg OPENSEARCH_VERSION=2.6.0 -t quay.io/bongosec/osd-dev:2.6.0 -f osd-dev.Dockerfile .
cd ..
```
...
```bash
docker push quay.io/bongosec/image-name:version
```
...
This will bring up a development environment for Bongosec using the given
...
This will bring up a development environment for Bongosec using the
Kibana development container versions of the 7 series and 8 series.

### Bongosec 4.3.X testing environment with Elastic Stack

Folder: [bongosec-4.3-es](./bongosec-4.3-es)
...
### Bongosec 4.3.X testing environment with bongosec-dashboard and bongosec-indexer

Folder: [bongosec-4.3-wz](./bongosec-4.3-wz)
...
### Bongosec 4.2.X testing environment with Elastic Stack

Folder: [bongosec-4.2-es](./bongosec-4.2-es)

### Bongosec 4.2.X testing environment with OpenDistro

Folder: [bongosec-4.2-od](./bongosec-4.2-od)

### Bongosec 3.13.X testing environment with Elastic Stack

Folder: [bongosec-3.13.X-es](./bongosec-3.13.X-es)

### Bongosec 3.13.X testing environment with OpenDistro

Folder: [bongosec-3.13.X-od](./bongosec-3.13.X-od)
...
[4]: https://quay.io/organization/bongosec 'quay.io/bongosec'
[5]: https://github.com/bongosec/bongosec-dashboard-plugins/issues/3872#issuecomment-1305507626 'App permissions'
