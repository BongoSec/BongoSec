...
usage() {
  echo
  echo "./dev.sh [-os os_version] [-osd osd_version] /bongosec_app_src action [saml/server] [server_version]"
  echo
  echo "where"
  echo "  -o os_version Specify the OS version (optional)"
  echo "  -d osd_version Specify the OSD version (optional)"
  echo "  bongosec_app_src is the path to the bongosec application source code"
...
PACKAGE_PATH="../../plugins/bongosec-core/package.json"
os_version=""
osd_version=""
...
profile="standard"
export BONGOSEC_DASHBOARD_CONF=./config/${OSD_MAJOR}/osd/opensearch_dashboards.yml
export SEC_CONFIG_FILE=./config/${OSD_MAJOR}/os/config.yml
if [[ "$3" =~ "saml" ]]; then
  cat /etc/hosts | grep -q "idp" || exit_with_message "Add idp to /etc/hosts"

  profile="saml"
  export BONGOSEC_DASHBOARD_CONF=./config/${OSD_MAJOR}/osd/opensearch_dashboards_saml.yml
  export SEC_CONFIG_FILE=./config/${OSD_MAJOR}/os/config-saml.yml
fi

if [[ "$3" =~ "server" ]]; then
  profile="server"
  export BONGOSEC_STACK="${4}"
fi
...
up)
   ...
   if [[ "$3" =~ "server" ]]; then
     ...
     echo "docker run --name ${COMPOSE_PROJECT_NAME}-agent-\$(date +%s) --network os-dev-${OS_VERSION} --label com.docker.compose.project=${COMPOSE_PROJECT_NAME} --env BONGOSEC_AGENT_VERSION=${BONGOSEC_STACK} -d ubuntu:20.04 bash -c '"
     echo "  apt update -y"
