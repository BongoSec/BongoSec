...
indexer="$1-bongosec.indexer-1"
dashboard="$1-bongosec.dashboard-1"

# Setup keycloack to be used with bongosec-dashboards
...
PC="{
  ...
  \"name\": \"bongosec\",
  \"clientId\": \"bongosec\",
  \"description\": \"bongosec saml integration\",
  ...
  \"attributes\" : {
    ...
    \"saml_single_logout_service_url_post\": \"https://bongosec.dashboard:5601/_opendistro/_security/saml/logout\",
...
curl -sS -L -X POST "${B}/admin/realms/${REALM}/clients" "${H[@]}" -d "$PC" | grep -v "Client bongosec already exists"

# Get a client json representation
CLIENT=$(curl -sS -L -X GET "${B}/admin/realms/${REALM}/clients" "${H[@]}" -G -d 'clientId=bongosec' |jq '.[] | select(.clientId=="bongosec")')
...
PU='{
  "username": "bongosec",
  "email": "hello@bongosec.github.io",
  "firstName": "Bongosec",
  "lastName": "Bongosec",
  "emailVerified": true,
  "enabled": true,
  "credentials": [{"temporary":false,"type":"password","value":"bongosec"}],
...
USER=$(curl -sS -L -X GET "${B}/admin/realms/${REALM}/users" "${H[@]}" -G -d 'username=bongosec' |jq '.[] | select(.username=="bongosec")')
...
certs="/usr/share/bongosec-indexer/config/certs"
ca="$certs/ca.pem"
cert="$certs/admin.pem"
key="$certs/admin-key.pem"

securityadmin="bash /usr/share/bongosec-indexer/plugins/opensearch-security/tools/securityadmin.sh"
config_path="/usr/share/bongosec-indexer/plugins/opensearch-security/securityconfig/"

echo "To update configuration in indexer, you can run:"
echo docker exec -e JAVA_HOME=/usr/share/bongosec-indexer/jdk $indexer $securityadmin -cacert $ca -cert $cert -key $key -cd $config_path
