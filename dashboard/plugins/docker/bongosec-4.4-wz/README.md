...
- For `CentOS/8` images:
  ...
  docker run --name wz-rel-agent-4.4.4 --rm --network wz-rel-444 --label com.docker.compose.project=wz-rel-444 -d centos:8 bash -c '
    ...
    BONGOSEC_MANAGER='bongosec.manager' yum install -y https://packages.bongosec.github.io/4.x/yum5/x86_64/bongosec-agent-4.4.4-1.el5.x86_64.rpm
...
- For `Ubuntu` images
  ...
  docker run --name wz-rel-agent-4.4.4 --network wz-rel-444 --label com.docker.compose.project=wz-rel-444 -d ubuntu:20.04 bash -c '
    ...
    curl -so bongosec-agent-4.4.4.deb https://packages.bongosec.github.io/4.x/apt/pool/main/w/bongosec-agent/bongosec-agent_4.4.4-1_amd64.deb && BONGOSEC_MANAGER='bongosec.manager' BONGOSEC_AGENT_GROUP='default' dpkg -i ./bongosec-agent-4.4.4.deb
