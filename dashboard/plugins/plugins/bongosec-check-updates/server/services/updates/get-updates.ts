...
import { IAPIHost } from '../../../../bongosec-core/server/services';

export const getUpdates = async (
  queryApi = false,
  forceQuery = false,
): Promise<AvailableUpdates> => {
  const { logger } = getBongosecCheckUpdatesServices();
  ...
  try {
    ...
    const { manageHosts, api: bongosecApiClient } = getBongosecCore();
    ...
    const apisAvailableUpdates = await Promise.all(
      hosts?.map(async api => {
        ...
        try {
          ...
          } = await bongosecApiClient.client.asInternalUser.request(
            'GET',
            '/',
            {},
            options,
        ...
        try {
          const response = await bongosecApiClient.client.asInternalUser.request(
            method,
            path,
            data,
            options,
