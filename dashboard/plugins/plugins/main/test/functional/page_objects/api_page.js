...
export function ApiProvider({ getService, getPageObjects }) {
  ...
  class ApiPage {
    ...
    async completeApiForm(type = 'apiConfig') {
      ...
      await testSubjects.setValue(`${type}Username`, 'bongosec');
      log.debug('insert the user');
      await testSubjects.setValue(`${type}Password`, 'bongosec');
    ...
    async checkTabDisabled(tab) {
      ...
      expect(await browser.getCurrentUrl()).to.contain('app/bongosec#/settings');
    }
