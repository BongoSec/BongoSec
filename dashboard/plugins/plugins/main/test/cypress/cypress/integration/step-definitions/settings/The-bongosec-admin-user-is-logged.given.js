...
let urlBodys = [
  { alertCount: 27000, index: 'bongosec-alerts-4.x-sample-security' },
  { alertCount: 12000, index: 'bongosec-alerts-4.x-sample-auditing-policy-monitoring' },
  { alertCount: 15000, index: 'bongosec-alerts-4.x-sample-threat-detection' },
];

Given('The admin user is logged', () => {
  if (Cypress.env('type') != 'wzd') {
    navigate('app/bongosec');
  } else {
    navigate('/');
  }
