/*
 * Bongosec app - Class for Bongosec-API functions
...
export class BongosecApiCtrl {
  ...
  async getToken(
    ...
    try {
      ...
      const { username } = await context.bongosec.security.getCurrentUser(
        request,
        context,
      );
      if (
        ...
        if (wzToken) {
          try {
            ...
            context.bongosec.logger.error(
              `Error decoding the API host entry token: ${error.message}`,
            );
          }
        }
      }
      const token = await context.bongosec.api.client.asCurrentUser.authenticate(
        idHost,
      );

      let textSecure = '';
      if (context.bongosec.server.info.protocol === 'https') {
        textSecure = ';Secure';
      }
      ...
      context.bongosec.logger.error(errorMessage);
  ...
  /**
   * Returns if the bongosec-api configuration is working
  ...
  async checkStoredAPI(
    ...
    try {
      ...
      context.bongosec.logger.debug(`Getting server API host by ID: ${id}`);
      const apiHostData = await context.bongosec_core.manageHosts.get(id, {
        excludePassword: true,
      });
      const api = { ...apiHostData };
      context.bongosec.logger.debug(
        `Server API host data: ${JSON.stringify(api)}`,
      );

      context.bongosec.logger.debug(`${id} exists`);

      // Fetch needed information about the cluster and the manager itself
      const responseManagerInfo =
        await context.bongosec.api.client.asInternalUser.request(
          'get',
          `/manager/info`,
          {},
      ...
      // If we have a valid response from the Bongosec API
      try {
        const { status, manager, node, cluster } =
          await context.bongosec_core.manageHosts.getRegistryDataByHost(
            apiHostData,
            {
              throwError: true,
            },
        ...
        // If we have an invalid response from the Bongosec API
        throw new Error(
      ...
      if (error.code === 'EPROTO') {
        ...
        try {
          const apis = await context.bongosec_core.manageHosts.get();
          ...
          context.bongosec.logger.error(error.message || error);
          return ErrorResponse(
            error.message || error,
            3020,
            error?.response?.status || HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR,
  ...
  /**
   * This check the bongosec-api configuration received in the POST body will work
   * @param {Object} context
   * @param {Object} request
   * @param {Object} response
   * @returns {Object} status obj or ErrorResponse
  ...
  async checkAPI(
    ...
    try {
      ...
      context.bongosec.logger.debug(`${request.body.id} is valid`);
      // Check if a Bongosec API id is given (already stored API)
      const data = await context.bongosec_core.manageHosts.get(request.body.id, {
        excludePassword: true,
      });
      if (data) {
        apiAvailable = data;
        ...
        context.bongosec.logger.debug(errorMessage);
        return ErrorResponse(
          errorMessage,
          3029,
      ...
      context.bongosec.logger.debug(`${request.body.id} credentials are valid`);
      if (
        responseManagerInfo.status === HTTP_STATUS_CODES.OK &&
        responseManagerInfo.data
      ) {
        const result =
          await context.bongosec_core.manageHosts.getRegistryDataByHost(data);
        return response.ok({
          body: result,
        });
      }
    } catch (error) {
      context.bongosec.logger.warn(error.message || error);
  ...
  /**
   * Check main Bongosec daemons status
   * @param {*} context Endpoint context
   * @param {*} api API entry stored in .bongosec
   * @param {*} path Optional. Bongosec API target path.
   */
  async checkDaemons(context, api, path) {
    try {
      const response = await context.bongosec.api.client.asInternalUser.request(
        'GET',
        '/manager/status',
        {},
        { apiHostID: api.id },
      ...
      const isCluster =
        ...
        typeof daemons['bongosec-clusterd'] !== 'undefined';
      const bongosecdbExists = typeof daemons['bongosec-db'] !== 'undefined';

      const execd = daemons['bongosec-execd'] === 'running';
      const modulesd = daemons['bongosec-modulesd'] === 'running';
      const bongosecdb = bongosecdbExists ? daemons['bongosec-db'] === 'running' : true;
      const clusterd = isCluster
        ? daemons['bongosec-clusterd'] === 'running'
        : true;

      const isValid = execd && modulesd && bongosecdb && clusterd;

      isValid && context.bongosec.logger.debug('Bongosec is ready');
  ...
  /**
   * This performs a request over Bongosec API and returns its response
  ...
  async makeRequest(context, method, path, data, id, response) {
    ...
    try {
      let api;
      try {
        api = await context.bongosec_core.manageHosts.get(id, {
          excludePassword: true,
        });
      } catch (error) {
        context.bongosec.logger.error('Could not get host credentials');
        //Can not get credentials from bongosec-hosts
        return ErrorResponse(
      ...
      if (delay) {
        ...
        addJobToQueue({
          ...
          run: async contextJob => {
            try {
              await context.bongosec.api.client.asCurrentUser.request(
                method,
                path,
                data,
                options,
              ...
              contextJob.bongosec.logger.error(
      ...
      context.bongosec.logger.debug(`${method} ${path}`);
  ...
  requestApi(
    ...
    if (!request.body.method) {
      ...
      context.bongosec.logger.error('Request method is not valid.');
      ...
      context.bongosec.logger.error('Request path is not valid.');
  ...
  /**
   * Get full data on CSV format from a list Bongosec API endpoint
   * @param {Object} ctx
   * @param {Object} request
   * @param {Object} response
  ...
  async csv(
    ...
    const appConfig = await context.bongosec_core.configuration.get();
    ...
    try {
      ...
      context.bongosec.logger.debug(`Report ${tmpPath}`);
      ...
      const output = await context.bongosec.api.client.asCurrentUser.request(
        'GET',
        `/${tmpPath}`,
        { params: params },
      ...
      if (totalItems && !isList) {
        ...
        while (
          ...
          const tmpData = await context.bongosec.api.client.asCurrentUser.request(
            'GET',
            `/${tmpPath}`,
            { params: params },
            { apiHostID: request.body.id },
      ...
      if (totalItems) {
        ...
        throw new Error(
          `An error occurred fetching data from the Bongosec API${
  ...
  /**
   * This get the bongosec setup settings
   * @param {Object} context
   * @param {Object} request
   * @param {Object} response
  ...
  async getSetupInfo(
    ...
    try {
      return response.ok({
        body: {
          ...
          data: {
            ...
            configuration_file: context.bongosec_core.configuration.store.file,
          },
        },
      });
    } catch (error) {
      context.bongosec.logger.error(error.message || error);
      return ErrorResponse(
        `Could not get data from bongosec-version registry due to ${
          error.message || error
        }`,
        4005,
        HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR,
  ...
  async getSyscollector(
    ...
    try {
      ...
      const data = await Promise.all([
        context.bongosec.api.client.asInternalUser.request(
          'GET',
  ...
  async getAppLogos(
    ...
    try {
      ...
      const logos = {
        [APP_LOGO]:
          await context.bongosec_core.configuration.getCustomizationSetting(
            APP_LOGO,
          ),
        [HEALTHCHECK_LOGO]:
          await context.bongosec_core.configuration.getCustomizationSetting(
