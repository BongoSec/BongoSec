/*
 * Bongosec app - Class for Bongosec reporting controller
...
import fs from 'fs';
import { BONGOSEC_MODULES } from '../../common/bongosec-modules';
...
import {
  ...
  BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
  AUTHORIZED_AGENTS,
...
export class BongosecReportingCtrl {
  constructor() {}
  /**
   * This do format to filters
   * @param {String} filters E.g: cluster.name: bongosec AND rule.groups: vulnerability
    ...
    try {
      ...
      if (section && typeof section === 'string') {
        if (!['agentConfig', 'groupConfig'].includes(section)) {
          printer.addContent({
            text: BONGOSEC_MODULES[tab].title + ' report',
            style: 'h1',
          });
      ...
      if (BONGOSEC_MODULES[tab] && BONGOSEC_MODULES[tab].description) {
        printer.addContentWithNewLine({
          text: BONGOSEC_MODULES[tab].description,
          style: 'standard',
        });
      }
    } catch (error) {
  ...
  private getConfigRows(context, data, labels) {
    context.bongosec.logger.debug('Building configuration rows');
    const result = [];
  ...
  private getConfigTables(context, data, section, tab, array = []) {
    context.bongosec.logger.debug('Building configuration tables');
    let plainData = {};
    const nestedData = [];
    const tableData = [];
  ...
  createReportsModules = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.bongosec.logger.debug('Report started');
        const {
          array,
          agents,
          browserTimezone,
        ...
        const printer = new ReportPrinter(
          context.bongosec.logger.get('report-printer'),
          context.bongosec_core.configuration,
        );

        createDataDirectoryIfNotExists();
        createDirectoryIfNotExists(BONGOSEC_DATA_DOWNLOADS_DIRECTORY_PATH);
        createDirectoryIfNotExists(BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH);
        createDirectoryIfNotExists(
          path.join(
            BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
            context.bongosecEndpointParams.hashUsername,
          ),
        );
        ...
        if (time) {
          additionalTables = await extendedInformation(
            ...
            serverSideQuery,
            indexPatternTitle ||
              context.bongosec_core.configuration.getSettingValue('pattern'),
            agents,
          );
        }
        ...
        await printer.print(context.bongosecEndpointParams.pathFilename);

        return response.ok({
          body: {
            success: true,
            message: `Report ${context.bongosecEndpointParams.filename} was created`,
            filename: context.bongosecEndpointParams.filename,
          },
        });
      } catch (error) {
  ...
  createReportsGroups = this.checkReportsUserDirectoryIsValidRouteDecorator(
    ...
    ({ params: { groupID } }) =>
      `bongosec-group-configuration-${groupID}-${this.generateReportTimestamp()}.pdf`,
  );
  ...
  createReportsAgentsConfiguration =
    this.checkReportsUserDirectoryIsValidRouteDecorator(
      async (
        ...
        try {
          ...
          try {
            ...
            context.bongosec.logger.debug(error.message || error);
          }
      ...
      ({ params: { agentID } }) =>
        `bongosec-agent-configuration-${agentID}-${this.generateReportTimestamp()}.pdf`,
    );
    ...
    this.checkReportsUserDirectoryIsValidRouteDecorator(
      async (
        ...
        try {
          ...
          const { hashUsername } = await context.bongosec.security.getCurrentUser(
            request,
            context,
          );
          createDataDirectoryIfNotExists();
          ...
          context.bongosec.logger.debug('Syscollector report');
          const [sanitizedFilters, agentsFilter] = filters
            ? this.sanitizeKibanaFilters(context, filters, searchBar)
            : [false, null];
          ...
          const requestInventory = async agentRequestInventory => {
            try {
              context.bongosec.logger.debug(agentRequestInventory.loggerMessage);
      ...
      ({ params: { agentID } }) =>
        `bongosec-agent-inventory-${agentID}-${this.generateReportTimestamp()}.pdf`,
    );
  ...
  async getReports(
    ...
    try {
      context.bongosec.logger.debug('Fetching created reports');
      const { hashUsername } = await context.bongosec.security.getCurrentUser(
        request,
        context,
      );
      ...
      context.bongosec.logger.debug(`Directory: ${userReportsDirectoryPath}`);
      ...
      context.bongosec.logger.debug(`Total reports: ${reports.length}`);
      return response.ok({
        body: { reports },
      });
    } catch (error) {
  ...
  getReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.bongosec.logger.debug(
          `Getting ${context.bongosecEndpointParams.pathFilename} report`,
        );
        const reportFileBuffer = fs.readFileSync(
          context.bongosecEndpointParams.pathFilename,
        );
        return response.ok({
  ...
  deleteReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.bongosec.logger.debug(
          `Deleting ${context.bongosecEndpointParams.pathFilename} report`,
        );
        fs.unlinkSync(context.bongosecEndpointParams.pathFilename);
        context.bongosec.logger.info(
          `${context.bongosecEndpointParams.pathFilename} report was deleted`,
        );
        return response.ok({
          body: { error: 0 },
        });
    ...
    return async (
      ...
      try {
        const { username, hashUsername } =
          await context.bongosec.security.getCurrentUser(request, context);
        const userReportsDirectoryPath = path.join(
          BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
          hashUsername,
        );
        ...
        return await routeHandler.bind(this)(
          {
            ...context,
            bongosecEndpointParams: { hashUsername, filename, pathFilename },
          },
          request,
          response,
        );
