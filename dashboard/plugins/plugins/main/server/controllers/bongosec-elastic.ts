/*
 * Bongosec app - Class for Bongosec-Elastic functions
...
import {
  BONGOSEC_SAMPLE_ALERTS_INDEX_SHARDS,
  BONGOSEC_SAMPLE_ALERTS_INDEX_REPLICAS,
...
import {
  BONGOSEC_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS,
  BONGOSEC_SAMPLE_ALERTS_DEFAULT_NUMBER_ALERTS,
} from '../../common/constants';
import { BONGOSEC_INDEXER_NAME } from '../../common/constants';
import { routeDecoratorProtectedAdministrator } from './decorators';

export class BongosecElasticCtrl {
  constructor() {}
  ...
  async buildSampleIndexByCategory(
    ...
    return `${await context.bongosec_core.configuration.get(
  ...
  async getTemplate(
    ...
    try {
      ...
      if (!templates || typeof templates !== 'string') {
        throw new Error(
          `An unknown error occurred when fetching templates from ${BONGOSEC_INDEXER_NAME}`,
        );
      }
      ...
      return ErrorResponse(
        `Could not retrieve templates from ${BONGOSEC_INDEXER_NAME} due to ${
          error.message || error
        }`,
        4002,
        500,
  ...
  async getCurrentPlatform(
    ...
    try {
      return response.ok({
        body: {
          platform: context.bongosec.security.platform,
        },
      });
    } catch (error) {
      context.bongosec.logger.error(error.message || error);
   ...
   * @param {String} id Index-pattern id to use in the visualizations. Eg: 'bongosec-alerts'
   */
  async buildVisualizationsRaw(context, app_objects, id, namespace = false) {
    const config = await context.bongosec_core.configuration.get();
    let monitoringPattern = `${config['bongosec.monitoring.pattern']}`;
    context.bongosec.logger.debug(`Building ${app_objects.length} visualizations`);
    context.bongosec.logger.debug(`Index pattern ID: ${id}`);
    const visArray = [];
    ...
    for (let element of app_objects) {
      ...
      if (
        ...
        const isMonitoring = defaultStr.includes('bongosec-monitoring');
        if (isMonitoring) {
          ...
          aux_source.kibanaSavedObjectMeta.searchSourceJSON =
            defaultStr.replace(/bongosec-alerts/g, id);
        }
      }

      // Replace index-pattern for selector visualizations
      if (typeof (aux_source || {}).visState === 'string') {
        aux_source.visState = aux_source.visState.replace(/bongosec-alerts/g, id);
      }
   ...
   * @param {String} name Cluster name. Eg: 'bongosec'
  ...
  buildClusterVisualizationsRaw(
    ...
    try {
      ...
      for (const element of app_objects) {
        ...
        aux_source = aux_source.replace(/bongosec-alerts/g, id);
  ...
  async haveSampleAlerts(
    ...
    try {
      // Check if bongosec sample alerts index exists
      const results = await Promise.all(
        Object.keys(BONGOSEC_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS).map(
          async category =>
  ...
  /**
   * This creates sample alerts in bongosec-sample-alerts
  ...
  createSampleAlerts = routeDecoratorProtectedAdministrator(1000)(
    async (
      ...
      try {
        ...
        const sampleAlerts = BONGOSEC_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS[
          request.params.category
        ]
          .map(typeAlert =>
            generateAlerts(
        ...
        if (!existsSampleIndex.body) {
          // Create bongosec sample alerts index

          const configuration = {
            settings: {
              index: {
                number_of_shards: BONGOSEC_SAMPLE_ALERTS_INDEX_SHARDS,
                number_of_replicas: BONGOSEC_SAMPLE_ALERTS_INDEX_REPLICAS,
              },
            },
          };
          ...
          context.bongosec.logger.info(`Index ${sampleAlertsIndex} created`);
        }
  ...
  deleteSampleAlerts = routeDecoratorProtectedAdministrator(1000)(
    async (
      ...
      // Delete Bongosec sample alert index
      const sampleAlertsIndex = await this.buildSampleIndexByCategory(
        context,
        request.params.category,
      );
      ...
      try {
        ...
        if (existsSampleIndex.body) {
          // Delete Bongosec sample alerts index
          await context.core.opensearch.client.asCurrentUser.indices.delete({
            index: sampleAlertsIndex,
          });
          context.bongosec.logger.info(`Deleted ${sampleAlertsIndex} index`);
          return response.ok({
