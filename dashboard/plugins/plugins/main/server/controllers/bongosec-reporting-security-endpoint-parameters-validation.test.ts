...
import { BongosecReportingRoutes } from '../routes/bongosec-reporting';
import md5 from 'md5';
import {
...
beforeAll(async () => {
  // Create <PLUGIN_PLATFORM_PATH>/data/bongosec directory.
  createDataDirectoryIfNotExists();
  // Create <PLUGIN_PLATFORM_PATH>/data/bongosec/downloads directory.
  createDirectoryIfNotExists(BONGOSEC_DATA_DOWNLOADS_DIRECTORY_PATH);
  // Create <PLUGIN_PLATFORM_PATH>/data/bongosec/downloads/reports directory.
  createDirectoryIfNotExists(BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH);
  // Create report files
  [
    { name: md5('admin'), files: ['bongosec-module-overview-general-1234.pdf'] },
    {
      name: md5('../../etc'),
      files: ['bongosec-module-overview-general-1234.pdf'],
    },
  ].forEach(({ name, files }) => {
    createDirectoryIfNotExists(
      path.join(BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, name),
    );
  ...
  // Register routes
  BongosecReportingRoutes(router);
...
afterAll(async () => {
  // Remove <PLUGIN_PLATFORM_PATH>/data/bongosec directory.
  execSync(`rm -rf ${BONGOSEC_DATA_ABSOLUTE_PATH}`);
...
describe('[endpoint][security] GET /reports/{name} - Parameters validation', () => {
  it.each`
    ...
    ${'Get report by filename'} | ${'admin'}     | ${'bongosec-module-overview-general-1234.pdf'}               | ${200}             | ${null}
    ${'Invalid parameters'}     | ${'admin'}     | ${'..%2fbongosec-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'}     | ${'admin'}     | ${'custom..%2fbongosec-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}        | ${'admin'}     | ${'../custom..%2fbongosec-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
    ${'Get report by filename'} | ${'../../etc'} | ${'bongosec-module-overview-general-1234.pdf'}               | ${200}             | ${null}
    ${'Invalid parameters'}     | ${'../../etc'} | ${'..%2fbongosec-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'}     | ${'../../etc'} | ${'custom..%2fbongosec-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}        | ${'../../etc'} | ${'../custom..%2fbongosec-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
  `(
    ...
    async ({
      ...
      const response = await supertest(innerServer.listener)
        ...
        .send({
          ...
          tables: [],
          section: 'overview',
          indexPatternTitle: 'bongosec-alerts-*',
          apiId: 'default',
        })
        .expect(responseStatusCode);
      if (responseBodyMessage) {
...
describe('[endpoint][security] DELETE /reports/{name} - Parameters validation', () => {
  it.each`
    testTitle               | username       | filename                                               | responseStatusCode | responseBodyMessage
    ${'Delete report file'} | ${'admin'}     | ${'bongosec-module-overview-general-1234.pdf'}            | ${200}             | ${null}
    ${'Invalid parameters'} | ${'admin'}     | ${'..%2fbongosec-module-overview-general-1234.pdf'}       | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'} | ${'admin'}     | ${'custom..%2fbongosec-module-overview-general-1234.pdf'} | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}    | ${'admin'}     | ${'../bongosec-module-overview-general-1234.pdf'}         | ${404}             | ${/Not Found/}
    ${'Delete report file'} | ${'../../etc'} | ${'bongosec-module-overview-general-1234.pdf'}            | ${200}             | ${null}
    ${'Invalid parameters'} | ${'../../etc'} | ${'..%2fbongosec-module-overview-general-1234.pdf'}       | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'} | ${'../../etc'} | ${'custom..%2fbongosec-module-overview-general-1234.pdf'} | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}    | ${'../../etc'} | ${'../bongosec-module-overview-general-1234.pdf'}         | ${404}             | ${/Not Found/}
  `(
