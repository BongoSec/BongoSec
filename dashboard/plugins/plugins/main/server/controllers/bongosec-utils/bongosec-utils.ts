...
export class BongosecUtilsCtrl {
  /**
   * Constructor
   * @param {*} server
   */
  ...
  async getConfiguration(
    ...
    try {
      context.bongosec.logger.debug('Getting configuration');
      const configuration = await context.bongosec_core.configuration.get();
  ...
  updateConfiguration = compose(
    ...
    async (
      ...
      const { requirements, update: updatedConfiguration } =
        await context.bongosec_core.configuration.set(updatedSettings);
      context.bongosec.logger.debug('Configuration updated');
  ...
  uploadFile = compose(
    ...
    async (
      ...
      const pluginSetting = context.bongosec_core.configuration._settings.get(key);
      ...
      context.bongosec.logger.debug(`Directory: ${targetDirectory}`);
      ...
      context.bongosec.logger.debug(`Storing file on : ${files.join(', ')}`);
      ...
      const { requirements, update } =
        await context.bongosec_core.configuration.set(updatedConfiguration);
  ...
  deleteFile = compose(
    ...
    async (
      ...
      // Update the setting in the configuration cache
      const { requirements, update } =
        await context.bongosec_core.configuration.clear(key);
