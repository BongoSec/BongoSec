...
import { BongosecApiCtrl } from '../controllers';
import { schema } from '@osd/config-schema';

export function BongosecApiRoutes(router: IRouter) {
  const ctrl = new BongosecApiCtrl();
  ...
  // Check if credentials on POST connect to Bongosec API. Not storing them!
  // Returns if the bongosec-api configuration received in the POST body will work
  router.post(
    {
      path: '/api/check-api',
      validate: {
  ...
  // Returns data from the Bongosec API on CSV readable format
  router.post(
    {
      path: '/api/csv',
      validate: {
