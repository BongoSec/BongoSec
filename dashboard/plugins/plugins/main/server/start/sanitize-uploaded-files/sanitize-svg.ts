...
export default async function sanitizeUploadedSVG(context) {
  ...
  const createLog = (level: string) => message => {
    context.bongosec.logger[level](`sanitize:sanitizeUploadedSVG: ${message}`);
  };
  ...
  try {
    ...
    logosSettingKeys.forEach(async logoKey => {
      const logoSetting =
        context.bongosec_core.configuration._settings.get(logoKey);
      ...
      if (originalSVGString.replace(/\s/g, '') != cleanSVG.replace(/\s/g, '')) {
        ...
        await context.bongosec_core.configuration.set({
