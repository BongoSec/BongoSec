/*
 * Bongosec app - Module for agent info fetching functions
...
import {
  BONGOSEC_MONITORING_DEFAULT_CRON_FREQ,
  BONGOSEC_MONITORING_TEMPLATE_NAME,
...
async function initMonitoringConfiguration(context) {
  try {
    context.bongosec.logger.debug('Reading configuration');
    const appConfig = await context.bongosec_core.configuration.get();
    MONITORING_ENABLED =
      (appConfig['bongosec.monitoring.enabled'] &&
        appConfig['bongosec.monitoring.enabled'] !== 'worker') ||
      appConfig['bongosec.monitoring.enabled'];
    MONITORING_FREQUENCY = appConfig['bongosec.monitoring.frequency'];
    try {
      MONITORING_CRON_FREQ = parseCron(MONITORING_FREQUENCY);
    } catch (error) {
      context.bongosec.logger.warn(
        `Using default value ${BONGOSEC_MONITORING_DEFAULT_CRON_FREQ} due to: ${
          error.message || error
        }`,
      );
      MONITORING_CRON_FREQ = BONGOSEC_MONITORING_DEFAULT_CRON_FREQ;
    }
    MONITORING_CREATION = appConfig['bongosec.monitoring.creation'];

    MONITORING_INDEX_PATTERN = appConfig['bongosec.monitoring.pattern'];
    ...
    context.bongosec.logger.debug(
      `bongosec.monitoring.enabled: ${MONITORING_ENABLED}`,
    );

    context.bongosec.logger.debug(
      `bongosec.monitoring.frequency: ${MONITORING_FREQUENCY} (${MONITORING_CRON_FREQ})`,
    );

    context.bongosec.logger.debug(
      `bongosec.monitoring.creation: ${MONITORING_CREATION}`,
    );

    context.bongosec.logger.debug(
      `bongosec.monitoring.pattern: ${MONITORING_INDEX_PATTERN} (index prefix: ${MONITORING_INDEX_PREFIX})`,
    );
  } catch (error) {
    context.bongosec.logger.error(error.message);
  }
}
...
/**
 * Verify bongosec-agent template
 */
async function checkTemplate(context) {
  try {
    try {
      context.bongosec.logger.debug(
        `Getting the ${BONGOSEC_MONITORING_TEMPLATE_NAME} template`,
      );
      // Check if the template already exists
      const currentTemplate =
        await context.core.opensearch.client.asInternalUser.indices.getTemplate(
          {
            name: BONGOSEC_MONITORING_TEMPLATE_NAME,
          },
        );
      // Copy already created index patterns
      monitoringTemplate.index_patterns =
        currentTemplate.body[BONGOSEC_MONITORING_TEMPLATE_NAME].index_patterns;
    } catch (error) {
      // Init with the default index pattern
      monitoringTemplate.index_patterns = [
        await context.bongosec_core.configuration.get('bongosec.monitoring.pattern'),
      ];
    }
    ...
    context.bongosec.logger.debug(
      `Updating the ${BONGOSEC_MONITORING_TEMPLATE_NAME} template`,
    );
    await context.core.opensearch.client.asInternalUser.indices.putTemplate({
      name: BONGOSEC_MONITORING_TEMPLATE_NAME,
      body: monitoringTemplate,
    });
    context.bongosec.logger.info(
      `Updated the ${BONGOSEC_MONITORING_TEMPLATE_NAME} template`,
    );
  } catch (error) {
    const errorMessage = `Something went wrong updating the ${BONGOSEC_MONITORING_TEMPLATE_NAME} template ${
      error.message || error
    }`;
    context.bongosec.logger.error(errorMessage);
    throw error;
  ...
  try {
    await tryCatchForIndexPermissionError(monitoringIndexName)(async () => {
      ...
      if (!exists.body) {
        ...
        context.bongosec.logger.debug(`The ${monitoringIndexName} index exists`);
      }
      ...
      const indexConfiguration = {
        settings: {
          index: {
            number_of_shards: appConfig['bongosec.monitoring.shards'],
            number_of_replicas: appConfig['bongosec.monitoring.replicas'],
          },
        },
      };
 ...
 * @param {String} indexName The name for the index (e.g. daily: bongosec-monitoring-YYYY.MM.DD)
 * @param {*} data
 */
...
/**
 * Create the bongosec-monitoring index
...
async function createIndex(context, indexName: string) {
  try {
    ...
    context.bongosec.logger.debug(`Creating ${indexName} index`);
    ...
    context.bongosec.logger.info(`${indexName} index created`);
  } catch (error) {
    context.bongosec.logger.error(
...
async function checkPluginPlatformStatus(context) {
  try {
    context.bongosec.logger.debug('Waiting for platform servers to be ready...');
...
async function cronTask(context) {
  try {
    ...
    const apiHosts = await context.bongosec_core.manageHosts.getEntries({
      excludePassword: true,
    });

    if (!apiHosts.length) {
      context.bongosec.logger.warn('There are no API host entries. Skip.');
      return;
    }
...
async function getApiInfo(context, apiHost) {
  try {
    context.bongosec.logger.debug(`Getting API info for ${apiHost.id}`);
...
async function fetchAllAgentsFromApiHost(context, apiHost) {
  let agents = [];
  try {
    context.bongosec.logger.debug(`Getting all agents from ApiID: ${apiHost.id}`);
    ...
    while (agents.length < agentsCount && payload.offset < agentsCount) {
      try {
        ...
        TODO: Improve the performance of request with:
          - Reduce the number of requests to the Bongosec API
...
export async function jobMonitoringRun(context) {
  context.bongosec.logger.debug('Task:Monitoring initializing');
  // Init the monitoring variables
  await initMonitoringConfiguration(context);
  // Check Kibana index and if it is prepared, start the initialization of Bongosec App.
  await checkPluginPlatformStatus(context);
