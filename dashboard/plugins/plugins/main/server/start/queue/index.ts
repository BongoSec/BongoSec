/*
 * Bongosec app - Add delayed jobs to a queue.
...
import { BONGOSEC_QUEUE_CRON_FREQ } from '../../../common/constants';
...
async function executePendingJobs(context: any) {
  try {
    ...
    context.bongosec.logger.debug(`Pending jobs: ${pendingJobs.length}`);
...
export function jobQueueRun(context) {
  cron.schedule(BONGOSEC_QUEUE_CRON_FREQ, async () => {
    try {
