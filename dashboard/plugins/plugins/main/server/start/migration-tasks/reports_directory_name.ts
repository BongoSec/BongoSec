...
import { BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH } from '../../../common/constants';
...
export default function migrateReportsDirectoryName(context) {
  ...
  const createLog = (level: string) => message => {
    context.bongosec.logger[level](`migration:reportsDirectoryName: ${message}`);
  };
  ...
  try {
    ...
    if (!fs.existsSync(BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH)) {
      logger.debug(
    ...
    logger.debug(
      `Reading reports directory: ${BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH}`,
    );
    fs.readdirSync(BONGOSEC_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, {
      withFileTypes: true,
    }).forEach(fileDirent => {
