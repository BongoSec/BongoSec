/*
 * Bongosec app - Module for app initialization
...
import {
  BONGOSEC_PLUGIN_PLATFORM_TEMPLATE_NAME,
...
export function jobInitializeRun(context) {
  ...
  context.bongosec.logger.info(`App revision: ${packageJSON.revision}`);

  try {
    // RAM in MB
    context.bongosec.logger.debug('Getting the total RAM memory');
    const ram = Math.ceil(totalmem() / 1024 / 1024);
    context.bongosec.logger.info(`Total RAM: ${ram}MB`);
  } catch (error) {
  ...
  const createKibanaTemplate = () => {
    ...
    try {
      ...
      context.bongosec.logger.error('Exception: ' + error.message);
    }

    return context.core.opensearch.client.asInternalUser.indices.putTemplate({
      name: BONGOSEC_PLUGIN_PLATFORM_TEMPLATE_NAME,
      order: 0,
      create: true,
  ...
  const createEmptyKibanaIndex = async () => {
    try {
      context.bongosec.logger.debug(`Creating ${PLUGIN_PLATFORM_INDEX} index.`);
      await context.core.opensearch.client.asInternalUser.indices.create({
        index: PLUGIN_PLATFORM_INDEX,
      });
      context.bongosec.logger.info(`${PLUGIN_PLATFORM_INDEX} index created`);
    } catch (error) {
      throw new Error(
  ...
  const fixKibanaTemplate = async () => {
    try {
      context.bongosec.logger.debug(`Fixing ${PLUGIN_PLATFORM_INDEX} template`);
      await createKibanaTemplate();
      context.bongosec.logger.info(`${PLUGIN_PLATFORM_INDEX} template created`);
  ...
  const getTemplateByName = async () => {
    try {
      context.bongosec.logger.debug(
        `Getting ${BONGOSEC_PLUGIN_PLATFORM_TEMPLATE_NAME} template`,
      );
  ...
  const checkKibanaStatus = async () => {
    try {
      ...
      if (response.body) {
        context.bongosec.logger.debug(`${PLUGIN_PLATFORM_INDEX} index exist`);
      } else {
        context.bongosec.logger.debug(
          `${PLUGIN_PLATFORM_INDEX} index does not exist`,
        );
        // No Kibana index created...
        context.bongosec.logger.info(`${PLUGIN_PLATFORM_INDEX} index not found`);
