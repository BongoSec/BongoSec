...
import {
  BONGOSEC_STATISTICS_DEFAULT_INDICES_SHARDS,
  BONGOSEC_STATISTICS_DEFAULT_INDICES_REPLICAS,
...
export class SaveDocument {
  ...
  async save(doc: object[], indexConfig: IIndexConfiguration) {
    ...
    try {
      ...
      this.context.bongosec.logger.debug('Bulk data');
      const response = await this.esClientInternalUser.bulk(
        createDocumentObject,
      );
      this.context.bongosec.logger.debug(
  ...
  private async checkIndexAndCreateIfNotExists(index, shards, replicas) {
    try {
      await tryCatchForIndexPermissionError(index)(async () => {
        ...
        if (!exists.body) {
          this.context.bongosec.logger.debug(`Creating ${index} index`);
          await this.esClientInternalUser.indices.create({
            ...
            body: {
              settings: {
                index: {
                  number_of_shards:
                    shards ?? BONGOSEC_STATISTICS_DEFAULT_INDICES_SHARDS,
                  number_of_replicas:
                    replicas ?? BONGOSEC_STATISTICS_DEFAULT_INDICES_REPLICAS,
                },
              },
            },
          });
          this.context.bongosec.logger.info(`${index} index created`);
        }
      })();
    } catch (error) {
  ...
  private async addIndexPrefix(index): string {
    const prefix = await this.context.bongosec_core.configuration.get(
      'cron.prefix',
    );
    return `${prefix}-${index}`;
  }
