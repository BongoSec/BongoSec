...
import { BONGOSEC_STATISTICS_TEMPLATE_NAME } from '../../../common/constants';
...
/**
 * Verify bongosec-statistics template
 */
const checkTemplate = async function (context) {
  try {
    ...
    try {
      ...
      context.bongosec.logger.debug(
        `Getting the ${BONGOSEC_STATISTICS_TEMPLATE_NAME} template`,
      );
      const currentTemplate =
        await context.core.opensearch.client.asInternalUser.indices.getTemplate(
          {
            name: BONGOSEC_STATISTICS_TEMPLATE_NAME,
          },
        );
      // Copy already created index patterns
      statisticsTemplate.index_patterns =
        currentTemplate.body[BONGOSEC_STATISTICS_TEMPLATE_NAME].index_patterns;
    } catch (error) {
    ...
    context.bongosec.logger.debug(
      `Updating the ${BONGOSEC_STATISTICS_TEMPLATE_NAME} template`,
    );
    ...
    context.bongosec.logger.info(
      `Updated the ${BONGOSEC_STATISTICS_TEMPLATE_NAME} template`,
    );
  } catch (error) {
    context.bongosec.logger.error(
      `Something went wrong updating the ${BONGOSEC_STATISTICS_TEMPLATE_NAME} template ${
        error.message || error
      }`,
    );
    throw error;
