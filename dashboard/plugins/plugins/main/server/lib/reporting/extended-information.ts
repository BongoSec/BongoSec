  ...
  try {
    ...
    if (agentsData.length) {
      ...
      printer.addSimpleTable({
        ...
        items: agentsData
          .filter(agent => agent) // Remove undefined agents when Bongosec API no longer finds and agentID
          .map(agent => {
            return {
              ...agent,
              os:
...
export async function extendedInformation(
  ...
  try {
    ...
    const agents = await context.bongosec.api.client.asCurrentUser.request(
      'GET',
      '/agents',
      { params: { limit: 1 } },
      { apiHostID: apiId },
