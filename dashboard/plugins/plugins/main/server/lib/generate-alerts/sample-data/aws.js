...
module.exports.instanceDetails = [
  ...
  },
  {
    ...
    imageDescription:
      'Canonical, Ubuntu, 18.04 LTS, UNSUPPORTED daily arm64 bionic image build on 2019-02-12',
    productCodes: {
      productCodeId: 'zud1u4kjmxu2j2jf0n36bqa',
      productCodeType: 'marketplace',
    },
    iamInstanceProfile: {
      id: Random.createHash(21, Random.ALPHABET),
      arn: 'arn:aws:iam::{data.aws.accountId}:instance-profile/opsworks-web-production',
    },
...
module.exports.guarddutyPortProbe = {
  ...
  rule: {
    ...
    level: 3,
    description:
      'AWS GuardDuty: PORT_PROBE - Unprotected port on EC2 instance {data.aws.resource.instanceDetails.instanceId} is being probed. [IP: {data.aws.service.action.portProbeAction.portProbeDetails.remoteIpDetails.ipAddressV4}] [Port: {data.aws.service.action.portProbeAction.portProbeDetails.localPortDetails.port}]',
    groups: ['amazon', 'aws', 'aws_guardduty'],
    id: '80305',
  },
  location: 'Bongosec-AWS',
...
module.exports.apiCall = {
  data: {
    aws: {
      ...
      log_info: {
        s3bucket: 'bongosec-aws-wodle',
        log_file:
          'guardduty/2020/04/22/10/firehose_guardduty-1-2020-04-22-10-36-02-d67c99dc-800a-486a-8339-59a7a8254ab2.zip',
      },
      description:
        'Unusual console login seen from principal {data.aws.resource.accessKeyDetails.userName}. Login activity using this client application, from the specific location has not been seen before from this principal.',
      source: 'guardduty',
      type: 'UnauthorizedAccess:IAMUser/ConsoleLogin',
      title:
        'Unusual console login was seen for principal {data.aws.resource.accessKeyDetails.userName}.',
  ...
  rule: {
    ...
    level: 6,
    description:
      'AWS GuardDuty: AWS_API_CALL - Unusual console login was seen for principal {data.aws.resource.accessKeyDetails.userName}.',
    groups: ['amazon', 'aws', 'aws_guardduty'],
    id: '80302',
  },
  location: 'Bongosec-AWS',
...
module.exports.networkConnection = {
  data: {
    ...
    aws: {
      ...
      description:
        'EC2 instance {data.aws.resource.instanceDetails.instanceId} is communicating with a remote host on an unusual server port 5060.',
      source: 'guardduty',
      type: 'Behavior:EC2/NetworkPortUnusual',
      title:
        'Unusual outbound communication seen from EC2 instance {data.aws.resource.instanceDetails.instanceId} on server port 5060.',
  ...
  rule: {
    ...
    level: 6,
    description:
      'AWS GuardDuty: NETWORK_CONNECTION - Unusual outbound communication seen from EC2 instance {data.aws.resource.instanceDetails.instanceId} on server port 5060.',
    groups: ['amazon', 'aws', 'aws_guardduty'],
    id: '80302',
  },
  location: 'Bongosec-AWS',
...
module.exports.iamPolicyGrantGlobal = {
  data: {
    aws: {
      severity: 'CRITICAL',
      actor: 'resources.bongosec.sample.com',
      summary: {
        Timestamps: '2020-04-22T00:11:44.617597Z,',
        Description:
          'S3 Bucket uses IAM policy to grant read rights to Everyone. Your IAM policy contains a clause that effectively grants read access to any user. Please audit this bucket, and data contained within and confirm that this is intentional. If intentional, please use the alert whitelist feature to prevent future alerts',
        Bucket: 'resources.bongosec.sample.com,',
        'Record Count': '1',
        'Event Count': '1',
      ...
      'alert-arn':
        'arn:aws:macie:{data.aws.region}:166157441400:trigger/b731d9ffb1fe61508d4a478c92efa666/alert/d78f0fd0a55ad458799e4c1fb6a0ed',
    },
  },
  rule: {
    mail: true,
    level: 12,
    description:
      'AWS Macie CRITICAL: S3 Bucket IAM policy grants global read rights - S3 Bucket uses IAM policy to grant read rights to Everyone. Your IAM policy contains a clause that effectively grants read access to any user. Please audit this bucket, and data contained within and confirm that this is intentional. If intentional, please use the alert whitelist feature to prevent future alerts',
