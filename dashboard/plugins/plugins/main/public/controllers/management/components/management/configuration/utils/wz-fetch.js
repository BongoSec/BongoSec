/*
 * Bongosec app - Fetch API function and utils.
    ...
    for (const section of sections) {
      ...
      try {
        ...
        if (agentId === '000') {
           ...
           * because the /agents/{agent_id}/config/logcollector/socket response has property named "target" instead of "socket" in versions before Bongosec 4.9.0
 ...
 * @param {*} location
 * @param updateBongosecNotReadyYet
...
export const handleError = async (
  ...
  try {
    if (messageIsString && message.includes('ERROR3099')) {
      updateBongosecNotReadyYet('Server not ready yet.');
      await makePing(updateBongosecNotReadyYet, isCluster);
      return;
    }
...
export const checkDaemons = async isCluster => {
  try {
    ...
    const isValid =
      execd && modulesd && bongosecdb && (isCluster ? clusterd : true);
...
export const makePing = async (
  ...
  try {
    ...
    return Promise.resolve('Bongosec is ready');
  } catch (error) {
...
/**
 * Get Cluster status from Bongosec API
 * @returns {Promise}
 */
...
export const restartNodeSelected = async (
  ...
  try {
    ...
    return await makePing(updateBongosecNotReadyYet, isCluster);
  } catch (error) {
    throw error;
  }
};
 ...
 */
export const restartClusterOrManager = async updateBongosecNotReadyYet => {
  try {
    const clusterStatus = (((await clusterReq()) || {}).data || {}).data || {};
    const isCluster =
      clusterStatus.enabled === 'yes' && clusterStatus.running === 'yes';
