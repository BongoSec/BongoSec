/*
 * Bongosec app - React hook for get query of plugin platform searchBar
...
import bongosecPermissions from '../../common/api-info/security-actions';
...
export class WzUserPermissions{
  ...
  static checkMissingUserPermissions = (requiredPermissions, userPermissions) => {
    const filtered = requiredPermissions.filter(permission => {
      ...
      let actionResource = (typeof permission === 'string' && bongosecPermissions[actionName].resources.length === 1) ? (bongosecPermissions[actionName].resources[0] + ':*') : permission.resource;
      ...
      const existInBongosecPermissions = (userResource) => {
        return !!bongosecPermissions[actionName].resources.find(function (resource) {
          return (
            resource ===
            userResource
              .split('&')
      ...
      const notAllowInBongosecPermissions = (userResource) => {
        if (userResource !== RESOURCE_ANY) {
          return existInBongosecPermissions(userResource)
      ...
      return userPermissions[actionName][actionResource]
        ? notAllowInBongosecPermissions(actionResource)
