...
import { IBongosecError, IBongosecErrorConstructor } from '../types';
...
interface IUrlRequestedTypes {
  [key: string]: IBongosecErrorConstructor;
}
...
export class ErrorHandler {
   ...
   * @returns 
   */
  static handleError(error: Error, customLogOptions?: ILogCustomOptions): Error | IBongosecError {
    if (!error) {
   ...
   * @returns
   */
  static createError(error: Error | AxiosError | string): IBongosecError | Error {
    if (!error) {
      throw Error('Error must be defined');
    }
  ...
  ): IBongosecErrorConstructor | null {
    let errorType = null;
   ...
   * @returns 
   */
  static isHttpError(error: Error | IBongosecError | AxiosError | OpenSearchDashboardsResponse): boolean {
   ...
   * @returns 
   */
  private static getErrorTypeByConfig(error: AxiosError): IBongosecErrorConstructor | null {
    const requestedUrlbyErrorTypes: IUrlRequestedTypes = {
      '/api': BongosecApiError,
      '/reports': BongosecReportingError,
   ...
   * @param error
   */
  private static logError(error: Error | IBongosecError, customLogOptions?: ILogCustomOptions) {
    ...
    if (error instanceof BongosecError) {
      defaultErrorLog = {
        ...error.logOptions,
        ...{
          error: {
