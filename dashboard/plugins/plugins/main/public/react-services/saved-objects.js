/*
 * Bongosec app - Saved Objects management service
...
import {
  ...
  BONGOSEC_INDEX_TYPE_ALERTS,
  BONGOSEC_INDEX_TYPE_MONITORING,
  BONGOSEC_INDEX_TYPE_STATISTICS,
...
export class SavedObject {
   ...
   */
  static async getListOfBongosecValidIndexPatterns(defaultIndexPatterns, where) {
    let result = [];
  ...
  /**
   * Creates the 'bongosec-alerts-*'  index pattern
   */
  static async createBongosecIndexPattern(pattern) {
    try {
      const fields = await SavedObject.getIndicesFields(
        pattern,
        BONGOSEC_INDEX_TYPE_ALERTS,
  ...
  static getIndicesFields = async (pattern, indexType) => {
    try {
      ...
      switch (indexType) {
        case BONGOSEC_INDEX_TYPE_MONITORING:
          return FieldsMonitoring;
        case BONGOSEC_INDEX_TYPE_STATISTICS:
          return FieldsStatistics;
        case BONGOSEC_INDEX_TYPE_ALERTS:
          return KnownFields;
        default:
   ...
   * It is usefull to validate if the endpoint works as expected. Related issue: https://github.com/bongosec/bongosec-dashboard-plugins/issues/4293
...
Restart the ${PLUGIN_PLATFORM_NAME} service to initialize the index. More information in troubleshooting guide: ${webDocumentationLink(
        'user-manual/bongosec-dashboard/troubleshooting.html#saved-object-for-index-pattern-not-found',
      )}.`);
    }
  }
}
