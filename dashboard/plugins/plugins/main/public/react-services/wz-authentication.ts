/*
 * Bongosec app - Authentication service for Bongosec
...
/**
 * Bongosec user authentication class
 */
export class WzAuthentication {
  ...
  static async refresh(force = false) {
    try {
      ...
      store.dispatch(
        updateUserAccount(
          getBongosecCorePlugin().dashboardSecurity.getAccountFromJWTAPIDecodedToken(
            jwtPayload,
          ),
        ),
      );
  ...
  /**
   * Sends a request to the Bongosec's API to delete the user's token.
   *
   * @returns {Object}
   */
