/*
 * Bongosec app - Reporting service
...
export class ReportingService {
  ...
  removeAgentStatusVis(idArray) {
    const monitoringEnabled =
      this.bongosecConfig.getConfig()['bongosec.monitoring.enabled'];
    if (!monitoringEnabled) {
      const visArray = idArray.filter(vis => {
        return vis !== 'Bongosec-App-Overview-General-Agents-status';
      });
      return visArray;
    }
    return idArray;
    ...
    return await Promise.all(
      Array.from(domVisualizations).map(async node => {
        return {
          ...
          See https://github.com/bongosec/bongosec-dashboard-plugins/issues/6900#issuecomment-2275495245
          */
