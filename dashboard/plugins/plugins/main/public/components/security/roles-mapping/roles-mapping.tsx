...
import { BongosecSecurity } from '../../../factories/bongosec-security';
...
export const RolesMapping = withUserAuthorizationPrompt([
  ...
  const getInternalUsers = async () => {
    try {
      const bongosecSecurity = new BongosecSecurity();
      const users = await bongosecSecurity.security.getUsers();
      const _users = users
        .map((item, idx) => {
          return {
            id: idx,
