/*
 * Bongosec app - React component building the API entries table.
 *
...
import {
  getBongosecCheckUpdatesPlugin,
  getBongosecCorePlugin,
...
import { BongosecConfig } from '../../../react-services/bongosec-config';
...
export const ApiTable = compose(withErrorBoundary)(
  class ApiTable extends Component {
    ...
    async getApisAvailableUpdates(queryApi = false, forceQuery = false) {
      try {
        ...
        const availableUpdates =
          await getBongosecCheckUpdatesPlugin().getAvailableUpdates(
            queryApi,
            forceQuery,
          );
    ...
    componentDidMount() {
      this.refresh();
      this.bongosecConfig = new BongosecConfig().getConfig();
      this.isUpdatesEnabled = !this.bongosecConfig?.['bongosec.updates.disabled'];
    ...
    render() {
      const { DismissNotificationCheck } = getBongosecCheckUpdatesPlugin();
        ...
        },
        {
          ...
          render: value => {
            return value ===
              ...
              getBongosecCorePlugin().API_USER_STATUS_RUN_AS.USER_NOT_ALLOWED ? (
              <EuiToolTip
                position='top'
                        ...
                        <EuiToolTip
                          ...
                          content={
                            this.state.availableUpdates?.last_check_date
                              ? getBongosecCorePlugin().utils.formatUIDate(
                                  this.state.availableUpdates.last_check_date,
                                )
                              : '-'
                          }
                                    ...
                                    {[
                                      {
                                        label: 'For Systemd',
                                        command:
                                          'sudo systemctl status bongosec-manager',
                                      },
                                      {
                                        label: 'For SysV Init',
                                        command:
                                          'sudo service bongosec-manager status',
                                      },
                                    ].map(({ label, command }) => (
                                      <>
