/*
 * Bongosec app - React component building the configuration component.
 *
...
const WzConfigurationSettingsProvider = props => {
  ...
  const {
    ...
    transformPluginSettingsToFormFields(
      ...
      Object.fromEntries(
        getBongosecCorePlugin().configuration._settings.entries(),
      ),
    ),
  );
  ...
  const visibleSettings = Object.entries(fields).map(
    ([fieldKey, fieldForm]) => {
      const pluginSetting =
        getBongosecCorePlugin().configuration._settings.get(fieldKey);
      const _categoryMeta = getBongosecCorePlugin().configuration._categories.get(
  ...
  const visibleCategories = groupSettingsByCategory(
    ...
    Array.from(getBongosecCorePlugin().configuration._categories.values()),
  );
    ...
    try {
      const settingsToUpdate = Object.entries(changed).reduce(
        (accum, [pluginSettingKey, currentValue]) => {
          const pluginSetting =
            getBongosecCorePlugin().configuration._settings.get(pluginSettingKey);
          ...
          {visibleCategories &&
            visibleCategories.map(({ category, settings }) => {
              const { description, documentationLink, title } = Array.from(
                getBongosecCorePlugin().configuration._categories.values(),
