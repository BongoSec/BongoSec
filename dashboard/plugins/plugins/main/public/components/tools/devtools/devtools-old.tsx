...
function calculateWhichGroup(editor, firstTime = false, groups = []) {
  try {
    ...
    try {
      ...
      $('#play_button').hide();
      $('#bongosec_dev_tools_documentation').hide();
      return null;
    }

    if (!$('#play_button').is(':visible')) $('#play_button').show();
    if (!$('#bongosec_dev_tools_documentation').is(':visible'))
      $('#bongosec_dev_tools_documentation').show();
    const currentPlayButton = $('#play_button').offset();
    $('#play_button').offset({
      top: cords.top /*+ 2*/,
      left: currentPlayButton.left,
    });
    $('#bongosec_dev_tools_documentation').offset({
      top: cords.top /*+ 2*/,
    });
    ...
    if (desiredGroup[0]) {
      ...
      if (apiEndpoint && apiEndpoint.documentation) {
        $('#bongosec_dev_tools_documentation')
          .attr('href', apiEndpoint.documentation)
          .show();
      } else {
        $('#bongosec_dev_tools_documentation').attr('href', '').hide();
      }
    }
        ...
        >
          <span className='wz-headline-title wz-dev-title'>
            ...
            <a
              ...
              id='bongosec_dev_tools_documentation'
            ></a>
          </span>
