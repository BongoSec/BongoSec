...
export const osdfucatePasswordInCommand = (password: string, commandText: string, os: tOperatingSystem['name']): string => {
  ...
  switch (osName){
    case 'macos':
    {
      const regex = /BONGOSEC_REGISTRATION_PASSWORD=\'((?:\\'|[^']|[\"'])*)'/g;
    ...
    case 'windows':
      {
        const replacedString = command.replace(
          `BONGOSEC_REGISTRATION_PASSWORD=\'${scapeSpecialCharsForWindows(password)}'`,
          () => {
            return `BONGOSEC_REGISTRATION_PASSWORD=\'${'*'.repeat(scapeSpecialCharsForWindows(password).length)}\'`;
          }
        );
        return replacedString;
      }
    ...
    {
      const replacedString = command.replace(
        `BONGOSEC_REGISTRATION_PASSWORD=\$'${scapeSpecialCharsForLinux(password)}'`,
        () => {
          return `BONGOSEC_REGISTRATION_PASSWORD=\$\'${'*'.repeat(scapeSpecialCharsForLinux(password).length)}\'`;
        }
      );
      return replacedString;
    }
