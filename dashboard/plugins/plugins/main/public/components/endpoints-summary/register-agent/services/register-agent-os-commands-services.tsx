...
export const getAllOptionals = (
  ...
  const paramNameOrderList: (keyof IOptionalParameters<tOptionalParameters>)[] =
    ['serverAddress', 'bongosecPassword', 'agentGroups', 'agentName', 'protocol'];
...
export const getDEBAMD64InstallCommand = (
  ...
  const { optionals, urlPackage, bongosecVersion } = props;
  const packageName = `bongosec-agent_${bongosecVersion}-1_amd64.deb`;
...
export const getDEBARM64InstallCommand = (
  ...
  const { optionals, urlPackage, bongosecVersion } = props;
  const packageName = `bongosec-agent_${bongosecVersion}-1_arm64.deb`;
  return `wget ${urlPackage} && sudo ${
    optionals && getAllOptionals(optionals)
  }dpkg -i ./${packageName}`;
};
...
export const getRPMAMD64InstallCommand = (
  ...
  const { optionals, urlPackage, bongosecVersion, architecture } = props;
  const packageName = `bongosec-agent-${bongosecVersion}-1.x86_64.rpm`;
...
export const getRPMARM64InstallCommand = (
  ...
  const { optionals, urlPackage, bongosecVersion, architecture } = props;
  const packageName = `bongosec-agent-${bongosecVersion}-1.aarch64.rpm`;
  return `curl -o ${packageName} ${urlPackage} && sudo ${
    optionals && getAllOptionals(optionals)
  }rpm -ihv ${packageName}`;
};
...
export const getWindowsInstallCommand = (
  ...
  return `Invoke-WebRequest -Uri ${urlPackage} -OutFile \$env:tmp\\bongosec-agent; msiexec.exe /i \$env:tmp\\bongosec-agent /q ${
...
export const getMacOsInstallCommand = (
  ...
  if (optionalsForCommand?.bongosecPassword) {
    /**
    ...
    */
    const scapedPasswordLength = JSON.stringify(
      optionalsForCommand?.bongosecPassword,
    ).length;
    // We need to remove the " added by JSON.stringify
    optionalsForCommand.bongosecPassword = `${JSON.stringify(
      optionalsForCommand?.bongosecPassword,
  ...
  const macOSInstallationSetEnvVariablesScript = macOSInstallationOptions
    ? `echo "${macOSInstallationOptions}" > /tmp/bongosec_envs && `
    : ``;

  // Merge environment variables with installation script
  const macOSInstallationScript = `curl -so bongosec-agent.pkg ${urlPackage} && ${macOSInstallationSetEnvVariablesScript}sudo installer -pkg ./bongosec-agent.pkg -target /`;
...
export const getMacosStartCommand = (
  ...
  return `sudo /Library/Ossec/bin/bongosec-control start`;
};
