/*
 * Bongosec app - Check alerts index pattern service
 *
...
import {
  ...
  BONGOSEC_PLUGIN_PLATFORM_SETTING_MAX_BUCKETS,
  BONGOSEC_PLUGIN_PLATFORM_SETTING_METAFIELDS,
  BONGOSEC_PLUGIN_PLATFORM_SETTING_TIME_FILTER,
...
const checkTimeFilter = decoratorHealthCheckRunCheckEnabled('timeFilter',
  checkPluginPlatformSettings(
    PLUGIN_PLATFORM_SETTING_NAME_TIME_FILTER,
    JSON.stringify(BONGOSEC_PLUGIN_PLATFORM_SETTING_TIME_FILTER),
    (checkLogger: CheckLogger, options: { defaultAppValue: any }) => {
      getDataPlugin().query.timefilter.timefilter.setTime(BONGOSEC_PLUGIN_PLATFORM_SETTING_TIME_FILTER)
