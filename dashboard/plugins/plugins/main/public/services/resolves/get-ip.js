/*
 * Bongosec app - Module to fetch index patterns
...
export async function getIp() {
  const checkBongosecPatterns = async indexPatterns => {
    const configuration = await getWzConfig(new BongosecConfig());
    const STATISTICS_PATTERN_IDENTIFIER =
      StatisticsDataSource.getIdentifierDataSourcePattern();
    const bongosecPatterns = [
      `${configuration['bongosec.monitoring.pattern']}`,
      STATISTICS_PATTERN_IDENTIFIER,
    ];
    return bongosecPatterns.every(pattern => {
  ...
  const buildSavedObjectsClient = async () => {
    try {
      ...
      if (
        ...
        !(await checkBongosecPatterns(savedObjects))
      ) {
        if (
      ...
      const onlyBongosecAlerts = savedObjects.filter(
        element => element.id === currentPattern,
      );

      if (!onlyBongosecAlerts || !onlyBongosecAlerts.length) {
      ...
      const options = {
        context: `${getIp.name}.checkBongosecPatterns`,
