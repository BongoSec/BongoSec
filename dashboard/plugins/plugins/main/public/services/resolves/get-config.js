/*
 * Bongosec app - Resolve function to parse configuration file
...
export async function getWzConfig(bongosecConfig) {
  try {
    const defaultConfig = await getBongosecCorePlugin().configuration.get();
    ...
    try {
      ...
      bongosecConfig.setConfig(defaultConfig);
    } catch (error) {
      bongosecConfig.setConfig(defaultConfig);
