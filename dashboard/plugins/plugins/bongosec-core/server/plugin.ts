...
import {
  PluginSetup,
  BongosecCorePluginSetup,
  BongosecCorePluginStart,
} from './types';
...
import {
  ...
  BONGOSEC_CORE_CONFIGURATION_CACHE_SECONDS,
  BONGOSEC_DATA_CONFIG_APP_PATH,
} from '../common/constants';
import { enhanceConfiguration } from './services/enhance-configuration';

export class BongosecCorePlugin
  implements Plugin<BongosecCorePluginSetup, BongosecCorePluginStart>
{
  ...
  public async setup(
    core: CoreSetup,
    plugins: PluginSetup,
  ): Promise<BongosecCorePluginSetup> {
    this.logger.debug('bongosec_core: Setup');
    ...
    this._internal.configurationStore = new ConfigurationStore(
      ...
      {
        cache_seconds: BONGOSEC_CORE_CONFIGURATION_CACHE_SECONDS,
        file: BONGOSEC_DATA_CONFIG_APP_PATH,
      },
    );
    ...
    core.http.registerRouteHandlerContext('bongosec_core', (context, request) => {
      return {
        ...this.services,
        api: {
          client: {
  ...
  public async start(core: CoreStart): Promise<BongosecCorePluginStart> {
    this.logger.debug('bongosecCore: Started');
