...
import { BONGOSEC_ROLE_ADMINISTRATOR_ID } from '../../../common/constants';
  ...
  dashboardSecurity.isAdministratorUser = async function (context, request) {
    try {
      ...
      if (
        ...
        !decodedToken.rbac_roles.includes(BONGOSEC_ROLE_ADMINISTRATOR_ID)
      ) {
        return {
          administrator: false,
      ...
      const responseTokenIsWorking =
        await context.bongosec_core.api.client.asCurrentUser.request(
          'GET',
          '/',
          {},
          { apiHostID },
