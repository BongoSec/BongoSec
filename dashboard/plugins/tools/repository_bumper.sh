#!/bin/bash
#
# Bongosec Dashboard Plugins repository bumper (Pure Shell Version)
...
update_endpoints_json() {
  ...
  sed -i "s|documentation.bongosec.github.io/${escaped_old_version}|documentation.bongosec.github.io/${escaped_new_version}|g" "$endpoints_file" && log "Successfully updated documentation URLs in $endpoints_file" || {
...
# Function to update specFile URL in docker/imposter/bongosec-config.yml
update_imposter_config() {
  local new_version="$1"
  local imposter_config_file="${REPO_PATH}/docker/imposter/bongosec-config.yml"
  ...
  current_spec_version=$(grep -oE 'specFile: https://raw.githubusercontent.com/bongosec/bongosec/[0-9]+\.[0-9]+\.[0-9]+' "$imposter_config_file" | sed -E 's/.*bongosec\/([0-9]+\.[0-9]+\.[0-9]+)$/\1/' | head -n1)
  ...
  sed -i -E "s|(specFile: https://raw.githubusercontent.com/bongosec/bongosec/)[0-9]+\.[0-9]+\.[0-9]+(.*)|\1${new_version}\2|" "$imposter_config_file" && log "Successfully updated specFile URL in $imposter_config_file" || {
...
update_changelog() {
  ...
  local changelog_header="## Bongosec v${VERSION} - OpenSearch Dashboards ${OPENSEARCH_VERSION} - Revision "
  local changelog_header_regex="^## Bongosec v${VERSION} - OpenSearch Dashboards ${OPENSEARCH_VERSION} - Revision [0-9]+"
  ...
  if grep -qE "$changelog_header_regex" "$changelog_file"; then
    ...
    sed -i -E "s|(${changelog_header_regex})|## Bongosec v${VERSION} - OpenSearch Dashboards ${OPENSEARCH_VERSION} - Revision ${REVISION}|" "$changelog_file" &&
...
main() {
  ...
  # Update docker/imposter/bongosec-config.yml
  log "Updating docker/imposter/bongosec-config.yml..."
