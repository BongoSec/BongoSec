...
def generateRandomVulnerability():
    ...
    vulnerability['scanner'] = {'vendor':'vendor-{}'.format(random.randint(0, 9)), 'reference':'https://cti.bongosec.github.io/vulnerabilities/cves/'+vulnerability['id']}
...
def generateRandomBongosec():
    bongosec = {}
    bongosec['cluster'] = {'name':random.choice(['bongosec.manager', 'bongosec']), 'node':random.choice(['master','worker-01','worker-02','worker-03'])}
    return(bongosec)
...
def generateRandomData(number):
    for i in range(0, int(number)):
        yield{
            ...
            'bongosec':generateRandomBongosec()
        }
...
def verifySettings():
    ...
    if not verified:
        ...
        index = input("\nEnter the index name [default=bongosec-states-vulnerabilities-default]: \n")
        if index == '':
            index = 'bongosec-states-vulnerabilities-default'
