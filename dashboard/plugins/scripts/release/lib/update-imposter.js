function updateImposterSpecificationReference(configuration, logger) {
  try {
    ...
    const specificationFile = path.join(
      __dirname,
      '../../..',
      'docker/imposter/bongosec-config.yml',
    );
    ...
    // specFile: https://raw.githubusercontent.com/bongosec/bongosec/<BRANCH_VERSION>/api/api/spec/spec.yaml
    const updatedContent = content.replace(
      /specFile:\s+\S+/m,
      `specFile: https://raw.githubusercontent.com/bongosec/bongosec/${version}/api/api/spec/spec.yaml`,
    );
