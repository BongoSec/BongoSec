...
# Foundation.
Summary:     Bongosec dashboard is a user interface and visualization tool for security-related data. Documentation can be found at https://documentation.bongosec.github.io/current/getting-started/components/bongosec-dashboard.html
...
buildroot:   %{_tmppath}/%{name}-%{version}-%{release}-bongosec-dashboard-%(%{__id_u} -n)
...
%global DASHBOARD_FILE bongosec-dashboard.tar.gz
...
%description
Bongosec dashboard is a user interface and visualization tool for security-related data. This Bongosec central component enables exploring, visualizing, and analyzing the stored security alerts generated by the Bongosec server. Bongosec dashboard enables inspecting the status and managing the configurations of the Bongosec cluster and agents as well as creating and managing users and roles. In addition, it allows testing the ruleset and making calls to the Bongosec API. Documentation can be found at https://documentation.bongosec.github.io/current/getting-started/components/bongosec-dashboard.html
...
cp bongosec-dashboard-base/config/node.options %{buildroot}%{CONFIG_DIR}
cp bongosec-dashboard-base/config/opensearch_dashboards.yml %{buildroot}%{CONFIG_DIR}
cp bongosec-dashboard-base/VERSION.json %{buildroot}%{INSTALL_DIR}

mv bongosec-dashboard-base/* %{buildroot}%{INSTALL_DIR}

# Set custom welcome styles

mkdir -p %{buildroot}%{INSTALL_DIR}/config

cp %{buildroot}%{INSTALL_DIR}/etc/services/bongosec-dashboard.service %{buildroot}/etc/systemd/system/bongosec-dashboard.service
cp %{buildroot}%{INSTALL_DIR}/etc/services/default %{buildroot}/etc/default/bongosec-dashboard

chmod 640 %{buildroot}/etc/systemd/system/bongosec-dashboard.service
chmod 640 %{buildroot}/etc/default/bongosec-dashboard
...
chown root:root %{buildroot}/etc/systemd/system/bongosec-dashboard.service

find %{buildroot}%{INSTALL_DIR}/plugins/bongosec/ -exec chown %{USER}:%{GROUP} {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/bongosec/ -type f -perm 644 -exec chmod 640 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/bongosec/ -type f -perm 755 -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/bongosec/ -type d -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/bongosec/ -type f -perm 744 -exec chmod 740 {} \;

# -----------------------------------------------------------------------------

%pre
# Create the bongosec-dashboard group if it doesn't exists
if [ $1 = 1 ]; then
  ...
  # Create the bongosec-dashboard user if it doesn't exists
  if ! getent passwd %{USER} > /dev/null 2>&1; then
    useradd -g %{GROUP} -G %{USER} -d %{INSTALL_DIR}/ -r -s /sbin/nologin bongosec-dashboard
  fi
fi
# Stop the services to upgrade the package
if [ $1 = 2 ]; then
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 && systemctl is-active --quiet bongosec-dashboard > /dev/null 2>&1; then
    systemctl stop bongosec-dashboard.service > /dev/null 2>&1
    touch %{INSTALL_DIR}/bongosec-dashboard.restart
  # Check for SysV
  elif command -v service > /dev/null 2>&1 && service bongosec-dashboard status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    service bongosec-dashboard stop > /dev/null 2>&1
    touch %{INSTALL_DIR}/bongosec-dashboard.restart
  fi
fi
...
rm -f /usr/share/bongosec-dashboard/VERSION
...
if [ $1 = 0 ];then
  ...
  # Remove the bongosec-dashboard user if it exists
  if getent passwd %{USER} > /dev/null 2>&1; then
    userdel %{USER} >/dev/null 2>&1
  fi
  # Remove the bongosec-dashboard group if it exists
  if command -v getent > /dev/null 2>&1 && getent group %{GROUP} > /dev/null 2>&1; then
  ...
  # Remove /etc/bongosec-dashboard and /usr/share/bongosec-dashboard dirs
  rm -rf %{INSTALL_DIR}
  if [ -d %{PID_DIR} ]; then
    rm -rf %{PID_DIR}
  fi
...
if [ -f %{INSTALL_DIR}/bongosec-dashboard.restart ]; then
  rm -f %{INSTALL_DIR}/bongosec-dashboard.restart
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1; then
...
%config(noreplace) %attr(0750, %{USER}, %{GROUP}) "/etc/default/bongosec-dashboard"
...
%attr(640, root, root) "/etc/systemd/system/bongosec-dashboard.service"

%changelog
* Fri May 05 2023 support <info@bongosec.github.io> - %{version}
- More info: https://documentation.bongosec.github.io/current/release-notes/
* Mon Apr 24 2023 support <info@bongosec.github.io> - 4.4.2
- More info: https://documentation.bongosec.github.io/current/release-notes/
