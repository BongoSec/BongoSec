...
- The base of Bongosec Dashboard, generated by running `yarn build --linux --skip-os-packages --release` or `yarn build --linux-arm --skip-os-packages --release` for arm packages
- The build of each plugin in `bongosec-dashboard-plugins` repo
- The build of the `bongosec-security-dashboards-plugin` repo
...
- `-a`, `--app`: URL or path to the zip that contains the `bongosec-dashboard-plugins` plugins build.
- `-b`, `--base`: URL or path to the zip that contains the `bongosec-dashboard build`.
- `-s`, `--security`: URL or path to the zip that contains the `bongosec-security-dashboards-plugin` build.
...
> [!WARNING]
> To build `arm` packages, you need to run the script in an arm machine, and use an arm build of the bongosec-dashboard base with `-b`

Example:

```bash
bash build-packages.sh \
    --app file:///home/user/packages/bongosec-package.zip \
