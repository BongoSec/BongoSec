...
mkdir -p ${tmp_dir}/bongosec-dashboard-base
cd ${tmp_dir}/bongosec-dashboard-base
log "Extracting base tar.gz..."
tar -zxf ${out_dir}/bongosec-dashboard-$version-$revision-linux-$architecture.tar.gz
log "Preparing the package..."
jq '.bongosec.revision="'${revision}'"' package.json > pkgtmp.json && mv pkgtmp.json package.json
cp $config_path/* .
...
cd ..
tar -czf bongosec-dashboard.tar.gz bongosec-dashboard-base
