...
clean() {
    ...
    rm -f ${current_path}/rpm/Docker/bongosec-dashboard.spec
...
get_packages(){
  ...
  packages_names=("Bongosec plugins" "Bongosec Dashboard" "Security plugin")
...
build_rpm() {
  ...
  cp ./bongosec-dashboard.spec ${dockerfile_path}
  docker build -t ${container_name} ${dockerfile_path} || return 1
  docker run -t --rm -v ${tmp_dir}/:/tmp:Z -v ${output_dir}/:/output:Z\
...
help() {
    ...
    echo "    -a, --app <url/path>          Set the location of the .zip file containing the Bongosec plugin."
    echo "    -b, --base <url/path>         Set the location of the .tar.gz file containing the base bongosec-dashboard build."
    echo "    -s, --security <url/path>     Set the location of the .zip file containing the bongosec-security-dashboards-plugin."
