...
PACKAGE=""
# Container name
CONTAINER_NAME="bongosec-dashboard"
# Files to check
FILES="/etc/bongosec-dashboard/opensearch_dashboards.yml /usr/share/bongosec-dashboard"
# Owner of the files
FILE_OWNER="bongosec-dashboard"
...
# Check if files exist and are owned by bongosec-dashboard
files_exist() {
...
check_opensearch_dashboard_yml() {
  ...
  diff_opensearch_dashboard_yml=$(docker exec $CONTAINER_NAME diff /etc/bongosec-dashboard/opensearch_dashboards.yml /tmp/opensearch_dashboards.yml)
  ...
  echo $(docker exec $CONTAINER_NAME diff /etc/bongosec-dashboard/opensearch_dashboards.yml /tmp/opensearch_dashboards.yml)
...
check_metadata_deb() {
  ...
  metadataVersion=$(docker exec $CONTAINER_NAME apt show bongosec-dashboard | grep Version | awk '{print $2}')
  metadataPackage=$(docker exec $CONTAINER_NAME apt show bongosec-dashboard | grep Package | awk '{print $2}')
  metadataStatus=$(docker exec $CONTAINER_NAME apt show bongosec-dashboard | grep Status)
...
check_metadata_rpm() {
  metadataVersion=$(docker exec $CONTAINER_NAME rpm -q --qf '%{VERSION}-%{RELEASE}' bongosec-dashboard)
  metadataPackage=$(docker exec $CONTAINER_NAME rpm -q --qf '%{NAME}' bongosec-dashboard)
...
help() {
  ...
  echo
  echo "    -p, --package <path>       Set Bongosec Dashboard rpm package name,which has to be in the <repository>/dev-tools/test-packages/<DISTRIBUTION>/ folder."
  echo
  exit $1
}
